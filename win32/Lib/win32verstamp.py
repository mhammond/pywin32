...
def stamp(pathname, options):
    ...
    try:
        ...
        raise ValueError(
            "--version must be a.b.c.d (all integers) - got %r" % ver)
    ...
    for k, v in sdata.items():
        if v is None:
            sdata[k] = ""
    vs = VS_VERSION_INFO(vmaj, vmin, vsub, vbuild,
                         sdata, vdata, is_debug, is_dll)
...
if __name__ == '__main__':
    ...
    if not args:
        parser.error(
            "You must supply a file to stamp.  Use --help for details.")

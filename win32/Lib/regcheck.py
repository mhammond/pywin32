...
def CheckRegisteredExe(exename):
    try:
        os.stat(win32api.RegQueryValue(regutil.GetRootKey(),
...
def CheckPythonPaths(verbose):
    ...
    if verbose:
        print "\tCore Path:",
    try:
        appPath = win32api.RegQueryValue(regutil.GetRootKey(
    ...
    ), regutil.BuildDefaultPythonKey() + "\\PythonPath", 0, win32con.KEY_READ)
    try:
        keyNo = 0
        while 1:
            try:
...
def CheckHelpFiles(verbose):
    ...
    try:
        key = win32api.RegOpenKey(regutil.GetRootKey(
        ), regutil.BuildDefaultPythonKey() + "\\Help", 0, win32con.KEY_READ)
    except win32api.error, exc:
        import winerror

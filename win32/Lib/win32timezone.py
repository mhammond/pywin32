...
class TimeZoneDefinition(DYNAMIC_TIME_ZONE_INFORMATION):
    ...
    def __init_from_other(self, other):
        ...
        for name in other.field_names():
            ...
            value = super(TimeZoneDefinition,
                          other).__getattribute__(other, name)
    ...
    def __getattribute__(self, attr):
        ...
        if 'bias' in attr:
            def make_minute_timedelta(m): return datetime.timedelta(minutes=m)
...
class TimeZoneInfo(datetime.tzinfo):
    ...
    def _get_time_zone_key(subkey=None):
        ...
        key = _RegKeyDict.open(
            _winreg.HKEY_LOCAL_MACHINE, TimeZoneInfo.tzRegKey)
        if subkey:
    ...
    def get_sorted_time_zone_names():
        ...
        def get_standard_name(tzi): return tzi.standardName
...
def GetTZCapabilities():
    ...
    DynamicTZSupport = not MissingTZPatch and datetime.datetime(
        2003, 11, 2, tzinfo=tzi).utctimetuple() == (2003, 11, 2, 7, 0, 0, 6, 306, 0)
    del tzi
    return vars()
...
class RangeMap(dict):
    ...
    def _find_first_match_(self, keys, item):
        def is_match(k): return self.match(item, k)

...
def FindHelpPath(helpFile, helpDesc, searchPaths):
    ...
    try:
        key = win32api.RegOpenKey(
            win32con.HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Windows\\Help", 0, win32con.KEY_ALL_ACCESS)
        try:
            try:
...
def FindAppPath(appName, knownFileName, searchPaths):
    ...
    for pathLook in searchPaths:
        if FileExists(os.path.join(pathLook, knownFileName)):
            # Found it
            return os.path.abspath(pathLook)
    raise error("The file %s can not be located for application %s" %
...
def FindPythonExe(exeAlias, possibleRealNames, searchPaths):
    ...
    if not FileExists(found):  # for developers
        if "64 bit" in sys.version:
            found = os.path.join(sys.prefix,  "PCBuild",
                                 "amd64", possibleRealNames)
        else:
...
def FindRegisterPackage(packageName, knownFile, searchPaths, registryAppName=None):
    ...
    try:
        pathLook, pathAdd = FindPackagePath(
            packageName, knownFile, searchPaths)
...
def SetupCore(searchPaths):
    ...
    hKey = win32api.RegCreateKey(
        regutil.GetRootKey(), regutil.BuildDefaultPythonKey())
    try:
        # Core Paths.
    ...
    win32paths = os.path.abspath(os.path.split(win32api.__file__)[0]) + ";" + \
        os.path.abspath(os.path.split(LocateFileName(
            "win32con.py;win32con.pyc", sys.path))[0])
...
def RegisterShellInfo(searchPaths):
    ...
    regutil.SetRegistryDefaultValue(
        ".py", "Python.File", win32con.HKEY_CLASSES_ROOT)
    regutil.RegisterShellCommand(
        "Open", QuotedFileName(exePath)+" \"%1\" %*", "&Run")
    regutil.SetRegistryDefaultValue(
        "Python.File\\DefaultIcon", "%s,0" % exePath, win32con.HKEY_CLASSES_ROOT)

    FindRegisterHelpFile("Python.hlp", searchPaths,
                         "Main Python Documentation")
    FindRegisterHelpFile("ActivePython.chm", searchPaths,
                         "Main Python Documentation")

...
class Registry(unittest.TestCase):
    ...
    def testValues(self):
        ...
        values = ((None, win32con.REG_SZ, 'This is default unnamed value'),
                  ('REG_SZ', win32con.REG_SZ, 'REG_SZ text data'),
                  ('REG_EXPAND_SZ', win32con.REG_EXPAND_SZ, '%systemdir%'),
                  # # REG_MULTI_SZ value needs to be a list since strings
                  # # are returned as a list
                  ('REG_MULTI_SZ', win32con.REG_MULTI_SZ, ['string 1',
                                                           'string 2',
                                                           'string 3',
                                                           'string 4'
                                                           ]),
    ...
    def testNotifyChange(self):
        ...
        win32api.RegNotifyChangeKeyValue(win32con.HKEY_CURRENT_USER,
                                         1,
                                         win32api.REG_NOTIFY_CHANGE_LAST_SET,
                                         evt,
                                         True)
        ret_code = win32event.WaitForSingleObject(evt,
                                                  0)
        # Should be no change.

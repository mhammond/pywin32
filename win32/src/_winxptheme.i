...
class PyHTHEME : public PyHANDLE {
   public:
    ...
    virtual const char *GetTypeName() { return "PyHTHEME"; }
};
...
Py_BEGIN_ALLOW_THREADS $function Py_END_ALLOW_THREADS if (FAILED($source))
{
    $cleanup return PyWin_SetAPIError("$name", $source);
}
}
...
static PyObject *MyCloseThemeData(PyObject *self, PyObject *args)
{
    ...
    if (!PyHANDLE_Check(obHandle))
        return PyErr_Format(PyExc_TypeError, "CloseThemeData requires a PyHTHEME object - got %s",
...
HRESULT DrawThemeBackground(HTHEME hTheme, HDC hdc, int iPartId, int iStateId, RECT *INPUT, RECT *INPUT_NULLOK);
...
HRESULT DrawThemeText(HTHEME hTheme, HDC hdc, int iPartId, int iStateId, WCHAR *pszText, int iCharCount,
                      DWORD dwTextFlags, DWORD dwTextFlags2, RECT *INPUT);
...
HRESULT GetThemeBackgroundContentRect(HTHEME hTheme, HDC hdc, int iPartId, int iStateId, RECT *INPUT, RECT *OUTPUT);
...
HRESULT GetThemeBackgroundExtent(HTHEME hTheme, HDC hdc, int iPartId, int iStateId, RECT *INPUT, RECT *OUTPUT);
...
static PyObject *MyGetCurrentThemeName(PyObject *self, PyObject *args)
{
    ...
    HRESULT hr = GetCurrentThemeName(nameBuf, sizeof(nameBuf) / sizeof(TCHAR), colorBuf,
                                     sizeof(colorBuf) / sizeof(TCHAR), sizeBuf, sizeof(sizeBuf) / sizeof(TCHAR));
    if (FAILED(hr))
        return PyWin_SetAPIError("GetCurrentThemeName", hr);
    return Py_BuildValue("NNN", PyWinObject_FromWCHAR(nameBuf), PyWinObject_FromWCHAR(colorBuf),

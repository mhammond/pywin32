...
{
    PyErr_SetString(PyExc_NotImplementedError, "not available in py3k");
    return NULL;
};
...
PyObject *MyGetNamedPipeHandleState(PyObject *self, PyObject *args)
{
    ...
    // @pyparm int|bGetCollectionData|0|Determines if the collection data should be retrieved.  If not, None is returned
    // in their place.
    // @pyparm int|bGetUserName|0|Determines if the username should be retrieved. Works only for a server handle and if
    // the client opened the pipe with SECURITY_IMPERSONATION access.
    ...
    BOOL ok;
    Py_BEGIN_ALLOW_THREADS ok = GetNamedPipeHandleState(hNamedPipe, &State, &CurInstances, pMaxCollectionCount,
                                                        pCollectDataTimeout, getUserName ? buf : NULL, 512);
    Py_END_ALLOW_THREADS if (!ok) return PyWin_SetAPIError("GetNamedPipeHandleState");
    ...
    if (getCollectData) {
        ...
        obMaxCollectionCount = Py_None;
        Py_INCREF(Py_None);
        obCollectDataTimeout = Py_None;
        Py_INCREF(Py_None);
    }
...
PyObject *MySetNamedPipeHandleState(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OOOO:SetNamedPipeHandleState", &obhNamedPipe, &obMode, &obMaxCollectionCount,
                          &obCollectDataTimeout))
        return NULL;
    if (!PyWinObject_AsHANDLE(obhNamedPipe, &hNamedPipe))
        return NULL;
    if (obMode != Py_None) {
        if (!PyLong_Check(obMode))
            return PyErr_Format(PyExc_TypeError, "mode param must be None or an integer (got %s)",
                                Py_TYPE(obMode)->tp_name);
        Mode = PyLong_AsLong(obMode);
        pMode = &Mode;
    }
    if (obMaxCollectionCount != Py_None) {
        if (!PyLong_Check(obMaxCollectionCount))
            return PyErr_Format(PyExc_TypeError, "maxCollectionCount param must be None or an integer (got %s)",
                                Py_TYPE(obMaxCollectionCount)->tp_name);
        MaxCollectionCount = PyLong_AsLong(obMaxCollectionCount);
        pMaxCollectionCount = &MaxCollectionCount;
    }
    if (obCollectDataTimeout != Py_None) {
        if (!PyLong_Check(obCollectDataTimeout))
            return PyErr_Format(PyExc_TypeError, "collectDataTimeout param must be None or an integer (got %s)",
                                Py_TYPE(obCollectDataTimeout)->tp_name);
        CollectDataTimeout = PyLong_AsLong(obCollectDataTimeout);
        pCollectDataTimeout = &CollectDataTimeout;
    }

    if (!SetNamedPipeHandleState(hNamedPipe, pMode, pMaxCollectionCount, pCollectDataTimeout))
...
PyObject *MyConnectNamedPipe(PyObject *self, PyObject *args)
{
    ...
    BOOL ok;
    Py_BEGIN_ALLOW_THREADS ok = ConnectNamedPipe(hNamedPipe, pOverlapped);
...
PyObject *MyTransactNamedPipe(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS ok =
        TransactNamedPipe(handle, write_buf.ptr(), write_buf.len(), readData, cbReadData, &numRead, pOverlapped);
...
PyObject *MyCallNamedPipe(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            ...
            &timeOut))     // @pyparm int|timeOut||Specifies the number of milliseconds to wait for the named pipe to be
                           // available. In addition to numeric values, the following special values can be specified.
        // @flagh Value|Meaning
        // @flag win32pipe.NMPWAIT_NOWAIT|Does not wait for the named pipe. If the named pipe is not available, the
        // function returns an error.
        // @flag win32pipe.NMPWAIT_WAIT_FOREVER|Waits indefinitely.
        // @flag win32pipe.NMPWAIT_USE_DEFAULT_WAIT|Uses the default time-out specified in a call to the CreateNamedPipe
        // function.
        return NULL;
    ...
    BOOL ok;
    Py_BEGIN_ALLOW_THREADS ok =
        CallNamedPipe(szPipeName, pybuf.ptr(), pybuf.len(), readBuf, readBufSize, &numRead, timeOut);
...
// @pyswig (<o PyHANDLE>, <o PyHANDLE>)|CreatePipe|Creates an anonymous pipe, and returns handles to the read and write
// ends of the pipe
PyObject *MyCreatePipe(SECURITY_ATTRIBUTES *INPUT,  // @pyparm <o PySECURITY_ATTRIBUTES>|sa||
...
PyObject *FdCreatePipe(SECURITY_ATTRIBUTES *INPUT,  // @pyparm <o PySECURITY_ATTRIBUTES>|sa||Specifies security and
                                                    // inheritance for the pipe
...
// @pyswig <o PyHANDLE>|CreateNamedPipe|Creates an instance of a named pipe and returns a handle for subsequent pipe
// operations
PyHANDLE CreateNamedPipe(TCHAR *lpName,                  // @pyparm <o PyUnicode>|pipeName||The name of the pipe
...
BOOLAPI DisconnectNamedPipe(PyHANDLE hFile  // @pyparm <o PyHANDLE>|hFile||The handle to the pipe to disconnect.
);
...
// @pyswig |WaitNamedPipe|Waits until either a time-out interval elapses or an instance of the specified named pipe is
// available to be connected to (that is, the pipe's server process has a pending <om win32pipe.ConnectNamedPipe>
...
BOOLAPI WaitNamedPipe(
    ...
    unsigned long timeout);  // @pyparm int|timeout||The number of milliseconds the function will wait.
                             ...
                             // @flagh Value|Meaning
// @flag NMPWAIT_USE_DEFAULT_WAIT|The time-out interval is the default value specified by the server process in the
...
BOOLAPI GetNamedPipeInfo(HANDLE hNamedPipe,  // @pyparm <o PyHANDLE>|hNamedPipe||Handle to a named pipe
                         DWORD *OUTPUT, DWORD *OUTPUT, DWORD *OUTPUT, DWORD *OUTPUT);

%{

// @pyswig (string, int, int)|PeekNamedPipe|Copies data from a named or anonymous pipe into a buffer without removing it
...
PyObject *MyPeekNamedPipe(PyObject *self, PyObject *args)
{
    ...
    if (PeekNamedPipe(hNamedPipe, buf, size, &bytesRead, &totalAvail, &bytesLeft)) {
        rc = Py_BuildValue("Nii", PyBytes_FromStringAndSize((char *)buf, bytesRead), totalAvail, bytesLeft);
    }
    else

...
static struct PyNET_STRUCT_ITEM ui20[] = {UI20_ENTRY(name, NSI_WSTR, 0),       // @prop string/<o PyUnicode>|name|
...
static struct PyNET_STRUCT_ITEM ui1005[] = {UI1005_ENTRY(priv, NSI_DWORD, 0),  // @prop int|priv|
                                            {NULL}};
...
static struct PyNET_STRUCT_ITEM ui1007[] = {UI1007_ENTRY(comment, NSI_WSTR, 0),  // @prop string/<o PyUnicode>|comment|
                                            {NULL}};

// @object PyUSER_INFO_1008|A dictionary holding the information in a Win32 USER_INFO_1008 structure.
static struct PyNET_STRUCT_ITEM ui1008[] = {UI1008_ENTRY(flags, NSI_DWORD, 0),  // @prop int|flags|
                                            {NULL}};
...
static struct PyNET_STRUCT_ITEM ui1010[] = {UI1010_ENTRY(auth_flags, NSI_WSTR, 0),  // @prop int|auth_flags|
                                            {NULL}};
...
static struct PyNET_STRUCT_ITEM umi0[] = {UMI0_ENTRY(min_passwd_len, NSI_DWORD, 0),     // @prop int|min_passwd_len|
...
static struct PyNET_STRUCT_ITEM umi1[] = {UMI1_ENTRY(role, NSI_DWORD, 0),    // @prop int|role|
...
**
******************************************************************************************************************/
// @pymethod [(groupName, attribute), ...]|win32net|NetUserGetGroups|Returns a list of groups,attributes for all groups
// for the user.
// @todo This needs to be extended to support the new model, while
// not breaking existing code.  A default arg would be perfect.
PyObject *PyNetUserGetGroups(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            args, "OO:NetUserGetGroups",
            &obServerName,  // @pyparm string|serverName||The name of the remote server on which the function is to
                            // execute. None or an empty string specifies the server program running on the local
                            // computer.
...
**
*************************************************************************************************************/
// @pymethod [groupName, ...]|win32net|NetUserGetLocalGroups|Retrieves a list of local groups to which a specified user
// belongs.
// @todo This needs to be extended to support the new model, while
// not breaking existing code.  A default arg would be perfect.
PyObject *PyNetUserGetLocalGroups(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            args, "OO|i:NetUserGetLocalGroups",
            &obServerName,  // @pyparm string|serverName||The name of the remote server on which the function is to
                            // execute. None or an empty string specifies the server program running on the local
                            // computer.
            &obUserName,    // @pyparm string|userName||The name of the user to search for in each group account. This
                            // parameter can be of the form \<UserName\>, in which case the username is expected to be
            // found on servername. The user name can also be of the form \<DomainName\>\\\<UserName\> in
            // which case \<DomainName\> is associated with servername and \<UserName\> is expected to be

...
#define REPORT_SUCCESS(i, l)                                                                                           \
    (MESSAGE_LEVEL >= l ? ReportEvent(hEventLog, EVENTLOG_INFORMATION_TYPE, 0, i, (PSID)NULL, 0, 0, NULL, (PVOID)NULL) \
                        : FALSE)
#define REPORT_INFORMATION(i, l)                                                                                       \
    (MESSAGE_LEVEL >= l ? ReportEvent(hEventLog, EVENTLOG_INFORMATION_TYPE, 0, i, (PSID)NULL, 0, 0, NULL, (PVOID)NULL) \
                        : FALSE)
#define REPORT_WARNING(i, l)                                                                                       \
    (MESSAGE_LEVEL >= l ? ReportEvent(hEventLog, EVENTLOG_WARNING_TYPE, 0, i, (PSID)NULL, 0, 0, NULL, (PVOID)NULL) \
                        : FALSE)
#define REPORT_ERROR(i, l)                                                                                       \
    (MESSAGE_LEVEL >= l ? ReportEvent(hEventLog, EVENTLOG_ERROR_TYPE, 0, i, (PSID)NULL, 0, 0, NULL, (PVOID)NULL) \
                        : FALSE)
#define REPORT_INFORMATION_DATA(i, l, d, s)                                                                           \
    (MESSAGE_LEVEL >= l ? ReportEvent(hEventLog, EVENTLOG_INFORMATION_TYPE, 0, i, (PSID)NULL, 0, s, NULL, (PVOID)(d)) \
                        : FALSE)
#define REPORT_WARNING_DATA(i, l, d, s)                                                                           \
    (MESSAGE_LEVEL >= l ? ReportEvent(hEventLog, EVENTLOG_WARNING_TYPE, 0, i, (PSID)NULL, 0, s, NULL, (PVOID)(d)) \
                        : FALSE)
...
--*/
{
    ...
    if (!dwOpenCount) {  // open Eventlog interface
        ...
        pControlData = (MappingManagerControlData *)MapViewOfFile(hSharedMemory, FILE_MAP_READ, 0, 0, 0);
        ...
        status = RegOpenKeyEx(HKEY_LOCAL_MACHINE, registryKeyName, 0L, KEY_ALL_ACCESS, &hKeyDriverPerf);
        ...
        size = sizeof(DWORD);
        status = RegQueryValueEx(hKeyDriverPerf, _T("First Counter"), 0L, &type, (LPBYTE)&dwModuleFirstCounter, &size);
        ...
        size = sizeof(DWORD);
        status = RegQueryValueEx(hKeyDriverPerf, _T("First Help"), 0L, &type, (LPBYTE)&dwModuleFirstHelp, &size);
...
DWORD APIENTRY CollectPerformanceData(IN LPWSTR lpValueName, IN OUT LPVOID *lppData, IN OUT LPDWORD lpcbTotalBytes,
...
--*/
{
    ...
    if (dwQueryType == QUERY_ITEMS) {
        if (!(IsNumberInUnicodeList(pPOT->ObjectNameTitleIndex + dwModuleFirstCounter, lpValueName))) {
    ...
    //
    memmove(pPOTResult, pPOT, SpaceNeeded);
    ...
    PERF_COUNTER_BLOCK *pPerfCounterBlock =
        (PERF_COUNTER_BLOCK *)(((LPBYTE)pPOTResult) + (pPOT->NumCounters * sizeof(PERF_COUNTER_DEFINITION)) +
...
--*/
{
    ...
    if (!MESSAGE_LEVEL) {
        ...
        if (lStatus == ERROR_SUCCESS) {
            lStatus = RegQueryValueEx(hAppKey, LogLevelValueName, (LPDWORD)NULL, &dwValueType, (LPBYTE)&dwLogLevel,
                                      &dwValueSize);
            if (lStatus == ERROR_SUCCESS) {
...
--*/
{
    ...
    if (bFound)
        return QUERY_GLOBAL;  // check for "Foreign" request
    pwcArgChar = lpValue;
    ...
    if (bFound)
        return QUERY_FOREIGN;  // check for "Costly" request
...
--*/
{
    ...
    if (lpwszUnicodeList == 0)
        return FALSE;  // null pointer, # not found

...
#define HEAP_PROBE()                                                                                  \
    {                                                                                                 \
        ...
        if (!(CallStatus = memprobe(dwHeapStatus, 16L, NULL))) {                                      \
            REPORT_INFORMATION_DATA(VGA_HEAP_STATUS, LOG_DEBUG, &dwHeapStatus, sizeof(dwHeapStatus)); \
            \                                                                                         \
        }                                                                                             \
        else {                                                                                        \
            REPORT_ERROR_DATA(VGA_HEAP_STATUS_ERROR, LOG_DEBUG, &CallStatus, sizeof(DWORD));          \
            \                                                                                         \
        }                                                                                             \
        \                                                                                             \
    }

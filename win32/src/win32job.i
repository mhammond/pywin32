...
BOOLAPI AssignProcessToJobObject(HANDLE hJob,     // @pyparm <o PyHANDLE>|hJob||
                                 HANDLE hProcess  // @pyparm <o PyHANDLE>|hProcess||
);

// @pyswig |CreateJobObject|Creates or opens a job object.
PyHANDLE CreateJobObject(SECURITY_ATTRIBUTES *lpJobAttributes,  // @pyparm <o PySECURITY_ATTRIBUTES>|jobAttributes||
                         WCHAR *lpName);                        // @pyparm unicode|name||

// @pyswig |OpenJobObject|Opens an existing job object.
PyHANDLE OpenJobObject(DWORD dwDesiredAccess,  // @pyparm int|desiredAccess||
...
BOOLAPI TerminateJobObject(HANDLE hJob,            // @pyparm <o PyHANDLE>|hJob||
                           unsigned int uExitCode  // @pyparm int|exitCode||
);

// @pyswig |UserHandleGrantAccess|Grants or denies access to a handle to a User object to a job that has a
// user-interface restriction.
BOOLAPI UserHandleGrantAccess(HANDLE hUserHandle,  // @pyparm <o PyHANDLE>|hUserHandle||
...
{
    return Py_BuildValue("{s:L,s:L,s:k,s:k,s:k,s:k,s:k,s:k,s:k}", "PerProcessUserTimeLimit",
                         jbli->PerProcessUserTimeLimit, "PerJobUserTimeLimit", jbli->PerJobUserTimeLimit, "LimitFlags",
                         jbli->LimitFlags, "MinimumWorkingSetSize", jbli->MinimumWorkingSetSize,
                         "MaximumWorkingSetSize", jbli->MaximumWorkingSetSize, "ActiveProcessLimit",
                         jbli->ActiveProcessLimit, "Affinity", jbli->Affinity, "PriorityClass", jbli->PriorityClass,
...
{
    return Py_BuildValue("{s:L,s:L,s:L,s:L,s:k,s:k,s:k,s:k}", "TotalUserTime", jbai->TotalUserTime, "TotalKernelTime",
                         jbai->TotalKernelTime, "ThisPeriodTotalUserTime", jbai->ThisPeriodTotalUserTime,
                         "ThisPeriodTotalKernelTime", jbai->ThisPeriodTotalKernelTime, "TotalPageFaultCount",
                         jbai->TotalPageFaultCount, "TotalProcesses", jbai->TotalProcesses, "ActiveProcesses",
                         jbai->ActiveProcesses, "TotalTerminatedProcesses", jbai->TotalTerminatedProcesses);
}
...
PyObject *PyQueryInformationJobObject(PyObject *self, PyObject *args)
{
    ...
    switch (infoclass) {
        // @flag JobObjectBasicAccountingInformation|Returns a dict representing a
        ...
        // @flag JobObjectBasicAndIoAccountingInformation|Returns a dict representing a
        ...
        case JobObjectBasicAndIoAccountingInformation: {
            ...
            if (!QueryInformationJobObject(jh, infoclass, &info, sizeof(info), NULL))
                return PyWin_SetAPIError("QueryInformationJobObject");
            return Py_BuildValue("{s:N,s:N}", "BasicInfo",
                                 PyWinObject_FromJOBOBJECT_BASIC_ACCOUNTING_INFORMATION(&info.BasicInfo), "IoInfo",
                                 PyWinObject_FromIO_COUNTERS(&info.IoInfo));
        }
        ...
        // @flag JobObjectExtendedLimitInformation|Returns a dict representing a JOBOBJECT_EXTENDED_LIMIT_INFORMATION
        // struct
        case JobObjectExtendedLimitInformation: {
            JOBOBJECT_EXTENDED_LIMIT_INFORMATION info;
            if (!QueryInformationJobObject(jh, infoclass, &info, sizeof(info), NULL))
                return PyWin_SetAPIError("QueryInformationJobObject");
            return Py_BuildValue("{s:N,s:N,s:N,s:N,s:N,s:N}", "BasicLimitInformation",
                                 PyWinObject_FromJOBOBJECT_BASIC_LIMIT_INFORMATION(&info.BasicLimitInformation),
                                 "IoInfo", PyWinObject_FromIO_COUNTERS(&info.IoInfo), "ProcessMemoryLimit",
                                 PyLong_FromUnsignedLongLong(info.ProcessMemoryLimit), "JobMemoryLimit",
                                 PyLong_FromUnsignedLongLong(info.JobMemoryLimit), "PeakProcessMemoryUsed",
                                 PyLong_FromUnsignedLongLong(info.PeakProcessMemoryUsed), "PeakJobMemoryUsed",
                                 PyLong_FromUnsignedLongLong(info.PeakJobMemoryUsed));
        }

        // @flag JobObjectEndOfJobTimeInformation|Returns a dict representing a JOBOBJECT_END_OF_JOB_TIME_INFORMATION
        // struct
        ...
        // @flag JobObjectJobSetInformation|Returns a dict representing a JOBOBJECT_JOBSET_INFORMATION struct (not
        // documented on MSDN)
...
BOOL PyWinObject_AsJOBOBJECT_END_OF_JOB_TIME_INFORMATION(PyObject *ob, PJOBOBJECT_END_OF_JOB_TIME_INFORMATION jeoj)
{
    ...
    if (dummyargs == NULL)
        return FALSE;
    BOOL bsuccess = PyArg_ParseTupleAndKeywords(dummyargs, ob, fmt, keywords, &jeoj->EndOfJobTimeAction);
    Py_DECREF(dummyargs);
    return bsuccess;
}
...
BOOL PyWinObject_AsJOBOBJECT_BASIC_UI_RESTRICTIONS(PyObject *ob, PJOBOBJECT_BASIC_UI_RESTRICTIONS jbur)
{
    ...
    if (dummyargs == NULL)
        return FALSE;
    BOOL bsuccess = PyArg_ParseTupleAndKeywords(dummyargs, ob, fmt, keywords, &jbur->UIRestrictionsClass);
    Py_DECREF(dummyargs);
    return bsuccess;
}
...
{
    static char *keywords[] = {"ReadOperationCount",
                               "WriteOperationCount",
                               "OtherOperationCount",
                               "ReadTransferCount",
                               "WriteTransferCount",
    ...
    BOOL bsuccess = PyArg_ParseTupleAndKeywords(
        dummyargs, ob, fmt, keywords, &pioc->ReadOperationCount, &pioc->WriteOperationCount, &pioc->OtherOperationCount,
        &pioc->ReadTransferCount, &pioc->WriteTransferCount, &pioc->OtherTransferCount);
    Py_DECREF(dummyargs);
    return bsuccess;
}
...
{
    static char *keywords[] = {"PerProcessUserTimeLimit",
                               "PerJobUserTimeLimit",
                               "LimitFlags",
    ...
    if (dummyargs == NULL)
        return FALSE;
    BOOL bsuccess = PyArg_ParseTupleAndKeywords(
        dummyargs, ob, fmt, keywords, &jbli->PerProcessUserTimeLimit, &jbli->PerJobUserTimeLimit, &jbli->LimitFlags,
        &jbli->MinimumWorkingSetSize, &jbli->MaximumWorkingSetSize, &jbli->ActiveProcessLimit, &jbli->Affinity,
        &jbli->PriorityClass, &jbli->SchedulingClass);
    Py_DECREF(dummyargs);
    return bsuccess;
}
...
BOOL PyWinObject_AsJOBOBJECT_EXTENDED_LIMIT_INFORMATION(PyObject *ob, PJOBOBJECT_EXTENDED_LIMIT_INFORMATION jeli)
{
    static char *keywords[] = {
        "BasicLimitInformation", "IoInfo", "ProcessMemoryLimit", "JobMemoryLimit", "PeakProcessMemoryUsed",
        "PeakJobMemoryUsed",     NULL};
#ifdef _WIN64
    ...
    if (dummyargs == NULL)
        return FALSE;
    BOOL bsuccess = PyArg_ParseTupleAndKeywords(
        dummyargs, ob, fmt, keywords, PyWinObject_AsJOBOBJECT_BASIC_LIMIT_INFORMATION, &jeli->BasicLimitInformation,
        PyWinObject_AsIO_COUNTERS, &jeli->IoInfo, &jeli->ProcessMemoryLimit, &jeli->JobMemoryLimit,
        &jeli->PeakProcessMemoryUsed, &jeli->PeakJobMemoryUsed);
    Py_DECREF(dummyargs);
    return bsuccess;
}
...
PyObject *PySetInformationJobObject(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            ...
            &infoclass,  // @pyparm int|JobObjectInfoClass||The type of data required, one of JobObject* values
            &PyDict_Type, &obinfo))  // @pyparm dict|JobObjectInfo||Dictionary containing info to be set, as returned by
                                     // <om win32job.QueryInformationJobObject>
        return NULL;
    if (!PyWinObject_AsHANDLE(objh, &jh))
        return NULL;
    ...
    switch (infoclass) {
        ...
        // @flag JobObjectSecurityLimitInformation|Input is a JOBOBJECT_SECURITY_LIMIT_INFORMATION dict - Not
        // implemented
        // @flag JobObjectAssociateCompletionPortInformation|Input is a JOBOBJECT_ASSOCIATE_COMPLETION_PORT dict - Not
        // implemented
        default:
            return PyErr_Format(PyExc_NotImplementedError, "Job information class %d is not supported yet");
    }

...
inline void SetDlgItemTemplate(DLGITEMTEMPLATE *t, DWORD s, WORD x, WORD y, WORD w, WORD h, WORD id)
{
    t->style = s;
    t->dwExtendedStyle = 0;
    t->x = x;
...
class CPythonDialogTemplate {
   ...
   public:
    CPythonDialogTemplate(LPCWSTR capt, DLGTEMPLATE *tmpl, WORD fontsize = 8, LPCWSTR font = NULL, LPCWSTR menu = NULL,
                          LPCWSTR wclass = NULL);
    ~CPythonDialogTemplate();
    BOOL Add(LPCWSTR wclass, DLGITEMTEMPLATE *tmpl, LPCWSTR txt = NULL, int datalen = 0, BYTE *data = NULL);

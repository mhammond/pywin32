...
void CALLBACK PyHINTERNET_StatusChange(HINTERNET hInternet, DWORD_PTR dwContext, DWORD dwInternetStatus,
...
class PyCallbackContext {
   public:
    ...
    PyCallbackContext(PyObject *context, HINTERNET hParent = NULL)
    {
        ...
        if (hParent && InternetQueryOption(hParent, INTERNET_OPTION_CONTEXT_VALUE, &parent_context, &bufsize) &&
            parent_context && parent_context->obCallback)
   ...
   public:
    ...
    virtual BOOL Close(void)
    {
        ...
        if (m_handle) {
            ...
            if (InternetQueryOption(h, INTERNET_OPTION_CONTEXT_VALUE, &context, &bufsize) && context)
    ...
    */
    ...
    switch (status) {
        ...
        case INTERNET_STATUS_REQUEST_COMPLETE: {
            ...
            return Py_BuildValue("{s:N, s:k}", "Result", PyWinLong_FromHANDLE((HANDLE)ias->dwResult), "Error",
                                 ias->dwError);
        }
        ...
        case INTERNET_STATUS_COOKIE_HISTORY: {  // InternetCookieHistory struct
            ...
            return Py_BuildValue("{s:N, s:N, s:N, s:N}", "Accepted", PyBool_FromLong(ich->fAccepted), "Leashed",
                                 PyBool_FromLong(ich->fLeashed), "Downgraded", PyBool_FromLong(ich->fDowngraded),
                                       ...
                                       LPVOID lpvStatusInformation, DWORD dwStatusInformationLength)
{
    ...
    if (context->obCallback != Py_None) {
        PyObject *args = Py_BuildValue(
            "NOkN", PyWinLong_FromHANDLE(hInternet), context->obContext, dwInternetStatus,
            PyWinObject_FromStatusInformation(dwInternetStatus, lpvStatusInformation, dwStatusInformationLength));
        if (args == NULL)
            PyErr_Print();
        else {
...
PyObject *PyObject_FromHINTERNET(HINTERNET hi)
{
    ...
    */
    PyCallbackContext *context;
    DWORD bufsize = sizeof(context);
    if (InternetQueryOption(hi, INTERNET_OPTION_CONTEXT_VALUE, &context, &bufsize) && context &&
...
BOOLAPI InternetSetCookie(TCHAR *lpszUrl,        // @pyparm string|url||
...
PyObject *PyInternetGetCookie(PyObject *self, PyObject *args)
{
    ...
    cch = cchallocated;
    Py_BEGIN_ALLOW_THREADS ok = InternetGetCookie(szUrl, szCookieName, buf, &cch);
    Py_END_ALLOW_THREADS if (!ok) PyWin_SetAPIError("InternetGetCookie");
    else
    {
    ...
    if (szCookieName)
        PyWinObject_FreeTCHAR(szCookieName);
    return ret;
}
%}
...
DWORDAPI InternetAttemptConnect(DWORD reserved = 0);  // @pyparm int|Reserved|0|Use only 0.

// @pyswig |InternetCheckConnection|Allows an application to check if a connection to the Internet can be established
BOOLAPI InternetCheckConnection(TCHAR *INPUT_NULLOK,  // @pyparm string|Url||Url to attempt to connect to, can be None
...
BOOLAPI InternetGoOnline(TCHAR *lpszUrl,    // @pyparm string|Url||Web site to connect to
...
BOOLAPI InternetCloseHandle(HINTERNET handle  // @pyparm <o PyHINTERNET>|handle||
);
...
PyObject *PyInternetConnect(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    static char *keywords[] = {"Internet", "ServerName", "ServerPort", "Username", "Password",
                               "Service",  "Flags",      "Context",    NULL};

    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "OOHOOkk|O:InternetConnect", keywords,
            &obhInternet,   // @pyparm <o PyHINTERNET>|Internet||Valid HINTERNET handle returned by a previous call to
                            // <om win32inet.InternetOpen>.
    ...
    Py_BEGIN_ALLOW_THREADS hret =
        InternetConnect(hInternet, ServerName, ServerPort, Username, Password, Service, Flags, (DWORD_PTR)context);
    Py_END_ALLOW_THREADS

        // In ansynchronous mode, returned handle may be NULL.  Handle will be passed to
        //	callback when it is created.
        if (hret) ret = PyObject_FromHINTERNET(hret);
    else
    {
...
PyHINTERNET InternetOpen(TCHAR *lpszAgent,  // @pyparm string|agent||A string that contains the name of the application
                                            // or entity calling the Internet functions. This name is used as the user
                                            // agent in the HTTP protocol.
                         DWORD dwAccessType,   // pyparm int|accessType||dwAccessType|Type of access required. This can
                                               // be one of INTERNET_OPEN_TYPE_DIRECT, INTERNET_OPEN_TYPE_PRECONFIG,
                         ...
                         DWORD dwFlags         // @pyparm int|flags||Combination of
                                               // INTERNET_FLAG_ASYNC,INTERNET_FLAG_FROM_CACHE, or INTERNET_FLAG_OFFLINE
);
...
PyObject *PyInternetOpenUrl(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    // @pyparm string|Url||A string that contains the URL to begin reading.  Only URLs beginning with ftp:, gopher:,
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OO|OkO:InternetOpenUrl", keywords, &obH, &obURL, &obHeaders, &flags,
                                     &obContext))
        return NULL;
    ...
    Py_BEGIN_ALLOW_THREADS hiret = InternetOpenUrl(hiin, szURL, szHeaders, headerLen, flags, (DWORD_PTR)context);
    Py_END_ALLOW_THREADS
        ...
        */
        if (hiret) ret = PyObject_FromHINTERNET(hiret);
    else
    {
        DWORD err = GetLastError();
        if (err == ERROR_IO_PENDING) {
...
PyObject *PyInternetCanonicalizeUrl(PyObject *self, PyObject *args)
{
    ...
    // @flag ICU_BROWSER_MODE|Does not encode or decode characters after "#" or "?", and does not remove trailing white
    ...
    // If no flags are specified (dwFlags = 0), the function converts all unsafe characters and meta sequences (such as
...
%{
// @pyswig int, string|InternetGetLastResponseInfo|Retrieves the last Win32® Internet function error description or
// server response on the thread calling this function.
...
PyObject *PyInternetReadFile(PyObject *self, PyObject *args)
{
    ...
    if (!PyWinObject_AsHANDLE(obH, (HANDLE *)&hiin))
        goto done;
    Py_BEGIN_ALLOW_THREADS ok = InternetReadFile(hiin, buf, size, &read);
...
PyObject *PyInternetWriteFile(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS ok = InternetWriteFile(hFile, pybuf.ptr(), pybuf.len(), &bytes_written);
    Py_END_ALLOW_THREADS if (!ok) return PyWin_SetAPIError("InternetWriteFile");
...
PyObject *PyFtpOpenFile(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "OOkk|O:FtpOpenFile", keywords,
            ...
            &Access,  // @pyparm int|Access||Integer value that determines how the file will be accessed. This can be
                      // GENERIC_READ or GENERIC_WRITE, but not both.
            ...
            &obContext))  // @pyparm object|Context|None|Arbitrary object that will be passed to handle's callback
                          // function
        return NULL;
    ...
    if (context == NULL)
        ...
        Py_BEGIN_ALLOW_THREADS hret = FtpOpenFile(hConnect, FileName, Access, Flags, (DWORD_PTR)context);
        Py_END_ALLOW_THREADS if (hret == NULL) PyWin_SetAPIError("FtpOpenFile");
        else ret = PyObject_FromHINTERNET(hret);
    }
...
PyObject *PyFtpCommand(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "OlkO|O:FtpCommand", keywords,
    ...
    if (context == NULL)
        ...
        Py_BEGIN_ALLOW_THREADS bsuccess =
            FtpCommand(hConnect, ExpectResponse, Flags, Command, (DWORD_PTR)context, &hret);
        Py_END_ALLOW_THREADS if (!bsuccess) PyWin_SetAPIError("FtpCommand");
        else
        {
...
PyObject *PyInternetQueryOption(PyObject *self, PyObject *args)
{
    ...
    void *buf = NULL;
    PyObject *ret = NULL;
    if (!PyArg_ParseTuple(args, "O&k:InternetQueryOption", PyWinObject_AsHANDLE,
    ...
    if (option == INTERNET_OPTION_CALLBACK || option == INTERNET_OPTION_CONTEXT_VALUE) {
        ...
        if (context == NULL) {
            ...
            PyErr_SetString(PyExc_RuntimeError,
                            "??? Internal error - PyHINTERNET does not have a PyCallbackContext ???");
            return NULL;
        }
    ...
    switch (option) {
        ...
        case INTERNET_OPTION_RECEIVE_TIMEOUT:  // @flag INTERNET_OPTION_RECEIVE_TIMEOUT|Int - timeout in millseconds
                                               // @flag INTERNET_OPTION_CONTROL_RECEIVE_TIMEOUT|Int - timeout in
                                               // millseconds
        ...
        case INTERNET_OPTION_CODEPAGE_EXTRA:   // @flag INTERNET_OPTION_CODEPAGE_EXTRA|Int - Codepage for path part of
                                               // URL
        case INTERNET_OPTION_CONNECT_RETRIES:  // @flag INTERNET_OPTION_CONNECT_RETRIES|Int - Number of time to try to
                                               // reconnect to host
        case INTERNET_OPTION_CONNECT_TIMEOUT:  // @flag INTERNET_OPTION_CONNECT_TIMEOUT|Int - Connection timeout in
                                               // milliseconds
        case INTERNET_OPTION_CONNECTED_STATE:  // @flag INTERNET_OPTION_CONNECTED_STATE|Int - Connection state,
                                               // INTERNET_STATE_*
        ...
        case INTERNET_OPTION_FROM_CACHE_TIMEOUT:  // @flag INTERNET_OPTION_FROM_CACHE_TIMEOUT|Int - Timeout in ms before
                                                  // cached copy is used
        ...
        case INTERNET_OPTION_REQUEST_FLAGS:             // @flag INTERNET_OPTION_REQUEST_FLAGS|Int, combination of
                                                        // INTERNET_REQFLAG_*
        ...
        case INTERNET_OPTION_USER_AGENT:            // @flag INTERNET_OPTION_USER_AGENT|String
            ...
            break;
        case INTERNET_OPTION_CACHE_TIMESTAMPS: {  // @flag INTERNET_OPTION_CACHE_TIMESTAMPS|dict - Expiration and last
                                                  // modified times
            INTERNET_CACHE_TIMESTAMPS *ct = (INTERNET_CACHE_TIMESTAMPS *)buf;
            ret = Py_BuildValue("{s:N, s:N}", "Expires", PyWinObject_FromFILETIME(ct->ftExpires), "LastModified",
                                PyWinObject_FromFILETIME(ct->ftLastModified));
            break;
        }
        case INTERNET_OPTION_HTTP_VERSION: {  // @flag INTERNET_OPTION_HTTP_VERSION|dict - HTTP_VERSION_INFO
            ...
            ret = Py_BuildValue("{s:k, s:k}", "MajorVersion", vi->dwMajorVersion, "MinorVersion", vi->dwMinorVersion);
            break;
        }
        ...
        case INTERNET_OPTION_PROXY: {  // @flag INTERNET_OPTION_PROXY|dict - INTERNET_PROXY_INFO
            ...
            ret = Py_BuildValue("{s:k, s:N, s:N}", "AccessType", pi->dwAccessType, "Proxy",
                                PyWinObject_FromTCHAR((TCHAR *)pi->lpszProxy), "ProxyBypass",
                                PyWinObject_FromTCHAR((TCHAR *)pi->lpszProxyBypass));
            break;
        }

        case INTERNET_OPTION_DIAGNOSTIC_SOCKET_INFO:  // @flag INTERNET_OPTION_DIAGNOSTIC_SOCKET_INFO|Not yet supported
                                                      // (INTERNET_DIAGNOSTIC_SOCKET_INFO)
        case INTERNET_OPTION_PER_CONNECTION_OPTION:   // @flag INTERNET_OPTION_PER_CONNECTION_OPTION|Not yet supported
                                                      // (INTERNET_PER_CONN_OPTION_LIST)
        case INTERNET_OPTION_SECURITY_CERTIFICATE_STRUCT:  // @flag INTERNET_OPTION_SECURITY_CERTIFICATE_STRUCT|Not yet
                                                           // supported (INTERNET_CERTIFICATE_INFO)
        ...
        case INTERNET_OPTION_DIGEST_AUTH_UNLOAD:  // @flag INTERNET_OPTION_DIGEST_AUTH_UNLOAD|Only valid for
                                                  // InternetSetOption
        case INTERNET_OPTION_END_BROWSER_SESSION:     // @flag INTERNET_OPTION_END_BROWSER_SESSION|Only valid for
                                                      // InternetSetOption
        case INTERNET_OPTION_REFRESH:                 // @flag INTERNET_OPTION_REFRESH|Only valid for InternetSetOption
        case INTERNET_OPTION_RESET_URLCACHE_SESSION:  // @flag INTERNET_OPTION_RESET_URLCACHE_SESSION|Only valid for
                                                      // InternetSetOption
        case INTERNET_OPTION_SETTINGS_CHANGED:        // @flag INTERNET_OPTION_SETTINGS_CHANGED|Only valid for
                                                      // InternetSetOption
        default:
...
BOOL PyWinObject_AsINTERNET_PROXY_INFO(PyObject *ob, INTERNET_PROXY_INFO *pipi)
{
    ...
    if (obdummy == NULL)
        return FALSE;
    bsuccess =
        PyArg_ParseTupleAndKeywords(obdummy, ob, "kOO", keywords, &pipi->dwAccessType, &obProxy, &obProxyBypass) &&
        PyWinObject_AsTCHAR(obProxy, &Proxy, TRUE) && PyWinObject_AsTCHAR(obProxyBypass, &ProxyBypass, TRUE);
...
PyObject *PyInternetSetOption(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O&kO:InternetSetOption", PyWinObject_AsHANDLE,
                          &h,       // @pyparm <o PyHINTERNET>|hInternet||Internet handle, or None for global defaults
    ...
    switch (option) {
        ...
        case INTERNET_OPTION_SECONDARY_CACHE_KEY:  // @flag INTERNET_OPTION_SECONDARY_CACHE_KEY|String
        case INTERNET_OPTION_USER_AGENT:           // @flag INTERNET_OPTION_USER_AGENT|String
        case INTERNET_OPTION_DATAFILE_EXT:  // @flag INTERNET_OPTION_DATAFILE_EXT|String - Extension to use for download
                                            // cache file
        ...
        case INTERNET_OPTION_CACHE_TIMESTAMPS:  // @flag INTERNET_OPTION_CACHE_TIMESTAMPS|Only valid for
                                                // InternetQueryOption
        ...
        case INTERNET_OPTION_SECURITY_CERTIFICATE:         // @flag INTERNET_OPTION_SECURITY_CERTIFICATE|Only valid for
                                                           // InternetQueryOption
        case INTERNET_OPTION_SECURITY_CERTIFICATE_STRUCT:  // @flag INTERNET_OPTION_SECURITY_CERTIFICATE_STRUCT|Only
                                                           // valid for InternetQueryOption
        case INTERNET_OPTION_SECURITY_FLAGS:  // @flag INTERNET_OPTION_SECURITY_FLAGS|Only valid for InternetQueryOption
        case INTERNET_OPTION_SECURITY_KEY_BITNESS:    // @flag INTERNET_OPTION_SECURITY_KEY_BITNESS|Only valid for
                                                      // InternetQueryOption
        case INTERNET_OPTION_DIAGNOSTIC_SOCKET_INFO:  // @flag INTERNET_OPTION_DIAGNOSTIC_SOCKET_INFO|Only valid for
                                                      // InternetQueryOption
...
PyObject *PyWinObject_FromINTERNET_CACHE_ENTRY_INFO(INTERNET_CACHE_ENTRY_INFO *buf)
{
    ...
    return Py_BuildValue(
        "{s:k, s:N, s:N, s:k, s:k, s:k, s:N, s:N, s:N, s:N, s:N, s:N, s:N, s:k}", "StructSize", buf->dwStructSize,
        "SourceUrlName", PyWinObject_FromTCHAR(buf->lpszSourceUrlName), "LocalFileName",
        PyWinObject_FromTCHAR(buf->lpszLocalFileName), "CacheEntryType", buf->CacheEntryType, "UseCount",
        buf->dwUseCount, "HitRate", buf->dwHitRate, "Size", PyWinObject_FromULARGE_INTEGER(ul), "LastModifiedTime",
        PyWinObject_FromFILETIME(buf->LastModifiedTime), "ExpireTime", PyWinObject_FromFILETIME(buf->ExpireTime),
        "LastAccessTime", PyWinObject_FromFILETIME(buf->LastAccessTime), "LastSyncTime",
        PyWinObject_FromFILETIME(buf->LastSyncTime), "HeaderInfo",
        PyWinObject_FromTCHAR(buf->lpHeaderInfo, buf->dwHeaderInfoSize), "FileExtension",
        PyWinObject_FromTCHAR(buf->lpszFileExtension), "ExemptDelta", buf->dwExemptDelta);
}
...
PyObject *PyFindFirstUrlCacheEntry(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    HANDLE h;
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "|O:FindFirstUrlCacheEntry", keywords,
    ...
    while (1) {
        ...
        if (h == NULL) {
            ...
            ret =
                Py_BuildValue("NN", PyWinObject_FromUrlCacheHANDLE(h), PyWinObject_FromINTERNET_CACHE_ENTRY_INFO(buf));
            break;
        }
    }
...
PyObject *PyFindNextUrlCacheEntry(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O:FindNextUrlCacheEntry", keywords,
                                     &obh))  // @pyparm <o PyUrlCacheHANDLE>|EnumHandle||Cache enumeration handle as
                                             // returned by <om win32inet.FindFirstUrlCacheEntry>
        return NULL;
...
PyObject *PyFindFirstUrlCacheEntryEx(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    HANDLE h;
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "|OkkK:FindFirstUrlCacheEntryEx", keywords,
            &obpattern,  // @pyparm str|SearchPattern|None|Type of entry to find, can be 'visited:', 'cookie:', or None
    ...
    while (1) {
        ...
        h = FindFirstUrlCacheEntryEx(pattern, Flags, Filter, GroupId, buf, &bufsize, GroupAttributes, NULL, Reserved);
        if (h == NULL) {
            err = GetLastError();
            if (err != ERROR_INSUFFICIENT_BUFFER) {
...
PyObject *PyFindNextUrlCacheEntryEx(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O:FindNextUrlCacheEntryEx", keywords,
                                     &obh))  // @pyparm <o PyUrlCacheHANDLE>|EnumHandle||Cache enumeration handle as
                                             // returned by <om win32inet.FindFirstUrlCacheEntryEx>
        return NULL;
    if (!PyWinObject_AsHANDLE(obh, &h))
        return NULL;
    while (1) {
...
PyObject *PyFindNextUrlCacheGroup(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O:FindNextUrlCacheGroup", keywords,
                                     &obh))  // @pyparm <o PyHANDLE>|Find||Group enumeration handle as returned by <om
...
PyObject *PyGetUrlCacheEntryInfo(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O:GetUrlCacheEntryInfo", keywords,
                                     &oburl))  // @pyparm str|UrlName||Cache enumeration handle as returned by <om
...
PyObject *PyDeleteUrlCacheGroup(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "K|k:DeleteUrlCacheGroup", keywords,
...
PyObject *PyCreateUrlCacheEntry(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "OkO:CreateUrlCacheEntry", keywords,
    ...
    if (PyWinObject_AsTCHAR(obUrlName, &UrlName, FALSE) && PyWinObject_AsTCHAR(obFileExtension, &FileExtension, TRUE)) {
...
PyObject *PyCommitUrlCacheEntry(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "OO|OOkOO:CommitUrlCacheEntry", keywords,
            &obUrlName,        // @pyparm str|UrlName||The Url for which to create an entry
    ...
    if (PyWinObject_AsTCHAR(obUrlName, &UrlName, FALSE) && PyWinObject_AsTCHAR(obLocalFileName, &LocalFileName, TRUE) &&
        ...
        if (!CommitUrlCacheEntry(UrlName, LocalFileName, ExpireTime, LastModifiedTime, CacheEntryType, HeaderInfo,
                                 HeaderSize, FileExtension, OriginalUrl))
...
PyObject *PySetUrlCacheEntryGroup(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "OkK:SetUrlCacheEntryGroup", keywords,
...
BOOL PyWinObject_AsINTERNET_CACHE_GROUP_INFO(PyObject *ob, INTERNET_CACHE_GROUP_INFO *GroupInfo)
{
    ...
    BOOL bsuccess =
        PyArg_ParseTupleAndKeywords(dummy_tuple, ob, "|kkkkkOO:INTERNET_CACHE_GROUP_INFO", keywords,
                                    &GroupInfo->dwGroupSize, &GroupInfo->dwGroupFlags, &GroupInfo->dwGroupType,
                                    &GroupInfo->dwDiskUsage, &GroupInfo->dwDiskQuota, &obOwnerStorage, &obGroupName) &&
...
PyObject *PyGetUrlCacheGroupAttribute(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "K|k:GetUrlCacheGroupAttribute", keywords,
                                     &groupid,      // @pyparm int|GroupId||Group id
                                     &Attributes))  // @pyparm int|Attributes|CACHEGROUP_ATTRIBUTE_GET_ALL|Attributes to
                                                    // retrieve, CACHEGROUP_ATTRIBUTE_*
        return NULL;
    bufsize = sizeof(buf);
    ...
    return Py_BuildValue("{s:k, s:k, s:k, s:k, s:k, s:N, s:N}", "GroupSize", buf.dwGroupSize, "GroupFlags",
                         buf.dwGroupFlags, "GroupType", buf.dwGroupType, "DiskUsage", buf.dwDiskUsage, "DiskQuota",
                         buf.dwDiskQuota, "OwnerStorage",
                         PyWinObject_FromDWORDArray(buf.dwOwnerStorage, GROUP_OWNER_STORAGE_SIZE), "GroupName",
                         PyWinObject_FromTCHAR(buf.szGroupName));
};
...
PyObject *PySetUrlCacheGroupAttribute(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "KkO|k:SetUrlCacheGroupAttribute", keywords,
                                     &GroupId,      // @pyparm int|GroupId||Id of a cache group
                                     &Attributes,   // @pyparm int|Attributes||Bitmask of CACHEGROUP_ATTRIBUTE_* flags
                                                    // indicating which attributes to set
                                     &obGroupInfo,  // @pyparm dict|GroupInfo||INTERNET_CACHE_GROUP_INFO dict as
                                                    // returned by <om win32inet.GetUrlCacheGroupAttribute>
...
for (PyMethodDef *pmd = win32inetMethods; pmd->ml_name; pmd++)
    if ((strcmp(pmd->ml_name, "InternetOpenUrl") == 0) || (strcmp(pmd->ml_name, "FtpOpenFile") == 0) ||
        (strcmp(pmd->ml_name, "FtpCommand") == 0) || (strcmp(pmd->ml_name, "InternetConnect") == 0) ||
        (strcmp(pmd->ml_name, "FindFirstUrlCacheEntry") == 0) || (strcmp(pmd->ml_name, "FindNextUrlCacheEntry") == 0) ||
        (strcmp(pmd->ml_name, "FindFirstUrlCacheEntryEx") == 0) ||
        (strcmp(pmd->ml_name, "FindNextUrlCacheEntryEx") == 0) || (strcmp(pmd->ml_name, "FindCloseUrlCache") == 0) ||
        (strcmp(pmd->ml_name, "FindFirstUrlCacheGroup") == 0) || (strcmp(pmd->ml_name, "FindNextUrlCacheGroup") == 0) ||
        (strcmp(pmd->ml_name, "GetUrlCacheGroupAttribute") == 0) ||
        (strcmp(pmd->ml_name, "GetUrlCacheEntryInfo") == 0) || (strcmp(pmd->ml_name, "DeleteUrlCacheGroup") == 0) ||
        (strcmp(pmd->ml_name, "CreateUrlCacheGroup") == 0) || (strcmp(pmd->ml_name, "CreateUrlCacheEntry") == 0) ||
        (strcmp(pmd->ml_name, "CommitUrlCacheEntry") == 0) || (strcmp(pmd->ml_name, "SetUrlCacheEntryGroup") == 0) ||

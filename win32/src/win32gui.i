...
BOOL PyWinObject_AsBLENDFUNCTION(PyObject *obbl, BLENDFUNCTION *pbl)
{
    ...
    return PyArg_ParseTuple(obbl, "BBBB:BLENDFUNCTION",
                            ...
                            &pbl->SourceConstantAlpha,  // @tupleitem 2|int|SourceConstantAlpha|Transparency to be
                                                        // applied to entire source. (255 is opaque)
                            &pbl->AlphaFormat);  // @tupleitem 3|int|AlphaFormat|Only defined flag is AC_SRC_ALPHA, used
                                                 // when src bitmap contains per-pixel alpha
}
...
class PyGdiHANDLE : public PyHANDLE {
   public:
    ...
    virtual const char *GetTypeName() { return "PyGdiHANDLE"; }
};
...
Py_BEGIN_ALLOW_THREADS $function Py_END_ALLOW_THREADS if ($source == NULL)
{
    $cleanup return PyWin_SetAPIError("$name");
}
}
/* ??? If you don't map these to a known type, swig obstinately ignores the input and output typemaps and tries to treat
   them as pointers. However, it doesn't seem to matter what you typedef them to as long as they have in and out
   typemaps. ??? */
...
Py_BEGIN_ALLOW_THREADS $function Py_END_ALLOW_THREADS if ($source == ERROR)
{
    $cleanup return PyWin_SetAPIError("$name");
}
}
...
class PyHDEVNOTIFY : public PyHANDLE {
   public:
    ...
    virtual const char *GetTypeName() { return "PyHDEVNOTIFY"; }
};
...
if (PyType_Ready(&PyWNDCLASSType) == -1 || PyType_Ready(&PyBITMAPType) == -1 || PyType_Ready(&PyLOGFONTType) == -1)
...
for (PyMethodDef *pmd = win32guiMethods; pmd->ml_name; pmd++)
    if (strcmp(pmd->ml_name, "SetLayeredWindowAttributes") == 0 ||
        strcmp(pmd->ml_name, "GetLayeredWindowAttributes") == 0 || strcmp(pmd->ml_name, "UpdateLayeredWindow") == 0 ||
        strcmp(pmd->ml_name, "AnimateWindow") == 0 || strcmp(pmd->ml_name, "GetOpenFileNameW") == 0 ||
        strcmp(pmd->ml_name, "GetSaveFileNameW") == 0 || strcmp(pmd->ml_name, "SystemParametersInfo") == 0 ||
...
%typemap(python,in) RECT *INPUT(RECT rect_input)
{
    if (PyTuple_Check($source)) {
        if (PyArg_ParseTuple($source, "llll", &rect_input.left, &rect_input.top, &rect_input.right,
                             &rect_input.bottom) == 0) {
...
{
    if (PyTuple_Check($source)) {
        if (PyArg_ParseTuple($source, "llll", &rect_input_nullok.left, &rect_input_nullok.top, &rect_input_nullok.right,
                             &rect_input_nullok.bottom) == 0) {
...
PyObject *o;
o = Py_BuildValue("(Nl(iiii)llN)", PyWinLong_FromHANDLE($source->hdc), $source->fErase, $source->rcPaint.left,
                  $source->rcPaint.top, $source->rcPaint.right, $source->rcPaint.bottom, $source->fRestore,
                  $source->fIncUpdate,
...
if (PyTuple_Check($source)) {
    if (!PyArg_ParseTuple($source, "Ol(iiii)llO", &obdc, &ps_input.fErase, &ps_input.rcPaint.left,
                          &ps_input.rcPaint.top, &ps_input.rcPaint.right, &ps_input.rcPaint.bottom, &ps_input.fRestore,
                          &ps_input.fIncUpdate, &obReserved))
        return NULL;
    ...
    if (lenReserved != sizeof(ps_input.rgbReserved))
        return PyErr_Format(PyExc_ValueError, "%s: last element must be string of %d bytes", "$name",
                            sizeof(ps_input.rgbReserved));
...
Py_BEGIN_ALLOW_THREADS SetLastError(0);
$function Py_END_ALLOW_THREADS DWORD le;
if ($source == 0 && (le = GetLastError())) {
    $cleanup return PyWin_SetAPIError("$name", le);
}
}
...
BOOL PyWndProc_Call(PyObject *obFuncOrMap, HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT *prc)
{
    ...
    PyObject *args = Py_BuildValue("NlNN", PyWinLong_FromHANDLE(hWnd), uMsg, PyWinObject_FromPARAM(wParam),
...
struct PyMethodDef PyWNDCLASS::methods[] = {
    {"SetDialogProc", PyWNDCLASS::PySetDialogProc,
     1},  // @pymeth SetDialogProc|Sets the WNDCLASS to be for a dialog box.
...
/*static*/ void PyWNDCLASS::deallocFunc(PyObject *ob) { delete (PyWNDCLASS *)ob; }
...
/*static*/ void PyBITMAP::deallocFunc(PyObject *ob) { delete (PyBITMAP *)ob; }
...
/*static*/ void PyLOGFONT::deallocFunc(PyObject *ob) { delete (PyLOGFONT *)ob; }
...
static PyObject *PyEnumFontFamilies(PyObject *self, PyObject *args)
{
    ...
    // @pyparm function|EnumFontFamProc||The Python function called with each font family. This function is called with
    // 4 arguments.
...
if (!PyLOGFONT_Check($source))
    return PyErr_Format(PyExc_TypeError, "Must be a LOGFONT object (got %s)", Py_TYPE($source)->tp_name);
...
HFONT CreateFontIndirect(
    LOGFONT *lf);  // @pyparm <o PyLOGFONT>|lplf||A LOGFONT object as returned by <om win32gui.LOGFONT>
    ...
    switch (typ) {
        ...
        case OBJ_PEN: {
            ...
            if (GetObject(hob, sizeof(LOGPEN), &lp) == 0)
                return PyWin_SetAPIError("GetObject");
            return Py_BuildValue("{s:I, s:l, s:k}", "Style", lp.lopnStyle, "Width",
                                 lp.lopnWidth.x,  // Docs say y member is not used, so ignore it
                                 "Color", lp.lopnColor);
        }
        default:
...
static PyObject *PySetString(PyObject *self, PyObject *args)
{
    ...
    if (IsBadWritePtr(addr, maxLen)) {
        PyErr_SetString(PyExc_ValueError, "The value is not a valid address for writing");
        return NULL;
    }
...
// @pyswig int|FlashWindow|The FlashWindow function flashes the specified window one time. It does not change the active
// state of the window.
...
PyObject *PyFlashWindowEx(PyObject *self, PyObject *args)
{
    ...
    if (!PyWinObject_AsHANDLE(obhwnd, (HANDLE *)&f.hwnd))
        return NULL;
    Py_BEGIN_ALLOW_THREADS rc = FlashWindowEx(&f);
    Py_END_ALLOW_THREADS ret = rc ? Py_True : Py_False;
    Py_INCREF(ret);
    return ret;
}
%}
...
// @pyswig int|SetWindowLong|Places a long value at the specified offset into the extra window memory of the given
// window.
...
static PyObject *PySetWindowLong(PyObject *self, PyObject *args)
{
    ...
    switch (index) {
        ...
        case GWLP_WNDPROC: {
            ...
            PyObject *key = PyWinLong_FromHANDLE(hwnd);
            PyObject *value =
                Py_BuildValue("ON", ob, PyWinLong_FromVoidPtr((void *)GetWindowLongPtr(hwnd, GWLP_WNDPROC)));
...
static PyObject *PyCallWindowProc(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OOIOO",
                          &obwndproc,  // @pyparm int|wndproc||The wndproc to call - this is generally the return value
                                       // of SetWindowLong(GWL_WNDPROC)
    ...
    if (!PyWinObject_AsPARAM(oblparam, &lparam))
        return NULL;
    LRESULT rc;
    Py_BEGIN_ALLOW_THREADS rc = CallWindowProc(wndproc, hwnd, msg, wparam, lparam);
    Py_END_ALLOW_THREADS return PyWinLong_FromVoidPtr((void *)rc);
}
%}
...
static PyObject *PySendMessage(PyObject *self, PyObject *args)
{
    ...
    LRESULT rc;
    Py_BEGIN_ALLOW_THREADS rc = SendMessage(hwnd, msg, wparam, lparam);
...
static PyObject *PySendMessageTimeout(PyObject *self, PyObject *args)
{
    ...
    LRESULT rc;
    DWORD_PTR dwresult;
    Py_BEGIN_ALLOW_THREADS rc = SendMessageTimeout(hwnd, msg, wparam, lparam, flags, timeout, &dwresult);
    Py_END_ALLOW_THREADS if (rc == 0) return PyWin_SetAPIError("SendMessageTimeout");
...
// @pyswig int|ReplyMessage|Used to reply to a message sent through the SendMessage function without returning control
// to the function that called SendMessage.
BOOLAPI ReplyMessage(int lResult);  // @pyparm int|result||Specifies the result of the message processing. The possible
                                    // values are based on the message sent.

// @pyswig int|RegisterWindowMessage|Defines a new window message that is guaranteed to be unique throughout the system.
...
BOOL CALLBACK PyEnumWindowsProc(HWND hwnd,     // handle to parent window
...
// @pyswig |EnumWindows|Enumerates all top-level windows on the screen by passing the handle to each window, in turn, to
// an application-defined callback function.
static PyObject *PyEnumWindows(PyObject *self, PyObject *args)
{
    BOOL rc;
    PyObject *obFunc, *obOther;
    // @pyparm function|callback||A Python function to be used as the callback.  Function can return False to stop
    // enumeration, or raise an exception.
    // @pyparm object|extra||Any python object - this is passed to the callback function as the second param (first is
    // the hwnd).
    ...
    cb.func = obFunc;
    cb.extra = obOther;
    Py_BEGIN_ALLOW_THREADS rc = EnumWindows(PyEnumWindowsProc, (LPARAM)&cb);
...
// @pyswig |EnumThreadWindows|Enumerates all top-level windows associated with a thread on the screen by passing the
// handle to each window, in turn, to an application-defined callback function. EnumThreadWindows continues until the
// last top-level window associated with the thread is enumerated or the callback function returns FALSE
static PyObject *PyEnumThreadWindows(PyObject *self, PyObject *args)
{
    ...
    cb.func = obFunc;
    cb.extra = obOther;
    Py_BEGIN_ALLOW_THREADS rc = EnumThreadWindows(dwThreadId, PyEnumWindowsProc, (LPARAM)&cb);
    Py_END_ALLOW_THREADS if (!rc)
    {
        // Callback may have raised an exception already
        if (PyErr_Occurred())
...
// @pyswig |EnumChildWindows|Enumerates the child windows that belong to the specified parent window by passing the
// handle to each child window, in turn, to an application-defined callback function. EnumChildWindows continues until
...
static PyObject *PyEnumChildWindows(PyObject *self, PyObject *args)
{
    ...
    Py_END_ALLOW_THREADS if (PyErr_Occurred()) return NULL;
    Py_RETURN_NONE;
}

// @pyswig |EnumDesktopWindows|Enumerates all top-level windows associated with a desktop on the screen by passing the
// handle to each window, in turn, to an application-defined callback function. EnumThreadWindows continues until the
// last top-level window associated with the thread is enumerated or the callback function returns FALSE
static PyObject *PyEnumDesktopWindows(PyObject *self, PyObject *args)
{
    ...
    cb.func = obFunc;
    cb.extra = obOther;
    Py_BEGIN_ALLOW_THREADS rc = EnumDesktopWindows(hDesktop, PyEnumWindowsProc, (LPARAM)&cb);
    Py_END_ALLOW_THREADS if (!rc)
    {
        // Callback may have raised an exception already
        if (PyErr_Occurred())
...
static PyObject *PyDialogBox(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            ...
            &param))     // @pyparm int|InitParam|0|Initialization data to be passed to above procedure during
    ...
    INT_PTR rc;
    Py_BEGIN_ALLOW_THREADS rc = DialogBoxParam(hinst, resid, hwnd, PyDlgProcHDLG, (LPARAM)obExtra);
    Py_END_ALLOW_THREADS
...
static PyObject *PyDialogBoxIndirect(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OOOO|O",
                          ...
                          &obList,   // @pyparm <o PyDialogTemplate>|controlList||Sequence of items defining the dialog
                                     // box and subcontrols
                          &obhwnd,   // @pyparm <o PyHANDLE>|hWndParent||Handle to dialog's parent window
                          &obDlgProc,  // @pyparm function|DialogFunc||Dialog box procedure to process messages
                          &obParam))   // @pyparm long|InitParam|0|Initialization data to be passed to above procedure
                                       // during WM_INITDIALOG processing
        return NULL;
    if (!PyWinObject_AsHANDLE(obhinst, (HANDLE *)&hinst))
        return NULL;
    if (!PyWinObject_AsHANDLE(obhwnd, (HANDLE *)&hwnd))
    ...
    Py_BEGIN_ALLOW_THREADS rc =
        DialogBoxIndirectParam(hinst, (const DLGTEMPLATE *)templ, hwnd, PyDlgProcHDLG, (LPARAM)obExtra);
    GlobalUnlock(h);
    GlobalFree(h);
    Py_END_ALLOW_THREADS Py_DECREF(obExtra);
    if (rc == -1)
...
static PyObject *PyCreateDialogIndirect(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OOOO|l",
                          &obhinst,    // @pyparm <o PyHANDLE>|hInstance||Handle to module creating the dialog box
                          &obList,     // @pyparm <o PyDialogTemplate>|controlList||Sequence containing a <o
                                       // PyDLGTEMPLATE>, followed by variable number of <o PyDLGITEMTEMPLATE>s
                          &obhwnd,     // @pyparm <o PyHANDLE>|hWndParent||Handle to dialog's parent window
                          &obDlgProc,  // @pyparm function|DialogFunc||Dialog box procedure to process messages
                          &param))     // @pyparm int|InitParam|0|Initialization data to be passed to above procedure
                                       // during WM_INITDIALOG processing
        return NULL;
    if (!PyWinObject_AsHANDLE(obhinst, (HANDLE *)&hinst))
        return NULL;
    ...
    HWND rc;
    Py_BEGIN_ALLOW_THREADS HGLOBAL templ = (HGLOBAL)GlobalLock(h);
...
HWND GetDlgItem(HWND hDlg,      // @pyparm <o PyHANDLE>|hDlg||Handle to a dialog window
...
BOOLAPI SetDlgItemInt(HWND hDlg,       // @pyparm <o PyHANDLE>|hDlg||Handle to a dialog window
                      int nIDDlgItem,  // @pyparm int|IDDlgItem||Identifier of one of the dialog's controls
...
BOOLAPI SetDlgItemText(HWND hDlg,       // @pyparm <o PyHANDLE>|hDlg||Handle to a dialog window
...
// @pyswig HWND|GetNextDlgTabItem|Retrieves a handle to the first control that has the WS_TABSTOP style that precedes
// (or follows) the specified control.
HWND GetNextDlgTabItem(HWND hDlg,        // @pyparm int|hDlg||handle to dialog box
                       HWND hCtl,        // @pyparm int|hCtl||handle to known control
                       BOOL bPrevious);  // @pyparm int|bPrevious||direction flag

// @pyswig HWND|GetNextDlgGroupItem|Retrieves a handle to the first control in a group of controls that precedes (or
// follows) the specified control in a dialog box.
HWND GetNextDlgGroupItem(HWND hDlg,        // @pyparm int|hDlg||handle to dialog box
                         HWND hCtl,        // @pyparm int|hCtl||handle to known control
                         BOOL bPrevious);  // @pyparm int|bPrevious||direction flag
...
static PyObject *PyGetWindowText(PyObject *self, PyObject *args)
{
    ...
    if (!PyWinObject_AsHANDLE(obhwnd, (HANDLE *)&hwnd))
        return NULL;
    Py_BEGIN_ALLOW_THREADS len = GetWindowText(hwnd, buffer, sizeof(buffer) / sizeof(TCHAR));
    Py_END_ALLOW_THREADS return PyWinObject_FromTCHAR(buffer, len);
}
%}
...
HCURSOR LoadCursor(HINSTANCE hInst,  // @pyparm int|hinstance||The module to load from
                   RESOURCE_ID name  // @pyparm int|resid||The resource ID
);

// @pyswig HCURSOR|SetCursor|
HCURSOR SetCursor(HCURSOR hc  // @pyparm int|hcursor||
);
...
BOOLAPI DrawIcon(HDC hDC,     // @pyparm int|hDC||handle to DC
...
HANDLE LoadImage(
    HINSTANCE hInst,  // @pyparm int|hinst||Handle to an instance of the module that contains the image to be loaded. To
                      // load an OEM image, set this parameter to zero.
    RESOURCE_ID name,  // @pyparm int/string|name||Specifies the image to load. If the hInst parameter is non-zero and
                       // the fuLoad parameter omits LR_LOADFROMFILE, name specifies the image resource in the hInst
                       // module. If the image resource is to be loaded by name, the name parameter is a string that
                       // contains the name of the image resource.
    UINT type,         // @pyparm int|type||Specifies the type of image to be loaded.
    int cxDesired,  // @pyparm int|cxDesired||Specifies the width, in pixels, of the icon or cursor. If this parameter
                    // is zero and the fuLoad parameter is LR_DEFAULTSIZE, the function uses the SM_CXICON or
                    // SM_CXCURSOR system metric value to set the width. If this parameter is zero and LR_DEFAULTSIZE is
                    // not used, the function uses the actual resource width.
    int cyDesired,  // @pyparm int|cyDesired||Specifies the height, in pixels, of the icon or cursor. If this parameter
                    // is zero and the fuLoad parameter is LR_DEFAULTSIZE, the function uses the SM_CYICON or
                    // SM_CYCURSOR system metric value to set the height. If this parameter is zero and LR_DEFAULTSIZE
                    // is not used, the function uses the actual resource height.
...
%{
// @pyswig |DeleteObject|Deletes a logical pen, brush, font, bitmap, region, or palette, freeing all system resources
...
BOOLAPI BitBlt(HDC hdcDest,  // @pyparm int|hdcDest||handle to destination DC
...
BOOLAPI StretchBlt(HDC hdcDest,       // @pyparm int|hdcDest||handle to destination DC
...
BOOLAPI PatBlt(HDC hdc,     // @pyparm <o PyHANDLE>|hdc||Handle to a device context
...
int SetStretchBltMode(
    ...
    int StretchMode);  // @pyparm int|StretchMode||One of
                       // BLACKONWHITE,COLORONCOLOR,HALFTONE,STRETCH_ANDSCANS,STRETCH_DELETESCANS,STRETCH_HALFTONE,STRETCH_ORSCANS,
                       // or WHITEONBLACK (from win32con)

// @pyswig int|GetStretchBltMode|Returns the stretching mode used by <om win32gui.StretchBlt>
// @rdesc Returns one of
// BLACKONWHITE,COLORONCOLOR,HALFTONE,STRETCH_ANDSCANS,STRETCH_DELETESCANS,STRETCH_HALFTONE,STRETCH_ORSCANS,WHITEONBLACK,
// or 0 on error.
...
static PyObject *PyTransparentBlt(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS ret =
        TransparentBlt(dst, dst_x, dst_y, dst_width, dst_height, src, src_x, src_y, src_width, src_height, transparent);
    Py_END_ALLOW_THREADS if (!ret) return PyWin_SetAPIError("TransparentBlt");
    Py_INCREF(Py_None);
    return Py_None;
}
...
static PyObject *PyMaskBlt(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OiiiiOiiOiik:MaskBlt",
                          ...
                          &rop))  // @pyparm int|Rop||Foreground and background raster operations.  See MSDN docs for
                                  // how to construct this value.
        return NULL;
    if (!PyWinObject_AsHANDLE(obdst, (HANDLE *)&dst))
        return NULL;
    if (!PyWinObject_AsHANDLE(obsrc, (HANDLE *)&src))
    ...
    if (!PyWinObject_AsHANDLE(obmask, (HANDLE *)&mask))
        return NULL;
    if (!MaskBlt(dst, dst_x, dst_y, dst_width, dst_height, src, src_x, src_y, mask, mask_x, mask_y, rop))
...
static PyObject *PyAlphaBlend(PyObject *self, PyObject *args)
{
    ...
    if (!PyWinObject_AsBLENDFUNCTION(obbl, &bl))
        return NULL;
    if (!AlphaBlend(dst, dst_x, dst_y, dst_width, dst_height, src, src_x, src_y, src_width, src_height, bl))
...
int ImageList_Add(HIMAGELIST himl,   // @pyparm int|himl||Handle to the image list.
                  HBITMAP hbmImage,  // @pyparm <o PyGdiHANDLE>|hbmImage||Handle to the bitmap that contains the image
                                     // or images. The number of images is inferred from the width of the bitmap.
                  HBITMAP hbmMask);  // @pyparm <o PyGdiHANDLE>|hbmMask||Handle to the bitmap that contains the mask. If
                                     // no mask is used with the image list, this parameter is ignored
...
BOOLAPI ImageList_DrawEx(HIMAGELIST himl, int i, HDC hdcDst, int x, int y, int dx, int dy, COLORREF rgbBk,
...
HIMAGELIST ImageList_LoadImage(HINSTANCE hInst, RESOURCE_ID name, int cx, int cGrow, COLORREF crMask, UINT uType,
                               UINT uFlags);

// @pyswig HANDLE|ImageList_LoadBitmap|Creates an image list from the specified bitmap resource.
HIMAGELIST ImageList_LoadBitmap(HINSTANCE hInst, TCHAR *name, int cx, int cGrow, COLORREF crMask);
...
// @pyswig |ImageList_SetOverlayImage|Adds a specified image to the list of images to be used as overlay masks. An image
// list can have up to four overlay masks in version 4.70 and earlier and up to 15 in version 4.71. The function assigns
// an overlay mask index to the specified image.
BOOLAPI ImageList_SetOverlayImage(HIMAGELIST himl,  // @pyparm int|hImageList||
...
HWND CreateWindow(STRING_OR_ATOM_CW lpClassName,  // @pyparm int/string|className||
...
BOOL EnableWindow(HWND hwnd,      // @pyparm <o PyHANDLE>|hWnd||Handle to window
                  BOOL bEnable);  // @pyparm boolean|bEnable||True to enable input to the window, False to disable input

// @pyswig <o PyHANDLE>|FindWindow|Retrieves a handle to the top-level window whose class name and window name match the
// specified strings.
HWND FindWindow(RESOURCE_ID_NULLOK
                    className,  // @pyparm <o PyResourceId>|ClassName||Name or atom of window class to find, can be None
                TCHAR *INPUT_NULLOK);  // @pyparm string|WindowName||Title of window to find, can be None

// @pyswig <o PyHANDLE>|FindWindowEx|Retrieves a handle to the top-level window whose class name and window name match
// the specified strings.
HWND FindWindowEx(HWND parent,      // @pyparm <o PyHANDLE>|Parent||Window whose child windows will be searched.  If 0,
                                    // desktop window is assumed.
                  HWND childAfter,  // @pyparm <o PyHANDLE>|ChildAfter||Child window after which to search in Z-order,
                                    // can be 0 to search all
                  RESOURCE_ID_NULLOK className,  // @pyparm <o PyResourceId>|ClassName||Name or atom of window class to
                                                 // find, can be None
                  TCHAR *INPUT_NULLOK);          // @pyparm string|WindowName||Title of window to find, can be None
...
// @pyswig |DragDetect|captures the mouse and tracks its movement until the user releases the left button, presses the
// ESC key, or moves the mouse outside the drag rectangle around the specified point.
// @pyparm int|hwnd||Handle to the Window
// @pyparm (int, int)|point||Initial position of the mouse, in screen coordinates. The function determines the
// coordinates of the drag rectangle by using this point.
// @rdesc If the user moved the mouse outside of the drag rectangle while holding down the left button , the return
// value is nonzero. <nl>If the user did not move the mouse outside of the drag rectangle while holding down the left
// button , the return value is zero.
...
BOOLAPI SetCaretPos(int X,  // @pyparm int|x||horizontal position
...
static PyObject *PyCascadeWindows(PyObject *self, PyObject *args)
{
    ...
    if (rectObject != Py_None) {
        if (!PyArg_ParseTuple(rectObject, "llll", &rect.left, &rect.top, &rect.right, &rect.bottom))
            return NULL;
        rectPtr = &rect;
    }
...
BOOL ShowWindow(HWND hWnd,      // @pyparm int|hWnd||The handle to the window
...
// @pyswig (left, top, right, bottom)|GetClientRect|Returns the rectangle of the client area of a window, in client
// coordinates
// @pyparm int|hwnd||The handle to the window
...
BOOLAPI DeleteDC(HDC dc  // @pyparm int|hdc||The source DC
);

// @pyswig HDC|CreateCompatibleDC|Creates a memory device context (DC) compatible with the specified device.
HDC CreateCompatibleDC(HDC hdc  // @pyparm int|dc||handle to DC
);

// @pyswig <o PyGdiHANDLE>|CreateCompatibleBitmap|Creates a bitmap compatible with the device that is associated with
// the specified device context.
HBITMAP CreateCompatibleBitmap(HDC hdc,     // @pyparm int|hdc||handle to DC
...
HBITMAP CreateBitmap(int nWidth,        // @pyparm int|width||bitmap width, in pixels
...
// @pyswig HGDIOBJ|SelectObject|Selects an object into the specified device context (DC). The new object replaces the
// previous object of the same type.
HGDIOBJ SelectObject(HDC hdc,        // @pyparm int|hdc||handle to DC
...
BOOLAPI SetWindowPos(
    ...
    HWND InsertAfter,  // @pyparm <o PyHANDLE>|InsertAfter||Window that hWnd will be placed below.  Can be a window
                       // handle or one of HWND_BOTTOM,HWND_NOTOPMOST,HWND_TOP, or HWND_TOPMOST
...
static PyObject *PyGetWindowPlacement(PyObject *self, PyObject *args)
{
    ...
    BOOL ok;
    Py_BEGIN_ALLOW_THREADS ok = GetWindowPlacement(hwnd, &pment);
    Py_END_ALLOW_THREADS if (!ok) return PyWin_SetAPIError("GetWindowPlacement");
...
static PyObject *PySetWindowPlacement(PyObject *self, PyObject *args)
{
    PyObject *obhwnd;
    HWND hwnd;
    WINDOWPLACEMENT pment;
    ...
    if (!PyArg_ParseTuple(args, "O(ii(ii)(ii)(iiii)):SetWindowPlacement", &obhwnd, &pment.flags, &pment.showCmd,
    ...
    if (!PyWinObject_AsHANDLE(obhwnd, (HANDLE *)&hwnd))
        return NULL;
    BOOL rc;
    Py_BEGIN_ALLOW_THREADS rc = SetWindowPlacement(hwnd, &pment);
    Py_END_ALLOW_THREADS if (!rc) return PyWin_SetAPIError("SetWindowPlacement");
    Py_INCREF(Py_None);
    return Py_None;
}
...
static PyObject *PyUnregisterClass(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            args, "OO",
            &obatom,    // @pyparm <o PyResourceId>|atom||The atom or classname identifying the class previously
                        // registered.
...
static PyObject *PyPumpMessages(PyObject *self, PyObject *args)
{
    ...
    Py_END_ALLOW_THREADS
        ...
        if (-1 == rc) return PyWin_SetAPIError("GetMessage");
...
static PyObject *PyPumpWaitingMessages(PyObject *self, PyObject *args)
{
    ...
    Py_END_ALLOW_THREADS return PyWinLong_FromVoidPtr((void *)result);
}
...
int TranslateAccelerator(HWND hwnd,      // @pyparm int|hwnd||
...
BOOL PyObject_AsNOTIFYICONDATA(PyObject *ob, NOTIFYICONDATA *pnid)
{
    ...
    if (!PyArg_ParseTuple(
            ob, "O|iiiOOOiOi:NOTIFYICONDATA tuple",
            ...
            &pnid->uCallbackMessage,  // @tupleitem 3|int|CallbackMessage|Message id to be pass to hWnd when processing
                                      // messages
...
static PyObject *PyEdit_GetLine(PyObject *self, PyObject *args)
{
    ...
    int numChars;
    TCHAR *buf;
    Py_BEGIN_ALLOW_THREADS if (size == 0) size = Edit_LineLength(hwnd, line) + 1;
    buf = (TCHAR *)malloc(size * sizeof(TCHAR));
    numChars = Edit_GetLine(hwnd, line, buf, size);
    Py_END_ALLOW_THREADS PyObject *ret;
    if (numChars == 0) {
        Py_INCREF(Py_None);
        ret = Py_None;
    }
...
if (!PyBytes_Check($source)) {
    PyErr_Format(PyExc_TypeError, "Argument must be a %d-byte string (got type %s)", size, Py_TYPE($source)->tp_name);
    return NULL;
}
if (size != PyBytes_GET_SIZE($source)) {
    PyErr_Format(PyExc_TypeError, "Argument must be a %d-byte string (got string of %d bytes)", size,
...
BOOLAPI ScreenToClient(HWND hWnd,     // @pyparm <o PyHANDLE>|hWnd||Handle to a window
...
BOOLAPI ClientToScreen(HWND hWnd,     // @pyparm <o PyHANDLE>|hWnd||Handle to a window
...
BOOLAPI PaintDesktop(HDC hdc);  // @pyparm <o PyHANDLE>|hdc||Handle to a device context

// @pyswig |RedrawWindow|Causes a portion of a window to be redrawn
BOOLAPI RedrawWindow(HWND hWnd,           // @pyparm <o PyHANDLE>|hWnd||Handle to window to be redrawn
                     RECT *INPUT_NULLOK,  // @pyparm (int,int,int,int)|rcUpdate||Rectangle (left, top, right, bottom)
                                          // identifying part of window to be redrawn, can be None
                     HRGN INPUT_NULLOK,   // @pyparm <o PyGdiHANDLE>|hrgnUpdate||Handle to region to be redrawn, can be
                                          // None to indicate entire client area
...
static PyObject *PyGetTextExtentPoint32(PyObject *self, PyObject *args)
{
    ...
    SIZE size = {0, 0};
    BOOL rc;
    Py_BEGIN_ALLOW_THREADS rc = GetTextExtentPoint32(hdc, szString, nchars, &size);
    Py_END_ALLOW_THREADS PyWinObject_FreeTCHAR(szString);
    if (!rc)
...
static PyObject *PyGetTextMetrics(PyObject *self, PyObject *args)
{
    ...
    PyObject *obdc;
    TEXTMETRICW tm;
    if (!PyArg_ParseTuple(args, "O:GetTextMetrics", &obdc))
        return NULL;
    if (!PyWinObject_AsHANDLE(obdc, (HANDLE *)&hdc))
        return NULL;
    ...
    return Py_BuildValue(
        "{s:l,s:l,s:l,s:l,s:l,s:l,s:l,s:l,s:l,s:l,s:l,s:N,s:N,s:N,s:N,s:B,s:B,s:B,s:B,s:B}", "Height", tm.tmHeight,
        "Ascent", tm.tmAscent, "Descent", tm.tmDescent, "InternalLeading", tm.tmInternalLeading, "ExternalLeading",
        tm.tmExternalLeading, "AveCharWidth", tm.tmAveCharWidth, "MaxCharWidth", tm.tmMaxCharWidth, "Weight",
        tm.tmWeight, "Overhang", tm.tmOverhang, "DigitizedAspectX", tm.tmDigitizedAspectX, "DigitizedAspectY",
        tm.tmDigitizedAspectY, "FirstChar", PyWinObject_FromWCHAR(&tm.tmFirstChar, 1), "LastChar",
        PyWinObject_FromWCHAR(&tm.tmLastChar, 1), "DefaultChar", PyWinObject_FromWCHAR(&tm.tmDefaultChar, 1),
        "BreakChar", PyWinObject_FromWCHAR(&tm.tmBreakChar, 1), "Italic", tm.tmItalic, "Underlined", tm.tmUnderlined,
        "StruckOut", tm.tmStruckOut, "PitchAndFamily", tm.tmPitchAndFamily, "CharSet", tm.tmCharSet);
}
...
BOOL PyWinObject_AsXFORM(PyObject *obxform, XFORM *pxform)
{
    ...
    if (dummy_tuple == NULL)
        return FALSE;
    BOOL ret = PyArg_ParseTupleAndKeywords(
        dummy_tuple, obxform, "|ffffff", keywords,
...
{
    return Py_BuildValue("{s:f,s:f,s:f,s:f,s:f,s:f}", "M11", pxform->eM11, "M12", pxform->eM12, "M21", pxform->eM21,
                         "M22", pxform->eM22, "Dx", pxform->eDx, "Dy", pxform->eDy);
}
...
BOOLAPI GetWindowOrgEx(HDC hdc,  // @pyparm <o PyHANDLE>|hdc||Handle to a device context
                       POINT *OUTPUT);

// @pyswig (int,int)|SetWindowOrgEx|Changes the window origin for a DC
// @rdesc Returns the previous origin
BOOLAPI SetWindowOrgEx(HDC hdc,  // @pyparm <o PyHANDLE>|hdc||Handle to a device context
...
BOOLAPI GetViewportOrgEx(HDC hdc,  // @pyparm <o PyHANDLE>|hdc||Handle to a device context
                         POINT *OUTPUT);

// @pyswig (int,int)|SetViewportOrgEx|Changes the viewport origin for a DC
// @rdesc Returns the previous origin as (x,y)
BOOLAPI SetViewportOrgEx(HDC hdc,  // @pyparm <o PyHANDLE>|hdc||Handle to a device context
                         int X,    // @pyparm int|X||New X coord in logical units
                         int Y,    // @pyparm int|Y||New Y coord in logical units
                         POINT *OUTPUT);
    ...
    *pmesh = malloc(bufsize);
    if (*pmesh == NULL) {
        ...
        for (tuple_index = 0; tuple_index < *item_cnt; tuple_index++) {
            ...
            else if (!PyArg_ParseTuple(tuple_item, "kk:GRADIENT_RECT",
...
static PyObject *PyGradientFill(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            ...
            &obmesh,  // @pyparm tuple|Mesh||Sequence of tuples containing either 2 or 3 ints that index into the
                      // trivertex array to define either triangles or rectangles
            &mode))   // @pyparm int|Mode||win32con.GRADIENT_FILL_* value defining whether to fill by triangle or by
                      // rectangle
        return NULL;
    if (!PyWinObject_AsHANDLE(obdc, (HANDLE *)&hdc))
        return NULL;
    ...
    if (!PyWinObject_AsMeshArray(obmesh, mode, &pmesh, &mesh_cnt))
        goto cleanup;
    Py_BEGIN_ALLOW_THREADS bres = GradientFill(hdc, ptv, tv_cnt, pmesh, mesh_cnt, mode);
    Py_END_ALLOW_THREADS if (!bres) PyWin_SetAPIError("GradientFill");
    else
    {
        Py_INCREF(Py_None);
        ret = Py_None;
...
// @pyswig int|GetOpenFileName|Creates an Open dialog box that lets the user specify the drive, directory, and the name
...
// (ie, a win32gui.error is raised).  If the user cancels the dialog, the winerror attribute of the exception will be
// zero.
...
// @pyparm int|fByPosition||Boolean value of True if uItem is set to a menu item position. This parameter is set to
// False if uItem is set to a menu item identifier.
...
static PyObject *PyInsertMenuItem(PyObject *self, PyObject *args)
{
    ...
    PyObject *info_obj;
    PyObject *menu_obj;
    if (!PyArg_ParseTuple(args, "OiiO:InsertMenuItem", &menu_obj, &uItem, &fByPosition, &info_obj))
        return NULL;
    ...
    if (sizeof(MENUITEMINFO) != pybuf.len())
        return PyErr_Format(PyExc_TypeError, "Argument must be a %d-byte string/buffer (got %d bytes)",
                            sizeof(MENUITEMINFO), pybuf.len());
    MENUITEMINFO *pmii = (MENUITEMINFO *)pybuf.ptr();

    BOOL result;
    Py_BEGIN_ALLOW_THREADS result = InsertMenuItem(hMenu, uItem, fByPosition, pmii);
    Py_END_ALLOW_THREADS

        if (!result) return PyWin_SetAPIError("InsertMenuItem");
...
static PyObject *PySetMenuItemInfo(PyObject *self, PyObject *args)
{
    ...
    PyObject *menu_obj;
    PyObject *info_obj;
    if (!PyArg_ParseTuple(args, "OiiO:SetMenuItemInfo", &menu_obj, &item, &fByPositon, &info_obj))
        return NULL;
    if (!PyWinObject_AsHANDLE(menu_obj, (HANDLE *)&hMenu))
        return NULL;
    ...
    BOOL result;
    Py_BEGIN_ALLOW_THREADS result = SetMenuItemInfo(hMenu, item, fByPositon, pmii);
    Py_END_ALLOW_THREADS

        if (!result) return PyWin_SetAPIError("SetMenuItemInfo");
...
// @pyparm int|fByPosition||Boolean value of True if uItem is set to a menu item position. This parameter is set to
// False if uItem is set to a menu item identifier.
// @pyparm buffer|menuItem||A string or buffer that will receive the information in the format of a <o MENUITEMINFO>
// structure.
static PyObject *PyGetMenuItemInfo(PyObject *self, PyObject *args)
{
    ...
    PyObject *menu_obj;
    PyObject *info_obj;
    if (!PyArg_ParseTuple(args, "OiiO:GetMenuItemInfo", &menu_obj, &item, &fByPosition, &info_obj))
        return NULL;
    if (!PyWinObject_AsHANDLE(menu_obj, (HANDLE *)&hMenu))
        return NULL;
    ...
    BOOL result;
    Py_BEGIN_ALLOW_THREADS result = GetMenuItemInfo(hMenu, item, fByPosition, pmii);
    Py_END_ALLOW_THREADS

        if (!result) return PyWin_SetAPIError("GetMenuItemInfo");
...
// @pyswig |ModifyMenu|Changes an existing menu item. This function is used to specify the content, appearance, and
// behavior of the menu item.
BOOLAPI ModifyMenu(HMENU hMnu,       // @pyparm int|hMnu||handle to menu
...
// @pyswig int|GetMenuItemID|Retrieves the menu item identifier of a menu item located at the specified position in a
// menu.
UINT GetMenuItemID(HMENU hMenu,  // @pyparm int|hMenu||handle to menu
                   int nPos      // @pyparm int|nPos||position of menu item
);

// @pyswig |SetMenuItemBitmaps|Associates the specified bitmap with a menu item. Whether the menu item is selected or
...
BOOLAPI CheckMenuRadioItem(HMENU hMenu,   // @pyparm int|hMenu||handle to menu
...
PyObject *PySetMenuInfo(PyObject *self, PyObject *args)
{
    ...
    if (sizeof(MENUINFO) != pybuf.len())
        return PyErr_Format(PyExc_TypeError, "Argument must be a %d byte string/buffer (got %d bytes)",
                            sizeof(MENUINFO), pybuf.len());

    Py_BEGIN_ALLOW_THREADS result = SetMenuInfo(hmenu, (MENUINFO *)pybuf.ptr());
    Py_END_ALLOW_THREADS if (!result) return PyWin_SetAPIError("SetMenuInfo");
    Py_INCREF(Py_None);
    return Py_None;
}
%}
...
PyObject *PyGetMenuInfo(PyObject *self, PyObject *args)
{
    ...
    if (!pybuf.ok())
        return NULL;
    if (sizeof(MENUINFO) != pybuf.len())
        return PyErr_Format(PyExc_TypeError, "Argument must be a %d byte buffer (got %d bytes)", sizeof(MENUINFO),
                            pybuf.len());

    Py_BEGIN_ALLOW_THREADS result = GetMenuInfo(hmenu, (MENUINFO *)pybuf.ptr());
    Py_END_ALLOW_THREADS if (!result) return PyWin_SetAPIError("GetMenuInfo");
    Py_INCREF(Py_None);
    return Py_None;
}
%}
...
int DrawText(HDC hDC,       // @pyparm int/<o PyHANDLE>|hDC||The device context on which to draw
...
BOOLAPI LineTo(HDC hdc,    // @pyparm <o PyHANDLE>|hdc||Handle to a device context
...
BOOLAPI Ellipse(HDC hdc,          // @pyparm <o PyHANDLE>|hdc||Device context on which to draw
...
BOOLAPI Pie(HDC hdc,         // @pyparm <o PyHANDLE>|hdc||Device context on which to draw
            int LeftRect,    // @pyparm int|LeftRect||Left limit of ellipse
            int TopRect,     // @pyparm int|TopRect||Top limit of ellipse
            int RightRect,   // @pyparm int|RightRect||Right limit of ellipse
            int BottomRect,  // @pyparm int|BottomRect||Bottom limit of ellipse
...
BOOLAPI Arc(HDC hdc,         // @pyparm <o PyHANDLE>|hdc||Device context on which to draw
            int LeftRect,    // @pyparm int|LeftRect||Left limit of ellipse
            int TopRect,     // @pyparm int|TopRect||Top limit of ellipse
            int RightRect,   // @pyparm int|RightRect||Right limit of ellipse
            int BottomRect,  // @pyparm int|BottomRect||Bottom limit of ellipse
...
BOOLAPI ArcTo(HDC hdc,         // @pyparm <o PyHANDLE>|hdc||Device context on which to draw
              int LeftRect,    // @pyparm int|LeftRect||Left limit of ellipse
              int TopRect,     // @pyparm int|TopRect||Top limit of ellipse
              int RightRect,   // @pyparm int|RightRect||Right limit of ellipse
              int BottomRect,  // @pyparm int|BottomRect||Bottom limit of ellipse
...
BOOLAPI Chord(HDC hdc,         // @pyparm <o PyHANDLE>|hdc||Device context on which to draw
              int LeftRect,    // @pyparm int|LeftRect||Left limit of ellipse
              int TopRect,     // @pyparm int|TopRect||Top limit of ellipse
              int RightRect,   // @pyparm int|RightRect||Right limit of ellipse
              int BottomRect,  // @pyparm int|BottomRect||Bottom limit of ellipse
...
BOOLAPI ExtFloodFill(HDC hdc,         // @pyparm <o PyHANDLE>||hdc|Handle to a device context
...
BOOLAPI SetPixelV(HDC hdc,          // @pyparm <o PyHANDLE>|hdc||Handle to a device context
...
BOOLAPI MoveToEx(HDC hdc,  // @pyparm <o PyHANDLE>|hdc||Device context handle
...
BOOLAPI GetCurrentPositionEx(HDC hdc,  // @pyparm <o PyHANDLE>|hdc||Device context
                             POINT *OUTPUT);
...
int GetArcDirection(HDC hdc);  // @pyparm <o PyHANDLE>|hdc||Handle to a device context
...
int SetArcDirection(HDC hdc,            // @pyparm <o PyHANDLE>|hdc||Handle to a device context
...
static PyObject *PyExtTextOut(PyObject *self, PyObject *args)
{
    ...
    HDC hdc;
    if (!PyArg_ParseTuple(
            args, "OiiiOO|O:ExtTextOut",
            &obdc,          // @pyparm <o PyHANDLE>|hdc||Handle to a device context
            &x,             // @pyparm x|int||The x coordinate to write the text to.
            &y,             // @pyparm y|int||The y coordinate to write the text to.
            &options,       // @pyparm nOptions|int||Specifies the rectangle type. This parameter can be one, both, or
                            // neither of ETO_CLIPPED and ETO_OPAQUE
            ...
            &widthObject))  // @pyparm (width1, width2, ...)|tuple||Optional array of values that indicate distance
                            // between origins of character cells.
        return NULL;
    if (!PyWinObject_AsHANDLE(obdc, (HANDLE *)&hdc))
        return NULL;
    // Parse out rectangle object
...
COLORREF GetTextColor(HDC hdc);  // @pyparm int|hdc||Handle to a device context
...
int SetTextColor(HDC hdc,          // @pyparm int|hdc||Handle to a device context
...
int GetBkMode(HDC hdc);  // @pyparm <o PyHANDLE>|hdc||Handle to a device context

// @pyswig int|SetBkMode|Sets the background mode for a device context
// @rdesc Returns the previous mode, or 0 on failure
int SetBkMode(HDC hdc,    // @pyparm int/<o PyHANDLE>|hdc||Handle to a device context
...
int GetBkColor(HDC hdc);  // @pyparm <o PyHANDLE>|hdc||Handle to a device context

// @pyswig int|SetBkColor|Sets the background color for a device context
// @rdesc Returns the previous color, or CLR_INVALID on failure
int SetBkColor(HDC hdc,        // @pyparm int/<o PyHANDLE>|hdc||Handle to a device context
...
int FillRect(HDC hDC,      // @pyparm <o PyHANDLE>|hDC||Handle to a device context
...
BOOLAPI FillRgn(HDC hdc,      // @pyparm <o PyHANDLE>|hdc||Handle to the device context
...
BOOLAPI PaintRgn(HDC hdc,     // @pyparm <o PyHANDLE>|hdc||Handle to the device context
                 HRGN hrgn);  // @pyparm <o PyGdiHANDLE>|hrgn||Handle to the region

// @pyswig |FrameRgn|Draws a frame around a region
BOOLAPI FrameRgn(HDC hdc,      // @pyparm <o PyHANDLE>|hdc||Handle to the device context
                 HRGN hrgn,    // @pyparm <o PyGdiHandle>|hrgn||Handle to the region
...
BOOLAPI InvertRgn(HDC hdc,     // @pyparm <o PyHANDLE>|hdc||Handle to the device context
                  HRGN hrgn);  // @pyparm <o PyGdiHandle>|hrgn||Handle to the region

// @pyswig boolean|EqualRgn|Determines if 2 regions are equal
BOOL EqualRgn(HRGN SrcRgn1,   // @pyparm <o PyGdiHandle>|SrcRgn1||Handle to a region
...
BOOL PtInRegion(HRGN hrgn,  // @pyparm <o PyGdiHandle>|hrgn||Handle to a region
                int X,      // @pyparm int|X||X coord
                int Y);     // @pyparm int|Y||Y coord

// @pyswig boolean|PtInRect|Determines if a rectangle contains a point
BOOL PtInRect(RECT *INPUT,   // @pyparm (int, int, int, int)|rect||The rect to check
...
BOOL RectInRegion(HRGN hrgn,     // @pyparm <o PyGdiHandle>|hrgn||Handle to a region
...
BOOLAPI SetRectRgn(HRGN hrgn,        // @pyparm <o PyGdiHandle>|hrgn||Handle to a region
...
BOOLAPI DrawAnimatedRects(HWND hwnd,    // @pyparm int|hwnd||handle to clipping window
...
HBRUSH CreateSolidBrush(COLORREF Color);  // @pyparm int|Color||RGB color value.  See <om win32api.RGB>.

// @pyswig <o PyGdiHANDLE>|CreatePatternBrush|Creates a brush using a bitmap as a pattern
HBRUSH CreatePatternBrush(HBITMAP hbmp);  // @pyparm <o PyGdiHANDLE>|hbmp||Handle to a bitmap

// @pyswig <o PyGdiHANDLE>|CreateHatchBrush|Creates a hatch brush with specified style and color
HBRUSH CreateHatchBrush(int Style,         // @pyparm int|Style||Hatch style, one of win32con.HS_* constants
                        COLORREF clrref);  // @pyparm int|clrref||Rgb color value.  See <om win32api.RGB>.

// @pyswig <o PyGdiHANDLE>|CreatePen|Create a GDI pen
HPEN CreatePen(int PenStyle,     // @pyparm int|PenStyle||One of win32con.PS_* pen styles
...
// @pyswig |ValidateRect|Validates the client area within a rectangle by removing the rectangle from the update region
...
BOOLAPI InvalidateRect(HWND hWnd,           // @pyparm <o PyHANDLE>|hWnd||Handle to the window
                       RECT *INPUT_NULLOK,  // @pyparm <o PyRECT>|Rect||Client coordinates defining area to be redrawn.
                                            // Use None for entire client area.
...
BOOLAPI FrameRect(HDC hDC,      // @pyparm <o PyHANDLE>|hDC||Handle to a device context
                  RECT *INPUT,  // @pyparm <o PyRECT>|rc||Rectangle around which to draw
                  HBRUSH hbr);  // @pyparm <o PyGdiHANDLE>|hbr||Handle to brush created using CreateHatchBrush,
                                // CreatePatternBrush, CreateSolidBrush, or GetStockObject

// @pyswig |InvertRect|Inverts the colors in a regtangular region
BOOLAPI InvertRect(HDC hDC,       // @pyparm <o PyHANDLE>|hDC||Handle to a device context
...
HWND WindowFromDC(HDC hDC);  // @pyparm <o PyHANDLE>|hDC||Handle to a device context
...
BOOLAPI SetWindowRgn(HWND hWnd,          // @pyparm <o PyHANDLE>|hWnd||Handle to a window
...
int_regiontype GetWindowRgnBox(HWND hWnd,  // @pyparm <o PyHANDLE>|hWnd||Handle to a window that has a window region.
                                           // (see <om win32gui.SetWindowRgn>)
                               RECT *OUTPUT);

// @pyswig |ValidateRgn|Removes a region from a window's update region
BOOLAPI ValidateRgn(HWND hWnd,   // @pyparm <o PyHANDLE>|hWnd||Handle to the window
                    HRGN hRgn);  // @pyparm <o PyGdiHANDLE>|hRgn||Region to be validated

// @pyswig |InvalidateRgn|Adds a region to a window's update region
BOOLAPI InvalidateRgn(HWND hWnd,    // @pyparm <o PyHANDLE>|hWnd||Handle to the window
...
int_regiontype GetRgnBox(HRGN hrgn,  // @pyparm <o PyGdiHANDLE>|hrgn||Handle to a region
                         RECT *OUTPUT);

// @pyswig int|OffsetRgn|Relocates a region
// @rdesc Returns type of region (COMPLEXREGION, NULLREGION, or SIMPLEREGION)
int_regiontype OffsetRgn(HRGN hrgn,     // @pyparm <o PyGdiHANDLE>|hrgn||Handle to a region
...
BOOLAPI Rectangle(HDC hdc,           // @pyparm <o PyHANDLE>|hdc||Handle to device context
...
BOOLAPI RoundRect(HDC hdc,          // @pyparm <o PyHANDLE>|hdc||Handle to device context
                  int nLeftRect,    // @pyparm int|LeftRect||Position of left edge of rectangle
                  int nTopRect,     // @pyparm int|TopRect||Position of top edge of rectangle
                  int nRightRect,   // @pyparm int|RightRect||Position of right edge of rectangle
                  int nBottomRect,  // @pyparm int|BottomRect||Position of bottom edge of rectangle
...
BOOLAPI CloseFigure(HDC hdc);  // @pyparm <o PyHANDLE>|hdc||Handle to a device context that contains an open path. See
                               // <om win32gui.BeginPath>.

// @pyswig |FlattenPath|Flattens any curves in current path into a series of lines
BOOLAPI FlattenPath(HDC hdc);  // @pyparm <o PyHANDLE>|hdc||Handle to a device context that contains a closed path. See
                               // <om win32gui.EndPath>.
...
BOOLAPI FillPath(HDC hdc);  // @pyparm <o PyHANDLE>|hdc||Handle to a device context that contains a finalized path. See
                            // <om win32gui.EndPath>.

// @pyswig |WidenPath|Widens current path by amount it would increase by if drawn with currently selected pen
BOOLAPI WidenPath(HDC hdc);  // @pyparm <o PyHANDLE>|hdc||Handle to a device context that contains a closed path. See
                             // <om win32gui.EndPath>.

// @pyswig |StrokePath|Draws current path with currently selected pen
BOOLAPI StrokePath(HDC hdc);  // @pyparm <o PyHANDLE>|hdc||Handle to a device context that contains a closed path. See
                              // <om win32gui.EndPath>.

// @pyswig |StrokeAndFillPath|Combines operations of StrokePath and FillPath with no overlap
BOOLAPI StrokeAndFillPath(HDC hdc);  // @pyparm <o PyHANDLE>|hdc||Handle to a device context that contains a closed
                                     // path. See <om win32gui.EndPath>.

// @pyswig float|GetMiterLimit|Retrieves the limit of miter joins for a DC
BOOLAPI GetMiterLimit(HDC hdc,  // @pyparm <o PyHANDLE>|hdc||Handle to a device context
                      float *OUTPUT);

// @pyswig float|SetMiterLimit|Set the limit of miter joins for a DC
// @rdesc Returns the previous limit
BOOLAPI SetMiterLimit(HDC hdc,         // @pyparm <o PyHANDLE>|hdc||Handle to a device context
...
HRGN PathToRegion(HDC hdc);  // @pyparm <o PyHANDLE>|hdc||Handle to a device context that contains a closed path. See
                             // <om win32gui.EndPath>.

%{
// @pyswig tuple,tuple|GetPath|Returns a sequence of points that describe the current path
// @rdesc Returns a sequence of POINT tuples, and a sequence of ints designating each point's function (combination of
// win32con.PT_* values)
static PyObject *PyGetPath(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O:GetPath",
                          &obdc))  // @pyparm <o PyHANDLE>|hdc||Handle to a device context containing a finalized path.
                                   // See <om win32gui.EndPath>
        return NULL;
    if (!PyWinObject_AsHANDLE(obdc, (HANDLE *)&hdc))
        return NULL;
...
HRGN CreateRoundRectRgn(int LeftRect,        // @pyparm int|LeftRect||Position of left edge of rectangle
...
HRGN CreateEllipticRgnIndirect(
    RECT *INPUT);  // @pyparm <o PyRECT>|rc||Coordinates of bounding rectangle in logical units

// @pyswig int|CreateWindowEx|Creates a new window with Extended Style.
HWND CreateWindowEx(DWORD dwExStyle,                // @pyparm int|dwExStyle||extended window style
...
HWND GetParent(HWND hWnd  // @pyparm int|child||handle to child window
);

// @pyswig int|SetParent|changes the parent window of the specified child window.
HWND SetParent(HWND hWndChild,     // @pyparm int|child||handle to window whose parent is changing
...
// @pyswig int|GetWindow|returns a window that has the specified relationship (Z order or owner) to the specified
// window.
HWND GetWindow(HWND hWnd,  // @pyparm int|hWnd||handle to original window
               UINT uCmd   // @pyparm int|uCmd||relationship flag
);

// @pyswig int|GetTopWindow|Examines the Z order of the child windows associated with the specified parent window and
// retrieves a handle to the child window at the top of the Z order.
HWND GetTopWindow(HWND hWnd  // @pyparm int|hWnd||handle to parent window
);

// @pyswig int|GetAncestor|retrieves the handle to the ancestor of the specified window.
HWND GetAncestor(HWND hWnd,    // @pyparm int|hWnd||handle to original window
                 UINT gaFlags  // @pyparm int|gaFlags||ancestor to be retrieved
);

// @pyswig int|GetWindowDC|returns the device context (DC) for the entire window, including title bar, menus, and scroll
// bars.
HDC GetWindowDC(HWND hWnd  // @pyparm int|hWnd||handle of window
);
...
BOOL IsChild(HWND hWndParent,  // @pyparm int|hWndParent||handle to parent window
...
// @pyswig |_TrackMouseEvent|Posts messages when the mouse pointer leaves a window or hovers over a window for a
...
int ReleaseDC(HWND hWnd,  // @pyparm int|hWnd||handle to window
              HDC hDC     // @pyparm int|hDC||handle to device context
);

// @pyswig |CreateCaret|Creates a new caret for a window
BOOLAPI CreateCaret(HWND hWnd,        // @pyparm int|hWnd||handle to owner window
...
int_regiontype ScrollWindowEx(
    ...
    HRGN INPUT_NULLOK,   // @pyparm <o PyGdiHandle>|hrgnUpdate||Handle to region which will be updated with area
                         // invalidated by scroll operation, can be None
    RECT *OUTPUT,
    UINT flags  // @pyparm int|flags||Scrolling flags, combination of
                // SW_ERASE,SW_INVALIDATE,SW_SCROLLCHILDREN,SW_SMOOTHSCROLL.
...
#define RETURN_ERR(err)                        \
    do {                                       \
        PyErr_SetString(ui_module_error, err); \
        return NULL;                           \
    } while (0)
#define RETURN_MEM_ERR(err)                      \
    do {                                         \
...
static PyObject *PySetScrollInfo(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OiO|i:SetScrollInfo", &obhwnd, &nBar, &obInfo, &bRedraw))
        return NULL;
    if (!PyWinObject_AsHANDLE(obhwnd, (HANDLE *)&hwnd))
        return NULL;
    SCROLLINFO info;
...
static PyObject *PyGetScrollInfo(PyObject *self, PyObject *args)
{
    HWND hwnd;
    PyObject *obhwnd;
    int nBar;
...
static PyObject *PyGetClassName(PyObject *self, PyObject *args)
{
    HWND hwnd;
    PyObject *obhwnd;
    TCHAR buf[MAX_CHARS];
...
static PyObject *PyRealGetWindowClass(PyObject *self, PyObject *args)
{
    HWND hwnd;
    PyObject *obhwnd;
    TCHAR buf[MAX_CHARS];
...
// @pyswig int|ChildWindowFromPoint|Determines which, if any, of the child windows belonging to a parent window contains
// the specified point.
...
static int CALLBACK PySortFunc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
    int rc = 0;
    PyObject *result = NULL;
    PyObject *args = NULL;
...
static PyObject *PyListView_SortItems(PyObject *self, PyObject *args)
{
    ...
    if (!PyCallable_Check(ob))
        return PyErr_Format(PyExc_TypeError, "2nd param must be callable (got type %s)", Py_TYPE(ob)->tp_name);
...
%{
// @pyswig |ListView_SortItemsEx|Uses an application-defined comparison function to sort the items of a list view
// control.
...
PyObject *PyReturn_OPENFILENAMEW_Output(OPENFILENAMEW *pofn)
{
    ...
    if (pofn->Flags & OFN_ALLOWMULTISELECT) {
        for (filechars = 0; filechars < pofn->nMaxFile - 1 &&
                            !(pofn->lpstrFile[filechars] == 0 && pofn->lpstrFile[filechars + 1] == 0);
             filechars++);
    }
    else {
    ...
    if (pofn->lpstrCustomFilter == NULL)
        return Py_BuildValue("NOk", PyWinObject_FromWCHAR(pofn->lpstrFile, filechars), Py_None, pofn->Flags);
    ...
    for (filterchars = pofn->nMaxCustFilter; filterchars > 0; filterchars--)
        if (pofn->lpstrCustomFilter[filterchars - 1] != 0)
            break;
    return Py_BuildValue("NNk", PyWinObject_FromWCHAR(pofn->lpstrFile, filechars),
                         // include trailing NULL so returned value can be passed back in as a filter unmodified
                         PyWinObject_FromWCHAR(pofn->lpstrCustomFilter, filterchars + 1), pofn->Flags);
}
%}
...
// First is the selected file(s). If multiple files are selected, returned string will be the directory followed by
// files names separated by nulls, otherwise it will be the full path.  In other words, if you use the
// OFN_ALLOWMULTISELECT flag you should split this value on \0 characters and if the length of the result list is 1, it
// will be the full path, otherwise element 0 will be the directory and the rest of the elements will be filenames in
// this directory.<nl>
...
// win32gui.error is raised.  If the user pressed cancel, the error number (ie, the winerror attribute of the exception)
// will be zero.
...
// @pyparm string|Filter|None|Contains pairs of descriptions and filespecs separated by NULLS, with a final trailing
// NULL. Example: 'Python Scripts\0*.py;*.pyw;*.pys\0Text files\0*.txt\0'
// @pyparm string|CustomFilter|None|Description to be used for filter that user selected or typed, can also contain a
// filespec as above
...
// @pyparm int|MaxFile|1024|Number of characters to allocate for selected filename(s), override if large number of files
// expected
...
{
    BOOL ret = FALSE;
    static char *keywords[] = {"hwndOwner",  "hInstance", "Filter", "CustomFilter", "FilterIndex",  "File", "MaxFile",
                               "InitialDir", "Title",     "Flags",  "DefExt",       "TemplateName", NULL};
    PyObject *obFilter = Py_None, *obCustomFilter = Py_None, *obFile = Py_None, *obInitialDir = Py_None,
             *obTitle = Py_None, *obDefExt = Py_None, *obTemplateName = Py_None, *obOwner = Py_None,
             *obhInstance = Py_None;
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "|OOOOlOlOOlOO:OPENFILENAME", keywords,
            ...
            &obFilter,  // @pyparm string|Filter|None|Contains pairs of descriptions and filespecs separated by NULLS,
                        // with a final trailing NULL. Example: 'Python Scripts\0*.py;*.pyw;*.pys\0Text files\0*.txt\0'
            &obCustomFilter,  // @pyparm string|CustomFilter|None|Description to be used for filter that user selected
                              // or typed, can also contain a filespec as above
            &pofn->nFilterIndex,  // @pyparm int|FilterIndex|0|Specifies which of the filters is initially selected, use
                                  // 0 for CustomFilter
            &obFile,              // @pyparm string|File|None|The file name initially displayed
            &pofn->nMaxFile,      // @pyparm int|MaxFile|1024|Number of characters to allocate for selected filename,
                                  // override if large number of files expected
...
// @pyswig |SystemParametersInfo|Queries or sets system-wide parameters. This function can also update the user profile
// while setting a parameter.
// @rdesc SPI_SET functions all return None on success.  Types returned by SPI_GET functions are dependent on the
// operation
...
BOOL PyWinObject_AsNONCLIENTMETRICS(PyObject *ob, NONCLIENTMETRICS *ncm)
{
    static char *keywords[] = {
        ...
        "lfCaptionFont", "iSmCaptionWidth", "iSmCaptionHeight", "lfSmCaptionFont", "iMenuWidth",
        "iMenuHeight",   "lfMenuFont",      "lfStatusFont",     "lfMessageFont",   NULL};
    BOOL ret;
    ...
    ret = PyArg_ParseTupleAndKeywords(
        dummy_args, ob, "iiiiiO&iiO&iiO&O&O&:NONCLIENTMETRICS", keywords, &ncm->iBorderWidth, &ncm->iScrollWidth,
        &ncm->iScrollHeight, &ncm->iCaptionWidth, &ncm->iCaptionHeight, PyWinObject_AsLOGFONT, &ncm->lfCaptionFont,
        &ncm->iSmCaptionWidth, &ncm->iSmCaptionHeight, PyWinObject_AsLOGFONT, &ncm->lfSmCaptionFont, &ncm->iMenuWidth,
        &ncm->iMenuHeight, PyWinObject_AsLOGFONT, &ncm->lfMenuFont, PyWinObject_AsLOGFONT, &ncm->lfStatusFont,
...
BOOL PyWinObject_AsMINIMIZEDMETRICS(PyObject *ob, MINIMIZEDMETRICS *mm)
{
    ...
    if (dummy_args == NULL)  // should not happen, interpreter apparently caches the empty tuple
        return FALSE;
    ret = PyArg_ParseTupleAndKeywords(dummy_args, ob, "iiii:MINIMIZEDMETRICS", keywords, &mm->iWidth, &mm->iHorzGap,
                                      &mm->iVertGap, &mm->iArrange);
    Py_DECREF(dummy_args);
    return ret;
}
...
static PyObject *PySystemParametersInfo(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            ...
            &WinIni))  // @pyparm int|WinIni|0|Flags specifying whether change should be permanent, and if all windows
                       // should be notified of change. Combination of SPIF_UPDATEINIFILE, SPIF_SENDCHANGE,
    ...
    switch (Action) {
        ...
        // @flag SPI_SETMOUSETRAILS|Param should be an int specifying the nbr of cursors in the trail (0 or 1 means
        // disabled)
        ...
        // @flag SPI_SETDOUBLECLICKTIME|Param is an int (in milliseconds),  Use <om win32gui.GetDoubleClickTime> to
        // retrieve the value.
        case SPI_SETDOUBLECLICKTIME:
        // @flag SPI_SETDOUBLECLKWIDTH|Param is an int.  Use win32api.GetSystemMetrics(SM_CXDOUBLECLK) to retrieve the
        // value.
        case SPI_SETDOUBLECLKWIDTH:
        // @flag SPI_SETDOUBLECLKHEIGHT|Param is an int,  Use win32api.GetSystemMetrics(SM_CYDOUBLECLK) to retrieve the
        // value.
        ...
        // @flag SPI_GETMOUSETRAILS|Returns an int specifying the nbr of cursor images in the trail, 0 or 1 indicates
        // disabled
        ...
        // @flag SPI_SETLANGTOGGLE|Param is ignored. Sets the language toggle hotkey from registry key HKCU\keyboard
        // layout\toggle
        ...
        // @flag SPI_GETMOUSE|Returns a tuple of 3 ints containing the x and y mouse thresholds and the acceleration
        // factor.
        case SPI_GETMOUSE:
        ...
        case SPI_SETMOUSE: {
            ...
            if (Action == SPI_SETMOUSE) {
                ...
                if (!PyArg_ParseTuple(param_tuple, "kkk", &((UINT *)pvParam)[0], &((UINT *)pvParam)[1],
                                      &((UINT *)pvParam)[2])) {
                    Py_DECREF(param_tuple);
                    goto done;
                }
        ...
        case SPI_SETANIMATION:
            ...
            break;
        // @flag SPI_ICONHORIZONTALSPACING|Functions as both a get and set operation.  If Param is None, functions as a
        // get operation, otherwise Param is an int to be set as the new value
        case SPI_ICONHORIZONTALSPACING:
        // @flag SPI_ICONVERTICALSPACING|Functions as both a get and set operation.  If Param is None, functions as a
        // get operation, otherwise Param is an int to be set as the new value
        ...
        // @flag SPI_SETNONCLIENTMETRICS|Param is a dict in the form of a NONCLIENTMETRICS struct, as returned by
        // SPI_GETNONCLIENTMETRICS operation
        case SPI_SETNONCLIENTMETRICS:
            ...
            if (Action == SPI_GETNONCLIENTMETRICS) {
                if (obParam != Py_None) {
                    PyErr_Format(PyExc_ValueError, "Don't supply a param for SPI_GETNONCLIENTMETRICS");
                    goto done;
                }
                memset(pvParam, 0, buflen);
                ((NONCLIENTMETRICS *)pvParam)->cbSize = buflen;
            }
            else if (!PyWinObject_AsNONCLIENTMETRICS(obParam, (NONCLIENTMETRICS *)pvParam))
                goto done;
            break;

        // @flag SPI_GETMINIMIZEDMETRICS|Returns a dict representing a MINIMIZEDMETRICS struct.  Param is not used.
        case SPI_GETMINIMIZEDMETRICS:
        // @flag SPI_SETMINIMIZEDMETRICS|Param should be a MINIMIZEDMETRICS dict as returned by SPI_GETMINIMIZEDMETRICS
        // action
        case SPI_SETMINIMIZEDMETRICS:
            ...
            if (Action == SPI_GETMINIMIZEDMETRICS) {
                if (obParam != Py_None) {
                    PyErr_Format(PyExc_ValueError, "Don't supply a param for SPI_GETMINIMIZEDMETRICS");
                    goto done;
                }
                memset(pvParam, 0, buflen);
                ((MINIMIZEDMETRICS *)pvParam)->cbSize = buflen;
            }
            else if (!PyWinObject_AsMINIMIZEDMETRICS(obParam, (MINIMIZEDMETRICS *)pvParam))
                goto done;
            break;
    ...
    switch (Action) {
        ...
        case SPI_GETNONCLIENTMETRICS: {
            ...
            ret = Py_BuildValue("{s:i,s:i,s:i,s:i,s:i,s:N,s:i,s:i,s:N,s:i,s:i,s:N,s:N,s:N}", "iBorderWidth",
                                p->iBorderWidth, "iScrollWidth", p->iScrollWidth, "iScrollHeight", p->iScrollHeight,
                                "iCaptionWidth", p->iCaptionWidth, "iCaptionHeight", p->iCaptionHeight, "lfCaptionFont",
                                new PyLOGFONT(&p->lfCaptionFont), "iSmCaptionWidth", p->iSmCaptionWidth,
                                "iSmCaptionHeight", p->iSmCaptionHeight, "lfSmCaptionFont",
                                new PyLOGFONT(&p->lfSmCaptionFont), "iMenuWidth", p->iMenuWidth, "iMenuHeight",
                                p->iMenuHeight, "lfMenuFont", new PyLOGFONT(&p->lfMenuFont), "lfStatusFont",
                                new PyLOGFONT(&p->lfStatusFont), "lfMessageFont", new PyLOGFONT(&p->lfMessageFont));
            break;
        }
        case SPI_GETMINIMIZEDMETRICS: {
            MINIMIZEDMETRICS *p = (MINIMIZEDMETRICS *)pvParam;
            ret = Py_BuildValue("{s:i,s:i,s:i,s:i}", "iWidth", p->iWidth, "iHorzGap", p->iHorzGap, "iVertGap",
                                p->iVertGap, "iArrange", p->iArrange);
            break;
        }
...
PyObject *PySetLayeredWindowAttributes(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    PyObject *obhwnd;
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "Okbk:SetLayeredWindowAttributes", keywords,
...
%{
// @pyswig (int,int,int)|GetLayeredWindowAttributes|Retrieves the layering parameters of a window with the WS_EX_LAYERED
// extended style
// @comm Accepts keyword arguments.
...
{
    static char *keywords[] = {"hwnd", "hdcDst", "ptDst", "size", "hdcSrc", "ptSrc", "Key", "blend", "Flags", NULL};
    HWND hwnd;
    HDC hdcDst, hdcSrc;
    ...
    DWORD Flags = 0;
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "O|OOOOOkOk:UpdateLayeredWindow", keywords,
            &obhwnd,  // @pyparm <o PyHANDLE>|hwnd||handle to layered window
            &obdst,   // @pyparm <o PyHANDLE>|hdcDst|None|handle to screen DC, can be None.  *Must* be None if hdcSrc is
                      // None
            &obptDst,  // @pyparm (x,y)|ptDst|None|New screen position, can be None.
            &obsize,   // @pyparm (cx, cy)|size|None|New size of the layered window, can be None.  *Must* be None if
                       // hdcSrc is None.
            ...
            &obblend,  // @pyparm (int, int, int, int)|blend|(0,0,255,0)|<o PyBLENDFUNCTION> specifying alpha blending
                       // parameters
    ...
    BOOL ret;
    Py_BEGIN_ALLOW_THREADS ret = UpdateLayeredWindow(hwnd, hdcDst, pptDst, psize, hdcSrc, pptSrc, crKey, &blend, Flags);
    Py_END_ALLOW_THREADS if (!ret) return PyWin_SetAPIError("UpdateLayeredWindow");
    Py_INCREF(Py_None);
    return Py_None;
}
...
%{
// @pyswig |AnimateWindow|Enables you to produce special effects when showing or hiding windows. There are three types
// of animation: roll, slide, and alpha-blended fade.
...
PyObject *PyAnimateWindow(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyWinObject_AsHANDLE(obhwnd, (HANDLE *)&hwnd))
        return NULL;
    BOOL ret;
    Py_BEGIN_ALLOW_THREADS ret = AnimateWindow(hwnd, duration, flags);
    Py_END_ALLOW_THREADS if (!ret) return PyWin_SetAPIError("AnimateWindow");
    Py_INCREF(Py_None);
    return Py_None;
}
...
%{
// @object PyLOGBRUSH|Dict representing a LOGBRUSH struct as used with <om win32gui.CreateBrushIndirect> and <om
...
BOOL PyWinObject_AsLOGBRUSH(PyObject *oblb, LOGBRUSH *plb)
{
    ...
    if (dummy_tuple == NULL)
        return FALSE;
    BOOL ret = PyArg_ParseTupleAndKeywords(
                   dummy_tuple, oblb, "kkO", keywords,
                   &plb->lbStyle,  // @prop int|Style|Brush style, one of win32con.BS_* values
                   &plb->lbColor,  // @prop int|Color|RGB color value.  Can also be DIB_PAL_COLORS or DIB_RGB_COLORS if
                                   // Style is BS_DIBPATTERN or BS_DIBPATTERNPT=
                   &obhatch)  // @prop int/<o PyHANDLE>|Hatch|For BS_HATCH style, one of win32con.HS_*. Not used For
                              // BS_SOLID or BS_HOLLOW. For a pattern brush, this should be a handle to a bitmap
...
static PyObject *PyExtCreatePen(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "kkO|O:ExtCreatePen",
                          &style,  // @pyparm int|PenStyle||Combination of win32con.PS_*.  Must contain either
                                   // PS_GEOMETRIC or PS_COSMETIC.
                          &width,  // @pyparm int|Width||Width of pen in logical units.  Must be 1 for PS_COSMETIC.
                          &oblb,   // @pyparm <o PyLOGBRUSH>|lb||Dict containing brush creation parameters
                          &obcustom_style))  // @pyparm (int, ...)|Style|None|Sequence containing lengths of dashes and
                                             // spaces  Used only with PS_USERSTYLE, otherwise must be None.
        return NULL;
    if (!PyWinObject_AsLOGBRUSH(oblb, &lb))
        return NULL;
...
PyObject *PyDrawTextW(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "OOiOI:DrawTextW", keywords,
    ...
    return Py_BuildValue("iN", height, PyWinObject_FromRECT(&rc));
}
...
BOOL CALLBACK PyEnumPropsExCallback(HWND hwnd, LPWSTR propname, HANDLE propdata, ULONG_PTR callback_data)
{
    ...
    args = Py_BuildValue("NNNO", PyWinLong_FromHANDLE(hwnd),
                         IS_INTRESOURCE(propname) ? PyWinLong_FromVoidPtr(propname) : PyWinObject_FromWCHAR(propname),
                         PyWinLong_FromHANDLE(propdata), callback_objects[1]);
    if (args == NULL)
        return FALSE;
...
PyObject *PyEnumPropsEx(PyObject *self, PyObject *args)
{
    ...
    BOOL ok;
    Py_BEGIN_ALLOW_THREADS ok = EnumPropsExW(hwnd, PyEnumPropsExCallback, (LPARAM)callback_objects);
...
%{
// @pyswig <o PyHDEVNOTIFY>|RegisterDeviceNotification|Registers the device or type of device for which a window will
...
PyObject *PyRegisterDeviceNotification(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OOk:RegisterDeviceNotification",
                          ...
                          &obFilter,  // @pyparm buffer|filter||A buffer laid out like one of the DEV_BROADCAST_*
                                      // structures, generally built by one of the win32gui_struct helpers.
    ...
    HDEVNOTIFY not ;
    Py_BEGIN_ALLOW_THREADS not = RegisterDeviceNotification(handle, pybuf.ptr(), flags);
    Py_END_ALLOW_THREADS if (not == NULL) return PyWin_SetAPIError("RegisterDeviceNotification");

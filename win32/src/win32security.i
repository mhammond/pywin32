...
// @object PyLSA_HANDLE|Object representing an Lsa policy handle (LSA_HANDLE), created by <om
...
class PyLSA_HANDLE : public PyHANDLE {
   public:
    ...
    virtual const char *GetTypeName() { return "PyLSA_HANDLE"; }
};

// @object PyLsaLogon_HANDLE|Lsa handle used to access authentication packages, returned by
//   <om win32security.LsaRegisterLogonProcess> or <om win32security.LsaConnectUntrusted>. Base low-level object is a
//   plain HANDLE. Inherits all properties and methods of <o PyHANDLE>, but Close uses LsaDeregisterLogonProcess
class PyLsaLogon_HANDLE : public PyHANDLE {
   public:
    PyLsaLogon_HANDLE(HANDLE hInit) : PyHANDLE(hInit) {}
    virtual BOOL Close(void)
    {
        NTSTATUS err = m_handle ? LsaDeregisterLogonProcess(m_handle) : STATUS_SUCCESS;
        m_handle = 0;
        if (err != STATUS_SUCCESS)
            PyWin_SetAPIError("LsaDeregisterLogonProcess", LsaNtStatusToWinError(err));
        return err == STATUS_SUCCESS;
    }
    virtual const char *GetTypeName() { return "PyLsaLogon_HANDLE"; }
};

BOOL PyWinObject_AsLSA_HANDLE(PyObject *ob, LSA_HANDLE *pRes, BOOL bNoneOK = FALSE);
PyObject *PyWinObject_FromLSA_HANDLE(LSA_HANDLE h) { return new PyLSA_HANDLE(h); }

BOOL PyWinObject_CloseLSA_HANDLE(PyObject *obHandle)
{
    if (PyHANDLE_Check(obHandle)) {
        // If it's a PyHANDLE, make sure it's the right type, since any other handle's Close method could be called
        // successfully
...
BOOL PyWinObject_CloseLsaLogon_HANDLE(PyObject *obHandle)
{
    if (PyHANDLE_Check(obHandle)) {
        ...
        if (strcmp(((PyHANDLE *)obHandle)->GetTypeName(), "PyLsaLogon_HANDLE") != 0) {
            PyErr_SetString(PyExc_TypeError,
                            "PyHANDLE passed to LsaDeregisterLogonProcess must be a PyLsaLogon_HANDLE");
            return FALSE;
        }
        return ((PyHANDLE *)obHandle)->Close();
    }
    ...
    NTSTATUS err = LsaDeregisterLogonProcess(lsahandle);
    if (err == STATUS_SUCCESS)
        return TRUE;
    PyWin_SetAPIError("LsaDeregisterLogonProcess", LsaNtStatusToWinError(err));
    return FALSE;
...
// @object PyTOKEN_GROUPS|A sequence of <o PySID_AND_ATTRIBUTES> sequences, eg [(<o PySID>,int),...] representing a
...
void PyWinObject_FreeTOKEN_PRIVILEGES(TOKEN_PRIVILEGES *pPriv) { free(pPriv); }
%}
...
for (PyMethodDef *pmd = win32securityMethods; pmd->ml_name; pmd++)
    if ((strcmp(pmd->ml_name, "DsGetDcName") == 0) || (strcmp(pmd->ml_name, "DuplicateTokenEx") == 0) ||
        (strcmp(pmd->ml_name, "AdjustTokenPrivileges") == 0) || (strcmp(pmd->ml_name, "AdjustTokenGroups") == 0) ||
        (strcmp(pmd->ml_name, "CreateRestrictedToken") == 0) || (strcmp(pmd->ml_name, "LsaAddAccountRights") == 0) ||
        (strcmp(pmd->ml_name, "LsaRemoveAccountRights") == 0) || (strcmp(pmd->ml_name, "LogonUser") == 0) ||
...
%native(DsGetSpn) PyDsGetSpn;
    // @pyswig (<o PyUnicode>,...)|DsGetSpn|Compose one or more service principal names to be registered using <om
    ...
    // @pyparm (<o PyUnicode>,...)|InstanceNames|None|A sequence of service instance names, can be None - not required
    // for for host Spn's
    // @pyparm (int,...)|InstancePorts|None|A sequence of extra instance ports.  If specified, must be same length as
    // InstanceNames.
...
%native (DsGetDcName) PYDSGETDCNAME;
    ...
    // You can supply DC selection criteria to this function to indicate preference for a DC with particular
    // characteristics.
...
%native (DsCrackNames) extern PyObject *PyDsCrackNames(PyObject *self, PyObject *args);
    // @pyswig [ (status, domain, name) ]|DsCrackNames|Converts an array of directory service object names from one
    // format to another.
...
BOOLAPI ImpersonateNamedPipeClient(PyHANDLE hNamedPipe  // @pyparm int|handle||handle of a named pipe.
);
...
%{
// @pyswig <o PyHANDLE>|LogonUser|Attempts to log a user on to the local computer, that is, to the computer from which
// LogonUser was called. You cannot use LogonUser to log on to a remote computer.
...
PyObject *PyLogonUser(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "OOOkk:LogonUser", keywords,
            ...
            &obpassword,  // @pyparm <o PyUnicode>|Password||User's password.  Use a blank string if Username contains a
                          // marshalled credential.
    ...
    if (PyWinObject_AsWCHAR(obusername, &username, FALSE) && PyWinObject_AsWCHAR(obdomain, &domain, TRUE) &&
        PyWinObject_AsWCHAR(obpassword, &password, FALSE)) {
        BOOL ok;
        DWORD err;
        Py_BEGIN_ALLOW_THREADS ok = LogonUser(username, domain, password, logontype, logonprovider, &htoken);
        // Capture error before Py_END_ALLOW_THREADS reacquires the GIL,
        // which may call Win32 functions that overwrite GetLastError().
        if (!ok)
            err = GetLastError();
        Py_END_ALLOW_THREADS if (!ok) PyWin_SetAPIError("LogonUser", err);
        else ret = PyWinObject_FromHANDLE(htoken);
    }
...
{
    return Py_BuildValue("{s:N, s:N, s:N, s:N, s:N, s:N}", "PagedPoolLimit",
                         PyLong_FromUnsignedLongLong(pql->PagedPoolLimit), "NonPagedPoolLimit",
                         PyLong_FromUnsignedLongLong(pql->NonPagedPoolLimit), "MinimumWorkingSetSize",
                         PyLong_FromUnsignedLongLong(pql->MinimumWorkingSetSize), "MaximumWorkingSetSize",
                         PyLong_FromUnsignedLongLong(pql->MaximumWorkingSetSize), "PagefileLimit",
                         PyLong_FromUnsignedLongLong(pql->PagefileLimit), "TimeLimit",
                         PyLong_FromUnsignedLongLong(pql->TimeLimit.QuadPart));
}
...
PyObject *PyLogonUserEx(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    static char *keywords[] = {"Username", "Domain", "Password", "LogonType", "LogonProvider", NULL};
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "OOOkk:LogonUserEx", keywords,
    ...
    if (PyWinObject_AsWCHAR(obusername, &username, FALSE) && PyWinObject_AsWCHAR(obdomain, &domain, TRUE) &&
        PyWinObject_AsWCHAR(obpassword, &password, FALSE)) {
        if (!LogonUserEx(username, domain, password, logontype, logonprovider, &htoken, &psid, &profile, &profilelen,
                         &quota_limits))
            PyWin_SetAPIError("LogonUserEx");
        else
            ret = Py_BuildValue("NNNN", PyWinObject_FromHANDLE(htoken), PyWinObject_FromSID(psid),
...
// @pyswig <o PySID>, string, int|LookupAccountName|Accepts the name of a system and an account as input. It retrieves a
// security identifier (SID) for the account and the name of the domain on which the account was found.
// @rdesc The result is a tuple of new SID object, the domain name where the account was found, and the type of account
// the SID is for.
...
PyObject *LookupAccountName(PyObject *self, PyObject *args)
{
    ...
    // Get the SID size.
    Py_BEGIN_ALLOW_THREADS LookupAccountName(szSystemName, szAcctName, pSid, &sidSize, refDomain, &refDomainSize,
                                             &sidType);
...
// @pyswig string, string, int|LookupAccountSid|Accepts a security identifier (SID) as input. It retrieves the name of
// the account for this SID and the name of the first domain on which this SID is found.
// @rdesc The result is a tuple of the name, the domain name where the account was found, and the type of account the
// SID is for.
...
PyObject *LookupAccountSid(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS ok =
        LookupAccountSid(szSystemName, pSid, szRetAcctName, &retAcctNameSize, refDomain, &refDomainSize, &sidType);
    Py_END_ALLOW_THREADS
...
GetBinarySid() accepts a buffer that contains the textual representation of a SID.This function returns NULL
    if it fails.If the SID can be constructed successfully,
    a valid binary SID is returned.*/ PSID GetBinarySid(LPTSTR TextualSid  // Buffer for Textual representation of SID.
                                      )
{
    PSID pSid = 0;
    ...
    *ptr1 = 0;
    ...
    if ((*ptr == '0') && (*(ptr + 1) == 'x')) {
        swscanf(ptr, _T("0x%02hx%02hx%02hx%02hx%02hx%02hx"), &identAuthority.Value[0], &identAuthority.Value[1],
                &identAuthority.Value[2], &identAuthority.Value[3], &identAuthority.Value[4], &identAuthority.Value[5]);
    }
    else {
        DWORD value;
    ...
    *ptr1 = '-';
    ...
    if (!AllocateAndInitializeSid(&identAuthority, nByteAuthorityCount, dwSubAuthority[0], dwSubAuthority[1],
                                  dwSubAuthority[2], dwSubAuthority[3], dwSubAuthority[4], dwSubAuthority[5],
                                  dwSubAuthority[6], dwSubAuthority[7], &pSid)) {
        pSid = 0;
    }
...
static PyObject *PyGetBinarySid(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O:GetBinarySid", &obTextualSid))
        return NULL;
...
PyObject *SetNamedSecurityInfo(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS err = SetNamedSecurityInfo(ObjectName, ObjectType, info, pSidOwner, pSidGroup, pDacl, pSacl);
    Py_END_ALLOW_THREADS if (err == ERROR_SUCCESS)
    {
        Py_INCREF(Py_None);
        ret = Py_None;
    }
    else PyWin_SetAPIError("SetNamedSecurityInfo", err);
...
static PyObject *PyGetNamedSecurityInfo(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS err =
        GetNamedSecurityInfoW(ObjectName, object_type, required_info, NULL, NULL, NULL, NULL, &pSD);
    Py_END_ALLOW_THREADS if (err == ERROR_SUCCESS)
    {
    ...
    else PyWin_SetAPIError("GetNamedSecurityInfo", err);
    PyWinObject_FreeWCHAR(ObjectName);
    if (pSD)
        LocalFree(pSD);
    return ret;
...
BOOLAPI OpenProcessToken(PyHANDLE ProcessHandle,  // @pyparm int|processHandle||The handle of the process to open.
...
PyObject *LookupPrivilegeName(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OO:LookupPrivilegeName",
                          &obsystem_name,  // @pyparm string/<o PyUnicode>|SystemName||System name, local system assumed
                                           // if not specified
...
PyObject *LookupPrivilegeDisplayName(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OO:LookupPrivilegeDisplayName",
                          &obsystem_name,  // @pyparm string/<o PyUnicode>|SystemName||System name, local system assumed
                                           // if not specified
                          &obpriv_name))  // @pyparm string/<o PyUnicode>|Name||Name of privilege, Se...Privilege string
                                          // constants (win32security.SE_*_NAME)
        return NULL;
...
static PyObject *PyAdjustTokenPrivileges(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "OlO:AdjustTokenPrivileges", keywords,
            ...
            &obNewState))           // @pyparm <o PyTOKEN_PRIVILEGES>|NewState||The new state, can be None if
                                    // bDisableAllPrivileges is True
        return NULL;
    ...
    if (!AdjustTokenPrivileges(TokenHandle, DisableAllPrivileges, NewState, origbufsize, PreviousState, &reqdbufsize))
        if (reqdbufsize > origbufsize) {
            ...
            AdjustTokenPrivileges(TokenHandle, DisableAllPrivileges, NewState, reqdbufsize, PreviousState,
                                  &reqdbufsize);
        }
...
static PyObject *PyAdjustTokenGroups(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "OiO:AdjustTokenGroups", keywords,
    ...
    if (oldstate == NULL) {
        PyErr_Format(PyExc_MemoryError, "AdjustTokenGroups: unable to allocate %d SID_AND_ATTRIBUTES structs",
                     origgroupcnt);
        ok = FALSE;
    }
    else {
...
// @pyswig object|GetTokenInformation|Retrieves a specified type of information about an access token. The calling
...
static PyObject *PyGetTokenInformation(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            args, "Ol:GetTokenInformation",
            &obHandle,  // @pyparm <o PyHANDLE>|TokenHandle||Handle to an access token.
            &typ))  // @pyparm int|TokenInformationClass||Specifies a value from the TOKEN_INFORMATION_CLASS enumerated
                    // type identifying the type of information the function retrieves.
        return NULL;
    HANDLE handle;
    if (!PyWinObject_AsHANDLE(obHandle, &handle))
        return NULL;
    ...
    switch (typ) {
        ...
        case TokenElevationType:  // @flag TokenElevationType|int - TokenElevation* value indicating what type of token
                                  // is linked to
    ...
    switch (typ) {
        ...
        case TokenSource: {
            ...
            ret = Py_BuildValue("NN", PyBytes_FromStringAndSize(ts->SourceName, 8),
        ...
        case TokenStatistics: {
            ...
            ret = Py_BuildValue("{s:N,s:N,s:N,s:l,s:l,s:l,s:l,s:l,s:l,s:N}", "TokenId",
                                PyWinObject_FromLARGE_INTEGER(*((LARGE_INTEGER *)&pts->TokenId)), "AuthenticationId",
                                PyWinObject_FromLARGE_INTEGER(*((LARGE_INTEGER *)&pts->AuthenticationId)),
                                "ExpirationTime", PyWinObject_FromTimeStamp(pts->ExpirationTime), "TokenType",
                                pts->TokenType, "ImpersonationLevel", pts->ImpersonationLevel, "DynamicCharged",
                                pts->DynamicCharged, "DynamicAvailable", pts->DynamicAvailable, "GroupCount",
                                pts->GroupCount, "PrivilegeCount", pts->PrivilegeCount, "ModifiedId",
                                PyWinObject_FromLARGE_INTEGER(*((LARGE_INTEGER *)&pts->ModifiedId)));
            break;
        }
        case TokenOrigin: {
        ...
        case TokenIntegrityLevel: {
            ...
            ret = Py_BuildValue("Nk", PyWinObject_FromSID(tml->Label.Sid), tml->Label.Attributes);
            break;
        }
...
BOOLAPI OpenThreadToken(PyHANDLE ThreadHandle,  // @pyparm <o PyHANDLE>|handle||handle to thread
...
// @pyswig <o PySECURITY_DESCRIPTOR>|GetFileSecurity|Obtains specified information about the security of a file or
// directory. The information obtained is constrained by the caller's access rights and privileges.
...
static PyObject *MyGetFileSecurity(PyObject *self, PyObject *args)
{
    ...
    // @pyparm int|info|OWNER_SECURITY_INFORMATION \| GROUP_SECURITY_INFORMATION \| DACL_SECURITY_INFORMATION \|
    // SACL_SECURITY_INFORMATION|Flags that specify the information requested.
...
// @pyswig |SetFileSecurity|Sets information about the security of a file or directory. The information obtained is
...
// @pyswig <o PySECURITY_DESCRIPTOR>|GetUserObjectSecurity|Obtains specified information about the security of a user
// object. The information obtained is constrained by the caller's access rights and privileges.
...
// @pyswig |SetUserObjectSecurity|Sets information about the security of a user object. The information obtained is
// constrained by the caller's access rights and privileges.
...
// @pyswig <o PySECURITY_DESCRIPTOR>|GetKernelObjectSecurity|Obtains specified information about the security of a
// kernel object. The information obtained is constrained by the caller's access rights and privileges.
...
// @pyswig |SetKernelObjectSecurity|Sets information about the security of a kernel object. The information obtained is
// constrained by the caller's access rights and privileges.
...
static PyObject *PySetTokenInformation(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            args, "OiO:SetTokenInformation",
            &obth,  // @pyparm <o PyHANDLE>|TokenHandle||Handle to an access token to be modified
            &typ,   // @pyparm int|TokenInformationClass||Specifies a value from the TOKEN_INFORMATION_CLASS enumerated
                    // type identifying the type of information to be modfied
    ...
    switch (typ) {
        ...
        case TokenVirtualizationAllowed:  // @flag TokenVirtualizationAllowed|Boolean
            ...
            *(DWORD *)buf = PyLong_AsUnsignedLong(obinfo);
            ...
            break;
        case TokenIntegrityLevel:  // @flag TokenIntegrityLevel|<o PySID_AND_ATTRIBUTES> containing an integrity SID and
                                   // SE_GROUP_INTEGRITY flag
...
static PyObject *PyLsaOpenPolicy(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "Oi:LsaOpenPolicy",
                          &obsystem_name,  // @pyparm string/<o PyUnicode>|system_name||System name, local system
    ...
    Py_BEGIN_ALLOW_THREADS ntsResult = LsaOpenPolicy(&system_name, &ObjectAttributes, access_mask, &lsahPolicyHandle);
    Py_END_ALLOW_THREADS if (ntsResult != STATUS_SUCCESS)
        PyWin_SetAPIError("LsaOpenPolicy", LsaNtStatusToWinError(ntsResult));
    else ret = PyWinObject_FromLSA_HANDLE(lsahPolicyHandle);
...
static PyObject *PyLsaQueryInformationPolicy(PyObject *self, PyObject *args)
{
    ...
    switch (info_class) {
        case PolicyAuditEventsInformation: {
            ...
            //   of POLICY_AUDIT_EVENT_UNCHANGED, POLICY_AUDIT_EVENT_SUCCESS, POLICY_AUDIT_EVENT_FAILURE,
        ...
        case PolicyLsaServerRoleInformation: {
            // @flag PolicyLsaServerRoleInformation|Returns an int, one of PolicyServerRoleBackup,
...
static PyObject *PyLsaAddAccountRights(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOO:LsaAddAccountRights", keywords, &policy_handle, &obsid, &privs))
        return NULL;
    ...
    if (plsau_start == NULL) {
        PyErr_Format(PyExc_MemoryError, "LsaAddAccountRights: Unable to allocate %d bytes",
                     priv_cnt * sizeof(LSA_UNICODE_STRING));
        goto done;
    }
    ...
    Py_BEGIN_ALLOW_THREADS err = LsaAddAccountRights(hpolicy, psid, plsau_start, priv_cnt);
    Py_END_ALLOW_THREADS if (err != STATUS_SUCCESS)
    {
...
static PyObject *PyLsaRemoveAccountRights(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOiO:LsaAddAccountRights", keywords, &policy_handle, &obsid,
                                     &AllRights, &privs))
        return NULL;
    if (!PyWinObject_AsHANDLE(policy_handle, &hpolicy))
        return NULL;
    ...
    plsau_start = (PLSA_UNICODE_STRING)calloc(priv_cnt, sizeof(LSA_UNICODE_STRING));
    if (plsau_start == NULL) {
        PyErr_Format(PyExc_MemoryError, "LsaRemoveAccountRights: Unable to allocate %d bytes",
                     priv_cnt * sizeof(LSA_UNICODE_STRING));
        goto done;
    }
...
static PyObject *PyLsaEnumerateAccountsWithUserRight(PyObject *self, PyObject *args)
{
    ...
    if (err == STATUS_SUCCESS) {
        ...
        // real error code is STATUS_NO_MORE_ENTRIES, which is only defined in ntstatus.h that only comes with Driver
        // Development Kit
...
static PyObject *PyConvertSidToStringSid(PyObject *self, PyObject *args)
{
    ...
    BOOL bConvert;
    DWORD err;
    Py_BEGIN_ALLOW_THREADS bConvert = ConvertSidToStringSid(psid, &stringsid);
    if (!bConvert)
        err = GetLastError();
    Py_END_ALLOW_THREADS if (!bConvert) PyWin_SetAPIError("ConvertSidToStringSid", err);
    else ret = PyWinObject_FromWCHAR(stringsid);
    if (stringsid != NULL)
        LocalFree(stringsid);
    return ret;
}
...
static PyObject *PyConvertStringSidToSid(PyObject *self, PyObject *args)
{
    ...
    if (!PyWinObject_AsWCHAR(obstringsid, &stringsid))
        return NULL;
    if (!ConvertStringSidToSid(stringsid, &psid))
...
// @pyswig string|ConvertSecurityDescriptorToStringSecurityDescriptor|Return string representation of a
// SECURITY_DESCRIPTOR
...
static PyObject *PyConvertSecurityDescriptorToStringSecurityDescriptor(PyObject *self, PyObject *args)
{
    ...
    if (!PyWinObject_AsSECURITY_DESCRIPTOR(obsd, &psd, FALSE))
        return NULL;
    if (!ConvertSecurityDescriptorToStringSecurityDescriptor(psd, sd_rev, info, &stringsd, NULL))
...
// @pyswig <o PySECURITY_DESCRIPTOR>|ConvertStringSecurityDescriptorToSecurityDescriptor|Turns string representation of
// a SECURITY_DESCRIPTOR into the real thing
...
static PyObject *PyConvertStringSecurityDescriptorToSecurityDescriptor(PyObject *self, PyObject *args)
{
    ...
    if (!PyWinObject_AsWCHAR(obssd, &stringsd, FALSE))
        return NULL;
    if (!ConvertStringSecurityDescriptorToSecurityDescriptor(stringsd, sd_rev, &psd, NULL))
...
static PyObject *PyLsaRegisterPolicyChangeNotification(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            args, "lO:LsaRegisterPolicyChangeNotification",
    ...
    if (!PyWinObject_AsHANDLE(obHandle, &hevent))
        return NULL;
    NTSTATUS err = LsaRegisterPolicyChangeNotification(info_class, hevent);
    if (err == STATUS_SUCCESS)
        ret = Py_None;
    else
...
static PyObject *PyLsaUnregisterPolicyChangeNotification(PyObject *self, PyObject *args)
{
    ...
    POLICY_NOTIFICATION_INFORMATION_CLASS info_class;
    if (!PyArg_ParseTuple(
            args, "lO:LsaUnregisterPolicyChangeNotification",
            (long *)&info_class,  // @pyparm int|InformationClass||POLICY_NOTIFICATION_INFORMATION_CLASS constant
            &obHandle))  // @pyparm <o PyHANDLE>|NotificationEventHandle||Event handle previously registered to receive
                         // policy change events
        return NULL;
    if (!PyWinObject_AsHANDLE(obHandle, &hevent))
        return NULL;
    NTSTATUS err = LsaUnregisterPolicyChangeNotification(info_class, hevent);
    if (err == STATUS_SUCCESS)
        ret = Py_None;
    else
...
static PyObject *PyCryptEnumProviders(PyObject *self, PyObject *args)
{
    ...
    while (TRUE) {
        ...
        pszProvName = NULL;
        ret_item = NULL;
        if (!CryptEnumProviders(dwIndex, NULL, dwFlags, &dwProvType, NULL, &cbProvName)) {
            err = GetLastError();
            break;
        }
        ...
        if (!CryptEnumProviders(dwIndex, NULL, dwFlags, &dwProvType, pszProvName, &cbProvName)) {
            err = GetLastError();
            break;
        }
...
// @pyswig (dict,...)|EnumerateSecurityPackages|List available security packages as a sequence of dictionaries
// representing SecPkgInfo structures
...
BOOLAPI AllocateLocallyUniqueId(LUID *OUTPUT);
...
BOOLAPI ImpersonateSelf(SECURITY_IMPERSONATION_LEVEL ImpersonationLevel);

// @pyswig <o PyHANDLE>|DuplicateToken|Creates a copy of an access token with specified impersonation level
// @pyparm <o PyHANDLE>|ExistingTokenHandle||Handle to an access token (see <om win32security.LogonUser>,<om
// win32security.OpenProcessToken>)
// @pyparm int|ImpersonationLevel||A value from SECURITY_IMPERSONATION_LEVEL enum
BOOLAPI DuplicateToken(HANDLE ExistingTokenHandle, SECURITY_IMPERSONATION_LEVEL ImpersonationLevel, PyHANDLE *OUTPUT);
...
{
    static char *keywords[] = {"ExistingToken", "ImpersonationLevel", "DesiredAccess",
                               "TokenType",     "TokenAttributes",    NULL};
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "Okkk|O:DuplicateTokenEx", keywords,
            ...
            &access,     // @pyparm int|DesiredAccess||Type of access required for the handle, combination of
                         // win32security.TOKEN_* flags
            &tokentype,  // @pyparm int|TokenType||Type of token to be created, TokenPrimary or TokenImpersonation
            &obsa))  // @pyparm <o PySECURITY_ATTRIBUTES>|TokenAttributes|None|Specifies security and inheritance for
                     // the new handle.  None results in default DACL and no inheritance,
        return NULL;
    if (!PyWinObject_AsHANDLE(obtoken, &htoken))
        return NULL;
...
// @pyswig <o PyHANDLE>|CreateRestrictedToken|Creates a restricted copy of an access token with reduced privs - requires
// win2K or higher
...
static PyObject *PyCreateRestrictedToken(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    BOOL bsuccess = TRUE;
    static char *keywords[] = {"ExistingTokenHandle", "Flags",          "SidsToDisable",
                               "PrivilegesToDelete",  "SidsToRestrict", NULL};
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "OlOOO:CreateRestrictedToken", keywords,
            &obExistingTokenHandle,  // @pyparm <o PyHANDLE>|ExistingTokenHandle||Handle to an access token (see <om
                                     // win32security.LogonUser>,<om win32security.OpenProcessToken>
            &Flags,            // @pyparm int|Flags||Valid values are zero or a combination of DISABLE_MAX_PRIVILEGE and
                               // SANDBOX_INERT
            &obSidsToDisable,  // @pyparm (<o PySID_AND_ATTRIBUTES>,...)|SidsToDisable||Ssequence of <o
                               // PySID_AND_ATTRIBUTES> tuples, or None
            &obPrivilegesToDelete,  // @pyparm (<o PyLUID_AND_ATTRIBUTES>,...)|PrivilegesToDelete||Privilege LUIDS to
                                    // remove from token (attributes are ignored), or None
            &obSidsToRestrict))     // @pyparm (<o PySID_AND_ATTRIBUTES>,...)|SidsToRestrict||Sequence of <o
                                    // PySID_AND_ATTRIBUTES> tuples (attributes must be 0).  Can be None.
        return NULL;
    if (PyWinObject_AsHANDLE(obExistingTokenHandle, &ExistingTokenHandle))
        if (PyWinObject_AsSID_AND_ATTRIBUTESArray(obSidsToDisable, &SidsToDisable, &DisableSidCount))
            if (PyWinObject_AsSID_AND_ATTRIBUTESArray(obSidsToRestrict, &SidsToRestrict, &RestrictedSidCount))
                if (PyWinObject_AsLUID_AND_ATTRIBUTESArray(obPrivilegesToDelete, &PrivilegesToDelete,
...
static PyObject *PyLsaRegisterLogonProcess(PyObject *self, PyObject *args)
{
    ...
    if (!PyWinObject_AsLSA_STRING(obname, &LogonProcessName))
        return NULL;
    NTSTATUS err = LsaRegisterLogonProcess(&LogonProcessName, &lsahandle, &dummy);
    if (err == STATUS_SUCCESS)
...
static PyObject *PyLsaConnectUntrusted(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, ":LsaConnectUntrusted"))
        return NULL;
    NTSTATUS err = LsaConnectUntrusted(&lsahandle);
    if (err == STATUS_SUCCESS)
        return new PyLsaLogon_HANDLE(lsahandle);
...
{
    PyObject *obhandle;
    // @pyparm <o PyLsaLogon_HANDLE>|LsaHandle||An Lsa handle as returned by <om win32security.LsaConnectUntrusted> or
    // <om win32security.LsaRegisterLogonProcess>
...
static PyObject *PyLsaLookupAuthenticationPackage(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OO:LsaLookupAuthenticationPackage", &obhandle, &obname))
        return NULL;
    if (!PyWinObject_AsHANDLE(obhandle, &lsahandle))
        return NULL;
    if (!PyWinObject_AsLSA_STRING(obname, &packagename))
        return NULL;
    NTSTATUS err = LsaLookupAuthenticationPackage(lsahandle, &packagename, &packageid);
    if (err != STATUS_SUCCESS)
...
static PyObject *PyLsaEnumerateLogonSessions(PyObject *self, PyObject *args)
{
    ...
    NTSTATUS err = LsaEnumerateLogonSessions(&sessioncount, &logonids);
    if (err != STATUS_SUCCESS)
...
#define HANDLE_OPT_ELT(Elt, Maker)                                  \
    PyObject *ob##Elt;                                              \
    if (pdata->Size > offsetof(SECURITY_LOGON_SESSION_DATA, Elt)) { \
        ob##Elt = Maker(pdata->Elt);                                \
    }                                                               \
    else {                                                          \
        ob##Elt = Py_None;                                          \
        Py_INCREF(Py_None);                                         \
    }
#define HANDLE_OPT_ELT_PTR(Elt, Maker)                              \
    PyObject *ob##Elt;                                              \
    if (pdata->Size > offsetof(SECURITY_LOGON_SESSION_DATA, Elt)) { \
        ob##Elt = Maker(&pdata->Elt);                               \
    }                                                               \
...
{
    return Py_BuildValue("{s:N,s:N,s:N}", "LastSuccessfulLogon", PyWinObject_FromTimeStamp(p->LastSuccessfulLogon),
                         "LastFailedLogon", PyWinObject_FromTimeStamp(p->LastFailedLogon),
                         "FailedAttemptCountSinceLastSuccessfulLogon",
                         PyLong_FromUnsignedLong(p->FailedAttemptCountSinceLastSuccessfulLogon));
}
...
static PyObject *PyLsaGetLogonSessionData(PyObject *self, PyObject *args)
{
    ...
    NTSTATUS err = LsaGetLogonSessionData(&logonid, &pdata);
    if (err != STATUS_SUCCESS)
        ...
        ret = Py_BuildValue("{s:N,s:N,s:N,s:N,s:l,s:l,s:N,s:N,s:N,s:N,s:N,s:N,s:N,s:N,s:N,s:N,s:N,s:N,s:N,s:N,s:N,s:N}",
                            "LogonId", PyWinObject_FromLARGE_INTEGER(*((LARGE_INTEGER *)&pdata->LogonId)), "UserName",
                            PyWinObject_FromLSA_UNICODE_STRING(pdata->UserName), "LogonDomain",
                            PyWinObject_FromLSA_UNICODE_STRING(pdata->LogonDomain), "AuthenticationPackage",
                            PyWinObject_FromLSA_UNICODE_STRING(pdata->AuthenticationPackage), "LogonType",
                            pdata->LogonType, "Session", pdata->Session, "Sid", PyWinObject_FromSID(pdata->Sid),
                            "LogonTime", PyWinObject_FromTimeStamp(pdata->LogonTime), "LogonServer", obLogonServer,
                            "DnsDomainName", obDnsDomainName, "Upn", obUpn, INSERT_ELT(UserFlags),
                            INSERT_ELT(LastLogonInfo), INSERT_ELT(LogonScript), INSERT_ELT(ProfilePath),
                            INSERT_ELT(HomeDirectory), INSERT_ELT(HomeDirectoryDrive), INSERT_ELT(LogoffTime),
                            INSERT_ELT(KickOffTime), INSERT_ELT(PasswordLastSet), INSERT_ELT(PasswordCanChange),
...
static PyObject *PyAcquireCredentialsHandle(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            args, "OOlOO|OO:AcquireCredentialsHandle",
            ...
            &CredentialUse,  // @pyparm int|CredentialUse||Intended use of requested credentials, SECPKG_CRED_INBOUND,
                             // SECPKG_CRED_OUTBOUND, or SECPKG_CRED_BOTH
            &obLogonID,      // @pyparm long|LogonID||LUID representing a logon session, can be None
            &obAuthData,     // @pyparm tuple|AuthData||Sequence of 3 strings: (User, Domain, Password) - use none for
    ...
    if (PyWinObject_AsWCHAR(obPrincipal, &Principal, TRUE) && PyWinObject_AsWCHAR(obPackage, &Package, FALSE)) {
        err = (*psecurityfunctiontable->AcquireCredentialsHandleW)(Principal, Package, CredentialUse, pLogonID,
                                                                   pAuthData, NULL, NULL, &Credential, &Expiry);
        if (err == SEC_E_OK)
...
// @pyswig (int, int, <o PyDateTime>)|InitializeSecurityContext|Creates a security context based on credentials created
// by AcquireCredentialsHandle
...
static PyObject *PyInitializeSecurityContext(PyObject *self, PyObject *args)
{
    ...
    PyObject *ret = NULL;
    if (!PyArg_ParseTuple(
            args, "OOOllOOO:InitializeSecurityContext",
            &obcredhandle,  // @pyparm <o PyCredHandle>|Credential||A credentials handle as returned by <om
                            // win32security.AcquireCredentialsHandle>
            &obctxt,  // @pyparm <o PyCtxtHandle>|Context||Use None on initial call, then handle returned in NewContext
                      // thereafter
            &obtargetname,   // @pyparm str/unicode|TargetName||Target of context, security package specific - Use None
                             // with NTLM
            ...
            &obsecbufferdescout))  // @pyparm <o PySecBufferDesc>|pOutput||Buffer that receives output data for
                                   // subsequent calls
        return NULL;
    ...
    if (PyWinObject_AsCredHandle(obcredhandle, &pcredhandle, FALSE) && PyWinObject_AsCtxtHandle(obctxt, &pctxt, TRUE) &&
        ...
        err = (*psecurityfunctiontable->InitializeSecurityContextW)(pcredhandle, pctxt, targetname, contextreq,
                                                                    reserved1, targetdatarep, psecbufferdesc, reserved2,
                                                                    pctxtout, psecbufferdescout, &contextattr, &expiry);
        if (err < 0)
...
static PyObject *PyAcceptSecurityContext(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            args, "OOOllOO:AcceptSecurityContext",
            &obcredhandle,  // @pyparm <o PyCredHandle>|Credential||Handle to server's credentials (see
            ...
            &targetdatarep,    // @pyparm int|TargetDataRep||One of SECURITY_NATIVE_DREP,SECURITY_NETWORK_DREP
            &obctxtout,        // @pyparm <o PyCtxtHandle>|NewContext||Uninitialized context handle to receive output
            &obsecbufferdescout))  // @pyparm <o PySecBufferDesc>|pOutput||Buffer that receives output data, to be
                                   // passed back as pInput on subsequent calls
        return NULL;
    if (contextreq & ISC_REQ_ALLOCATE_MEMORY) {
        PyErr_SetString(PyExc_NotImplementedError, "Use of ISC_REQ_ALLOCATE_MEMORY is not yet supported");
        return NULL;
    ...
    if (PyWinObject_AsCredHandle(obcredhandle, &pcredhandle, FALSE) && PyWinObject_AsCtxtHandle(obctxt, &pctxt, TRUE) &&
        ...
        err = (*psecurityfunctiontable->AcceptSecurityContext)(pcredhandle, pctxt, psecbufferdesc, contextreq,
                                                               targetdatarep, pctxtout, psecbufferdescout, &contextattr,
                                                               &expiry);
        if (err < 0)
...
{
    // @pyparm <o PyLsaLogon_HANDLE>|LsaHandle||Lsa handle as returned by <om win32security.LsaRegisterLogonProcess> or
    // <om win32security.LsaConnectUntrusted>
    // @pyparm int|AuthenticationPackage||Id of authentication package to call, as returned by <om
    ...
    switch (msgtype) {
        ...
        case KerbRetrieveTicketMessage:
            ...
            if (!PyWinObject_AsLARGE_INTEGER(obinputbuf,
                                             (LARGE_INTEGER *)&((PKERB_QUERY_TKT_CACHE_REQUEST)inputbuf)->LogonId))
                goto done;
            break;
        // @flag KerbPurgeTicketCacheMessage|(long, <o PyUnicode>, <o PyUnicode>) - tuple containing (LogonId,
        ...
        case KerbPurgeTicketCacheMessage:
            ...
            if (!PyArg_ParseTuple(obinputbuf, "OOO:KERB_PURGE_TKT_CACHE_REQUEST", &obLogonId, &obServerName,
                                  &obRealmName))
                goto done;
            if (!PyWinObject_AsLARGE_INTEGER(obLogonId,
                                             (LARGE_INTEGER *)&((PKERB_PURGE_TKT_CACHE_REQUEST)inputbuf)->LogonId) ||
                !PyWinObject_AsLSA_UNICODE_STRING(obServerName, &((PKERB_PURGE_TKT_CACHE_REQUEST)inputbuf)->ServerName,
                                                  TRUE) ||
                !PyWinObject_AsLSA_UNICODE_STRING(obRealmName, &((PKERB_PURGE_TKT_CACHE_REQUEST)inputbuf)->RealmName,
                                                  TRUE))
                goto done;
            ((PKERB_PURGE_TKT_CACHE_REQUEST)inputbuf)->MessageType = (KERB_PROTOCOL_MESSAGE_TYPE)msgtype;
            break;
        // @flag KerbRetrieveEncodedTicketMessage|(LogonId, TargetName, TicketFlags, CacheOptions, EncryptionType,
        // CredentialsHandle)
    ...
    err = LsaCallAuthenticationPackage(lsahandle, pkgid, inputbuf, inputbuflen, &outputbuf, &outputbuflen,
                                       &protocol_status);
    if (err != STATUS_SUCCESS) {
    ...
    switch (msgtype) {
        // @flag KerbQueryTicketCacheMessage|(dict,...) - Returns all tickets for the specified logon session (form is
        ...
        case KerbQueryTicketCacheMessage:
            ...
            for (tkt_ind = 0; tkt_ind < kqtcr->CountOfTickets; tkt_ind++) {
                obktci = Py_BuildValue(
                    "{s:N,s:N,s:N,s:N,s:N,s:l,s:l}", "ServerName",
                    PyWinObject_FromLSA_UNICODE_STRING(kqtcr->Tickets[tkt_ind].ServerName), "RealmName",
                    PyWinObject_FromLSA_UNICODE_STRING(kqtcr->Tickets[tkt_ind].RealmName), "StartTime",
                    PyWinObject_FromTimeStamp(kqtcr->Tickets[tkt_ind].StartTime), "EndTime",
                    PyWinObject_FromTimeStamp(kqtcr->Tickets[tkt_ind].EndTime), "RenewTime",
                    PyWinObject_FromTimeStamp(kqtcr->Tickets[tkt_ind].RenewTime), "EncryptionType",
                    kqtcr->Tickets[tkt_ind].EncryptionType, "TicketFlags", kqtcr->Tickets[tkt_ind].TicketFlags);
                if (obktci == NULL) {
                    Py_DECREF(ret);
                    ret = NULL;
                    goto done;
        ...
        case KerbPurgeTicketCacheMessage:
            ...
            ret = Py_None;
            break;
        // @flag KerbRetrieveTicketMessage|Returns the ticket granting ticket for the logon session as a
...
static PyObject *PyTranslateName(PyObject *self, PyObject *args)
{
    ...
    BOOL ok;
    if (!PyArg_ParseTuple(
            args, "Oii|l:TranslateName",
            &obAcctName,     // @pyparm <o PyUnicode>|accountName||object name
            &format,         // @pyparm int|accountNameFormat||A value from the EXTENDED_NAME_FORMAT enumeration type
                             // indicating the format of the accountName name.
            &desiredFormat,  // @pyparm int|accountNameFormat||A value from the EXTENDED_NAME_FORMAT enumeration type
                             // indicating the format of the desired name.
    ...
    Py_BEGIN_ALLOW_THREADS ok =
        TranslateName(szAcctName, (EXTENDED_NAME_FORMAT)format, (EXTENDED_NAME_FORMAT)desiredFormat, buf, &numChars);
    Py_END_ALLOW_THREADS PyObject *ret = NULL;
    if (ok) {
...
static PyObject *PyCreateWellKnownSid(PyObject *self, PyObject *args)
{
    ...
    if (!PyWinObject_AsSID(obDomainSid, &DomainSid, TRUE))
        return NULL;
    if (!CreateWellKnownSid(sidtype, DomainSid, outsid, &bufsize))
...
static PyObject *PyMapGenericMask(PyObject *self, PyObject *args)
{
    ...
    // @pyparm (int,int,int,int)|GenericMapping||A tuple of 4 bitmasks (GenericRead, GenericWrite, GenericExecute,
    // GenericAll) containing the standard and specific rights that correspond to the generic rights.
    if (!PyArg_ParseTuple(args, "k(kkkk):MapGenericMask", &mask, &mapping.GenericRead, &mapping.GenericWrite,
        ...
        return NULL;
    Py_BEGIN_ALLOW_THREADS MapGenericMask(&mask, &mapping);
    Py_END_ALLOW_THREADS return PyLong_FromUnsignedLong(mask);
}
%}
...
#define TOKEN_ALL_ACCESS \
    TOKEN_ALL_ACCESS  // Combines the STANDARD_RIGHTS_REQUIRED standard access rights and all individual access rights
                      // for tokens.
#define TOKEN_ASSIGN_PRIMARY \
    TOKEN_ASSIGN_PRIMARY  // Required to attach a primary token to a process in addition to the SE_CREATE_TOKEN_NAME
                          // privilege.
#define TOKEN_DUPLICATE TOKEN_DUPLICATE  // Required to duplicate an access token.
#define TOKEN_EXECUTE \
    TOKEN_EXECUTE  // Combines the STANDARD_RIGHTS_EXECUTE standard access rights and the TOKEN_IMPERSONATE access
                   // right.
...
#define TOKEN_WRITE \
    TOKEN_WRITE  // Combines the STANDARD_RIGHTS_WRITE standard access rights and the TOKEN_ADJUST_PRIVILEGES,
...
#define LOGON32_LOGON_BATCH \
    LOGON32_LOGON_BATCH  // This logon type is intended for batch servers, where processes may be executing on behalf of
                         // a user without their direct intervention; or for higher performance servers that process
                         // many clear-text authentication attempts at a time, such as mail or web servers. LogonUser
...
#define LOGON32_LOGON_INTERACTIVE \
    LOGON32_LOGON_INTERACTIVE  // This logon type is intended for users who will be interactively using the machine,
                               ...
                               // disconnected operation, and is therefore inappropriate for some client/server
                               // applications, such as a mail server.
#define LOGON32_LOGON_SERVICE \
    LOGON32_LOGON_SERVICE  // Indicates a service-type logon. The account provided must have the service privilege
                           // enabled.
#define LOGON32_LOGON_NETWORK \
    LOGON32_LOGON_NETWORK  // This logon type is intended for high performance servers to authenticate clear text
                           // passwords. LogonUser does not cache credentials for this logon type. This is the fastest
                           // logon path, but there are two limitations. First, the function returns an impersonation
                           // token, not a primary token. You cannot use this token directly in the CreateProcessAsUser
                           // function. However, you can call the DuplicateTokenEx function to convert the token to a
                           // primary token, and then use it in CreateProcessAsUser. Second, if you convert the token to
                           // a primary token and use it in CreateProcessAsUser to start a process, the new process will
                           // not be able to access other network resources, such as remote servers or printers, through
                           // the redirector.
...
#define LOGON32_PROVIDER_DEFAULT \
    LOGON32_PROVIDER_DEFAULT  // Use the standard logon provider for the system. This is the recommended value for the
                              // dwLogonProvider parameter. It provides maximum compatibility with current and future
...
#define ACCESS_ALLOWED_OBJECT_ACE_TYPE \
    ACCESS_ALLOWED_OBJECT_ACE_TYPE  // Object-specific access-allowed ACE that uses the ACCESS_ALLOWED_OBJECT_ACE
                                    // structure.
...
#define ACCESS_DENIED_OBJECT_ACE_TYPE \
    ACCESS_DENIED_OBJECT_ACE_TYPE  // Object-specific access-denied ACE that uses the ACCESS_DENIED_OBJECT_ACE
                                   // structure.
...
#define POLICY_AUDIT_EVENT_UNCHANGED \
    POLICY_AUDIT_EVENT_UNCHANGED  // For set operations, specify this value to leave the current options unchanged. This
                                  // is the default.

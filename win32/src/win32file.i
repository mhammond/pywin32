...
// Specifies read access to the object. Data can be read from the file and the file pointer can be moved. Combine with
...
// Opens the file. Once opened, the file is truncated so that its size is zero bytes. The calling process must open the
// file with at least GENERIC_WRITE access. The function fails if the file does not exist.
...
// The file or directory is compressed. For a file, this means that all of the data in the file is compressed. For a
// directory, this means that compression is the default for newly created files and subdirectories.
...
// The data of the file is not immediately available. Indicates that the file data has been physically moved to offline
// storage.
...
// The file is being used for temporary storage. File systems attempt to keep all of the data in memory for quicker
// access rather than flushing the data back to mass storage. A temporary file should be deleted by the application as
// soon as it is no longer needed.

#define FILE_FLAG_WRITE_THROUGH FILE_FLAG_WRITE_THROUGH
// Instructs the system to write through any intermediate cache and go directly to disk. Windows can still cache write
// operations, but cannot lazily flush them.
#define FILE_FLAG_OVERLAPPED FILE_FLAG_OVERLAPPED
// Instructs the system to initialize the object, so that operations that take a significant amount of time to process
// return ERROR_IO_PENDING. When the operation is finished, the specified event is set to the signaled state. When you
// specify FILE_FLAG_OVERLAPPED, the ReadFile and WriteFile functions must specify an OVERLAPPED structure. That is,
// when FILE_FLAG_OVERLAPPED is specified, an application must perform overlapped reading and writing. When
// FILE_FLAG_OVERLAPPED is specified, the system does not maintain the file pointer. The file position must be passed as
// part of the lpOverlapped parameter (pointing to an OVERLAPPED structure) to the ReadFile and WriteFile functions.
// This flag also enables more than one operation to be performed simultaneously with the handle (a simultaneous read
// and write operation, for example).
...
// <nl>-	File access must begin at byte offsets within the file that are integer multiples of the volume's sector
// size. <nl>-	File access must be for numbers of bytes that are integer multiples of the volume's sector size. For
// example, if the sector size is 512 bytes, an application can request reads and writes of 512, 1024, or 2048 bytes,
// but not of 335, 981, or 7171 bytes. <nl>-	Buffer addresses for read and write operations must be aligned on
// addresses in memory that are integer multiples of the volume's sector size. One way to align buffers on integer
// multiples of the volume sector size is to use VirtualAlloc to allocate the buffers. It allocates memory that is
// aligned on addresses that are integer multiples of the operating system's memory page size. Because both memory page
// and volume sector sizes are powers of 2, this memory is also aligned on addresses that are integer multiples of a
// volume's sector size. An application can determine a volume's sector size by calling the GetDiskFreeSpace function.
...
// Indicates that the file is to be accessed sequentially from beginning to end. The system can use this as a hint to
// optimize file caching. If an application moves the file pointer for random access, optimum caching may not occur;
// however, correct operation is still guaranteed. Specifying this flag can increase performance for applications that
// read large files using sequential access. Performance gains can be even more noticeable for applications that read
// large files mostly sequentially, but occasionally skip over small ranges of bytes.
...
// not just the handle for which you specified FILE_FLAG_DELETE_ON_CLOSE. Subsequent open requests for the file will
// fail, unless FILE_SHARE_DELETE is used.
...
// The operating system ensures that the calling process overrides file security checks, provided it has the necessary
// permission to do so. The relevant permissions are SE_BACKUP_NAME and SE_RESTORE_NAME. You can also set this flag to
// obtain a handle to a directory. A directory handle can be passed to some Win32 functions in place of a file handle.
...
// Use care when using this option because files created with this flag may not be accessible by applications written
...
// Specifies that the security tracking mode is dynamic. If this flag is not specified, Security Tracking Mode is
// static.
#define SECURITY_EFFECTIVE_ONLY SECURITY_EFFECTIVE_ONLY
// Specifies that only the enabled aspects

// @pyswig int|AreFileApisANSI|Determines whether a set of Win32 file functions is using the ANSI or OEM character set
...
BOOLAPI CreateDirectoryEx(
    TCHAR *templateName,  // @pyparm string|templateName||Specifies the path of the directory to use as a template when
                          // creating the new directory.
    TCHAR *newDirectory,  // @pyparm string|newDirectory||Specifies the name of the new directory
    SECURITY_ATTRIBUTES *pSA);  // @pyparm <o PySECURITY_ATTRIBUTES>|sa||The security attributes, or None

// @pyswig <o PyHANDLE>|CreateFile|Creates or opens the a file or other object and returns a handle that can be used to
// access the object.
// @comm The following objects can be opened:<nl>files<nl>pipes<nl>mailslots<nl>communications resources<nl>disk devices
// (Windows NT only)<nl>consoles<nl>directories (open only)
PyHANDLE CreateFile(
    ...
    DWORD dwDesiredAccess,  // @pyparm int|desiredAccess||access (read-write) mode
                            ...
                            // @flagh Value|Meaning
                            // @flag 0|Specifies device query access to the object. An application can query device
                            // attributes without accessing the device.
                            // @flag GENERIC_READ|Specifies read access to the object. Data can be read from the file
                            // and the file pointer can be moved. Combine with GENERIC_WRITE for read-write access.
                            // @flag GENERIC_WRITE|Specifies write access to the object. Data can be written to the file
                            // and the file pointer can be moved. Combine with GENERIC_READ for read-write access.
    DWORD dwShareMode,      // @pyparm int|shareMode||Set of bit flags that specifies how the object can be shared. If
                        // dwShareMode is 0, the object cannot be shared. Subsequent open operations on the object will
                        // fail, until the handle is closed. To share the object, use a combination of one or more of
                        // the following values:
                        // @flagh Value|Meaning
                        // @flag FILE_SHARE_DELETE|Windows NT: Subsequent open operations on the object will succeed
                        // only if delete access is requested.
                        // @flag FILE_SHARE_READ|Subsequent open operations on the object will succeed only if read
                        // access is requested.
                        // @flag FILE_SHARE_WRITE|Subsequent open operations on the object will succeed only if write
                        // access is requested.
    SECURITY_ATTRIBUTES
        *lpSecurityAttributes,    // @pyparm <o PySECURITY_ATTRIBUTES>|attributes||The security attributes, or None
    DWORD dwCreationDisposition,  // @pyparm int|CreationDisposition||Specifies which action to take on files that
                                  // exist, and which action to take when files do not exist. For more information about
                                  // this parameter, see the Remarks section. This parameter must be one of the
                                  // following values:
                                  // @flagh Value|Meaning
                                  // @flag CREATE_NEW|Creates a new file. The function fails if the specified file
                                  // already exists.
                                  // @flag CREATE_ALWAYS|Creates a new file. If the file exists, the function overwrites
                                  ...
                                  // @flag OPEN_ALWAYS|Opens the file, if it exists. If the file does not exist, the
                                  // function creates the file as if dwCreationDisposition were CREATE_NEW.
                                  // @flag TRUNCATE_EXISTING|Opens the file. Once opened, the file is truncated so that
                                  // its size is zero bytes. The calling process must open the file with at least
    ...
    PyHANDLE INPUT_NULLOK  // @pyparm <o PyHANDLE>|hTemplateFile||Specifies a handle with GENERIC_READ access to a
                           // template file. The template file supplies file attributes and extended attributes for the
                           // file being created.
);
...
%{
// @pyswig <o PyHANDLE>|CreateIoCompletionPort|Can associate an instance of an opened file with a newly created or an
// existing input/output (I/O) completion port; or it can create an I/O completion port without associating it with a
// file.
...
PyObject *MyCreateIoCompletionPort(PyObject *self, PyObject *args)
{
    ...
    PyObject *obRet = NULL;
    if (!PyArg_ParseTuple(
            args, "OOOi:CreateIoCompletionPort",
    ...
    HANDLE hRet;
    Py_BEGIN_ALLOW_THREADS hRet = CreateIoCompletionPort(hFile, hExisting, key, nt);
    Py_END_ALLOW_THREADS if (!hRet)
    {
        Py_XDECREF(obRet);
...
PyHANDLE CreateMailslot(
    ...
    SECURITY_ATTRIBUTES *INPUT_NULLOK  // @pyparm <o PySECURITY_ATTRIBUTES>|SecurityAttributes||Determines if returned
                                       // handle is inheritable, can be None
);
...
BOOLAPI GetMailslotInfo(HANDLE Mailslot,  // @pyparm <o PyHANDLE>|Mailslot||Handle to a mailslot
                        DWORD *OUTPUT, DWORD *OUTPUT, DWORD *OUTPUT, DWORD *OUTPUT);

// @pyswig |SetMailslotInfo|Sets a mailslot's timeout
// @pyseeapi SetMailslotInfo
BOOLAPI SetMailslotInfo(HANDLE Mailslot,     // @pyparm <o PyHANDLE>|Mailslot||Handle to a mailslot
                        DWORD ReadTimeout);  // @pyparm int|ReadTimeout||Timeout in milliseconds, use -1 for no timeout

// @pyswig |DefineDosDevice|Lets an application define, redefine, or delete MS-DOS device names.
BOOLAPI DefineDosDevice(DWORD dwFlags,        // @pyparm int|flags||flags specifying aspects of device definition
...
BOOLAPI DefineDosDeviceW(DWORD dwFlags,        // @pyparm int|flags||flags specifying aspects of device definition
...
PyObject *py_DeviceIoControl(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "OkOO|O:DeviceIoControl", keywords,
            ...
            &obInBuffer,       // @pyparm str/buffer|InBuffer||The input data for the operation, can be None for some
                               // operations.
            &obOutBuffer,    // @pyparm int/buffer|OutBuffer||Size of the buffer to allocate for output, or a writeable
                             // buffer
                             //	as returned by <om win32file.AllocateReadBuffer>.
            &obOverlapped))  // @pyparm <o PyOVERLAPPED>|Overlapped|None|An overlapped object for async operations.
                             // Device
    ...
    Py_BEGIN_ALLOW_THREADS
        ...
        ok = DeviceIoControl(hDevice, dwIoControlCode, in_buf.ptr(), in_buf.len(), OutBuffer, OutBufferSize, &numRead,
                             pOverlapped);
...
// @pyswig int|FindFirstChangeNotification|Creates a change notification handle and sets up initial change notification
// filter conditions. A wait on a notification handle succeeds when a change matching the filter conditions occurs in
// the specified directory or subtree.
HANDLE FindFirstChangeNotification(
    ...
    DWORD dwNotifyFilter  // @pyparm int|notifyFilter||filter conditions to watch for.  See <om
                          // win32api.FindFirstChangeNotification> for details.
);

// @pyswig int|FindNextChangeNotification|Requests that the operating system signal a change notification handle the
...
typedef struct {
    PyObject_HEAD HANDLE hFind;
    WIN32_FIND_DATAW buffer;
    BOOL seen_first;
    BOOL empty;
} FindFileIterator;

static void ffi_dealloc(FindFileIterator *it)
{
...
static PyObject *ffi_iternext(PyObject *iterator)
{
    ...
    if (!ffi->seen_first)
        ...
        BOOL ok;
        Py_BEGIN_ALLOW_THREADS memset(&ffi->buffer, 0, sizeof(ffi->buffer));
...
PyTypeObject FindFileIterator_Type = {
    PYWIN_OBJECT_HEAD "FindFileIterator", /* tp_name */
...
// @pyswig |FlushFileBuffers|Clears the buffers for the specified file and causes all buffered data to be written to the
// file.
BOOLAPI FlushFileBuffers(
    PyHANDLE hFile  // @pyparm <o PyHANDLE>|hFile||open handle to file whose buffers are to be flushed
);

// @pyswig int|GetBinaryType|Determines whether a file is executable, and if so, what type of executable file it is.
// That last property determines which subsystem an executable file runs under.
BOOLAPI GetBinaryType(TCHAR *lpApplicationName,  // @pyparm string|appName||Fully qualified path of file to test
...
BOOLAPI GetDiskFreeSpace(TCHAR *lpRootPathName,  // @pyparm string|rootPathName||address of root path
                         unsigned long *OUTPUT, unsigned long *OUTPUT, unsigned long *OUTPUT, unsigned long *OUTPUT
                         // @rdesc The result is a tuple of integers representing (sectors per cluster, bytes per
                         // sector, number of free clusters, total number of clusters)
);

// GetDiskFreeSpaceEx
// @pyswig long, long, long|GetDiskFreeSpaceEx|Determines the free space on a device.
BOOLAPI GetDiskFreeSpaceEx(TCHAR *lpRootPathName,  // @pyparm string|rootPathName||address of root path
                           ULARGE_INTEGER *OUTPUT, ULARGE_INTEGER *OUTPUT, ULARGE_INTEGER *OUTPUT
                           // @rdesc The result is a tuple of long integers:
                           // @tupleitem 0|long integer|freeBytes|The total number of free bytes on the disk that are
                           // available to the user associated with the calling thread.
                           // @tupleitem 1|long integer|totalBytes|The total number of bytes on the disk that are
                           // available to the user associated with the calling thread. If per-user quotas are in use,
                           // this value may be less than the total number of bytes on the disk.
...
long GetDriveType(TCHAR *rootPathName  // @pyparm string|rootPathName||
...
long GetDriveTypeW(WCHAR *rootPathName  // @pyparm string|rootPathName||
                                        // @rdesc The result is one of the DRIVE_* constants.
);
...
DWORD GetFileAttributes(TCHAR *fileName);  // @pyparm string|fileName||Name of the file to retrieve attributes for.

// @pyswig int|GetFileAttributesW|Determines a files attributes
DWORD GetFileAttributesW(WCHAR *fileName);  // @pyparm string|fileName||Name of the file to retrieve attributes for.

// @pyswig (<o PyDateTime>, <o PyDateTime>, <o PyDateTime>)|GetFileTime|Returns a file's creation, last access, and
// modification times.
// @comm Times are returned in UTC time.
BOOLAPI GetFileTime(HANDLE handle,     // @pyparm <o PyHANDLE>|handle||Handle to the file.
...
static BOOL PyWinTime_DateTimeCheck(PyObject *ob) { return PyDateTimeAPI && PyDateTime_Check(ob); }
...
{
    PyObject
        *obHandle;  // @pyparm <o PyHANDLE>|File||Previously opened handle (opened with FILE_WRITE_ATTRIBUTES access).
    PyObject *obCreationTime =
        Py_None;  // @pyparm <o PyDateTime>|CreationTime|None|File created time. None for no change.
    PyObject *obLastAccessTime =
        Py_None;  // @pyparm <o PyDateTime>|LastAccessTime|None|File access time. None for no change.
    PyObject *obLastWriteTime =
        Py_None;            // @pyparm <o PyDateTime>|LastWriteTime|None|File written time. None for no change.
    BOOL UTCTimes = FALSE;  // @pyparm boolean|UTCTimes|False|If True, input times are treated as UTC and no conversion
                            // is done, otherwise they are treated as local times.  Defaults to False for backward
                            // compatibility. This parameter is ignored in Python 3, where you should always pass
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|OOOi:SetFileTime", keywords, &obHandle, &obCreationTime,
                                     &obLastAccessTime, &obLastWriteTime, &UTCTimes))
        return NULL;
...
static PyObject *PyGetFileInformationByHandle(PyObject *self, PyObject *args)
{
    ...
    // @pyparm <o PyHANDLE>/int|handle||Handle to the file for which to obtain information.<nl>This handle should not be
    // a pipe handle. The GetFileInformationByHandle function does not work with pipe handles.
    if (!PyArg_ParseTuple(args, "O", &obHandle))
        return NULL;
    HANDLE handle;
    if (!PyWinObject_AsHANDLE(obHandle, &handle))
        return NULL;
    Py_BEGIN_ALLOW_THREADS memset(&fi, 0, sizeof(fi));
    rc = GetFileInformationByHandle(handle, &fi);
    Py_END_ALLOW_THREADS if (!rc) return PyWin_SetAPIError("GetFileInformationByHandle");
...
PyObject *MyGetCompressedFileSize(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS ulsize.LowPart = GetCompressedFileSize(fname, &ulsize.HighPart);
    Py_END_ALLOW_THREADS

        PyWinObject_FreeTCHAR(fname);
    // If we failed ...
    if (ulsize.LowPart == 0xFFFFFFFF && GetLastError() != NO_ERROR)
...
PyObject *MyGetFileSize(PyObject *self, PyObject *args)
{
    ...
    if (!PyWinObject_AsHANDLE(obHandle, &hFile))
        return NULL;
    ULARGE_INTEGER ulsize;
    Py_BEGIN_ALLOW_THREADS ulsize.LowPart = GetFileSize(hFile, &ulsize.HighPart);
    Py_END_ALLOW_THREADS
        // If we failed ...
        if (ulsize.LowPart == 0xFFFFFFFF && GetLastError() != NO_ERROR) return PyWin_SetAPIError("GetFileSize");
    return PyWinObject_FromULARGE_INTEGER(ulsize);
}
...
%{
// @pyswig <o PyOVERLAPPEDReadBuffer>|AllocateReadBuffer|Allocates a buffer which can be used with an overlapped Read
// operation using <om win32file.ReadFile>
...
// If the overlapped param is not None, then the result is a <o PyOVERLAPPEDReadBuffer>.  Once the overlapped IO
// operation has completed, you can convert this to a string (str(object)) [py2k] or (bytes(object)) [py3k] to obtain
// the data. While the operation is in progress, you can use the slice operations (object[:end]) to obtain the data read
// so far. You must use the OVERLAPPED API functions to determine how much of the data is valid.
PyObject *MyReadFile(PyObject *self, PyObject *args)
{
    ...
    // @comm in a multi-threaded overlapped environment, it is likely to be necessary to pre-allocate the read buffer
    // using the <om win32file.AllocateReadBuffer> method, otherwise the I/O operation may complete before you can
    // assign to the resulting buffer.
    ...
    DWORD numRead;
    BOOL ok;
    Py_BEGIN_ALLOW_THREADS ok = ReadFile(hFile, buf, bufSize, &numRead, pOverlapped);
    Py_END_ALLOW_THREADS DWORD err = 0;
    if (!ok) {
        err = GetLastError();
...
PyObject *MyWriteFile(PyObject *self, PyObject *args)
{
    ...
    BOOL ok;
    DWORD err = 0;
    Py_BEGIN_ALLOW_THREADS ok = WriteFile(hFile, pybuf.ptr(), pybuf.len(), &numWritten, pOverlapped);
    Py_END_ALLOW_THREADS if (!ok)
    {
        err = GetLastError();
        if (err != ERROR_IO_PENDING)
...
static PyObject *MyLockFileEx(PyObject *self, PyObject *args)
{
    OVERLAPPED *pOverlapped;
    PyObject *obhFile;
    HANDLE hFile;
    ...
    BOOL ok;
    DWORD err = 0;
    Py_BEGIN_ALLOW_THREADS ok = LockFileEx(hFile, dwFlags, 0, nbytesLow, nbytesHigh, pOverlapped);
    Py_END_ALLOW_THREADS
...
static PyObject *MyUnlockFileEx(PyObject *self, PyObject *args)
{
    OVERLAPPED *pOverlapped;
    PyObject *obhFile;
    HANDLE hFile;
    ...
    Py_BEGIN_ALLOW_THREADS ok = UnlockFileEx(hFile, 0, nbytesLow, nbytesHigh, pOverlapped);
    Py_END_ALLOW_THREADS
...
// @pyswig (int, int, int, <o PyOVERLAPPED>)|GetQueuedCompletionStatus|Attempts to dequeue an I/O completion packet from
// a specified input/output completion port.
...
static PyObject *myGetQueuedCompletionStatus(PyObject *self, PyObject *args)
{
    ...
    OVERLAPPED *pOverlapped = NULL;
    UINT errCode;
    Py_BEGIN_ALLOW_THREADS BOOL ok = GetQueuedCompletionStatus(handle, &bytes, &key, &pOverlapped, timeout);
    errCode = ok ? 0 : GetLastError();
    Py_END_ALLOW_THREADS PyObject *rc = Py_BuildValue("ilNN", errCode, bytes, PyWinObject_FromULONG_PTR(key),
                                                      PyWinObject_FromQueuedOVERLAPPED(pOverlapped));
    return rc;
}

// @pyswig None|PostQueuedCompletionStatus|lets you post an I/O completion packet to an I/O completion port. The I/O
// completion packet will satisfy an outstanding call to the GetQueuedCompletionStatus function.
PyObject *myPostQueuedCompletionStatus(PyObject *self, PyObject *args)
{
    ...
    if (!PyWinObject_AsQueuedOVERLAPPED(obOverlapped, &pOverlapped, TRUE))
        return NULL;
    BOOL ok;
    Py_BEGIN_ALLOW_THREADS ok = ::PostQueuedCompletionStatus(handle, bytesTransferred, key, pOverlapped);
    Py_END_ALLOW_THREADS if (!ok) return PyWin_SetAPIError("PostQueuedCompletionStatus");
    Py_INCREF(Py_None);
    return Py_None;
...
// @comm The result is the number of bytes transferred.  The overlapped object's attributes will be changed during this
// call.
...
BOOLAPI MoveFile(TCHAR *lpExistingFileName,  // @pyparm string|existingFileName||Name of the existing file
...
BOOLAPI MoveFileW(WCHAR *lpExistingFileName,  // @pyparm string|existingFileName||Name of the existing file
...
#define MOVEFILE_COPY_ALLOWED \
    MOVEFILE_COPY_ALLOWED  // If the file is to be moved to a different volume, the function simulates the move by using
                           // the CopyFile and DeleteFile functions. Cannot be combined with the
                           // MOVEFILE_DELAY_UNTIL_REBOOT flag.
#define MOVEFILE_DELAY_UNTIL_REBOOT \
    MOVEFILE_DELAY_UNTIL_REBOOT  // Windows NT only: The function does not move the file until the operating system is
                                 // restarted. The system moves the file immediately after AUTOCHK is executed, but
                                 // before creating any paging files. Consequently, this parameter enables the function
                                 // to delete paging files from previous startups.
#define MOVEFILE_REPLACE_EXISTING \
    MOVEFILE_REPLACE_EXISTING  // If a file of the name specified by lpNewFileName already exists, the function replaces
                               // its contents with those specified by lpExistingFileName.
#define MOVEFILE_WRITE_THROUGH \
    MOVEFILE_WRITE_THROUGH  // Windows NT only: The function does not return until the file has actually been moved on
                            // the disk. Setting this flag guarantees that a move performed as a copy and delete
                            // operation is flushed to disk before the function returns. The flush occurs at the end of
                            // the copy operation.<nl>This flag has no effect if the MOVEFILE_DELAY_UNTIL_REBOOT flag is
                            // set.
    ...
    while (1) {
        ...
        if (nbytes_read > nbytes) {
            PyErr_SetString(PyExc_RuntimeError,
                            "internal error decoding - running off end of buffer before seeing end-of-buffer marker");
            Py_DECREF(ret);
            return NULL;
        }
    }
...
static PyObject *PyReadDirectoryChangesW(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            args, "OOii|OO:ReadDirectoryChangesW",
            &obhandle,  // @pyparm <o PyHANDLE>|handle||Handle to the directory to be monitored. This directory must be
                        // opened with the FILE_LIST_DIRECTORY access right.
            &obBuffer,  // @pyparm int|size||Size of the buffer to allocate for the results.
            &bWatchSubtree,  // @pyparm int|bWatchSubtree||Specifies whether the ReadDirectoryChangesW function will
                             // monitor the directory or the directory tree. If TRUE is specified, the function monitors
                             // the directory tree rooted at the specified directory. If FALSE is specified, the
                             // function monitors only the directory specified by the hDirectory parameter.
            &filter,  // @pyparm int|dwNotifyFilter||Specifies filter criteria the function checks to determine if the
                      // wait operation has completed. This parameter can be one or more of the FILE_NOTIFY_CHANGE_*
                      // values.
            &obOverlapped,  // @pyparm <o PyOVERLAPPED>|overlapped|None|An overlapped object.  The directory must also
                            // be opened with FILE_FLAG_OVERLAPPED.
            &obOverlappedRoutine))
        return NULL;
    ...
    Py_BEGIN_ALLOW_THREADS ok =
        ::ReadDirectoryChangesW(handle, buf, bufSize, bWatchSubtree, filter, &bytes_returned, pOverlapped, NULL);
    Py_END_ALLOW_THREADS if (!ok)
    {
...
PyObject *PyFILE_NOTIFY_INFORMATION(PyObject *self, PyObject *args)
{
    ...
    if (!pybuf.ok())
        return NULL;
    if (size > pybuf.len())
        return PyErr_Format(PyExc_ValueError, "buffer is only %d bytes long, but %d bytes were requested", pybuf.len(),
                            size);
...
%name(SetCurrentDirectory) BOOLAPI SetCurrentDirectoryW(
    ...
    // @pyswig |SetEndOfFile|Moves the end-of-file (EOF) position for the specified file to the current position of the
    // file pointer.
    BOOL SetEndOfFile(PyHANDLE hFile  // @pyparm <o PyHANDLE>|hFile||handle of file whose EOF is to be set
    );

    // @pyswig |SetFileApisToANSI|Causes a set of Win32 file functions to use the ANSI character set code page. This
    // function is useful for 8-bit console input and output operations.
    void SetFileApisToANSI(void);

    // @pyswig |SetFileApisToOEM|Causes a set of Win32 file functions to use the OEM character set code page. This
    // function is useful for 8-bit console input and output operations.
    void SetFileApisToOEM(void);

    // @pyswig |SetFileAttributes|Changes a file's attributes.
    BOOLAPI SetFileAttributes(TCHAR *lpFileName,      // @pyparm string|filename||filename
...
%{
    ...
    PyObject *MySetFilePointer(PyObject *self, PyObject *args)
    {
        ...
        if (!PyArg_ParseTuple(
                ...
                &iMethod))  // @pyparm int|moveMethod||Starting point for the file pointer move. This parameter can be
        ...
        Py_BEGIN_ALLOW_THREADS offset.LowPart = SetFilePointer(handle, offset.LowPart, &offset.HighPart, iMethod);
        Py_END_ALLOW_THREADS
            // If we failed ...
            if (offset.LowPart == 0xFFFFFFFF && GetLastError() != NO_ERROR) return PyWin_SetAPIError("SetFilePointer");
...
%}
    ...
    int _setmaxstdio(int newmax  // @pyparm int|newmax||Maximum number of open stdio streams, 2048 max
    );
...
%{
    ...
    static PyObject *py_TransmitFile(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        static char *keywords[] = {
            "Socket", "File", "NumberOfBytesToWrite", "NumberOfBytesPerSend", "Overlapped", "Flags", "Head",
            "Tail",   NULL};
        if (!PyArg_ParseTupleAndKeywords(
                args, kwargs, "OOiiOi|OO:TransmitFile", keywords,
                ...
                &bytes_to_write,  // @pyparm int|NumberOfBytesToWrite||The number of bytes in the file to transmit, use
                                  // 0 for entire file.
                &bytes_per_send,  // @pyparm int|NumberOfBytesPerSend||The size, in bytes, of each block of data sent in
                                  // each send operation.
                &obOverlapped,    // @pyparm <o PyOVERLAPPED>|Overlapped||An overlapped structure, can be None.
                &flags,    // @pyparm int|Flags||A set of flags used to modify the behavior of the TransmitFile function
                           // call. (win32file.TF_*)
        ...
        error = WSAIoctl(s, SIO_GET_EXTENSION_FUNCTION_POINTER, &guid, sizeof(GUID), &lpfnTransmitFile,
...
%{
    ...
    static PyObject *py_ConnectEx(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(
                args, kwargs, "OOO|O:ConnectEx", keywords,
        ...
        error = WSAIoctl(sConnecting, SIO_GET_EXTENSION_FUNCTION_POINTER, &guid, sizeof(GUID), &lpfnConnectEx,
                         sizeof(lpfnConnectEx), &dwBytes, NULL, NULL);
        if (error == SOCKET_ERROR) {
            rc = WSAGetLastError();
            PyWin_SetAPIError("WSAIoctl", rc);
            return NULL;
        ...
        error = getsockopt(sConnecting, SOL_SOCKET, SO_PROTOCOL_INFO, (char *)&prot_info, &prot_info_len);
        if (error) {
        ...
        rc = 0;
        Py_BEGIN_ALLOW_THREADS;
        if (!lpfnConnectEx(sConnecting, res->ai_addr, (int)res->ai_addrlen, pybuf.ptr(), pybuf.len(), &sent,
                           pOverlapped))
            rc = WSAGetLastError();
        Py_END_ALLOW_THREADS;
        WspiapiFreeAddrInfo(res);
...
%{
    ...
    static PyObject *MyAcceptEx(PyObject *self, PyObject *args)
    {
        OVERLAPPED *pOverlapped = NULL;
        SOCKET sListening;
        SOCKET sAccepting;
        ...
        if (!PyArg_ParseTuple(
                ...
                &obAccepting,  // @pyparm <o PySocket>/int|sAccepting||Socket that will be used as the incoming
                               // connection.
                &obBuf,  // @pyparm <o buffer>|buffer||Buffer to read incoming data and connection point information
                         // into. This buffer MUST be big enough to recieve your connection endpoints... AF_INET sockets
                         // need to be at least 64 bytes. The correct minimum of the buffer is determined by the
                         // protocol family that the listening socket is using.
                &obOverlapped))  // @pyparm <o PyOVERLAPPED>|ol||An overlapped structure
        {
            return NULL;
        }

        // @comm In order to make sure the connection has been accepted, either use the hEvent in PyOVERLAPPED,
        ...
        // @ex To have sAccepting inherit the properties of sListening, you need to do the following after a connection
        // is successfully accepted| import struct sAccepting.setsockopt(socket.SOL_SOCKET,
        // win32file.SO_UPDATE_ACCEPT_CONTEXT, struct.pack("I", sListening.fileno()))
        ...
        Py_BEGIN_ALLOW_THREADS rc =
            getsockopt(sListening, SOL_SOCKET, SO_PROTOCOL_INFO, (char *)&wsProtInfo, (int *)&cbSize);
        Py_END_ALLOW_THREADS if (rc == SOCKET_ERROR)
        {
            PyWin_SetAPIError("getsockopt", WSAGetLastError());
            return NULL;
        }
        ...
        if (pybuf.len() < (DWORD)iMinBufferSize) {
            PyErr_Format(PyExc_ValueError, "Second param must be at least %ld bytes long", iMinBufferSize);
            return NULL;
        }

        // Phew... finally, all the arguments are converted...
        Py_BEGIN_ALLOW_THREADS ok =
            AcceptEx(sListening, sAccepting, pybuf.ptr(), pybuf.len() - iMinBufferSize, wsProtInfo.iMaxSockAddr + 16,
                     wsProtInfo.iMaxSockAddr + 16, &cBytesRecvd, pOverlapped);
        Py_END_ALLOW_THREADS if (!ok)
        {
            rc = WSAGetLastError();
    ...
    static PyObject *MyMakeIPAddr(SOCKADDR_IN *addr)
    {
        long x = ntohl(addr->sin_addr.s_addr);
        char buf[100];
        sprintf(buf, "%d.%d.%d.%d", (int)(x >> 24) & 0xff, (int)(x >> 16) & 0xff, (int)(x >> 8) & 0xff,
                (int)(x >> 0) & 0xff);
        return PyBytes_FromString(buf);
    }

    static PyObject *MyMakeSockaddr(SOCKADDR *addr, INT cbAddr)
    {
        if (cbAddr == 0) {
        ...
        switch (addr->sa_family) {
            ...
            default:
                ...
                return Py_BuildValue("iN", addr->sa_family,
                                     PyBytes_FromStringAndSize(addr->sa_data, sizeof(addr->sa_data)));
        }
    }

    // @pyswig int|CalculateSocketEndPointSize|Calculate how many bytes are needed for the connection endpoints data for
    // a socket.
    PyObject *MyCalculateSocketEndPointSize(PyObject *self, PyObject *args)
    {
        ...
        // Grab the protocol information for the socket
        Py_BEGIN_ALLOW_THREADS rc = getsockopt(s, SOL_SOCKET, SO_PROTOCOL_INFO, (char *)&wsProtInfo, (int *)&cbSize);
        Py_END_ALLOW_THREADS if (rc == SOCKET_ERROR)
        {
            PyWin_SetAPIError("getsockopt", WSAGetLastError());
            return NULL;
...
%{
    // @pyswig (iFamily, <o LocalSockAddr>, <o RemoteSockAddr>)|GetAcceptExSockaddrs|Parses the connection endpoints
    // from the buffer passed into AcceptEx
    PyObject *MyGetAcceptExSockaddrs(PyObject *self, PyObject *args)
    {
        PyObject *rv = NULL;
        PyObject *obAccepting = NULL;
        PyObject *obBuf = NULL;
        ...
        if (!PyArg_ParseTuple(args, "OO:GetAcceptExSockaddrs",
                              &obAccepting,  // @pyparm <o PySocket>/int|sAccepting||Socket that was passed into the
                                             // sAccepting parameter of AcceptEx
        ...
        // So we can compute the correct minimum buffer size.
        Py_BEGIN_ALLOW_THREADS rc =
            getsockopt(sAccepting, SOL_SOCKET, SO_PROTOCOL_INFO, (char *)&wsProtInfo, (int *)&cbSize);
        Py_END_ALLOW_THREADS if (rc == SOCKET_ERROR)
        {
            PyWin_SetAPIError("getsockopt", WSAGetLastError());
            return NULL;
        ...
        Py_BEGIN_ALLOW_THREADS GetAcceptExSockaddrs(pybuf.ptr(), pybuf.len() - iMinBufferSize, cbLocal, cbRemote,
                                                    &psaddrLocal, &cbLocal, &psaddrRemote, &cbRemote);
        Py_END_ALLOW_THREADS
...
%{
    PyObject *MyWSAEventSelect(SOCKET *s, PyHANDLE hEvent, LONG lNetworkEvents)
    {
        int rc;
        Py_BEGIN_ALLOW_THREADS;
...
%}
    ...
    // @pyswig |WSAEventSelect|Specifies an event object to be associated with the supplied set of FD_XXXX network
    // events.
%name(WSAEventSelect) PyObject *MyWSAEventSelect
    (SOCKET *s,           // @pyparm <o PySocket>|socket||socket to attach to the event
     ...
     LONG lNetworkEvents  // @pyparm int|networkEvents||A bitmask of network events that will cause hEvent to be
                          // signaled. e.g. (FD_CLOSE \| FD_READ)
    );

%native(WSAEnumNetworkEvents) MyWSAEnumNetworkEvents;

%{
    static int MyCopyEvent(PyObject *dict, WSANETWORKEVENTS *events, long event, int eventbit)
    {
        int res = 0;
    ...
    // @pyswig dict|WSAEnumNetworkEvents|Return network events that caused the event associated with the socket to be
    // signaled.
    // @rdesc A dictionary mapping network events that occurred for the specified socket since the last call to this
    // function (e.g. FD_READ, FD_WRITE) to their associated error code, or 0 if the event occurred without an error.
    // The events returned are a subset of events previously registered for this socket with WSAEventSelect.
    static PyObject *MyWSAEnumNetworkEvents(PyObject *self, PyObject *args)
    {
        PyObject *socket, *event = NULL;
        // @pyparm <o PySocket>|s||Socket to check for netork events, previously registered for network event
        // notification with WSAEventSelect.
        SOCKET s;
        // @pyparm <o PyHANDLE>|hEvent||Optional handle to the event associated with socket s in the last call to
        // WSAEventSelect. If specified, the event will be reset.
        HANDLE hEvent = NULL;
...
%{
    ...
    PyObject *MyWSAAsyncSelect(SOCKET *s, HWND hwnd, LONG wMsg, LONG lNetworkEvents)
    {
        int rc;
        Py_BEGIN_ALLOW_THREADS;
...
%name(WSAAsyncSelect) PyObject *MyWSAAsyncSelect
    (SOCKET *s,           // @pyparm <o PySocket>|socket||socket to attach to the event
     ...
     LONG lNetworkEvents  // @pyparm int|networkEvents||A bitmask of network events that will cause wMsg to be posted.
                          // e.g. (FD_CLOSE \| FD_READ)
    );
...
%{
    ...
    PyObject *MyWSASend(PyObject *self, PyObject *args)
    {
        SOCKET s;
        ...
        if (!PyArg_ParseTuple(args, "OOO|i:WSASend",
        ...
        Py_BEGIN_ALLOW_THREADS;
        rc = WSASend(s, &wsBuf, 1, &cbSent, dwFlags, pOverlapped, NULL);
        Py_END_ALLOW_THREADS;
    ...
    PyObject *MyWSARecv(PyObject *self, PyObject *args)
    {
        SOCKET s;
        PyObject *obSocket = NULL;
        WSABUF wsBuf;
        ...
        if (!PyArg_ParseTuple(args, "OOO|i:WSARecv",
                              &obSocket,      // @pyparm <o PySocket>/int|s||Socket to send data on.
        ...
        Py_BEGIN_ALLOW_THREADS;
        rc = WSARecv(s, &wsBuf, 1, &cbRecvd, &dwFlags, pOverlapped, NULL);
        Py_END_ALLOW_THREADS;
...
%typemap(python,ignore) COMMTIMEOUTS *OUTPUT(COMMTIMEOUTS temp)
    ...
    // @pyswig <o PyDCB>|BuildCommDCB|Fills the specified DCB structure with values specified in a device-control
    // string. The device-control string uses the syntax of the mode command
    BOOLAPI BuildCommDCB(TCHAR *lpDef,  // @pyparm string|def||device-control string
                         DCB *OUTOUT    // @pyparm <o PyDCB>|dcb||The device-control block
    );

%{
    // @pyswig int, <o PyCOMSTAT>|ClearCommError|retrieves information about a communications error and reports the
...
%native (ClearCommError) PyClearCommError;
    ...
    BOOLAPI EscapeCommFunction(
        ...
        int func  // int|func||Specifies the code of the extended function to perform. This parameter can be one of the
                  // following values.
                  // @flagh Value|Meaning
        ...
        // @flag SETBREAK|Suspends character transmission and places the transmission line in a break state until the
        // ClearCommBreak function is called (or EscapeCommFunction is called with the CLRBREAK extended function code).
        // The SETBREAK extended function code is identical to the SetCommBreak function. Note that this extended
        // function does not flush data that has not been transmitted.
        // @flag CLRBREAK|Restores character transmission and places the transmission line in a nonbreak state. The
        // CLRBREAK extended function code is identical to the ClearCommBreak function.
    );

    // @pyswig <o PyDCB>|GetCommState|Returns a device-control block (a DCB structure) with the current control settings
    // for a specified communications device.
    BOOLAPI GetCommState(PyHANDLE handle,  // @pyparm <o PyHANDLE>|handle||The handle to the communications device.
                         DCB *OUTPUT);
    // @pyswig |SetCommState|Configures a communications device according to the specifications in a device-control
    // block. The function reinitializes all hardware and control settings, but it does not empty output or input
    // queues.
    BOOLAPI SetCommState(PyHANDLE handle,  // @pyparm <o PyHANDLE>|handle||The handle to the communications device.
                         DCB *dcb          // @pyparm <o PyDCB>|dcb||The control settings.
    );

    // @pyswig |ClearCommBreak|Restores character transmission for a specified communications device and places the
    // transmission line in a nonbreak state
    BOOLAPI ClearCommBreak(PyHANDLE handle  // @pyparm <o PyHANDLE>|handle||The handle to the communications device.
    );

    // @pyswig int|GetCommMask|Retrieves the value of the event mask for a specified communications device.
    BOOLAPI GetCommMask(PyHANDLE handle,  // @pyparm <o PyHANDLE>|handle||The handle to the communications device.
                        unsigned long *OUTPUT);

    // @pyswig int|SetCommMask|Sets the value of the event mask for a specified communications device.
    BOOLAPI SetCommMask(PyHANDLE handle,   // @pyparm <o PyHANDLE>|handle||The handle to the communications device.
    ...
    // @pyswig <o PyCOMMTIMEOUTS>|GetCommTimeouts|Retrieves the time-out parameters for all read and write operations on
    // a specified communications device.
    BOOLAPI GetCommTimeouts(PyHANDLE handle,  // @pyparm <o PyHANDLE>|handle||The handle to the communications device.
                            COMMTIMEOUTS *OUTPUT);

    // @pyswig int|SetCommTimeouts|Sets the time-out parameters for all read and write operations on a specified
    // communications device.
    BOOLAPI SetCommTimeouts(PyHANDLE handle,  // @pyparm <o PyHANDLE>|handle||The handle to the communications device.
                            COMMTIMEOUTS *timeouts  // @pyparm <o PyCOMMTIMEOUTS>|val||The new time-out parameters.
    );

    // @pyswig |PurgeComm|Discards all characters from the output or input buffer of a specified communications
    // resource. It can also terminate pending read or write operations on the resource.
    BOOLAPI PurgeComm(PyHANDLE handle,   // @pyparm <o PyHANDLE>|handle||The handle to the communications device.
                      unsigned long val  // @pyparm int|action||The action to perform.  This parameter can be one or
                                         // more of the following values.
                                         // @flagh Value|Meaning
                      // @flag PURGE_TXABORT|Terminates all outstanding overlapped write operations and returns
                      // immediately, even if the write operations have not been completed.
                      // @flag PURGE_RXABORT|Terminates all outstanding overlapped read operations and returns
    ...
    // @pyswig |SetCommBreak|Suspends character transmission for a specified communications device and places the
    // transmission line in a break state until the <om win32file.ClearCommBreak> function is called.
    BOOLAPI SetCommBreak(PyHANDLE handle  // @pyparm <o PyHANDLE>|handle||The handle to the communications device.
    );

    // @pyswig |SetupComm|Initializes the communications parameters for a specified communications device.
    BOOLAPI SetupComm(PyHANDLE handle,          // @pyparm <o PyHANDLE>|handle||The handle to the communications device.
                      unsigned long dwInQueue,  // @pyparm int|dwInQueue||Specifies the recommended size, in bytes, of
                                                // the device's internal input buffer.
                      unsigned long dwOutQueue  // @pyparm int|dwOutQueue||Specifies the recommended size, in bytes, of
                                                // the device's internal output buffer.
    );

    // @pyswig |TransmitCommChar|Transmits a specified character ahead of any pending data in the output buffer of the
    // specified communications device.
    BOOLAPI TransmitCommChar(PyHANDLE handle,  // @pyparm <o PyHANDLE>|handle||The handle to the communications device.
                             char ch           // @pyparm char|cChar||The character to transmit.
                             // @comm The TransmitCommChar function is useful for sending an interrupt character (such
                             // as a CTRL+C) to a host system. <nl>If the device is not transmitting, TransmitCommChar
                             // cannot be called repeatedly. Once TransmitCommChar places a character in the output
                             // buffer, the character must be transmitted before the function can be called again. If
                             // the previous character has not yet been sent, TransmitCommChar returns an error.
    );

%{
    // @pyswig |WaitCommEvent|Waits for an event to occur for a specified communications device. The set of events that
    // are monitored by this function is contained in the event mask associated with the device handle.
    static PyObject *MyWaitCommEvent(PyObject *self, PyObject *args)
    {
        ...
        if (!PyArg_ParseTuple(args, "O|O",
                              ...
                              &obOverlapped))  // @pyparm <o PyOVERLAPPED>|overlapped||This structure is required if
                                               // hFile was opened with FILE_FLAG_OVERLAPPED.
            // <nl>If hFile was opened with FILE_FLAG_OVERLAPPED, the lpOverlapped parameter must not be NULL. It must
            // point to a valid OVERLAPPED structure. If hFile was opened with FILE_FLAG_OVERLAPPED and lpOverlapped is
            // NULL, the function can incorrectly report that the operation is complete. <nl>If hFile was opened with
            // FILE_FLAG_OVERLAPPED and lpOverlapped is not NULL, WaitCommEvent is performed as an overlapped operation.
            // In this case, the OVERLAPPED structure must contain a handle to a manual-reset event object (created by
            // using the CreateEvent function). <nl>If hFile was not opened with FILE_FLAG_OVERLAPPED, WaitCommEvent
            // does not return until one of the specified events or an error occurs.
            return NULL;
        HANDLE handle;
        if (!PyWinObject_AsHANDLE(obHandle, &handle))
            return NULL;
        ...
        BOOL ok;
        Py_BEGIN_ALLOW_THREADS ok = WaitCommEvent(handle, pmask, pyoverlapped ? pyoverlapped->GetOverlapped() : NULL);
        Py_END_ALLOW_THREADS DWORD rc = ok ? 0 : GetLastError();
        if (rc != 0 && rc != ERROR_IO_PENDING)
...
%{
    ...
    static PyObject *py_SetVolumeMountPoint(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OO:SetVolumeMountPoint", keywords,
                                         &mount_point_obj,  // @pyparm string|VolumeMountPoint||The mount point - must
                                                            // be an existing empty directory on an NTFS volume
    ...
    static PyObject *py_DeleteVolumeMountPoint(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O:DeleteVolumeMountPoint", keywords,
                                         &mount_point_obj))  // @pyparm string|VolumeMountPoint||The mount point to
                                                             // delete - must have a trailing backslash.
            return NULL;
        if (!PyWinObject_AsWCHAR(mount_point_obj, &mount_point, FALSE))
            return NULL;
    ...
    static PyObject *py_GetVolumeNameForVolumeMountPoint(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O:GetVolumeNameForVolumeMountPoint", keywords,
                                         &obmount_point))  // @pyparm string|VolumeMountPoint||Volume mount point or
                                                           // root drive - trailing backslash required
            return NULL;
    ...
    static PyObject *py_GetVolumePathName(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(
                args, kwargs, "O|l:GetVolumePathName", keywords,
        ...
        else if (!GetVolumePathName(path, mount_point, bufsize))
    ...
    static PyObject *py_GetVolumePathNamesForVolumeName(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O:GetVolumePathNamesForVolumeName", keywords,
                                         &obvolume))  // @pyparm string|VolumeName||Name of a volume as returned by <om
    ...
    static PyObject *py_CreateHardLink(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(
                args, kwargs, "OO|OO:CreateHardLink", keywords,
                &new_file_obj,       // @pyparm string|FileName||The name of the new directory entry to be created.
                &existing_file_obj,  // @pyparm string|ExistingFileName||The name of the existing file to which the new
                                     // link will point.
                &sa_obj,      // @pyparm <o PySECURITY_ATTRIBUTES>|SecurityAttributes|None|Optional SECURITY_ATTRIBUTES
                              // object. MSDN describes this parameter as reserved, so use only None
                &trans_obj))  // @pyparm <o PyHANDLE>|Transaction|None|Handle to a transaction, as returned by <om
    ...
    static PyObject *py_CreateSymbolicLink(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(
                args, kwargs, "OO|kO:CreateSymbolicLink", keywords,
                ...
                &flags,         // @pyparm int|Flags|0|SYMBOLIC_LINK_FLAG_DIRECTORY and
                                // SYMBOLIC_LINK_FLAG_ALLOW_UNPRIVILEGED_CREATE are the only defined flags
        ...
        if (PyWinObject_AsWCHAR(oblinkname, &linkname, FALSE) &&
            PyWinObject_AsWCHAR(obtargetname, &targetname, FALSE)) {
            BOOLEAN bsuccess;
            if (htrans)
    ...
    PyObject *PyWinObject_FromPENCRYPTION_CERTIFICATE_LIST(PENCRYPTION_CERTIFICATE_LIST pecl)
    {
        ...
        for (user_cnt = 0; user_cnt < pecl->nUsers; user_cnt++) {
            ret_item = Py_BuildValue(
                "NN", PyWinObject_FromSID((*user_item)->pUserSid),
    ...
    PyObject *PyWinObject_FromPENCRYPTION_CERTIFICATE_HASH_LIST(PENCRYPTION_CERTIFICATE_HASH_LIST pechl)
    {
        ...
        for (user_cnt = 0; user_cnt < pechl->nCert_Hash; user_cnt++) {
            ret_item = Py_BuildValue(
                "NNu", PyWinObject_FromSID((*user_item)->pUserSid),
    ...
    {
        char *format_msg =
            "ENCRYPTION_CERTIFICATE_LIST must be represented as a sequence of sequences of (PySID, str, int "
            "dwCertEncodingType )";
        BOOL bSuccess = TRUE;
        DWORD cert_ind = 0;
        PENCRYPTION_CERTIFICATE *ppec = NULL;
        ...
        if (ppec == NULL) {
            PyErr_SetString(PyExc_MemoryError,
                            "PyWinObject_AsENCRYPTION_CERTIFICATE_LIST: unable to allocate hash list");
            return NULL;
        }
        ...
        for (cert_ind = 0; cert_ind < cert_cnt; cert_ind++) {
            ...
            if (bSuccess) {
                *ppec = new (ENCRYPTION_CERTIFICATE);
                if (*ppec == NULL) {
                    PyErr_SetString(
                        PyExc_MemoryError,
                        "PyWinObject_AsENCRYPTION_CERTIFICATE_LIST: unable to allocate ENCRYPTION_CERTIFICATE");
                    bSuccess = FALSE;
                }
            }
            if (bSuccess) {
            ...
            if (bSuccess) {
                ...
                if ((*ppec)->pCertBlob == NULL) {
                    PyErr_SetString(
                        PyExc_MemoryError,
                        "PyWinObject_AsENCRYPTION_CERTIFICATE_LIST: unable to allocate EFS_CERTIFICATE_BLOB");
                    bSuccess = FALSE;
                }
            }
            if (bSuccess) {
                ...
                if (!PyLong_Check(obcert_member)) {
                    PyErr_SetString(PyExc_TypeError,
                                    "Second item (dwCertEncodingType) of ENCRYPTION_CERTIFICATE must be an integer");
                    bSuccess = FALSE;
                }
                else
            ...
            if (bSuccess) {
                ...
                Py_ssize_t cbData;
                if (PyBytes_AsStringAndSize(obcert_member, (char **)&((*ppec)->pCertBlob->pbData), &cbData) == -1) {
                    PyErr_SetString(
                        PyExc_TypeError,
                        "Third item of ENCRYPTION_CERTIFICATE must be a string containing encoded certificate data");
                    bSuccess = FALSE;
                }
                else {
    ...
    {
        char *err_msg =
            "ENCRYPTION_CERTIFICATE_HASH_LIST must be represented as a sequence of sequences of (PySID, bytes, string)";
        BOOL bSuccess = TRUE;
        DWORD hash_ind = 0;
        PENCRYPTION_CERTIFICATE_HASH *ppech = NULL;
        ...
        if (ppech == NULL) {
            PyErr_SetString(
                PyExc_MemoryError,
                "PyWinObject_AsENCRYPTION_CERTIFICATE_HASH_LIST: unable to allocate ENCRYPTION_CERTIFICATE_HASH_LIST");
            return FALSE;
        }
        ...
        for (hash_ind = 0; hash_ind < hash_cnt; hash_ind++) {
            ...
            if (bSuccess) {
                *ppech = new (ENCRYPTION_CERTIFICATE_HASH);
                if (*ppech == NULL) {
                    PyErr_SetString(PyExc_MemoryError,
                                    "PyWinObject_AsPENCRYPTION_CERTIFICATE_HASH_LIST: unable to allocate "
                                    "EMCRYPTION_CERTIFICATE_HASH");
                    bSuccess = FALSE;
                }
            }
            if (bSuccess) {
            ...
            if (bSuccess) {
                ...
                if ((*ppech)->pHash == NULL) {
                    PyErr_SetString(
                        PyExc_MemoryError,
                        "PyWinObject_AsPENCRYPTION_CERTIFICATE_HASH_LIST: unable to allocate EFS_HASH_BLOB");
                    bSuccess = FALSE;
                }
            }
            ...
            if (bSuccess) {
                ...
                Py_ssize_t cbData;
                if (PyBytes_AsStringAndSize(obhash_item, (char **)&((*ppech)->pHash->pbData), &cbData) == -1) {
                    PyErr_SetString(PyExc_TypeError,
                                    "Second item of ENCRYPTION_CERTIFICATE_HASH tuple must be a string containing "
    ...
    // @pyswig (<o PySID>,bytes,string)|QueryUsersOnEncryptedFile|Returns list of users for an encrypted file as tuples
    // of (SID, certificate hash blob, display info)
    ...
    // @pyswig (<o PySID>,bytes,string)|QueryRecoveryAgentsOnEncryptedFile|Lists recovery agents for file as a tuple of
    // tuples.
    ...
    // @pyswig |RemoveUsersFromEncryptedFile|Removes specified certificates from file - if certificate is not found, it
    // is ignored
    static PyObject *py_RemoveUsersFromEncryptedFile(PyObject *self, PyObject *args)
    {
        ...
        // @pyparm ((<o PySID>,bytes,string),...)|pHashes||Sequence representing an ENCRYPTION_CERTIFICATE_HASH_LIST
        // structure, as returned by QueryUsersOnEncryptedFile
    ...
    // @pyswig |AddUsersToEncryptedFile|Allows user identified by SID and EFS certificate access to decrypt specified
    // file
    ...
    static PyObject *py_DuplicateEncryptionInfoFile(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        DWORD disp, attr;
        static char *keywords[] = {"SrcFileName", "DstFileName",        "CreationDisposition",
                                   "Attributes",  "SecurityAttributes", NULL};

        if (!PyArg_ParseTupleAndKeywords(
                args, kwargs, "OOkk|O:DuplicateEncryptionInfoFile", keywords,
                ...
                &disp,   // @pyparm int|CreationDisposition||Specifies whether an existing file should be overwritten
                         // (CREATE_NEW or CREATE_ALWAYS)
                &attr,   // @pyparm int|Attributes||File attributes
                &obsa))  // @pyparm <o PySECURITY_ATTRIBUTES>|SecurityAttributes|None|Specifies security for destination
                         // file
            return NULL;
    ...
    static PyObject *py_BackupRead(PyObject *self, PyObject *args)
    {
        ...
        if (!PyArg_ParseTuple(args, "OlOllO:BackupRead", &obh, &bytes_requested, &obbuf, &bAbort, &bProcessSecurity,
                              &obctxt))
            return NULL;
        ...
        if (obbuf == Py_None) {
            ...
            if (pybuf.len() < bytes_requested)
                return PyErr_Format(PyExc_ValueError, "Buffer size (%d) less than requested read size (%d)",
                                    pybuf.len(), bytes_requested);
            Py_INCREF(obbufout);
        }
    ...
    static PyObject *py_BackupSeek(PyObject *self, PyObject *args)
    {
        ...
        if (!BackupSeek(h, bytes_to_seek.LowPart, bytes_to_seek.HighPart, &bytes_moved.LowPart, &bytes_moved.HighPart,
                        &ctxt)) {
    ...
    static PyObject *py_BackupWrite(PyObject *self, PyObject *args)
    {
        ...
        if (!PyArg_ParseTuple(args, "OlOllO:BackupWrite", &obh, &bytes_to_write, &obbuf, &bAbort, &bProcessSecurity,
                              &obctxt))
            return NULL;
        if (!PyWinObject_AsHANDLE(obh, &h))
            return NULL;
        ...
        if (pybuf.len() < bytes_to_write)
            return PyErr_Format(PyExc_ValueError, "Buffer size (%d) less than requested write size (%d)", pybuf.len(),
                                bytes_to_write);
    ...
    static PyObject *py_SetFileShortName(PyObject *self, PyObject *args)
    {
        ...
        if (!PyWinObject_AsWCHAR(obshortname, &shortname, FALSE))
            return NULL;
        Py_BEGIN_ALLOW_THREADS bsuccess = SetFileShortName(h, (LPCWSTR)shortname);
        Py_END_ALLOW_THREADS PyWinObject_FreeWCHAR(shortname);
        if (bsuccess) {
            Py_INCREF(Py_None);
            return Py_None;
        }
    ...
    // @object CopyProgressRoutine|Python function used as a callback for <om win32file.CopyFileEx> and <om
    // win32file.MoveFileWithProgress><nl> Function will receive 9 parameters:<nl> (TotalFileSize,
    // TotalBytesTransferred, StreamSize, StreamBytesTransferred,
    ...
    DWORD CALLBACK CopyFileEx_ProgressRoutine(LARGE_INTEGER TotalFileSize, LARGE_INTEGER TotalBytesTransferred,
                                              ...
                                              HANDLE hDestinationFile, LPVOID lpData)
    {
        ...
        args = Py_BuildValue("LLLLkkNNO", TotalFileSize, TotalBytesTransferred, StreamSize, StreamBytesTransferred,
                             dwStreamNumber, dwCallbackReason, PyWinLong_FromHANDLE(hSourceFile),
                             PyWinLong_FromHANDLE(hDestinationFile), callback_objects[1]);
    ...
    static PyObject *py_CopyFileEx(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(
                args, kwargs, "OO|OOikO:CopyFileEx", keywords,
                &obsrc,       // @pyparm string|ExistingFileName||File to be copied
                &obdst,       // @pyparm string|NewFileName||Place to which it will be copied
                &obcallback,  // @pyparm <o CopyProgressRoutine>|ProgressRoutine|None|A python function that receives
                              // progress updates, can be None
                &obdata,      // @pyparm object|Data|None|An arbitrary object to be passed to the callback function
                &bcancel,     // @pyparm boolean|Cancel|False|Pass True to cancel a restartable copy that was previously
                              // interrupted
        ...
        if (PyWinObject_AsWCHAR(obsrc, &src, FALSE) && PyWinObject_AsWCHAR(obdst, &dst, FALSE)) {
            Py_BEGIN_ALLOW_THREADS if (htrans) bsuccess =
                CopyFileTransacted(src, dst, callback, callback_data, &bcancel, flags, htrans);
            else bsuccess = CopyFileEx(src, dst, callback, callback_data, &bcancel, flags);
    ...
    static PyObject *py_MoveFileWithProgress(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        DWORD flags = 0;
        HANDLE htrans;
        static char *keywords[] = {"ExistingFileName", "NewFileName", "ProgressRoutine", "Data", "Flags",
                                   "Transaction",      NULL};

        if (!PyArg_ParseTupleAndKeywords(
                args, kwargs, "OO|OOkO:MoveFileWithProgress", keywords,
                &obsrc,       // @pyparm string|ExistingFileName||File or directory to be moved
                &obdst,       // @pyparm string|NewFileName||Destination, can be None if flags contain
                              // MOVEFILE_DELAY_UNTIL_REBOOT
                &obcallback,  // @pyparm <o CopyProgressRoutine>|ProgressRoutine|None|A python function that receives
                              // progress updates, can be None
                &obdata,      // @pyparm object|Data|None|An arbitrary object to be passed to the callback function
                &flags,       // @pyparm int|Flags|0|Combination of MOVEFILE_* flags
                &obtrans))    // @pyparm <o PyHANDLE>|Transaction|None|Handle to a transaction (optional).  See <om
                              // win32transaction.CreateTransaction>.
            return NULL;
        ...
        if (PyWinObject_AsWCHAR(obsrc, &src, FALSE) && PyWinObject_AsWCHAR(obdst, &dst, TRUE)) {
            Py_BEGIN_ALLOW_THREADS if (htrans) bsuccess =
                MoveFileTransacted(src, dst, callback, callback_data, flags, htrans);
            else bsuccess = MoveFileWithProgress(src, dst, callback, callback_data, flags);
            Py_END_ALLOW_THREADS if (!bsuccess)
            {
                // progress routine may have already thrown an exception
                if (!PyErr_Occurred())
    ...
    static PyObject *py_ReplaceFile(PyObject *self, PyObject *args)
    {
        ...
        if (!PyArg_ParseTuple(args, "OO|OkOO:ReplaceFile",
                              ...
                              &obbackup,  // @pyparm string|BackupFileName|None|Place at which to create a backup of the
                                          // replaced file, can be None
        ...
        if (PyWinObject_AsWCHAR(obsrc, &dst, FALSE) && PyWinObject_AsWCHAR(obdst, &src, FALSE) &&
            PyWinObject_AsWCHAR(obbackup, &backup, TRUE)) {
            Py_BEGIN_ALLOW_THREADS bsuccess = ReplaceFile(dst, src, backup, flags, Exclude, Reserved);
            Py_END_ALLOW_THREADS if (bsuccess)
            {
                Py_INCREF(Py_None);
                ret = Py_None;
            }
            else PyWin_SetAPIError("ReplaceFile");
        }
        PyWinObject_FreeWCHAR(dst);
        PyWinObject_FreeWCHAR(src);
    ...
    static PyObject *py_OpenEncryptedFileRaw(PyObject *self, PyObject *args)
    {
        ...
        if (!PyWinObject_AsWCHAR(obfname, &fname, FALSE))
            return NULL;
        Py_BEGIN_ALLOW_THREADS err = OpenEncryptedFileRaw(fname, flags, &ctxt),
                               Py_END_ALLOW_THREADS if (err != ERROR_SUCCESS)
    ...
    // &nbsp&nbsp Data: Read-only buffer containing the raw data read from the file.  Must not be referenced outside of
    // the callback function.<nl> &nbsp&nbsp CallbackContext: Arbitrary object passed to ReadEncryptedFileRaw.<nl>
    // &nbsp&nbsp Length: Number of bytes in the Data buffer.<nl>
    // On success, function should return ERROR_SUCCESS.  Otherwise, it can return a win32 error code, or simply raise
    // an exception.
    ...
    static PyObject *py_ReadEncryptedFileRaw(PyObject *self, PyObject *args)
    {
        ...
        if (!PyArg_ParseTuple(args, "OOO:ReadEncryptedFileRaw",
                              &obcallback,  // @pyparm <o ExportCallBack>|ExportCallback||Python function that receives
                                            // chunks of data as it is read
                              &obcallback_data,  // @pyparm object|CallbackContext||Arbitrary Python object to be passed
                                                 // to callback function
                              &obctxt))          // @pyparm PyCObject|Context||Context object returned from <om
        ...
        Py_BEGIN_ALLOW_THREADS retcode = ReadEncryptedFileRaw(PyExportCallback, callback_objects, ctxt);
        Py_END_ALLOW_THREADS if (retcode == ERROR_SUCCESS)
        {
            Py_INCREF(Py_None);
            return Py_None;
        }
    ...
    // Function is called with 3 parameters: (Data, CallbackContext, Length)<nl>
    // &nbsp&nbsp Data: Writeable buffer to be filled with raw encrypted data.  Buffer memory is only valid within the
    // callback function.<nl> &nbsp&nbsp CallbackContext: The arbitrary object passed to WriteEncryptedFileRaw.<nl>
    ...
    static PyObject *py_WriteEncryptedFileRaw(PyObject *self, PyObject *args)
    {
        ...
        if (!PyArg_ParseTuple(args, "OOO:WriteEncryptedFileRaw",
                              &obcallback,  // @pyparm <o ImportCallBack>|ImportCallback||Python function that supplies
                                            // data to be written
                              &obcallback_data,  // @pyparm object|CallbackContext||Arbitrary Python object to be passed
                                                 // to callback function
                              &obctxt))          // @pyparm PyCObject|Context||Context object returned from <om
        ...
        Py_BEGIN_ALLOW_THREADS retcode = WriteEncryptedFileRaw(PyImportCallback, callback_objects, ctxt);
        Py_END_ALLOW_THREADS if (retcode == ERROR_SUCCESS)
        {
            Py_INCREF(Py_None);
            return Py_None;
    ...
    static PyObject *py_CloseEncryptedFileRaw(PyObject *self, PyObject *args)
    {
        ...
        if (!PyCapsule_IsValid(obctxt, NULL))
            return PyErr_Format(PyExc_TypeError, "param must be handle to an encrypted file (got type %s)",
                                Py_TYPE(obctxt)->tp_name);
        if (PyCapsule_GetDestructor(obctxt) != encryptedfilecontextdestructor)
            return PyErr_Format(PyExc_TypeError,
                                "param must be handle to an encrypted file (got a CObject with invalid destructor)");
    ...
    // @pyswig <o PyHANDLE>|CreateFileW|Unicode version of CreateFile - see <om win32file.CreateFile> for more
    // information.
    // @pyseeapi CreateFile
    ...
    {
        WCHAR *filename = NULL;
        PyObject *obfilename, *obsa, *obhtemplate = Py_None, *obhtransaction = Py_None, *obminiversion = Py_None,
                                     *obextendedparameter = Py_None;
        ...
        static char *keywords[] = {"FileName",
                                   ...
                                   NULL};
        if (!PyArg_ParseTupleAndKeywords(
                args, kwargs, "OkkOkk|OOOO:CreateFileW", keywords,
                ...
                &sharemode,      // @pyparm int|ShareMode||Combination of FILE_SHARE_READ, FILE_SHARE_WRITE,
                                 // FILE_SHARE_DELETE
                &obsa,  // @pyparm <o PySECURITY_ATTRIBUTES>|SecurityAttributes||Specifies security descriptor and
                        // handle inheritance, can be None
                &creationdisposition,  // @pyparm int|CreationDisposition||One of
                                       // CREATE_ALWAYS,CREATE_NEW,OPEN_ALWAYS,OPEN_EXISTING or TRUNCATE_EXISTING
                &flags,        // @pyparm int|FlagsAndAttributes||Combination of FILE_ATTRIBUTE_* and FILE_FLAG_* flags
                &obhtemplate,  // @pyparm <o PyHANDLE>|TemplateFile|None|Handle to file to be used as template, can be
                               // None
                &obhtransaction,  // @pyparm <o PyHANDLE>|Transaction|None|Handle to the transaction as returned by <om
                                  // win32transaction.CreateTransaction>
        ...
        Py_BEGIN_ALLOW_THREADS if (htransaction) hret =
            CreateFileTransacted(filename, desiredaccess, sharemode, psa, creationdisposition, flags, htemplate,
                                 htransaction, pminiversion, extendedparameter);
        else hret = CreateFileW(filename, desiredaccess, sharemode, psa, creationdisposition, flags, htemplate);
        Py_END_ALLOW_THREADS
    ...
    static PyObject *py_DeleteFileW(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|O:DeleteFileW", keywords,
                                         ...
                                         &obhtransaction))  // @pyparm <o PyHANDLE>|Transaction|None|Transaction handle
                                                            // as returned by <om win32transaction.CreateTransaction>
            return NULL;
        if (!PyWinObject_AsHANDLE(obhtransaction, &htransaction))
            return NULL;
        if (!PyWinObject_AsWCHAR(obfilename, &filename, FALSE))
        ...
        BOOL ret;
        Py_BEGIN_ALLOW_THREADS if (htransaction) ret = DeleteFileTransacted(filename, htransaction);
        else ret = DeleteFileW(filename);
        Py_END_ALLOW_THREADS
    ...
    static PyObject *PyObject_FromFILEX_INFO(GET_FILEEX_INFO_LEVELS level, void *p)
    {
        switch (level) {
            case GetFileExInfoStandard: {
                ...
                return Py_BuildValue("iNNNN", pa->dwFileAttributes, PyWinObject_FromFILETIME(pa->ftCreationTime),
    ...
    // @pyparm <o PyHANDLE>|Transaction|None|Handle to a transaction (optional).  See <om
    // win32transaction.CreateTransaction>.
    ...
    static PyObject *py_GetFileAttributesEx(PyObject *self, PyObject *args, PyObject *kwargs, BOOL bUnicode)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|kO:GetFileAttributesEx", keywords, &obfname, &lvl, &obtrans))
            return NULL;
        if (!PyWinObject_AsHANDLE(obtrans, &htrans))
            return NULL;
        ...
        Py_END_ALLOW_THREADS
            ...
            if (!ok) PyWin_SetAPIError("GetFileAttributesEx");
        else ret = PyObject_FromFILEX_INFO(lvl, buf);
    ...
    static PyObject *py_SetFileAttributesW(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(
                args, kwargs, "Ok|O:SetFileAttributesW", keywords,
                ...
                &obtrans))  // @pyparm <o PyHANDLE>|Transaction|None|Handle to the transaction.  See <om
                            // win32transaction.CreateTransaction>.
            return NULL;
        if (!PyWinObject_AsHANDLE(obtrans, &htrans))
            return NULL;
    ...
    static PyObject *py_CreateDirectoryExW(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(
                args, kwargs, "OO|OO:CreateDirectoryExW", keywords,
        ...
        BOOL bsuccess;
        if (PyWinObject_AsWCHAR(obdirname, &dirname, FALSE) && PyWinObject_AsWCHAR(obtemplatedir, &templatedir, TRUE)) {
            if (htrans)
    ...
    static PyObject *py_RemoveDirectory(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|O:RemoveDirectory", keywords,
                                         ...
                                         &obtrans))   // @pyparm <o PyHANDLE>|Transaction|None|Handle to a transaction
                                                      // (optional). See <om win32transaction.CreateTransaction>.
            return NULL;
        if (!PyWinObject_AsHANDLE(obtrans, &htrans))
            return NULL;
    ...
    // @pyswig list|FindFilesW|Retrieves a list of matching filenames, using the Windows Unicode API.  An interface to
    // the API FindFirstFileW/FindNextFileW/Find close functions.
    // @comm Accepts keyword args.
    ...
    static PyObject *py_FindFilesW(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(
                args, kwargs, "O|O:FindFilesW", keywords,
                &obfileSpec,  // @pyparm string|FileName||A string that specifies a valid directory or path and
                              // filename, which can contain wildcard characters (* and ?).
                &obtrans))    // @pyparm <o PyHANDLE>|Transaction|None|Transaction handle as returned by <om
                              // win32transaction.CreateTransaction>.  Can be None.
                              //	If this parameter is not None, FindFirstFileTransacted will be called to perform a
                              // transacted search
            return NULL;
        if (!PyWinObject_AsHANDLE(obtrans, &htrans))
            return NULL;
        ...
        if (htrans != NULL)
            hFind = FindFirstFileTransacted(fileSpec, FindExInfoStandard, &findData, FindExSearchNameMatch, NULL, 0,
                                            htrans);
        else
    ...
    static PyObject *py_FindFilesIterator(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(
                args, kwargs, "O|O:FindFilesIterator", keywords,
                &obfileSpec,  // @pyparm string|FileName||A string that specifies a valid directory or path and
                              // filename, which can contain wildcard characters (* and ?).
        ...
        Py_BEGIN_ALLOW_THREADS if (htrans != NULL) it->hFind =
            FindFirstFileTransacted(fileSpec, FindExInfoStandard, &it->buffer, FindExSearchNameMatch, NULL, 0, htrans);
        else it->hFind = ::FindFirstFileW(fileSpec, &it->buffer);
        Py_END_ALLOW_THREADS PyWinObject_FreeWCHAR(fileSpec);
    ...
    static PyObject *py_FindStreams(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(
                args, kwargs, "O|O:FindStreams", keywords,
    ...
    static PyObject *py_FindFileNames(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(
                args, kwargs, "O|O:FindFileNames", keywords,
    ...
    static PyObject *py_GetFinalPathNameByHandle(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(
                args, kwargs, "Ok:GetFinalPathNameByHandle", keywords,
                &obhfile,  // @pyparm <o PyHANDLE>|File||An open file handle
                &flags))   // @pyparm int|Flags||Specifies type of path to return.
                          // (win32con.FILE_NAME_NORMALIZED,FILE_NAME_OPENED,VOLUME_NAME_DOS,VOLUME_NAME_GUID,VOLUME_NAME_NONE,VOLUME_NAME_NT)
            return NULL;
        if (!PyWinObject_AsHANDLE(obhfile, &hfile))
            return NULL;
    ...
    static PyObject *py_GetLongPathName(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|O:GetLongPathName", keywords,
                                         ...
                                         &obtrans))  // @pyparm <o PyHANDLE>|Transaction|None|Handle to a transaction.
                                                     // If specified, GetLongPathNameTransacted will be called.
            return NULL;
        if (!PyWinObject_AsHANDLE(obtrans, &htrans))
            return NULL;
        ...
        while (1) {
            ...
            Py_BEGIN_ALLOW_THREADS if (htrans) retlen =
                GetLongPathNameTransacted(short_path, long_path, pathlen, htrans);
            else retlen = GetLongPathName(short_path, long_path, pathlen);
            Py_END_ALLOW_THREADS if (retlen == 0)
            {
    ...
    static PyObject *py_GetFullPathName(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|O:GetFullPathName", keywords,
                                         ...
                                         &obtrans))  // @pyparm <o PyHANDLE>|Transaction|None|Handle to a transaction as
                                                     // returned by <om win32transaction.CreateTransaction>
            return NULL;
        if (!PyWinObject_AsHANDLE(obtrans, &htrans))
            return NULL;
        ...
        if (TmpWCHAR wpathin = obpathin) {
            ...
            while (1) {
                ...
                Py_BEGIN_ALLOW_THREADS if (htrans) retlen =
                    GetFullPathNameTransactedW(wpathin, pathlen, wpathret, &wfilepart, htrans);
                else retlen = GetFullPathNameW(wpathin, pathlen, wpathret, &wfilepart);
                Py_END_ALLOW_THREADS if (retlen == 0)
                {
        ...
        if (cpathin = PyBytes_AsString(obpathin)) {
            ...
            while (1) {
                ...
                Py_BEGIN_ALLOW_THREADS if (htrans) retlen =
                    GetFullPathNameTransactedA(cpathin, pathlen, cpathret, &cfilepart, htrans);
                else retlen = GetFullPathNameA(cpathin, pathlen, cpathret, &cfilepart);
                Py_END_ALLOW_THREADS if (retlen == 0)
                {
    ...
    // @pyswig int|Wow64DisableWow64FsRedirection|Disables file system redirection for 32-bit processes running on a
    // 64-bit system
    ...
    // @pyswig |Wow64RevertWow64FsRedirection|Reenables file system redirection for 32-bit processes running on a 64-bit
    // system
...
%{
    ...
    // @flag FileIdBothDirectoryInfo|Sequence of dicts representing FILE_ID_BOTH_DIR_INFO structs.  Call in loop until
    // no more files are returned.
    ...
    static PyObject *py_GetFileInformationByHandleEx(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O&i:GetFileInformationByHandleEx", keywords,
                                         PyWinObject_AsHANDLE, &handle, &info_class))
            return NULL;
        ...
        while (true) {
            ...
            Py_BEGIN_ALLOW_THREADS rc = GetFileInformationByHandleEx(handle, info_class, buf, buflen);
            Py_END_ALLOW_THREADS if (rc) break;
            err = GetLastError();
        ...
        switch (info_class) {
            case FileBasicInfo: {
                ...
                ret = Py_BuildValue("{s:N, s:N, s:N, s:N, s:k}", "CreationTime",
                                    PyWinObject_FromTimeStamp(pbi->CreationTime), "LastAccessTime",
                                    PyWinObject_FromTimeStamp(pbi->LastAccessTime), "LastWriteTime",
                                    PyWinObject_FromTimeStamp(pbi->LastWriteTime), "ChangeTime",
                                    PyWinObject_FromTimeStamp(pbi->ChangeTime), "FileAttributes", pbi->FileAttributes);
                break;
            }
            case FileStandardInfo: {
                FILE_STANDARD_INFO *psi = (FILE_STANDARD_INFO *)buf;
                ret = Py_BuildValue(
                    "{s:N, s:N, s:k, s:N, s:N}", "AllocationSize", PyWinObject_FromLARGE_INTEGER(psi->AllocationSize),
                    "EndOfFile", PyWinObject_FromLARGE_INTEGER(psi->EndOfFile), "NumberOfLinks", psi->NumberOfLinks,
                    "DeletePending", PyBool_FromLong(psi->DeletePending), "Directory", PyBool_FromLong(psi->Directory));
                break;
            }
            case FileNameInfo: {
            ...
            case FileCompressionInfo: {
                ...
                ret = Py_BuildValue("{s:N, s:H, s:B, s:B, s:B, s:(BBB)}", "CompressedFileSize",
                                    PyWinObject_FromLARGE_INTEGER(pci->CompressedFileSize), "CompressionFormat",
                                    pci->CompressionFormat, "CompressionUnitShift", pci->CompressionUnitShift,
                                    "ChunkShift", pci->ChunkShift, "ClusterShift", pci->ClusterShift, "Reserved",
                                    pci->Reserved[0], pci->Reserved[1], pci->Reserved[2]);
                break;
            }
            case FileAttributeTagInfo: {
                FILE_ATTRIBUTE_TAG_INFO *pati = (FILE_ATTRIBUTE_TAG_INFO *)buf;
                ret =
                    Py_BuildValue("{s:k, s:k}", "FileAttributes", pati->FileAttributes, "ReparseTag", pati->ReparseTag);
                break;
            }
            case FileIdBothDirectoryInfo:
            case FileIdBothDirectoryRestartInfo: {
                ...
                for (ULONG i = 0; i < file_cnt; i++) {
                    PyObject *file_info =
                        Py_BuildValue("{s:k, s:N, s:N, s:N, s:N, s:N, s:N, s:k, s:k, s:N, s:N, s:N}", "FileIndex",
                                      pdi->FileIndex, "CreationTime", PyWinObject_FromTimeStamp(pdi->CreationTime),
                                      "LastAccessTime", PyWinObject_FromTimeStamp(pdi->LastAccessTime), "LastWriteTime",
                                      PyWinObject_FromTimeStamp(pdi->LastWriteTime), "ChangeTime",
                                      PyWinObject_FromTimeStamp(pdi->ChangeTime), "EndOfFile",
                                      PyWinObject_FromLARGE_INTEGER(pdi->EndOfFile), "AllocationSize",
                                      PyWinObject_FromLARGE_INTEGER(pdi->AllocationSize), "FileAttributes",
                                      pdi->FileAttributes, "EaSize", pdi->EaSize, "ShortName",
                                      PyWinObject_FromWCHAR(pdi->ShortName, pdi->ShortNameLength / sizeof(WCHAR)),
                                      "FileId", PyWinObject_FromLARGE_INTEGER(pdi->FileId), "FileName",
                                      PyWinObject_FromWCHAR(pdi->FileName, pdi->FileNameLength / sizeof(WCHAR)));
                    if (file_info == NULL) {
                        Py_DECREF(ret);
                        ret = NULL;
                        break;
            ...
            case FileStreamInfo: {
                ...
                for (DWORD i = 0; i < stream_cnt; i++) {
                    PyObject *stream_info = Py_BuildValue(
                        "{s:N, s:N, s:N}", "StreamSize", PyWinObject_FromLARGE_INTEGER(psi->StreamSize),
                        "StreamAllocationSize", PyWinObject_FromLARGE_INTEGER(psi->StreamAllocationSize), "StreamName",
                        PyWinObject_FromWCHAR(psi->StreamName, psi->StreamNameLength / sizeof(WCHAR)));
                    if (stream_info == NULL) {
                        Py_DECREF(ret);
                        ret = NULL;
                        break;
...
%{
    ...
    static PyObject *py_SetFileInformationByHandle(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O&iO:SetFileInformationByHandle", keywords,
                                         PyWinObject_AsHANDLE, &handle, &info_class, &info))
            return NULL;
        ...
        switch (info_class) {
            ...
            case FileBasicInfo: {
                ...
                rc = PyArg_ParseTupleAndKeywords(dummy_tuple, info, "O&O&O&O&k", keywords, PyWinObject_AsFILETIME,
                                                 &pbi->CreationTime, PyWinObject_AsFILETIME, &pbi->LastAccessTime,
                                                 PyWinObject_AsFILETIME, &pbi->LastWriteTime, PyWinObject_AsFILETIME,
                                                 &pbi->ChangeTime, &pbi->FileAttributes);
                break;
            }
            ...
            // MSDN says the RootDirectory is "A handle to the root directory in which the file to be renamed is
            // located". However, this is actually the destination dir, can be None to stay in same dir.
            case FileRenameInfo: {
                TmpPyObject dummy_tuple = PyTuple_New(0);
                if (dummy_tuple == NULL)
                    return NULL;
                ...
                rc = PyArg_ParseTupleAndKeywords(dummy_tuple, info, "iO&O", keywords, &ReplaceIfExists,
                                                 PyWinObject_AsHANDLE, &RootDirectory, &obFileName) &&
        ...
        Py_BEGIN_ALLOW_THREADS rc = SetFileInformationByHandle(handle, info_class, buf, buflen);
...
%{
    ...
    static PyObject *py_ReOpenFile(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O&kkk:ReOpenFile", keywords, PyWinObject_AsHANDLE, &horig,
                                         &DesiredAccess, &ShareMode, &Flags))
            return NULL;

        Py_BEGIN_ALLOW_THREADS hret = ReOpenFile(horig, DesiredAccess, ShareMode, Flags);
        Py_END_ALLOW_THREADS if (hret == INVALID_HANDLE_VALUE) return PyWin_SetAPIError("ReOpenFile");
        return PyWinObject_FromHANDLE(hret);
    }
...
%{
    ...
    {
        static char *keywords[] = {"File", "FileID", "DesiredAccess", "ShareMode", "Flags", "SecurityAttributes", NULL};
        HANDLE hvol, hret;
        DWORD DesiredAccess, ShareMode, Flags;
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O&Okkk|O:OpenFileById", keywords, PyWinObject_AsHANDLE, &hvol,
                                         &obfileid, &DesiredAccess, &ShareMode, &Flags, &obsa))
            return NULL;
        ...
        Py_BEGIN_ALLOW_THREADS hret = OpenFileById(hvol, &fileid, DesiredAccess, ShareMode, sa, Flags);
        Py_END_ALLOW_THREADS if (hret == INVALID_HANDLE_VALUE) return PyWin_SetAPIError("OpenFileById");
        return PyWinObject_FromHANDLE(hret);
    }
...
%init %{
    ...
    if (PyType_Ready(&FindFileIterator_Type) == -1 || PyType_Ready(&PyDCB::type) == -1 ||
    ...
    for (PyMethodDef *pmd = win32fileMethods; pmd->ml_name; pmd++)
        if ((strcmp(pmd->ml_name, "CreateFileW") == 0) || (strcmp(pmd->ml_name, "DeleteFileW") == 0) ||
            (strcmp(pmd->ml_name, "MoveFileWithProgress") == 0) || (strcmp(pmd->ml_name, "CopyFileEx") == 0) ||
            (strcmp(pmd->ml_name, "GetFileAttributesEx") == 0) || (strcmp(pmd->ml_name, "GetFileAttributesExW") == 0) ||
            (strcmp(pmd->ml_name, "SetFileAttributesW") == 0) || (strcmp(pmd->ml_name, "CreateHardLink") == 0) ||
            (strcmp(pmd->ml_name, "CreateSymbolicLink") == 0) || (strcmp(pmd->ml_name, "CreateDirectoryExW") == 0) ||
            (strcmp(pmd->ml_name, "RemoveDirectory") == 0) || (strcmp(pmd->ml_name, "FindFilesW") == 0) ||
            (strcmp(pmd->ml_name, "FindFilesIterator") == 0) || (strcmp(pmd->ml_name, "FindStreams") == 0) ||
            (strcmp(pmd->ml_name, "FindFileNames") == 0) || (strcmp(pmd->ml_name, "GetFinalPathNameByHandle") == 0) ||
            ...
            (strcmp(pmd->ml_name, "GetLongPathName") == 0) || (strcmp(pmd->ml_name, "GetFullPathName") == 0) ||
            (strcmp(pmd->ml_name, "GetFileInformationByHandleEx") == 0) ||
            (strcmp(pmd->ml_name, "SetFileInformationByHandle") == 0) ||
            (strcmp(pmd->ml_name, "DeviceIoControl") == 0) || (strcmp(pmd->ml_name, "TransmitFile") == 0) ||
            (strcmp(pmd->ml_name, "ConnectEx") == 0) || (strcmp(pmd->ml_name, "ReOpenFile") == 0) ||
            (strcmp(pmd->ml_name, "OpenFileById") == 0) || (strcmp(pmd->ml_name, "SetFileTime") == 0))
...
#define EV_RXFLAG \
        EV_RXFLAG  // The event character was received and placed in the input buffer. The event character is specified
                   // in the device's DCB structure, which is applied to a serial port by using the SetCommState
                   // function.
...
#define DTR_CONTROL_HANDSHAKE \
        DTR_CONTROL_HANDSHAKE  // Enables DTR handshaking. If handshaking is enabled, it is an error for the application
...
#define RTS_CONTROL_TOGGLE \
        RTS_CONTROL_TOGGLE  // Specifies that the RTS line will be high if bytes are available for transmission. After
...
#define SETBREAK \
        SETBREAK  // Suspends character transmission and places the transmission line in a break state until the
                  // ClearCommBreak function is called (or EscapeCommFunction is called with the CLRBREAK extended
                  // function code). The SETBREAK extended function code is identical to the SetCommBreak function. Note
                  // that this extended function does not flush data that has not been transmitted.
#define CLRBREAK \
        CLRBREAK  // Restores character transmission and places the transmission line in a nonbreak state. The CLRBREAK
                  // extended function code is identical to the ClearCommBreak function.
#define PURGE_TXABORT \
        PURGE_TXABORT  // Terminates all outstanding overlapped write operations and returns immediately, even if the
                       // write operations have not been completed.
#define PURGE_RXABORT \
        PURGE_RXABORT  // Terminates all outstanding overlapped read operations and returns immediately, even if the

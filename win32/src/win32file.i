...
static PyObject *myGetQueuedCompletionStatus(PyObject *self, PyObject *args)
{
    ...
    Py_END_ALLOW_THREADS
    PyObject *rc = Py_BuildValue("ilNN", errCode, bytes, PyWinObject_FromULONG_PTR(key),
...
%{
    ...
    static PyObject *MyAcceptEx(PyObject *self, PyObject *args)
    {
        ...
        Py_BEGIN_ALLOW_THREADS
            rc = getsockopt(sListening, SOL_SOCKET, SO_PROTOCOL_INFO, (char *)&wsProtInfo, (int *)&cbSize);
        Py_END_ALLOW_THREADS
        if (rc == SOCKET_ERROR) {
            PyWin_SetAPIError("getsockopt", WSAGetLastError());
            return NULL;
        ...
        Py_BEGIN_ALLOW_THREADS
            ok = AcceptEx(sListening, sAccepting, pybuf.ptr(), pybuf.len() - iMinBufferSize,
                          wsProtInfo.iMaxSockAddr + 16, wsProtInfo.iMaxSockAddr + 16, &cBytesRecvd, pOverlapped);
        Py_END_ALLOW_THREADS
        if (!ok) {
            rc = WSAGetLastError();
    ...
    PyObject *MyCalculateSocketEndPointSize(PyObject *self, PyObject *args)
    {
        ...
        // Grab the protocol information for the socket
        Py_BEGIN_ALLOW_THREADS
            rc = getsockopt(s, SOL_SOCKET, SO_PROTOCOL_INFO, (char *)&wsProtInfo, (int *)&cbSize);
        Py_END_ALLOW_THREADS
        if (rc == SOCKET_ERROR) {
            PyWin_SetAPIError("getsockopt", WSAGetLastError());
            return NULL;
...
%{
    ...
    PyObject *MyGetAcceptExSockaddrs(PyObject *self, PyObject *args)
    {
        ...
        // So we can compute the correct minimum buffer size.
        Py_BEGIN_ALLOW_THREADS
            rc = getsockopt(sAccepting, SOL_SOCKET, SO_PROTOCOL_INFO, (char *)&wsProtInfo, (int *)&cbSize);
        Py_END_ALLOW_THREADS
        if (rc == SOCKET_ERROR) {
            PyWin_SetAPIError("getsockopt", WSAGetLastError());
            return NULL;
        ...
        Py_BEGIN_ALLOW_THREADS
            GetAcceptExSockaddrs(pybuf.ptr(), pybuf.len() - iMinBufferSize, cbLocal, cbRemote, &psaddrLocal, &cbLocal,
                                 &psaddrRemote, &cbRemote);
        Py_END_ALLOW_THREADS
...
%{
    ...
    static PyObject *MyWaitCommEvent(PyObject *self, PyObject *args)
    {
        ...
        BOOL ok;
        Py_BEGIN_ALLOW_THREADS
            ok = WaitCommEvent(handle, pmask, pyoverlapped ? pyoverlapped->GetOverlapped() : NULL);
        Py_END_ALLOW_THREADS
...
%{
    ...
    static PyObject *py_CreateFileW(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        Py_BEGIN_ALLOW_THREADS
            if (htransaction)
                hret = CreateFileTransacted(filename, desiredaccess, sharemode, psa, creationdisposition, flags,
                                            htemplate, htransaction, pminiversion, extendedparameter);
            else
                hret = CreateFileW(filename, desiredaccess, sharemode, psa, creationdisposition, flags, htemplate);
        Py_END_ALLOW_THREADS
    ...
    static PyObject *py_FindFilesIterator(PyObject *self, PyObject *args, PyObject *kwargs)
    {
        ...
        Py_BEGIN_ALLOW_THREADS
            if (htrans != NULL)
                it->hFind = FindFirstFileTransacted(fileSpec, FindExInfoStandard, &it->buffer, FindExSearchNameMatch,
                                                    NULL, 0, htrans);
            else

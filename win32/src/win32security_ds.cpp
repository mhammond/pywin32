...
static PyObject *PyObject_FromDS_NAME_RESULT(DS_NAME_RESULT *dsresult)
{
    ...
    for (DWORD i = 0; i < dsresult->cItems; i++) {
        ...
        PyList_SET_ITEM(
            ret, i,
            Py_BuildValue("iNN", pi->status, PyWinObject_FromWCHAR(pi->pDomain), PyWinObject_FromWCHAR(pi->pName)));
    }
    return ret;
}
...
class PyDS_HANDLE : public PyHANDLE {
   public:
    ...
    virtual const char *GetTypeName() { return "PyDS_HANDLE"; }
};
...
PyObject *PyDsGetDcName(PyObject *self, PyObject *args, PyObject *kw)
{
    ...
    DWORD flags = 0;
    DWORD err;
    if (!PyArg_ParseTupleAndKeywords(args, kw, "|OOOOi:DsGetDcName", kw_items, &obServer, &obDomain, &obGUID,
                                     &obSiteName, &flags))
        return NULL;
    ...
    ret = Py_BuildValue("{s:N,s:N,s:i,s:N,s:N,s:N,s:i,s:N,s:N}", "DomainControllerName",
                        PyWinObject_FromTCHAR(pdci->DomainControllerName), "DomainControllerAddress",
                        PyWinObject_FromTCHAR(pdci->DomainControllerAddress), "DomainControllerAddressType",
                        pdci->DomainControllerAddressType, "DomainGuid", PyWinObject_FromIID(pdci->DomainGuid),
                        "DomainName", PyWinObject_FromTCHAR(pdci->DomainName), "DnsForestName",
                        PyWinObject_FromTCHAR(pdci->DnsForestName), "Flags", pdci->Flags, "DcSiteName",
                        PyWinObject_FromTCHAR(pdci->DcSiteName), "ClientSiteName",
                        PyWinObject_FromTCHAR(pdci->ClientSiteName));

...
)
{
    ...
    Py_BEGIN_ALLOW_THREADS
        sh = CreateService(hSCManager, lpServiceName, lpDisplayName, dwDesiredAccess, dwServiceType, dwStartType,
                           dwErrorControl, lpBinaryPathName, lpLoadOrderGroup, pTagID, lpDeps, lpServiceStartName,
                           lpPassword);
...
static PyObject *MyEnumServicesStatusEx(PyObject *self, PyObject *args)
{
    ...
    while (true) {
        Py_BEGIN_ALLOW_THREADS
            bsuccess = EnumServicesStatusExW(hscm, (SC_ENUM_TYPE)lvl, service_type, service_state, buf, buf_size,
                                             &buf_needed, &nbr_returned, &resume_handle, grp);
        Py_END_ALLOW_THREADS
        if (!bsuccess) {
            err = GetLastError();
...
static PyObject *MyEnumDependentServices(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS
        ...
        if (!result && GetLastError() == ERROR_MORE_DATA) {
            ...
            services = (LPENUM_SERVICE_STATUS)buffer;
            result =
                EnumDependentServices(hsc, serviceState, services, bytesNeeded + 1, &bytesNeeded, &servicesReturned);
        }

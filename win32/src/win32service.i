...
if (PyType_Ready(&PyHWINSTAType) == -1 || PyType_Ready(&PyHDESKType) == -1)
    return NULL;
...
class PySC_HANDLE : public PyHANDLE {
   public:
    ...
    virtual const char *GetTypeName() { return "PySC_HANDLE"; }
};
...
// @object PyHWINSTA|Wrapper for a handle to a window station - returned by CreateWindowStation, OpenWindowStation, or
...
struct PyMethodDef PyHWINSTA::methods[] = {
    {"EnumDesktops", PyHWINSTA::EnumDesktops, METH_VARARGS,
     "List desktop names within the window station"},  // @pymeth EnumDesktops|List desktop names within the window
                                                       // station
    {"SetProcessWindowStation", PyHWINSTA::SetProcessWindowStation, METH_VARARGS,
     "Associates the calling process with the window station"},  // @pymeth SetProcessWindowStation|Associates the
                                                                 // calling process with the window station
    {"CloseWindowStation", PyHWINSTA::CloseWindowStation, METH_VARARGS,
     "Closes the window station handle"},  // @pymeth CloseWindowStation|Closes the window station handle
    {"Detach", PyHANDLE::Detach, METH_VARARGS,
     "Releases reference to handle without closing it"},  //@pymeth Detach|Releases reference to handle without closing
                                                          // it
    {NULL}};
...
PyTypeObject PyHWINSTAType = {PYWIN_OBJECT_HEAD "PyHWINSTA",
                              sizeof(PyHWINSTA),
                              0,
...
PyHWINSTA::PyHWINSTA(HWINSTA hwinsta) : PyHANDLE((HANDLE)hwinsta) { ob_type = &PyHWINSTAType; }
PyHWINSTA::~PyHWINSTA(void) { ::CloseWindowStation((HWINSTA)m_handle); }

void PyHWINSTA::deallocFunc(PyObject *ob) { delete (PyHWINSTA *)ob; }
...
struct PyMethodDef PyHDESK::methods[] = {
    {"SetThreadDesktop", PyHDESK::SetThreadDesktop, METH_VARARGS,
     "Assigns desktop to calling thread"},  // @pymeth SetThreadDesktop|Assigns desktop to calling thread
    {"EnumDesktopWindows", PyHDESK::EnumDesktopWindows, METH_VARARGS,
     "Lists all top-level windows on the desktop"},  // @pymeth EnumDesktopWindows|Lists all top-level windows on the
                                                     // desktop
    {"SwitchDesktop", PyHDESK::SwitchDesktop, METH_VARARGS,
     "Activates the desktop"},  // @pymeth SwitchDesktop|Activates the desktop
    {"CloseDesktop", PyHDESK::CloseDesktop, METH_VARARGS,
     "Closes the handle"},  //@pymeth CloseDesktop|Closes the desktop handle
    {"Detach", PyHANDLE::Detach, METH_VARARGS,
     "Releases reference to handle without closing it"},  //@pymeth Detach|Releases reference to handle without closing
                                                          // it
    {NULL}};
...
PyTypeObject PyHDESKType = {PYWIN_OBJECT_HEAD "PyHDESK",
                            sizeof(PyHDESK),
                            0,
...
PyHDESK::PyHDESK(HDESK hdesk) : PyHANDLE((HANDLE)hdesk) { ob_type = &PyHDESKType; }

PyHDESK::~PyHDESK(void) { ::CloseDesktop((HDESK)m_handle); }

void PyHDESK::deallocFunc(PyObject *ob) { delete (PyHDESK *)ob; }
...
PyObject *PyGetUserObjectInformation(PyObject *self, PyObject *args)
{
    ...
    if (!bsuccess) {
        ...
        if (err == ERROR_INSUFFICIENT_BUFFER) {
            ...
            if (buf == NULL)
                return PyErr_Format(PyExc_MemoryError, "GetUserObjectInformation unable to allocate %d bytes",
                                    reqdbuflen);
    ...
    if (!bsuccess)
        ...
        switch (info_type) {
            ...
            case UOI_FLAGS: {
                ret = Py_BuildValue("{s:N,s:N,s:l}", "Inherit", PyBool_FromLong(((USEROBJECTFLAGS *)buf)->fInherit),
                                    "Reserved", PyBool_FromLong(((USEROBJECTFLAGS *)buf)->fReserved), "Flags",
                                    ((USEROBJECTFLAGS *)buf)->dwFlags);
                break;
            }
            default:
...
PyObject *PySetUserObjectInformation(PyObject *self, PyObject *args)
{
    ...
    // @pyparm <o PyHANDLE>|Handle||Handle to window station or desktop
    // @pyparm object|info||Information to set for handle, currently only a dictionary representing USEROBJECTFLAGS
    // struct
    ...
    if (!PyArg_ParseTupleAndKeywords(dummy_tuple, obinfo, "lll", uof_members, &uof.fInherit, &uof.fReserved,
                                     &uof.dwFlags)) {
        PyErr_SetString(PyExc_TypeError, uof_format);
        return NULL;
    }
...
PyObject *PyCreateWindowStation(PyObject *self, PyObject *args)
{
    ...
    // @pyparm <o PySECURITY_ATTRIBUTES>|SecurityAttributes||Specifies security for window station, and whether handle
    // is inheritable
...
%{
PyObject *MyCreateService(SC_HANDLE hSCManager,     // handle to service control manager database
...
)
{
    ...
    Py_BEGIN_ALLOW_THREADS sh = CreateService(hSCManager, lpServiceName, lpDisplayName, dwDesiredAccess, dwServiceType,
                                              dwStartType, dwErrorControl, lpBinaryPathName, lpLoadOrderGroup, pTagID,
                                              lpDeps, lpServiceStartName, lpPassword);
    Py_END_ALLOW_THREADS if (sh == 0)
    {
...
PyObject *MyChangeServiceConfig(SC_HANDLE hSCManager,     // handle to service control manager database
                                DWORD dwServiceType,      // type of service
                                DWORD dwStartType,        // when to start service
                                DWORD dwErrorControl,     // severity if service fails to start
                                TCHAR *lpBinaryPathName,  // pointer to name of binary file
...
)
{
    ...
    if (!ChangeServiceConfig(hSCManager, dwServiceType, dwStartType, dwErrorControl, lpBinaryPathName, lpLoadOrderGroup,
                             pTagID, lpDeps, lpServiceStartName, lpPassword, lpDisplayName))
...
PyObject *MyStartService(SC_HANDLE scHandle, PyObject *serviceArgs)
{
    ...
    PyObject *rc;
    BOOL ok = FALSE;
    Py_BEGIN_ALLOW_THREADS ok = StartService(scHandle, numStrings, (LPCWSTR *)pArgs);
    Py_END_ALLOW_THREADS if (ok)
    {
        rc = Py_None;
        Py_INCREF(Py_None);
    }
    else rc = PyWin_SetAPIError("StartService");
...
// @rdesc Returns a sequence of tuples representing ENUM_SERVICE_STATUS structs: (ServiceName, DisplayName, <o
// SERVICE_STATUS>)
...
{
    // @pyparm <o PySC_HANDLE>|hSCManager||Handle to service control manager as returned by <om
    ...
    Py_BEGIN_ALLOW_THREADS
        ...
        EnumServicesStatus(hscm, serviceType, serviceState, services, sizeof(tmp), &bytesNeeded, &servicesReturned,
                           &resumeHandle);

    if (GetLastError() == ERROR_MORE_DATA) {
        buffer = new char[bytesNeeded + 1];
        services = (LPENUM_SERVICE_STATUS)buffer;
        result = EnumServicesStatus(hscm, serviceType, serviceState, services, bytesNeeded + 1, &bytesNeeded,
    ...
    for (DWORD i = 0; i < servicesReturned; i++) {
        ...
        PyTuple_SetItem(
            retval, i,
            Py_BuildValue("OO(lllllll)", obServiceName, obDisplayName, services[i].ServiceStatus.dwServiceType,
                          services[i].ServiceStatus.dwCurrentState, services[i].ServiceStatus.dwControlsAccepted,
                          services[i].ServiceStatus.dwWin32ExitCode,
                          services[i].ServiceStatus.dwServiceSpecificExitCode, services[i].ServiceStatus.dwCheckPoint,
...
{
    // @pyparm <o PySC_HANDLE>|SCManager||Handle to service control manager as returned by <om
    // win32service.OpenSCManager>
    // @pyparm int|ServiceType|SERVICE_WIN32|Types of services to enumerate (SERVICE_DRIVER and/or SERVICE_WIN32)
    // @pyparm int|ServiceState|SERVICE_STATE_ALL|Limits to services in specified state
    ...
    if (!PyArg_ParseTuple(args, "O&|kkOk:EnumServicesStatusEx", PyWinObject_AsHANDLE, &hscm, &service_type,
                          &service_state, &obgrp, &lvl))
        return NULL;
    ...
    while (true) {
        Py_BEGIN_ALLOW_THREADS bsuccess =
            EnumServicesStatusExW(hscm, (SC_ENUM_TYPE)lvl, service_type, service_state, buf, buf_size, &buf_needed,
                                  &nbr_returned, &resume_handle, grp);
        ...
        for (DWORD i = 0; i < nbr_returned; i++) {
            ...
            TmpPyObject stat = Py_BuildValue(
                "{s:N, s:N, s:k, s:k, s:k, s:k, s:k, s:k, s:k, s:k, s:k}", "ServiceName",
                PyWinObject_FromTCHAR(essp[i].lpServiceName), "DisplayName",
                PyWinObject_FromTCHAR(essp[i].lpDisplayName), "ServiceType", essp[i].ServiceStatusProcess.dwServiceType,
                "CurrentState", essp[i].ServiceStatusProcess.dwCurrentState, "ControlsAccepted",
                essp[i].ServiceStatusProcess.dwControlsAccepted, "Win32ExitCode",
                essp[i].ServiceStatusProcess.dwWin32ExitCode, "ServiceSpecificExitCode",
                essp[i].ServiceStatusProcess.dwServiceSpecificExitCode, "CheckPoint",
                essp[i].ServiceStatusProcess.dwCheckPoint, "WaitHint", essp[i].ServiceStatusProcess.dwWaitHint,
                "ProcessId", essp[i].ServiceStatusProcess.dwProcessId, "ServiceFlags",
                essp[i].ServiceStatusProcess.dwServiceFlags);
...
{
    // @pyparm <o PySC_HANDLE>|hService||Handle to service for which to list dependent services (as returned by <om
    // win32service.OpenService>)
    // @pyparm int|ServiceState|SERVICE_STATE_ALL|Limits to services in specified state - One of SERVICE_STATE_ALL,
    ...
    Py_BEGIN_ALLOW_THREADS
        ...
        result = EnumDependentServices(hsc, serviceState, services, sizeof(tmp), &bytesNeeded, &servicesReturned);

    if (!result && GetLastError() == ERROR_MORE_DATA) {
        buffer = new char[bytesNeeded + 1];
        services = (LPENUM_SERVICE_STATUS)buffer;
        result = EnumDependentServices(hsc, serviceState, services, bytesNeeded + 1, &bytesNeeded, &servicesReturned);
    }
...
static PyObject *MyQueryServiceConfig(PyObject *self, PyObject *args)
{
    ...
    PyObject *retval =
        Py_BuildValue("lllNNlNNN", config->dwServiceType, config->dwStartType, config->dwErrorControl,
                      PyWinObject_FromTCHAR(config->lpBinaryPathName), PyWinObject_FromTCHAR(config->lpLoadOrderGroup),
                      config->dwTagId, PyWinObject_FromMultipleString(config->lpDependencies),
...
$target = &junk;
if (!PyArg_ParseTuple($source, "lllllll", &$target->dwServiceType, &$target->dwCurrentState,
                      &$target->dwControlsAccepted, &$target->dwWin32ExitCode, &$target->dwServiceSpecificExitCode,
                      &$target->dwCheckPoint, &$target->dwWaitHint))
    return NULL;
}
...
%name (StartService) PyObject *MyStartService (
     ...
     SC_HANDLE OpenSCManager(TCHAR *INPUT_NULLOK,  // @pyparm string|machineName||The name of the computer, or None
                             TCHAR *INPUT_NULLOK,  // @pyparm string|dbName||The name of the service database, or None
                             unsigned long desiredAccess);  // @pyparm int|desiredAccess||The access desired.
                                                            // (combination of win32service.SC_MANAGER_* flags)
...
%{
     PyObject *MyQueryServiceStatusEx(PyObject *self, PyObject *args)
     {
         ...
         return Py_BuildValue("{s:l,s:l,s:l,s:l,s:l,s:l,s:l,s:l,s:l}", "ServiceType", info.dwServiceType,
                              "CurrentState", info.dwCurrentState, "ControlsAccepted", info.dwControlsAccepted,
                              "Win32ExitCode", info.dwWin32ExitCode, "ServiceSpecificExitCode",
                              info.dwServiceSpecificExitCode, "CheckPoint", info.dwCheckPoint, "WaitHint",
                              info.dwWaitHint, "ProcessId", info.dwProcessId, "ServiceFlags", info.dwServiceFlags);
     }
%}
...
%{
     PyObject *MySetServiceObjectSecurity(PyObject *self, PyObject *args)
     {
         ...
         // @pyparm int|SecurityInformation||Type of infomation to set, combination of values from SECURITY_INFORMATION
         // enum
...
%}
     ...
     // @pyswig <o PySECURITY_DESCRIPTOR>|QueryServiceObjectSecurity|Retrieves information from the security descriptor
     // for a service
...
%{
     PyObject *MyQueryServiceObjectSecurity(PyObject *self, PyObject *args)
     {
         ...
         PyObject *obhsvc;
         // @pyparm <o PySC_HANDLE>|Handle||Service handle
         // @pyparm int|SecurityInformation||Type of infomation to retrieve, combination of values from
         // SECURITY_INFORMATION enum
         ...
         if (!QueryServiceObjectSecurity(hsvc, info, pSD, origbufsize, &reqdbufsize)) {
             ...
             if (err == ERROR_INSUFFICIENT_BUFFER) {
                 ...
                 if (pSD == NULL)
                     PyErr_Format(PyExc_MemoryError, "QueryServiceObjectSecurity: unable to reallocatate %d bytes",
                                  reqdbufsize);
                 else if (!QueryServiceObjectSecurity(hsvc, info, pSD, reqdbufsize, &reqdbufsize))
...
%}
     ...
     BOOLAPI ControlService(SC_HANDLE handle,  // @pyparm <o PySC_HANDLE>|scHandle||Handle to control
...
%name (ChangeServiceConfig) PyObject * MyChangeServiceConfig(
    ...
    SC_LOCK LockServiceDatabase(SC_HANDLE handle  // @pyparm <o PySC_HANDLE>|sc_handle||A handle to the SCM.
    );
...
%{
    // @pyswig (int, string, int)|QueryServiceLockStatus|Retrieves the lock status of the specified service control
    // manager database.
...
%{
    ...
    BOOL PyWinObject_AsSERVICE_FAILURE_ACTIONS(PyObject *obinfo, LPSERVICE_FAILURE_ACTIONSW psfa)
    {
        ...
        static char *err =
            ...
            "{'ResetPeriod':int,'RebootMsg':unicode,'Command':unicode,'Actions':sequence of 2 tuples(int,int)";
    ...
    PyObject *PyWinObject_FromSERVICE_FAILURE_ACTIONS(LPSERVICE_FAILURE_ACTIONSW psfa)
    {
        ...
        return Py_BuildValue("{s:l,s:u,s:u,s:N}", "ResetPeriod", psfa->dwResetPeriod, "RebootMsg", psfa->lpRebootMsg,
                             "Command", psfa->lpCommand, "Actions", obActions);
    }
...
%{
    PyObject *PyQueryServiceConfig2(PyObject *self, PyObject *args)
    {
        ...
        if (buf == NULL)
            return PyErr_Format(PyExc_MemoryError, "QueryServiceConfig2: Unable to allocate buffer of %d bytes",
                                bytes_needed);
        ...
        if (QueryServiceConfig2(hService, level, buf, bufsize, &bytes_needed))
            switch (level) {
                ...
                case SERVICE_CONFIG_REQUIRED_PRIVILEGES_INFO:
                    ret = PyWinObject_FromMultipleString(
                        ((SERVICE_REQUIRED_PRIVILEGES_INFO *)buf)->pmszRequiredPrivileges);
                    break;
                default:
...
#define SERVICE_CONTROL_INTERROGATE SERVICE_CONTROL_INTERROGATE
    // Requests the service to update immediately its current status information to the service control manager. The
...
#define SERVICE_ACCEPT_PAUSE_CONTINUE SERVICE_ACCEPT_PAUSE_CONTINUE
    // The service can be paused and continued. This enables the SERVICE_CONTROL_PAUSE and SERVICE_CONTROL_CONTINUE
    // values.
#define SERVICE_ACCEPT_SHUTDOWN SERVICE_ACCEPT_SHUTDOWN
    // The service is notified when system shutdown occurs. This enables the system to send a SERVICE_CONTROL_SHUTDOWN
    // value to the service. The ControlService function cannot send this control
...
#define SERVICE_BOOT_START SERVICE_BOOT_START
    // Specifies a device driver started by the operating system loader. This value is valid only if the service type is
...
#define SERVICE_AUTO_START SERVICE_AUTO_START
    // Specifies a device driver or Win32 service started by the service control manager automatically during system
    // startup.
#define SERVICE_DEMAND_START SERVICE_DEMAND_START
    // Specifies a device driver or Win32 service started by the service control manager when a process calls the
...
#define SERVICE_QUERY_STATUS SERVICE_QUERY_STATUS
    // Enables calling of the QueryServiceStatus function to ask the service control manager about the status of the
    // service.

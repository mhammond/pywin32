                        ...
                        PyObject *obData)     // raw data
{
    ...
    BOOL ok;
    Py_BEGIN_ALLOW_THREADS
        ok = ReportEventW(hEventLog, wType, wCategory, dwEventID, sid, (WORD)numStrings, pybuf.len(),
...
%{
     ...
     static PyObject *RenderEventValues(EVT_HANDLE render_context, EVT_HANDLE event)
     {
         ...
         Py_BEGIN_ALLOW_THREADS
             bsuccess = EvtRender(render_context, event, EvtRenderEventValues, allocated_size, variants, &returned_size,
                                  &prop_count);
         Py_END_ALLOW_THREADS
         ...
         if (!bsuccess) {
             ...
             Py_BEGIN_ALLOW_THREADS
                 bsuccess = EvtRender(render_context, event, EvtRenderEventValues, allocated_size, variants,
                                      &returned_size, &prop_count);
             Py_END_ALLOW_THREADS
         }
     ...
     // to an event log record (if not called due to an error) 	If an event handle is passed in, a pull subscription is
     // created.  The event handle will be 	signalled when events are available, and the subscription handle can be
     ...
     static PyObject *PyEvtSubscribe(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         Py_BEGIN_ALLOW_THREADS
             ret = EvtSubscribe(session, signalevent, path, query, bookmark, (void *)obuserdata, pfncallback, flags);
         Py_END_ALLOW_THREADS
         if (ret == NULL)

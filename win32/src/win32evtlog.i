...
class PyEVTLOG_HANDLE : public PyHANDLE {
   public:
    ...
    virtual const char *GetTypeName() { return "PyEVTLOG_HANDLE"; }
};
...
class PyEVT_HANDLE : public PyHANDLE {
   public:
    ...
    virtual const char *GetTypeName() { return "PyEVT_HANDLE"; }
...
Py_BEGIN_ALLOW_THREADS $function Py_END_ALLOW_THREADS if ($source == 0 || $source == INVALID_HANDLE_VALUE)
{
...
PyTypeObject PyEventLogRecordType = {
    PYWIN_OBJECT_HEAD "PyEventLogRecord",
...
/*static*/ void PyEventLogRecord::deallocFunc(PyObject *ob) { delete (PyEventLogRecord *)ob; }
...
PyObject *_MyReadEventLog(HANDLE hEventLog, DWORD dwReadFlags, DWORD dwRecordOffset, DWORD nNumberOfBytesToRead)
{
    ...
    while (1) {
        ...
        Py_BEGIN_ALLOW_THREADS ok = ReadEventLogW(hEventLog, dwReadFlags, dwRecordOffset, buf, needed, &read, &needed);
...
PyObject *MyReadEventLog(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O&kk|k:ReadEventLog", PyWinObject_AsHANDLE,
                          &hEventLog,       // @pyparm <o Py_HANDLE>|Handle||Handle to a an opened event log (see <om
                        ...
                        PyObject *obData)     // raw data
{
    ...
    BOOL ok;
    Py_BEGIN_ALLOW_THREADS ok = ReportEventW(hEventLog, wType, wCategory, dwEventID, sid, (WORD)numStrings, pybuf.len(),
                                             (const WCHAR **)pStrings, pybuf.ptr());
...
ClearEventLogW(HANDLE hEventLog,    // @pyparm int|handle||Handle to the event log to clear.
...
BackupEventLogW(HANDLE hEventLog,        // @pyparm int|handle||Handle to the event log to backup.
...
BOOLAPI
CloseEventLog(HANDLE hEventLog  // @pyparm int|handle||Handle to the event log to close
);

// @pyswig |DeregisterEventSource|Deregisters an Event Source
BOOLAPI
DeregisterEventSource(HANDLE hEventLog  // @pyparm int|handle||Identifies the event log whose handle was returned by <om
                                        // win32evtlog.RegisterEventSource.>
);

// @pyswig |NotifyChangeEventLog|Lets an application receive notification when an event is written to the event log file
// specified by the hEventLog parameter. When the event is written to the event log file, the function causes the event
// object specified by the hEvent parameter to become signaled.
BOOLAPI
NotifyChangeEventLog(
    HANDLE hEventLog,  // @pyparm int|handle||Handle to an event log file, obtained by calling <om
                       // win32evtlog.OpenEventLog> function. When an event is written to this log file, the event
                       // specified by hEvent becomes signaled.
    PyHANDLE hEvent    // @pyparm int|handle||A handle to a Win32 event. This is the event that becomes signaled when an
...
BOOLAPI
GetNumberOfEventLogRecords(HANDLE hEventLog,  // @pyparm int|handle||Handle to the event log to query.
...
BOOLAPI
GetOldestEventLogRecord(HANDLE hEventLog, unsigned long *OUTPUT);
...
%name (RegisterEventSource) HANDLE
    RegisterEventSourceW(WCHAR *INPUT_NULLOK,  // @pyparm <o PyUnicode>|serverName||The server name, or None
...
%{
     ...
     static PyObject *PyEvtOpenChannelEnum(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "|O&k:EvtOpenChannelEnum", keywords, PyWinObject_AsHANDLE,
                 &session,  // @pyparm <o PyEVT_HANDLE>|Session|None|Handle to a remote session (see <om
                            // win32evtlog.EvtOpenSession>), or None for local machine.
                 &flags))   // @pyparm int|Flags|0|Reserved, use only 0
             return NULL;
         Py_BEGIN_ALLOW_THREADS enum_handle = EvtOpenChannelEnum(session, flags);
     ...
     static PyObject *FormatMessageInternal(EVT_HANDLE metadata, EVT_HANDLE event, DWORD flags, DWORD resourceId)
     {
         ...
         Py_BEGIN_ALLOW_THREADS
             ...
             bsuccess =
                 EvtFormatMessage(metadata, event, resourceId, 0, NULL, flags, allocated_size, buf, &returned_size);
         Py_END_ALLOW_THREADS
     ...
     static PyObject *PyEvtFormatMessage(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         DWORD resourceId = 0;
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "O&O&k|k:EvtFormatMessage", keywords, PyWinObject_AsHANDLE,
                 &metadata_handle,  // @pyparm <o PyEVT_HANDLE>|Metadata||Handle to provider metadata returned by <om
                                    // win32evtlog.EvtOpenPublisherMetadata>
                 PyWinObject_AsHANDLE, &event_handle,  // @pyparm <o PyEVT_HANDLE>|Event||Handle to an event
                 &flags,  // @pyparm int|Flags||Type of message to format. EvtFormatMessageEvent or
                          // EvtFormatMessageLevel or EvtFormatMessageTask or EvtFormatMessageOpcode or
                          // EvtFormatMessageKeyword or EvtFormatMessageChannel or EvtFormatMessageProvider or
                          // EvtFormatMessageId or EvtFormatMessageXml.  If set to EvtFormatMessageId, callers should
                          // also set the 'ResourceId' parameter
                 &resourceId))  // @pyparm int|ResourceId|0|The resource identifier of a message string returned by <om
                                // win32evtlog.EvtGetPublisherMetadataProperty>.  Only set this if flags =
                                // EvtFormatMessageId.
             return NULL;
     ...
     static PyObject *PyEvtNextChannelPath(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         PyObject *ret = NULL;
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "O&:EvtNextChannelPath", keywords, PyWinObject_AsHANDLE,
                 &enum_handle))  // @pyparm <o PyEVT_HANDLE>|ChannelEnum||Handle to an enumeration as returned by <om
         ...
         while (true) {
             ...
             Py_BEGIN_ALLOW_THREADS bsuccess = EvtNextChannelPath(enum_handle, allocated_size, buf, &returned_size);
     ...
     static PyObject *PyEvtOpenLog(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         PyObject *obpath;
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "Ok|O&:EvtOpenLog", keywords,
                 ...
                 PyWinObject_AsHANDLE, &session))  // @pyparm <o PyEVT_HANDLE>|Session|None|Handle to a remote session
                                                   // (see <om win32evtlog.EvtOpenSession>), or None for local machine.
             return NULL;
         if (!PyWinObject_AsWCHAR(obpath, &path, FALSE))
             return NULL;
         Py_BEGIN_ALLOW_THREADS log_handle = EvtOpenLog(session, path, flags);
         Py_END_ALLOW_THREADS PyWinObject_FreeWCHAR(path);
         if (log_handle == NULL)
     ...
     static PyObject *PyEvtClearLog(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "O|OO&k:EvtClearLog", keywords,
                 &obpath,         // @pyparm str|ChannelPath||Name of event log to be cleared
                 &obexport_path,  // @pyparm str|TargetFilePath|None|Name of file in which cleared events will be
                                  // archived, or None
                 PyWinObject_AsHANDLE, &session,  // @pyparm <o PyEVT_HANDLE>|Session|None|Handle to a remote session
                                                  // (see <om win32evtlog.EvtOpenSession>), or None for local machine.
                 &flags))                         // @pyparm int|Flags|0|Reserved, use only 0
         ...
         if (!PyWinObject_AsWCHAR(obexport_path, &export_path, TRUE))
             return NULL;
         BOOL bsuccess;
         Py_BEGIN_ALLOW_THREADS bsuccess = EvtClearLog(session, path, export_path, flags);
         Py_END_ALLOW_THREADS if (bsuccess)
         {
             Py_INCREF(Py_None);
             return Py_None;
     ...
     static PyObject *PyEvtExportLog(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "OOk|OO&:EvtExportLog", keywords,
         ...
         if (!PyWinObject_AsWCHAR(obquery, &query, TRUE))
             return NULL;
         BOOL bsuccess;
         Py_BEGIN_ALLOW_THREADS bsuccess = EvtExportLog(session, path, query, export_path, flags);
         Py_END_ALLOW_THREADS if (bsuccess)
         {
             Py_INCREF(Py_None);
             return Py_None;
     ...
     static PyObject *PyEvtArchiveExportedLog(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         PyObject *obpath;
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "Ol|O&k:EvtArchiveExportedLog", keywords,
         ...
         BOOL bsuccess;
         Py_BEGIN_ALLOW_THREADS bsuccess = EvtArchiveExportedLog(session, path, lcid, flags);
         Py_END_ALLOW_THREADS if (bsuccess)
         {
             Py_INCREF(Py_None);
             return Py_None;
     ...
     static PyObject *PyEvtGetExtendedStatus(PyObject *self, PyObject *args)
     {
         ...
         while (1) {
             ...
             Py_BEGIN_ALLOW_THREADS bsuccess = EvtGetExtendedStatus(buflen, msg, &bufneeded);
             Py_END_ALLOW_THREADS if (bsuccess)
             {
     ...
     static PyObject *PyEvtQuery(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "Ol|OO&:EvtQuery", keywords,
         ...
         if (!PyWinObject_AsWCHAR(obquery, &query, TRUE))
             return NULL;
         Py_BEGIN_ALLOW_THREADS ret = EvtQuery(session, path, query, flags);
     ...
     static PyObject *PyEvtNext(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "O&k|kk:EvtNext", keywords, PyWinObject_AsHANDLE,
                 &query,          // @pyparm <o PyEVT_HANDLE>|ResultSet||Handle to event query or subscription
         ...
         BOOL bsuccess;
         Py_BEGIN_ALLOW_THREADS bsuccess = EvtNext(query, nbr_requested, events, timeout, flags, &nbr_returned);
         Py_END_ALLOW_THREADS if (!bsuccess)
         {
             free(events);
             DWORD err = GetLastError();
     ...
     static PyObject *PyEvtSeek(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         LONGLONG position;
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "O&Lk|O&k:EvtSeek", keywords, PyWinObject_AsHANDLE,
                 &query,     // @pyparm <o PyEVT_HANDLE>|ResultSet||Handle to event query or subscription
                 ...
                 PyWinObject_AsHANDLE, &bookmark,  // @pyparm <o PyEVT_HANDLE>|Bookmark|None|Used as seek origin only if
                                                   // Flags contains EvtSeekRelativeToBookmark
                 &timeout))                        // @pyparm int|Timeout|0|Reserved, use only 0
             return NULL;
         BOOL bsuccess;
         Py_BEGIN_ALLOW_THREADS bsuccess = EvtSeek(query, position, bookmark, timeout, flags);
     ...
     static PyObject *PyEvtCreateRenderContext(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "k:EvtCreateRenderContext", keywords,
                 &flags))  // @pyparm int|Flags||EvtRenderContextSystem or EvtRenderContextUser. EvtRenderContextValues
                           // not currently supported
             return NULL;

         EVT_HANDLE ret = NULL;
         Py_BEGIN_ALLOW_THREADS ret = EvtCreateRenderContext(0, NULL, flags);
         Py_END_ALLOW_THREADS
     ...
     static PyObject *RenderEventValues(EVT_HANDLE render_context, EVT_HANDLE event)
     {
         ...
         Py_BEGIN_ALLOW_THREADS bsuccess = EvtRender(render_context, event, EvtRenderEventValues, allocated_size,
                                                     variants, &returned_size, &prop_count);
         Py_END_ALLOW_THREADS
     ...
     static PyObject *PyEvtRender(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "O&k|O&:EvtRender", keywords, PyWinObject_AsHANDLE,
                 &event,  // @pyparm <o PyEVT_HANDLE>|Event||Handle to an event or bookmark
                 &flags,  // @pyparm int|Flags||EvtRenderEventValues or EvtRenderEventXml or EvtRenderBookmark
                          // indicating type of handle
                 PyWinObject_AsHANDLE, &render_context))  // @pyparm <o PyEVT_HANDLE>|Context|None|Handle to a render
                                                          // context returned by <om win32evtlog.EvtCreateRenderContext>
             return NULL;
         ...
         while (1) {
             ...
             Py_BEGIN_ALLOW_THREADS bsuccess = EvtRender(NULL, event, flags, bufsize, buf, &bufneeded, &propcount);
             Py_END_ALLOW_THREADS if (bsuccess)
             {
     ...
     DWORD CALLBACK PyEvtSubscribe_callback(EVT_SUBSCRIBE_NOTIFY_ACTION action, void *context, EVT_HANDLE event)
     {
     ...
     //		First is an integer specifying why the function was called (EvtSubscribeActionError or
     // EvtSubscribeActionDeliver) 		Second is the context object passed to EvtSubscribe. 		Third is the handle
     // to an event log record (if not called due to an error)
     // If an event handle is passed in, a pull subscription is created.  The event handle will be signalled when events
     // are available, and the subscription handle can be passed to <om win32evtlog.EvtNext> to obtain the events.
     ...
     static PyObject *PyEvtSubscribe(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "Ok|O&OOOO&O&:EvtSubscribe", keywords,
                 &obpath,  // @pyparm str|ChannelPath||Name of an event log channel
                 &flags,   // @pyparm int|Flags||Combination of EvtSubscribe* flags determining how subscription is
                           // initiated
                 PyWinObject_AsHANDLE, &signalevent,  // @pyparm <o Py_HANDLE>|SignalEvent|None|An event handle to be
                                                      // set when events are available (see <om win32event.CreateEvent>)
                 ...
                 &obquery,  // @pyparm str|Query|None|XML query used to select specific events, use None or '*' for all
                            // events
                 PyWinObject_AsHANDLE, &session,    // @pyparm <o PyEVT_HANDLE>|Session|None|Handle to a session on
                                                    // another machine, or None for local
                 PyWinObject_AsHANDLE, &bookmark))  // @pyparm <o PyEVT_HANDLE>|Bookmark|None|If Flags contains
                                                    // EvtSubscribeStartAfterBookmark, used as starting point
             return NULL;
         ...
         Py_BEGIN_ALLOW_THREADS ret =
             EvtSubscribe(session, signalevent, path, query, bookmark, (void *)obuserdata, pfncallback, flags);
     ...
     static PyObject *PyEvtCreateBookmark(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         // @pyparm str|BookmarkXML|None|XML representation of a bookmark as returned by <om win32evtlog.EvtRender>, or
         // None for a new bookmark
         if (!PyArg_ParseTupleAndKeywords(args, kwargs, "|O:EvtCreateBookmark", keywords, &obxml))
             return NULL;
         if (!PyWinObject_AsWCHAR(obxml, &xml, TRUE))
             return NULL;
         Py_BEGIN_ALLOW_THREADS ret = EvtCreateBookmark(xml);
     ...
     static PyObject *PyEvtUpdateBookmark(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O&O&:EvtUpdateBookmark", keywords, PyWinObject_AsHANDLE,
                                          &bookmark,  // @pyparm <o PyEVT_HANDLE>|Bookmark||Handle to a bookmark
                                          PyWinObject_AsHANDLE,
                                          &evt))  // @pyparm <o PyEVT_HANDLE>|Event||Handle to an event
             return NULL;

         BOOL bsuccess;
         Py_BEGIN_ALLOW_THREADS bsuccess = EvtUpdateBookmark(bookmark, evt);
     ...
     static PyObject *PyEvtGetChannelConfigProperty(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         DWORD flags = 0;
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "O&i|k:EvtGetChannelConfigProperty", keywords, PyWinObject_AsHANDLE,
                 &config_handle,  // @pyparm <o PyEVT_HANDLE>|ChannelConfig||Config handle as returned by <om
         ...
         Py_BEGIN_ALLOW_THREADS EvtGetChannelConfigProperty(config_handle, prop_id, flags, buf_size, val, &buf_needed);
         Py_END_ALLOW_THREADS err = GetLastError();
         if (err != ERROR_INSUFFICIENT_BUFFER)
     ...
     static PyObject *PyEvtOpenChannelConfig(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|O&k:EvtOpenChannelConfig", keywords,
                                          ...
                                          PyWinObject_AsHANDLE,
                                          &session,  // @pyparm <o PyEVT_HANDLE>|Session|None|Session handle as returned
                                                     // by <om win32evtlog.EvtOpenSession>, or None for local machine
                                          &flags))   // @pyparm int|Flags|0|Reserved, use only 0
             return NULL;
         if (!PyWinObject_AsWCHAR(obchannel, &channel, FALSE))
             return NULL;
         Py_BEGIN_ALLOW_THREADS ret = EvtOpenChannelConfig(session, channel, flags);
     ...
     BOOL PyWinObject_AsEVT_RPC_LOGIN(PyObject *ob, EVT_RPC_LOGIN *erl)
     {
         ...
         if (!PyArg_ParseTuple(ob, "O|OOOk", &observer, &obuser, &obdomain, &obpassword, &erl->Flags))
             return FALSE;
         if (PyWinObject_AsWCHAR(observer, &erl->Server, FALSE) && PyWinObject_AsWCHAR(obuser, &erl->User, TRUE) &&
             PyWinObject_AsWCHAR(obdomain, &erl->Domain, TRUE) && PyWinObject_AsWCHAR(obpassword, &erl->Password, TRUE))
             return TRUE;
     ...
     static PyObject *PyEvtOpenSession(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "O|ikk:EvtOpenSession", keywords,
                 &oblogin,     // @pyparm <o PyEVT_RPC_LOGIN>|Login||Credentials to be used to access remote machine
                 &loginclass,  // @pyparm int|LoginClass|EvtRpcLogin|Type of login to perform, EvtRpcLogin is only
                               // defined value
                 &timeout,     // @pyparm int|Timeout|0|Reserved, use only 0
                 &flags))      // @pyparm int|Flags|0|Reserved, use only 0
             return NULL;
         ...
         if (!PyWinObject_AsEVT_RPC_LOGIN(oblogin, &login))
             return NULL;
         Py_BEGIN_ALLOW_THREADS ret = EvtOpenSession(loginclass, &login, timeout, flags);
         Py_END_ALLOW_THREADS PyWinObject_FreeEVT_RPC_LOGIN(&login);
         if (ret == NULL)
     ...
     static PyObject *PyEvtOpenPublisherEnum(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         DWORD flags = 0;
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "|O&k:EvtOpenPublisherEnum", keywords, PyWinObject_AsHANDLE,
                 &session,  // @pyparm <o PyEVT_HANDLE>|Session|None|Handle to a remote session (see <om
                            // win32evtlog.EvtOpenSession>), or None for local machine.
                 &flags))   // @pyparm int|Flags|0|Reserved, use only 0
             return NULL;
         Py_BEGIN_ALLOW_THREADS enum_handle = EvtOpenPublisherEnum(session, flags);
     ...
     static PyObject *PyEvtNextPublisherId(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         PyObject *ret = NULL;
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "O&:EvtNextPublisherId", keywords, PyWinObject_AsHANDLE,
                 &enum_handle))  // @pyparm <o PyEVT_HANDLE>|PublisherEnum||Handle to an enumeration as returned by <om
         ...
         while (true) {
             ...
             Py_BEGIN_ALLOW_THREADS bsuccess = EvtNextPublisherId(enum_handle, allocated_size, buf, &returned_size);
             Py_END_ALLOW_THREADS if (bsuccess)
             {
                 ret = PyWinObject_FromWCHAR(buf);
                 break;
     ...
     // @pyswig <o PyEVT_HANDLE>|EvtOpenPublisherMetadata|Opens a publisher to retrieve properties using <om
     ...
     static PyObject *PyEvtOpenPublisherMetadata(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "O|O&Okk:EvtOpenPublisherMetadata", keywords,
                 &obpublisher,  // @pyparm str|PublisherIdentity||Publisher id as returned by <om
                                // win32evtlog.EvtNextPublisherId>
                 PyWinObject_AsHANDLE,
                 &session,  // @pyparm <o PyEVT_HANDLE>|Session|None|Handle to remote session, or None for local machine
                 &oblogfile,  // @pyparm str|LogFilePath|None|Log file from which to retrieve publisher, or None for
                              // locally registered publisher
         ...
         Py_BEGIN_ALLOW_THREADS ret = EvtOpenPublisherMetadata(session, publisher, logfile, locale, flags);
     ...
     static PyObject *PyEvtGetPublisherMetadataProperty(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         DWORD flags = 0;
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "O&i|k:EvtGetPublisherMetadataProperty", keywords, PyWinObject_AsHANDLE,
                 &hpublisher,  // @pyparm <o PyEVT_HANDLE>|PublisherMetadata||Publisher handle as returned by <om
                               // win32evtlog.EvtOpenPublisherMetadata>
         ...
         PEVT_VARIANT val = NULL;
         DWORD buf_size = 0, buf_needed, err;
         Py_BEGIN_ALLOW_THREADS EvtGetPublisherMetadataProperty(hpublisher, prop_id, flags, buf_size, val, &buf_needed);
         Py_END_ALLOW_THREADS err = GetLastError();
         if (err != ERROR_INSUFFICIENT_BUFFER)
     ...
     static PyObject *PyEvtOpenEventMetadataEnum(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         DWORD flags = 0;
         if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O&|k:EvtOpenEventMetadataEnum", keywords, PyWinObject_AsHANDLE,
                                          &hpublisher,  // @pyparm <o PyEVT_HANDLE>|PublisherMetadata||Publisher handle
                                                        // as returned by <om win32evtlog.EvtOpenPublisherMetadata>
                                          &flags))      // @pyparm int|Flags|0|Reserved, use only 0
             return NULL;
         Py_BEGIN_ALLOW_THREADS enum_handle = EvtOpenEventMetadataEnum(hpublisher, flags);
     ...
     static PyObject *PyEvtNextEventMetadata(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         DWORD flags = 0;
         if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O&|k:EvtNextEventMetadata", keywords, PyWinObject_AsHANDLE,
                                          &henum,   // @pyparm <o PyEVT_HANDLE>|EventMetadataEnum||Enumeration handle as
                                                    // returned by <om win32evtlog.EvtOpenEventMetadataEnum>
                                          &flags))  // @pyparm int|Flags|0|Reserved, use only 0
             return NULL;

         Py_BEGIN_ALLOW_THREADS ret = EvtNextEventMetadata(henum, flags);
     ...
     static PyObject *PyEvtGetEventMetadataProperty(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O&i|k:EvtGetEventMetadataProperty", keywords,
                                          PyWinObject_AsHANDLE,
                                          &hevent,   // @pyparm <o PyEVT_HANDLE>|EventMetadata||Event metadata handle as
                                                     // returned by <om win32evtlog.EvtNextEventMetadata>
         ...
         PEVT_VARIANT val = NULL;
         DWORD buf_size = 0, buf_needed, err;
         Py_BEGIN_ALLOW_THREADS EvtGetEventMetadataProperty(hevent, prop_id, flags, buf_size, val, &buf_needed);
         Py_END_ALLOW_THREADS err = GetLastError();
         if (err != ERROR_INSUFFICIENT_BUFFER)
     ...
     static PyObject *PyEvtGetLogInfo(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "O&i:EvtGetLogInfo", keywords, PyWinObject_AsHANDLE,
                 &hlog,  // @pyparm <o PyEVT_HANDLE>|Log||Event log handle as returned by <om win32evtlog.EvtOpenLog>
                 &prop_id))  // @pyparm int|PropertyId||Property to retreive, EvtLog*
             return NULL;

         PEVT_VARIANT val = NULL;
         DWORD buf_size = 0, buf_needed, err;
         Py_BEGIN_ALLOW_THREADS EvtGetLogInfo(hlog, prop_id, buf_size, val, &buf_needed);
         Py_END_ALLOW_THREADS err = GetLastError();
         if (err != ERROR_INSUFFICIENT_BUFFER)
         ...
         buf_size = buf_needed;
         BOOL bsuccess;
         Py_BEGIN_ALLOW_THREADS bsuccess = EvtGetLogInfo(hlog, prop_id, buf_size, val, &buf_needed);
         Py_END_ALLOW_THREADS PyObject *ret = NULL;
         if (!bsuccess)
     ...
     static PyObject *PyEvtGetEventInfo(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O&i:EvtGetEventInfo", keywords, PyWinObject_AsHANDLE,
                                          &hevent,    // @pyparm <o PyEVT_HANDLE>|Event||Handle to an event
                                          &prop_id))  // @pyparm int|PropertyId||Property to retreive, EvtEvent*
             return NULL;

         PEVT_VARIANT val = NULL;
         DWORD buf_size = 0, buf_needed, err;
         Py_BEGIN_ALLOW_THREADS EvtGetEventInfo(hevent, prop_id, buf_size, val, &buf_needed);
         Py_END_ALLOW_THREADS err = GetLastError();
         if (err != ERROR_INSUFFICIENT_BUFFER)
         ...
         buf_size = buf_needed;
         BOOL bsuccess;
         Py_BEGIN_ALLOW_THREADS bsuccess = EvtGetEventInfo(hevent, prop_id, buf_size, val, &buf_needed);
         Py_END_ALLOW_THREADS PyObject *ret = NULL;
         if (!bsuccess)
     ...
     static PyObject *PyEvtGetObjectArraySize(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         DWORD size;
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "O&:EvtGetObjectArraySize", keywords, PyWinObject_AsHANDLE,
                 &harray))  // @pyparm <o PyEVT_HANDLE>|ObjectArray||Handle to an array of objects as returned by <om
                            // win32evtlog.EvtGetPublisherMetadataProperty> for some ProperyId's
             return NULL;
         BOOL bsuccess;
         Py_BEGIN_ALLOW_THREADS bsuccess = EvtGetObjectArraySize(harray, &size);
     ...
     static PyObject *PyEvtGetObjectArrayProperty(PyObject *self, PyObject *args, PyObject *kwargs)
     {
         ...
         if (!PyArg_ParseTupleAndKeywords(
                 args, kwargs, "O&kk|k:EvtGetObjectArrayProperty", keywords, PyWinObject_AsHANDLE,
                 &harray,   // @pyparm <o PyEVT_HANDLE>|ObjectArray||Handle to an array of objects as returned by <om
                            // win32evtlog.EvtGetPublisherMetadataProperty> for some ProperyId's
         ...
         PEVT_VARIANT val = NULL;
         DWORD buf_size = 0, buf_needed, err;
         Py_BEGIN_ALLOW_THREADS EvtGetObjectArrayProperty(harray, prop_id, index, flags, buf_size, val, &buf_needed);
         Py_END_ALLOW_THREADS err = GetLastError();
         if (err != ERROR_INSUFFICIENT_BUFFER)
...
%init %{
     ...
     for (PyMethodDef *pmd = win32evtlogMethods; pmd->ml_name; pmd++)
         if ((strcmp(pmd->ml_name, "EvtOpenChannelEnum") == 0) ||
             (strcmp(pmd->ml_name, "EvtCreateRenderContext") == 0) || (strcmp(pmd->ml_name, "EvtFormatMessage") == 0) ||
             (strcmp(pmd->ml_name, "EvtNextChannelPath") == 0) || (strcmp(pmd->ml_name, "EvtOpenLog") == 0) ||
             (strcmp(pmd->ml_name, "EvtClearLog") == 0) || (strcmp(pmd->ml_name, "EvtOpenSession") == 0) ||
             (strcmp(pmd->ml_name, "EvtExportLog") == 0) || (strcmp(pmd->ml_name, "EvtArchiveExportedLog") == 0) ||
             (strcmp(pmd->ml_name, "EvtQuery") == 0) || (strcmp(pmd->ml_name, "EvtNext") == 0) ||
             (strcmp(pmd->ml_name, "EvtSeek") == 0) || (strcmp(pmd->ml_name, "EvtRender") == 0) ||
             (strcmp(pmd->ml_name, "EvtSubscribe") == 0) || (strcmp(pmd->ml_name, "EvtCreateBookmark") == 0) ||
             (strcmp(pmd->ml_name, "EvtUpdateBookmark") == 0) ||
             (strcmp(pmd->ml_name, "EvtGetChannelConfigProperty") == 0) ||
             (strcmp(pmd->ml_name, "EvtOpenChannelConfig") == 0) || (strcmp(pmd->ml_name, "EvtOpenSession") == 0) ||
             (strcmp(pmd->ml_name, "EvtOpenPublisherEnum") == 0) || (strcmp(pmd->ml_name, "EvtNextPublisherId") == 0) ||
             ...
             (strcmp(pmd->ml_name, "EvtGetLogInfo") == 0) || (strcmp(pmd->ml_name, "EvtGetEventInfo") == 0) ||

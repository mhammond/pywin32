...
$function if (!$source) { $cleanup return PyWin_SetAPIError("$name"); }
}
...
#define EVENT_MODIFY_STATE \
    EVENT_MODIFY_STATE  // Enables use of the event handle in the SetEvent and ResetEvent functions to modify the
                        // event's state.

#define SYNCHRONIZE \
    SYNCHRONIZE  // Windows NT only: Enables use of the event handle in any of the wait functions to wait for the
                 // event's state to be signaled.
...
PyHANDLE CreateEvent(SECURITY_ATTRIBUTES *inNullSA,  // @pyparm <o PySECURITY_ATTRIBUTES>|EventAttributes||The security
                                                     // attributes, or None
...
PyHANDLE CreateMutex(
    SECURITY_ATTRIBUTES *inNullSA,  // @pyparm <o PySECURITY_ATTRIBUTES>|MutexAttributes||Specifies inheritance and
                                    // security descriptor for object, or None for defaults
...
PyHANDLE CreateSemaphore(
    SECURITY_ATTRIBUTES *SemaphoreAttributes,  // @pyparm <o PySECURITY_ATTRIBUTES>|SemaphoreAttributes||Specifies
                                               // inheritance and security descriptor for object, or None for defaults
...
PyHANDLE CreateWaitableTimer(
    SECURITY_ATTRIBUTES *TimerAttributes,  // @pyparm <o PySECURITY_ATTRIBUTES>|TimerAttributes||Specifies inheritance
                                           // and security descriptor for object, or None for defaults
    BOOL bManualReset,   // @pyparm bool|ManualReset||True for manual reset timer, or False to create a synchronization
                         // timer
    TCHAR *INPUT_NULLOK  // @pyparm str|TimerName||Timer object name, or None
);

// @pyswig <o PyHANDLE>|CreateWaitableTimerEx|Creates or opens a waitable timer object and returns a handle to the
// object
// @rdesc The result is a handle to the object
...
%{
    static PyObject *MyMsgWaitForMultipleObjects(PyObject *handleList,
                                                 ...
                                                 DWORD dwWakeMask)
    {
        ...
        if (!MakeHandleList(handleList, &pItems, &numItems))
            return NULL;
        DWORD rc;
        Py_BEGIN_ALLOW_THREADS rc = MsgWaitForMultipleObjects(numItems, pItems, fWaitAll, dwMilliseconds, dwWakeMask);
        Py_END_ALLOW_THREADS PyObject *obrc;
...
%{
    static PyObject *MyMsgWaitForMultipleObjectsEx(PyObject *handleList,
                                                   DWORD dwMilliseconds,  // time-out interval in milliseconds
                                                   DWORD dwWakeMask,
    ...
    )
    {
        ...
        Py_BEGIN_ALLOW_THREADS rc = MsgWaitForMultipleObjectsEx(numItems, pItems, dwMilliseconds, dwWakeMask, dwFlags);
        Py_END_ALLOW_THREADS PyObject *obrc;
        if (rc == (DWORD)0xFFFFFFFF)
...
%}
    ...
    PyHANDLE OpenEvent(
        DWORD dwDesiredAccess,  // @pyparm int|desiredAccess||access flag - one of <om win32event.EVENT_ALL_ACCESS>, <om
                                // win32event.EVENT_MODIFY_STATE>, or (NT only) <om win32event.SYNCHRONIZE>
    ...
    PyHANDLE OpenMutex(DWORD dwDesiredAccess,  // @pyparm int|desiredAccess||access flag
                       BOOL bInheritHandle,    // @pyparm bool|bInheritHandle||inherit flag
    ...
    PyHANDLE OpenSemaphore(DWORD dwDesiredAccess,  // @pyparm int|desiredAccess||access flag
                           BOOL bInheritHandle,    // @pyparm bool|bInheritHandle||inherit flag
    ...
    PyHANDLE OpenWaitableTimer(DWORD dwDesiredAccess,  // @pyparm int|desiredAccess||access flag
                               BOOL bInheritHandle,    // @pyparm bool|bInheritHandle||inherit flag
                               TCHAR *lpTimerName      // @pyparm str|timerName||pointer to timer object name
    );

    // @pyswig |PulseEvent|Provides a single operation that sets (to signaled) the state of the specified event object
    // and then resets it (to nonsignaled) after releasing the appropriate number of waiting threads.
    BOOLAPI PulseEvent(PyHANDLE hEvent  // @pyparm <o PyHANDLE>|hEvent||handle of event object
    );

    // @pyswig |ReleaseMutex|Releases a mutex.
    BOOLAPI ReleaseMutex(PyHANDLE hMutex  // @pyparm <o PyHANDLE>|hEvent||handle of mutex object
    );

    // @pyswig int|ReleaseSemaphore|Releases a semaphore.
    BOOLAPI ReleaseSemaphore(PyHANDLE hSemaphore,  // @pyparm <o PyHANDLE>|hEvent||handle of the semaphore object
    ...
    BOOLAPI ResetEvent(PyHANDLE hEvent  // @pyparm <o PyHANDLE>|hEvent||handle of event object
    );

    // @pyswig |SetEvent|Sets an event
    BOOLAPI SetEvent(PyHANDLE hEvent  // @pyparm <o PyHANDLE>|hEvent||handle of event object
    );
    ...
    BOOLAPI SignalObjectAndWait(PyHANDLE hObjectToSignal,  // handle of object to signal
...
%{
    static PyObject *MyWaitForMultipleObjects(PyObject *handleList,
    ...
    )
    {
        ...
        if (!MakeHandleList(handleList, &pItems, &numItems))
            return NULL;
        DWORD rc;
        Py_BEGIN_ALLOW_THREADS rc = WaitForMultipleObjects(numItems, pItems, bWaitAll, dwMilliseconds);
        Py_END_ALLOW_THREADS PyObject *obrc;
        if (rc == WAIT_FAILED)
...
%{
    static PyObject *MyWaitForMultipleObjectsEx(PyObject *handleList,
                                                BOOL bWaitAll,         // wait flag
                                                DWORD dwMilliseconds,  // time-out interval in milliseconds
    ...
    )
    {
        ...
        if (!MakeHandleList(handleList, &pItems, &numItems))
            return NULL;
        DWORD rc;
        Py_BEGIN_ALLOW_THREADS rc = WaitForMultipleObjectsEx(numItems, pItems, bWaitAll, dwMilliseconds, bAlertable);
        Py_END_ALLOW_THREADS PyObject *obrc;
        if (rc == WAIT_FAILED)
...
%typemap(python,except) DWORD_WAITAPI {
    Py_BEGIN_ALLOW_THREADS $function Py_END_ALLOW_THREADS if ($source == WAIT_FAILED)
    {
    ...
    // @rdesc If the function succeeds, the return value indicates the event that caused the function to return. This
    // value can be one of the following.
    // @flagh Value|Meaning
    // @flag WAIT_ABANDONED|The specified object is a mutex object that was not released by the thread that owned the
    // mutex object before the owning thread terminated. Ownership of the mutex object is granted to the calling thread,
    ...
    // @pyswig int|WaitForInputIdle|Waits until the given process is waiting for user input with no input pending, or
    // until the time-out interval has elapsed
    DWORD_WAITAPI WaitForInputIdle(HANDLE hProcess,      // @pyparm <o PyHANDLE>|hProcess||handle of process to wait for
                                   DWORD dwMilliseconds  // @pyparm int|milliseconds||time-out interval in milliseconds
    );
    // @rdesc The return value indicates wether the process is ready or wether it timed out. This value can be one of
    // the following.
    // @flagh Value|Meaning

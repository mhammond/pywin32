    ...
    def __init__(self, **kw):
        ...
        for name, val in kw.iteritems():
            if name not in self.__dict__:
                raise ValueError(
                    "LVITEM structures do not have an item '%s'" % (name,))
...
class DemoWindowBase:
    ...
    def _GetDialogTemplate(self, dlgClassName):
        ...
        dlg = [[title, (0, 0, 210, 250), style, None,
                (8, "MS Sans Serif"), None, dlgClassName], ]

        # ID label and text box
        dlg.append([130, "Enter something", -1,
                    (5, 5, 200, 9), cs | win32con.SS_LEFT])
        ...
        dlg.append([128, "Fill List", IDC_BUTTON_SEARCH,
                    (5, 35, 50, 14), s | win32con.BS_DEFPUSHBUTTON])
...
##        dlg.append(['SysListView32', "Title", IDC_LISTBOX, (5, 505, 200, 200), s])
    ...
    def _SetupList(self):
        ...
        self.hwndList = win32gui.CreateWindow(
            "SysListView32", None, child_style, 0, 0, 100, 100, self.hwnd, IDC_LISTBOX, self.hinst, None)
        ...
        lvc.cx = 200
        win32gui.SendMessage(
            self.hwndList, commctrl.LVM_INSERTCOLUMN, 0, lvc.toparam())
        lvc.iSubItem = 0
        lvc.text = "Order"
        lvc.cx = 50
        win32gui.SendMessage(
    ...
    def AddListItem(self, data, *columns):
        num_items = win32gui.SendMessage(
            self.hwndList, commctrl.LVM_GETITEMCOUNT)
        ...
        for col in columns[1:]:
            ...
            win32gui.SendMessage(
                self.hwndList, commctrl.LVM_SETITEM, 0, item.toparam())
            col_no += 1
    ...
    def _DoSize(self, cx, cy, repaint=1):
        ...
        new_width = cx - \
            win32gui.SendMessage(self.hwndList, commctrl.LVM_GETCOLUMNWIDTH, 0)
        win32gui.SendMessage(
            self.hwndList, commctrl.LVM_SETCOLUMNWIDTH, 1, new_width)

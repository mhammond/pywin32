...
class MainWindow:
    ...
    def createMenu(self):
        ...
        if self.iconPathName:
            hicon = LoadImage(0, self.iconPathName, win32con.IMAGE_ICON,
        ...
        # draw the icon
        DrawIconEx(hdcBitmap, 0, 0, hicon, ico_x,
        ...
        # A 'radio' menu.
        InsertMenu(sub_menu, 0, win32con.MF_BYPOSITION,
    ...
    def OnCommand(self, hwnd, msg, wparam, lparam):
        ...
        if id == 1000:
            ...
            if new_state & win32con.MF_CHECKED != check_flags:
                raise RuntimeError(
                    "The new item didn't get the new checked state!")
    ...
    def OnDrawItem(self, hwnd, msg, wparam, lparam):
        ...
        if text is None:
            ...
            if selected:
                crText = SetTextColor(hDC, GetSysColor(
                    win32con.COLOR_HIGHLIGHTTEXT))
                crBkgnd = SetBkColor(
                    hDC, GetSysColor(win32con.COLOR_HIGHLIGHT))

            each_pad = self.icon_x_pad // 2
            x_icon = left + \
                GetSystemMetrics(win32con.SM_CXMENUCHECK) + each_pad

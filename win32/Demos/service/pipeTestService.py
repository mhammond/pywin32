...
class TestPipeService(win32serviceutil.ServiceFramework):
    ...
    def CreatePipeSecurityObject(self):
        ...
        acl.AddAccessAllowedAce(
            FILE_GENERIC_READ | FILE_GENERIC_WRITE, sidEveryone)
    ...
    def DoProcessClient(self, pipeHandle, tid):
        try:
            ...
            if ok:
                msg = ("%s (on thread %d) sent me %s" % (
                    GetNamedPipeHandleState(pipeHandle)[4], tid, d)).encode('ascii')
    ...
    def ProcessClient(self, pipeHandle):
        try:
            ...
            th = DuplicateHandle(procHandle, GetCurrentThread(
            ), procHandle, 0, 0, win32con.DUPLICATE_SAME_ACCESS)
            try:

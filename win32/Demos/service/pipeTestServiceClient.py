...
def testClient(server, msg):
    if verbose:
        print "Sending", msg
    data = CallPipe(CallNamedPipe, ("\\\\%s\\pipe\\PyPipeTest" %
                                    server, msg, 256, NMPWAIT_WAIT_FOREVER))
    if verbose:
...
def testLargeMessage(server, size=4096):
    ...
    msg = "*" * size
    data = CallPipe(CallNamedPipe, ("\\\\%s\\pipe\\PyPipeTest" %
                                    server, msg, 512, NMPWAIT_WAIT_FOREVER))
    if len(data)-size:
    ...
    try:
        try:
            for i in xrange(numMessages):
                r = CallPipe(CallNamedPipe, ("\\\\%s\\pipe\\PyPipeTest" %
                                             server, "#" * 512, 1024, NMPWAIT_WAIT_FOREVER))
        except:
            traceback.print_exc()

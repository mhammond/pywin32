...
coord = win32console.PyCOORDType(
    X=window_size.Right+20, Y=window_size.Bottom+20)
...
newbuffer.WriteConsole(
    'Press some keys, click some characters with the mouse\n')
        ...
        if input_record.EventType == win32console.KEY_EVENT:
            if input_record.KeyDown:
                if input_record.Char == '\0':
                    newbuffer.WriteConsole(virtual_keys.get(
                        input_record.VirtualKeyCode, 'VirtualKeyCode: %s' % input_record.VirtualKeyCode))
                else:
            ...
            if input_record.EventFlags == 0:  # 0 indicates a button event
                if input_record.ButtonState != 0:  # exclude button releases
                    ...
                    attr = newbuffer.ReadConsoleOutputAttribute(
                        Length=1, ReadCoord=pos)[0]
                    new_attr = attr
                    if attr & win32console.FOREGROUND_BLUE:
                        new_attr = (
                            new_attr & ~win32console.FOREGROUND_BLUE) | win32console.BACKGROUND_BLUE
                    if attr & win32console.FOREGROUND_RED:
                        new_attr = (
                            new_attr & ~win32console.FOREGROUND_RED) | win32console.BACKGROUND_RED
                    if attr & win32console.FOREGROUND_GREEN:
                        new_attr = (
                            new_attr & ~win32console.FOREGROUND_GREEN) | win32console.BACKGROUND_GREEN

                    if attr & win32console.BACKGROUND_BLUE:
                        new_attr = (
                            new_attr & ~win32console.BACKGROUND_BLUE) | win32console.FOREGROUND_BLUE
                    if attr & win32console.BACKGROUND_RED:
                        new_attr = (
                            new_attr & ~win32console.BACKGROUND_RED) | win32console.FOREGROUND_RED
                    if attr & win32console.BACKGROUND_GREEN:
                        new_attr = (
                            new_attr & ~win32console.BACKGROUND_GREEN) | win32console.FOREGROUND_GREEN

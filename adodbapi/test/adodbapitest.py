...
import adodbapi
# will set sys.path to find correct version of adodbapi
...
class CommonDBTests(unittest.TestCase):
    ...
    def testErrorHandlerInherits(self):
        if not self.remote:
            ...
            def mycallable(connection, cursor, errorclass,
                           errorvalue): return 1
    ...
    def testOwnErrorHandlerConnection(self):
        ...
        def mycallable(connection, cursor, errorclass,
                       errorvalue): return 1  # does not raise anything
        conn = self.getConnection()
        conn.errorhandler = mycallable
        conn.close()
        conn.commit()  # Should not be able to use connection after it is closed
        ...
        assert len(
            conn.messages) > 0, "Setting errorhandler to none  should bring back the standard error handler"
    ...
    def testOwnErrorHandlerCursor(self):
        ...
        # The Standard errorhandler appends error to messages attribute
        assert len(
            crsr.messages) > 0, "Setting errorhandler to none  should bring back the standard error handler"

    def testUserDefinedConversions(self):
        if self.remote:  # Todo: should be a "skip"
            return
        try:
            def duplicatingConverter(aStringField): return aStringField*2
            ...
            self.assertRaises(
                AttributeError, lambda x: conn.variantConversions[x], [2])
            if not self.remote:
                ...
                def upcaseConverter(aStringField): return aStringField.upper()
        ...
        for inParam in inputs:
            ...
            try:
                ...
                if self.remote:
                    ...
                    conn.printADOerrors()
                raise
            crsr.execute("SELECT fldData FROM xx_%s WHERE ?=fldID" %
                         config.tmp, [fldId])
            rs = crsr.fetchone()
            if allowedReturnValues:
                allowedTypes = tuple([type(aRV)
                                      for aRV in allowedReturnValues])
                assert isinstance(rs[0], allowedTypes), \
                    'result type "%s" must be one of %s' % (
                        type(rs[0]), allowedTypes)
            else:
                assert isinstance(rs[0], type(pyData)), \
                    'result type "%s" must be instance of %s' % (
                        type(rs[0]), type(pyData))

            if compareAlmostEqual and DBAPIDataTypeString == 'DATETIME':
                iso1 = adodbapi.dateconverter.DateObjectToIsoFormatString(
                    rs[0])
                ...
                assert abs(v-s)/s < 0.00001, \
                    "Values not almost equal recvd=%s, expected=%f" % (
                        rs[0], s)
            else:
                if allowedReturnValues:
                    ok = False
    ...
    def testDataTypeFloat(self):
        ...
        self.helpTestDataType("float", 'NUMBER', 1.79e37,
    ...
    def testDataTypeMoney(self):  # v2.1 Cole -- use decimal for money
        if self.getEngine() == 'MySQL':
            self.helpTestDataType("DECIMAL(20,4)", 'NUMBER',
            ...
            self.helpTestDataType("smallmoney", 'NUMBER',
                                  decimal.Decimal('214748.02'))
            self.helpTestDataType(
    ...
    def testDataTypeChar(self):
        for sqlDataType in ("char(6)", "nchar(6)"):
            self.helpTestDataType(sqlDataType, 'STRING', u'spam  ', allowedReturnValues=[
                                  u'spam', 'spam', u'spam  ', 'spam  '])
    ...
    def helpCreateAndPopulateTableTemp(self, crsr):
        ...
        for i in range(9):  # note: this poor SQL code, but a valid test
            crsr.execute("INSERT INTO xx_%s (fldData) VALUES (%i)" %
                         (config.tmp, i))
    ...
    def testExecuteMany(self):
        ...
        crsr.executemany("INSERT INTO xx_%s (fldData) VALUES (?)" %
                         config.tmp, seq_of_values)
        if crsr.rowcount == -1:
    ...
    def testErrorConnect(self):
        ...
        conn.close()
        self.assertRaises(api.DatabaseError, self.db,
                          'not a valid connect string', kw)
    ...
    def testRowIterator(self):
        ...
        for inParam in inputs:
            ...
            try:
                ...
                raise
            crsr.execute(
                "SELECT fldTwo,fldThree,fldFour FROM xx_%s WHERE ?=fldID" % config.tmp, [fldId])
            rec = crsr.fetchone()
    ...
    def testFormatParamstyle(self):
        ...
        for inParam in inputs:
            ...
            rec = crsr.fetchone()
            self.assertEqual(
                rec[0], inParam, 'returned value:"%s" != test value:"%s"' % (rec[0], inParam))
        ...
        sel = "insert into xx_" + config.tmp + \
            " (fldId,fldData) VALUES (%s,'four%sfive')"
        params = (20,)
        crsr.execute(sel, params)

        # test the .query implementation
        assert '(?,' in crsr.query, 'expected:"%s" in "%s"' % (
            '(?,', crsr.query)
        ...
        for inParam in inputs:
            ...
            try:
                ...
                if self.remote:
                    ...
                    conn.printADOerrors()
                raise
            crsr.execute("SELECT fldData FROM xx_%s WHERE :Id=fldID" %
                         config.tmp, {'Id': fldId})
            rec = crsr.fetchone()
            self.assertEqual(
                rec[0], inParam, 'returned value:"%s" != test value:"%s"' % (rec[0], inParam))
        # now a test with a ":" as part of a literal
        crsr.execute("insert into xx_%s (fldId,fldData) VALUES (:xyz,'six:five')" %
                     config.tmp, {'xyz': 30})
        ...
        for inParam in inputs:
            ...
            try:
                ...
                if self.remote:
                    ...
                    conn.printADOerrors()
                raise
            crsr.execute("SELECT fldData FROM xx_%s WHERE %%(Id)s=fldID" %
                         config.tmp, {'Id': fldId})
            rec = crsr.fetchone()
            self.assertEqual(
                rec[0], inParam, 'returned value:"%s" != test value:"%s"' % (rec[0], inParam))
        # now a test with a "%" as part of a literal
        crsr.execute("insert into xx_%s (fldId,fldData) VALUES (%%(xyz)s,'six%%five')" %
                     config.tmp, {'xyz': 30})
        crsr.execute("SELECT fldData FROM xx_%s WHERE fldID=30" % config.tmp)
        rec = crsr.fetchone()
        self.assertEqual(rec[0], 'six%five')
    ...
    def testAutoRollback(self):
        ...
        assert row == None, 'cursor.fetchone should return None if a query retrieves no rows. Got %s' % repr(
            row)
        self.helpRollbackTblTemp()
...
class TestADOwithSQLServer(CommonDBTests):
    ...
    def testVariableReturningStoredProcedure(self):
        ...
        assert retvalues[0] == 'Dodsworth', '%s is not "Dodsworth"' % repr(
            retvalues[0])
    ...
    def testDatetimeProcedureParameter(self):
        ...
        result = crsr.callproc('sp_DeleteMeOnlyForTesting', [
...
class TestADOwithAccessDB(CommonDBTests):
    ...
    def getAnotherConnection(self, addkeys=None):
        raise NotImplementedError(
            'Jet cannot use a second connection to the database')
...
class TestPythonTimeConverter(TimeConverterInterfaceTest):
    ...
    def testCOMDate(self):
        ...
        cmd = self.tc.COMDate(t)
        assert abs(cmd - 37435.7604282) < 1.0 / \
            24, "%f more than an hour wrong" % cmd

    def testDateObjectFromCOMDate(self):
        cmd = self.tc.DateObjectFromCOMDate(37435.7604282)
        t1 = time.gmtime(time.mktime(
            (2002, 6, 28, 0, 14, 1, 4, 31+28+31+30+31+28, -1)))
        # there are errors in the implementation of gmtime which we ignore
        t2 = time.gmtime(time.mktime(
            (2002, 6, 29, 12, 14, 2, 4, 31+28+31+30+31+28, -1)))
        assert t1 < cmd < t2, '"%s" should be about 2002-6-28 12:15:01' % repr(
            cmd)
    ...
    def testTime(self):
        self.assertEquals(self.tc.Time(18, 15, 2),
                          time.gmtime(18*60*60+15*60+2))

    def testTimestamp(self):
        t1 = time.localtime(time.mktime(
            (2002, 6, 28, 18, 14, 1, 4, 31+28+31+30+31+28, -1)))
        t2 = time.localtime(time.mktime(
            (2002, 6, 28, 18, 16, 1, 4, 31+28+31+30+31+28, -1)))

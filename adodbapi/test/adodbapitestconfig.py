...
# will (hopefully) be imported using the "pth" discovered above
import adodbapi
...
# yes, I do use a dvorak keyboard!
_alphabet = "PYFGCRLAOEUIDHTNTQJKXBMWVZ1234567890"
...
# -- change this
THE_PROXY_HOST = '25.44.77.176' if node != SQL_HOST_NODE or not onWindows else '::1'
...
if doAccessTest:
    ...
    c['macro_is64bit'] = [
        'driver', "Microsoft.ACE.OLEDB.12.0", "Microsoft.Jet.OLEDB.4.0"]
...
if doSqlServerTest:
    ...
    if node != SQL_HOST_NODE:
        if THE_PROXY_HOST:
            # the SQL server runs a proxy for this test
            c["proxy_host"] = THE_PROXY_HOST
        else:
            c["pyro_connection"] = "PYRONAME:ado.connection"
    print('    ...Testing MS-SQL login...')
    doSqlServerTest, connStrSQLServer, dbSqlServerconnect = tryconnection.try_connection(

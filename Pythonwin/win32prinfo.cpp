...
{
    CPrintInfo *pPrInfo = (CPrintInfo *)ui_assoc_object::GetGoodCppObject(ui_type_check);
    if (pPrInfo == NULL) {
        PyErr_Clear();
        RETURN_NONE;
    }
...
// @pymethod |PyCPrintInfo|GetDwFlags|A flags specifying DocObject printing operations. Valid only if data member
...
// @pymethod |PyCPrintInfo|SetDwFlags|Set a flag specifying DocObject printing operations. Valid only if data member
// m_bDocObject is TRUE.
// @pyseemfc CPrintInfo|m_dwFlags
...
// @pymethod |PyCPrintInfo|GetDocOffsetPage|Get the number of pages preceding the first page of a particular DocObject
...
// @pymethod |PyCPrintInfo|SetDocOffsetPage|Set the number of pages preceding the first page of a particular DocObject
// in a combined DocObject print job.
// @pyseemfc CPrintInfo|m_nOffsetPage
...
// @pymethod |PyCPrintInfo|SetPrintDialog|Set a pointer to the CPrintDialog object used to display the Print dialog box
// for the print job.
...
// @pymethod |PyCPrintInfo|SetDirect|Sets to TRUE if the Print dialog box will be bypassed for direct printing; FALSE
// otherwise.
// @pyseemfc CPrintInfo|m_bDirect
...
static PyObject *ui_get_draw(PyObject *self, PyObject *args)
{
    ...
    if (!pInfo)
        return NULL;
    return Py_BuildValue("(iiii)", pInfo->m_rectDraw.left, pInfo->m_rectDraw.top, pInfo->m_rectDraw.right,
...
// @pymethod |PyCPrintInfo|GetOffsetPage|Get the number of pages preceding the first page of a DocObject item being
// printed in a combined DocObject print job.  This currently does NOT work, as, if I include the symbol
// pInfo->GetOffsetPage(), the link fails to find its definition.  Allways returns 0.
...
// @pymethod |PyCPrintInfo|SetHDC|Sets the printer DC compatible with the users choices, call after the print dialog
// DoModal finishes.
// @pyseemfc CPrintInfo|m_pPD
...
// @pymethod |PyCPrintInfo|CreatePrinterDC|Handle to the newly created printer device context, call only after DoModal
// finishes.
...
// @pymethod |PyCPrintInfo|GetDefaults|Nonzero if the function was successful; otherwise 0.  Call this function to
...
// @pymethod |PyCPrintInfo|FreeDefaults|After a call to GetDefaults, and you are through with the CPrintDialog object,
...
// @pymethod |PyCPrintInfo|GetDriverName|The name of the currently selected printer device driver, call only after
// DoModal finishes.
...
// @pymethod |PyCPrintInfo|GetPortName|The name of the currently selected printer port, call only after DoModal
// finishes.
// @pyseemfc CPrintDialog|GetPortName
MAKE_STR_METH(ui_get_port_name, GetPortName)

// @pymethod |PyCPrintInfo|GetPrinterDC|A handle to the printer device context if successful; otherwise NULL.  If the
...
// @pymethod |PyCPrintInfo|PrintAll|Nonzero if all pages in the document are to be printed; otherwise 0, call only after
// DoModal finishes.
// @pyseemfc CPrintDialog|PrintAll
MAKE_INT_METH(ui_print_all, PrintAll)

// @pymethod |PyCPrintInfo|PrintCollate|Nonzero if the user selects the collate check box in the dialog box; otherwise
// 0, call only after DoModal finishes.
// @pyseemfc CPrintDialog|PrintCollate
MAKE_INT_METH(ui_print_collate, PrintCollate)

// @pymethod |PyCPrintInfo|PrintRange|Nonzero if only a range of pages in the document are to be printed; otherwise 0,
// call only after DoModal finishes.
// @pyseemfc CPrintDialog|PrintRange
MAKE_INT_METH(ui_print_range, PrintRange)

// @pymethod |PyCPrintInfo|PrintSelection|Nonzero if only the selected items are to be printed; otherwise 0., call only
...
// @pymethod |PyCPrintInfo|GetHDC|Identifies a device context or an information context, depending on whether the Flags
...
// @pymethod |PyCPrintInfo|GetFlags|A set of bit flags that you can use to initialize the Print common dialog box. When
...
// @pymethod |PyCPrintInfo|SetFlags|A set of bit flags that you can use to initialize the Print common dialog box. When
// the dialog box returns, it sets these flags to indicate the user's input. This member can be a combination of the
// following flags: PD_ALLPAGES, PD_COLLATE, PD_DISABLEPRINTTOFILE, PD_ENABLEPRINTHOOK, PD_ENABLEPRINTTEMPLATE,
// PD_ENABLEPRINTTEMPLATEHANDLE, PD_ENABLESETUPHOOK, PD_ENABLESETUPTEMPLATE, PD_ENABLESETUPTEMPLATEHANDLE,
// PD_HIDEPRINTTOFILE, PD_NONETWORKBUTTON, PD_NOPAGENUMS, PD_NOSELECTION, PD_NOWARNING, PD_PAGENUMS, PD_PRINTSETUP,
...
// @pymethod |PyCPrintInfo|GetPRINTDLGMinPage|Get the minimum value for the page range specified in the From and To page
...
// @pymethod |PyCPrintInfo|SetPRINTDLGMinPage|Set the minimum value for the page range specified in the From and To page
// edit controls. If nMinPage equals nMaxPage, the Pages radio button and the starting and ending page edit controls are
// disabled.
// @pyseemfc PRINTDLG|nMinPage
...
// @pymethod |PyCPrintInfo|GetPRINTDLGCopies|Get the initial number of copies for the Copies edit control if hDevMode is
...
// @pymethod |PyCPrintInfo|SetPRINTDLGCopies|Set the initial number of copies for the Copies edit control if hDevMode is
// NULL; otherwise, the dmCopies member of theDEVMODE structure contains the initial value. When PrintDlg returns,
// nCopies contains the actual number of copies to print. This value depends on whether the application or the printer
// driver is responsible for printing multiple copies. If the PD_USEDEVMODECOPIESANDCOLLATE flag is set in the Flags
// member, nCopies is always 1 on return, and the printer driver is responsible for printing multiple copies. If the
...
/////////////////////////////////////////////////////////////////////
// PrintInfo Methods
// @object PyCPrintInfo|Encapsulates an MFC CPrintInfo class, its member <c CPrintDialog> class, and the <c PRINTDLG>
// structure member of the CPrintDialog.
static struct PyMethodDef ui_prinfo_methods[] = {

    // CPrintInfo

    {"DocObject", ui_is_doc_object,
     1},  // @pymeth DocObject|A flag indicating whether the document being printed is a DocObject.
    {"GetDwFlags", ui_get_dwflags, 1},  // @pymeth GetDwFlags|A flags specifying DocObject printing operations. Valid
                                        // only if data member m_bDocObject is TRUE.
    {"SetDwFlags", ui_set_dwflags, 1},  // @pymeth SetDwFlags|Set a flag specifying DocObject printing operations. Valid
                                        // only if data member m_bDocObject is TRUE.
    {"GetDocOffsetPage", ui_get_doc_offset_page,
     1},  // @pymeth GetDocOffsetPage|Get the number of pages preceding the first page of a particular DocObject in a
          // combined DocObject print job.
    {"SetDocOffsetPage", ui_set_doc_offset_page,
     1},  // @pymeth SetDocOffsetPage|Set the number of pages preceding the first page of a particular DocObject in a
          // combined DocObject print job.
    {"SetPrintDialog", ui_set_print_dialog, 1},  // @pymeth SetPrintDialog|Set a pointer to the CPrintDialog object used
                                                 // to display the Print dialog box for the print job.
    {"GetDirect", ui_get_direct,
     1},  // @pymeth GetDirect|TRUE if the Print dialog box will be bypassed for direct printing; FALSE otherwise.
    {"SetDirect", ui_set_direct, 1},    // @pymeth SetDirect|Sets to TRUE if the Print dialog box will be bypassed for
                                        // direct printing; FALSE otherwise.
    ...
    {"GetContinuePrinting", ui_get_continue_printing,
     1},  // @pymeth GetContinuePrinting|A flag indicating whether the framework should continue the print loop.
    {"SetContinuePrinting", ui_set_continue_printing,
     1},  // @pymeth SetContinuePrinting|Set whether the framework should continue the print loop.
    ...
    {"GetNumPreviewPages", ui_get_num_preview_pages,
     1},  // @pymeth GetNumPreviewPages|Get the number of pages displayed in preview mode.
    {"SetNumPreviewPages", ui_set_num_preview_pages,
     1},  // @pymeth SetNumPreviewPages|Set the number of pages displayed in preview mode.
    ...
    {"GetPageDesc", ui_get_page_desc,
     1},  // @pymeth GetPageDesc|Get the format string used to display the page numbers during print preview
    {"SetPageDesc", ui_set_page_desc,
     1},  // @pymeth SetPageDesc|Set the format string used to display the page numbers during print preview
    ...
    {"GetOffsetPage", ui_get_offset_page, 1},  // @pymeth GetOffsetPage|Get the number of pages preceding the first page
                                               // of a DocObject item being printed in a combined DocObject print job.
    ...
    {"SetHDC", ui_set_hdc, 1},  // @pymeth SetHDC|Sets the printer DC compatible with the users choices, call after the
                                // print dialog DoModal finishes.
    {"CreatePrinterDC", ui_create_printer_dc, 1},  // @pymeth CreatePrinterDC|Handle to the newly created printer device
                                                   // context, call only after DoModal finishes.
    {"DoModal", ui_do_modal, 1},                   // @pymeth DoModal|Call DoModal on the dialog.
    {"GetCopies", ui_get_copies,
     1},  // @pymeth GetCopies|The number of copies requested, call only after DoModal finishes.
    {"GetDefaults", ui_get_defaults,
     1},  // @pymeth GetDefaults|Retrieves device defaults without displaying a dialog box.
    {"FreeDefaults", ui_free_defaults,
     1},  // @pymeth FreeDefaults|After a call to GetDefaults, and you are through with the CPrintDialog object, this
          // call deletes the printer DC and calls GlobalFree function on the handles.
    {"GetDeviceName", ui_get_device_name,
     1},  // @pymeth GetDeviceName|The name of the currently selected printer, call only after DoModal finishes.
    {"GetDriverName", ui_get_driver_name, 1},  // @pymeth GetDriverName|The name of the currently selected printer
                                               // device driver, call only after DoModal finishes.
    {"GetDlgFromPage", ui_get_dlg_from_page,
     1},                                      // @pymeth GetDlgFromPage|Retrieves the starting page of the print range.
    {"GetDlgToPage", ui_get_dlg_to_page, 1},  // @pymeth GetDlgToPage|Retrieves the ending page of the print range.
    {"GetPortName", ui_get_port_name,
     1},  // @pymeth GetPortName|The name of the currently selected printer port, call only after DoModal finishes.
    {"GetPrinterDC", ui_get_printer_dc,
     1},  // @pymeth GetPrinterDC|A handle to the printer device context if successful; otherwise NULL.  If the
          // bPrintSetupOnly parameter of the CPrintDialog constructor was FALSE (indicating that the Print dialog box
          // is displayed), then GetPrinterDC returns a handle to the printer device context. You must call the
          // WindowsDeleteDC function to delete the device context when you are done using it.
    {"PrintAll", ui_print_all, 1},          // @pymeth PrintAll|Nonzero if all pages in the document are to be printed;
                                            // otherwise 0, call only after DoModal finishes.
    {"PrintCollate", ui_print_collate, 1},  // @pymeth PrintCollate|Nonzero if the user selects the collate check box in
                                            // the dialog box; otherwise 0, call only after DoModal finishes.
    {"PrintRange", ui_print_range, 1},  // @pymeth PrintRange|Nonzero if only a range of pages in the document are to be
                                        // printed; otherwise 0, call only after DoModal finishes.
    {"PrintSelection", ui_print_selection, 1},  // @pymeth PrintSelection|Nonzero if only the selected items are to be
                                                // printed; otherwise 0., call only after DoModal finishes

    // PRINTDLG (CPrintInfo->CPrintDialog->m_pd)

    {"GetHDC", ui_get_hdc,
     1},  // @pymeth GetHDC|Identifies a device context or an information context, depending on whether the Flags member
          // specifies the PD_RETURNDC or PC_RETURNIC flag. If neither flag is specified, the value of this member is
          // undefined. If both flags are specified, PD_RETURNDC has priority.
    {"GetFlags", ui_get_flags,
     1},  // @pymeth GetFlags|A set of bit flags that you can use to initialize the Print common dialog box. When the
          // dialog box returns, it sets these flags to indicate the user's input.
    {"SetFlags", ui_set_flags,
     1},  // @pymeth SetFlags|A set of bit flags that you can use to initialize the Print common dialog box. When the
          // dialog box returns, it sets these flags to indicate the user's input.
    ...
    {"GetPRINTDLGMinPage", ui_get_printdlg_min_page,
     1},  // @pymeth GetPRINTDLGMinPage|Get the minimum value for the page range specified in the From and To page edit
          // controls. If nMinPage equals nMaxPage, the Pages radio button and the starting and ending page edit
          // controls are disabled.
    {"SetPRINTDLGMinPage", ui_set_printdlg_min_page,
     1},  // @pymeth SetPRINTDLGMinPage|Set the minimum value for the page range specified in the From and To page edit
          // controls. If nMinPage equals nMaxPage, the Pages radio button and the starting and ending page edit
          // controls are disabled.
    {"GetPRINTDLGCopies", ui_get_printdlg_copies,
     1},  // @pymeth GetPRINTDLGCopies|Gets the initial number of copies for the Copies edit control if hDevMode is
          // NULL; otherwise, the dmCopies member of the DEVMODE structure contains the initial value.
    {"SetPRINTDLGCopies", ui_set_printdlg_copies,
     1},  // @pymeth SetPRINTDLGCopies|Sets the initial number of copies for the Copies edit control if hDevMode is
          // NULL; otherwise, the dmCopies member of the DEVMODE structure contains the initial value.
    {NULL, NULL}};

ui_type ui_prinfo_object::type("PyCPrintInfo", &ui_assoc_object::type, sizeof(ui_prinfo_object),
                               PYOBJ_OFFSET(ui_prinfo_object), ui_prinfo_methods, GET_PY_CTOR(ui_prinfo_object));

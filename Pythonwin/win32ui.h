...
#define MAKE_PY_CTOR(classname) \
    static ui_base_class *classname::PyObConstruct(void) { return new classname; }
#else
#define MAKE_PY_CTOR(classname)                          \
    static ui_base_class *classname::PyObConstruct(void) \
    {                                                    \
...
enum EnumExceptionHandlerAction { EHA_PRINT_ERROR, EHA_DISPLAY_DIALOG };
...
PYW_EXPORT BOOL Python_OnCmdMsg(CCmdTarget *, UINT nID, int nCode, void *pExtra,
                                AFX_CMDHANDLERINFO *pHandlerInfo);  // TRUE if fully processed.
PYW_EXPORT BOOL Python_OnNotify(CWnd *pFrom, WPARAM, LPARAM lParam, LRESULT *pResult);

// Conversion routines
PYW_EXPORT BOOL CreateStructFromPyObject(LPCREATESTRUCT lpcs, PyObject *ob, const char *fnName = NULL,
                                         BOOL bFromTuple = FALSE);

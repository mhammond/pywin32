...
BOOL CPythonPropertySheet::OnCmdMsg(UINT nID, int nCode, void *pExtra, AFX_CMDHANDLERINFO *pHandlerInfo)
{
...
BOOL CPythonPropertySheet::OnCommand(WPARAM wParam, LPARAM lParam)
{
    ...
    if (m_customizeFont && (ID_APPLY_NOW == wParam || ID_WIZNEXT == wParam || ID_WIZBACK == wParam))
        PostMessage(WM_RESIZEPAGE);

    return CPropertySheet::OnCommand(wParam, lParam);
}

BOOL CPythonPropertySheet::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT *pResult)
{
    if (m_customizeFont) {

...
class SplitVector {
   protected:
    ...
    void GapTo(int position)
    {
        if (position != part1Length) {
            if (position < part1Length) {
                memmove(body + position + gapLength, body + position, sizeof(T) * (part1Length - position));
            }
            else {  // position > part1Length
                memmove(body + part1Length, body + part1Length + gapLength, sizeof(T) * (position - part1Length));
            }
   ...
   public:
    ...
    void SetGrowSize(int growSize_) { growSize = growSize_; }
    ...
    void InsertValue(int position, int insertLength, T v)
    {
        ...
        if (insertLength > 0) {
            ...
            GapTo(position);
            for (int i = 0; i < insertLength; i++) body[part1Length + i] = v;

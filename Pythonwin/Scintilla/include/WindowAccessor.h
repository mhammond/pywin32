...
class WindowAccessor : public Accessor {
   ...
   public:
    WindowAccessor(WindowID id_, PropSet &props_)
        : Accessor(), id(id_), props(props_), lenDoc(-1), validLen(0), chFlags(0), chWhile(0)
    {
    }
    ~WindowAccessor();
    ...
    int GetPropertyInt(const char *key, int defaultValue = 0) { return props.GetInt(key, defaultValue); }

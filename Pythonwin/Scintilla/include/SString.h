...
class SContainer {
   ...
   public:
     ...
     * @return the pointer to the new string
     */
    static char *StringAllocate(const char *s,                   ///< The string to duplicate
...
class SBuffer : protected SContainer {
   ...
   public:
    ...
    /** Size of buffer. */
    lenpos_t size() const { return SContainer::size(); }
};
...
class SString : protected SContainer {
   ...
   public:
    ...
    char operator[](lenpos_t i) const { return (s && i < sSize) ? s[i] : '\0'; }
    ...
    bool contains(char ch) const { return (s && *s) ? strchr(s, ch) != 0 : false; }
    ...
    SString &operator+=(const char *sOther) { return append(sOther, static_cast<lenpos_t>(measure_length)); }
    SString &operator+=(const SString &sOther) { return append(sOther.s, sOther.sLen); }
    SString &operator+=(char ch) { return append(&ch, 1); }
    SString &appendwithseparator(const char *sOther, char sep) { return append(sOther, strlen(sOther), sep); }
    ...
    int value() const { return s ? atoi(s) : 0; }
    bool startswith(const char *prefix);
    bool endswith(const char *suffix);
    int search(const char *sFind, lenpos_t start = 0) const;
    bool contains(const char *sFind) const { return search(sFind) >= 0; }

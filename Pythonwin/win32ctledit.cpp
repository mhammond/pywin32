...
// @pymethod <o PyCEdit>|win32ui|CreateEdit|Creates an Edit object.  <om PyCEdit.CreateWindow> creates the actual
// control.
PyObject *PyCEdit_create(PyObject *self, PyObject *args)
{
    CHECK_NO_ARGS(args);
...
static PyObject *PyCEdit_create_window(PyObject *self, PyObject *args)
{
    int style, id;
    PyObject *obParent;
    RECT rect;
...
static PyObject *PyCEdit_set_sel(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            ...
            &bNoScroll)) {  // @pyparm int|bNoScroll|0|Indicates whether the caret should be scrolled into view. If 0,
                            // the caret is scrolled into view. If 1, the caret is not scrolled into view.
        PyErr_Clear();
        bNoScroll = FALSE;
        if (!PyArg_ParseTuple(
                args, "(ii)|i:SetSel",
                ...
                &end,
                &bNoScroll))  // @pyparmalt2 int|bNoScroll|0|Indicates whether the caret should be scrolled into view.
                              // If 0, the caret is scrolled into view. If 1, the caret is not scrolled into view.
            return NULL;
    }
    if (start != end && end == 0)
        end = start;
...
static PyObject *PyCEdit_set_readonly(PyObject *self, PyObject *args)
{
    CEdit *pEdit = GetEditCtrl(self);
    if (!pEdit)
        return NULL;
...
static PyObject *PyCEdit_get_line_count(PyObject *self, PyObject *args)
{
    CHECK_NO_ARGS(args);
    CEdit *pEdit = GetEditCtrl(self);
    if (!pEdit)
...
static PyObject *PyCEdit_get_first_visible(PyObject *self, PyObject *args)
{
    CHECK_NO_ARGS(args);
    CEdit *pEdit = GetEditCtrl(self);
    if (!pEdit)
...
static PyObject *PyCEdit_line_index(PyObject *self, PyObject *args)
{
...
static PyObject *PyCEdit_line_scroll(PyObject *self, PyObject *args)
{
    // @comm This method only works on multi-linr edit controls.
    CEdit *pEdit = GetEditCtrl(self);
    if (!pEdit)
...
static PyObject *PyCEdit_line_from_char(PyObject *self, PyObject *args)
{
    CEdit *pEdit = GetEditCtrl(self);
    if (!pEdit)
        return NULL;
...
static PyObject *PyCEdit_get_line(PyObject *self, PyObject *args)
{
    CEdit *pEdit = GetEditCtrl(self);
    if (!pEdit)
        return NULL;
...
static PyObject *PyCEdit_clear(PyObject *self, PyObject *args)
{
    CHECK_NO_ARGS(args);
    CEdit *pEdit = GetEditCtrl(self);
    if (!pEdit)
...
static struct PyMethodDef PyCEdit_methods[] = {
    ...
    {"GetFirstVisibleLine", PyCEdit_get_first_visible,
    ...
    {"GetLineCount", PyCEdit_get_line_count,
...
ui_type_CObject PyCEdit::type("PyCEdit", &ui_control_object::type, RUNTIME_CLASS(CEdit), sizeof(PyCEdit),
                              PYOBJ_OFFSET(PyCEdit), PyCEdit_methods, GET_PY_CTOR(PyCEdit));

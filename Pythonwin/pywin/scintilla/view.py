...
wordbreaks = "._" + string.ascii_uppercase + \
...
class CScintillaView(docview.CtrlView, control.CScintillaColorEditInterface):
    def __init__(self, doc):
        docview.CtrlView.__init__(self, doc, "Scintilla", win32con.WS_CHILD | win32con.WS_VSCROLL |
                                  win32con.WS_HSCROLL | win32con.WS_CLIPCHILDREN | win32con.WS_VISIBLE)
...
#		return self._obj_.SendMessage(msg, w, l)
    ...
    def HookHandlers(self):
        ...
        for command, reflection in command_reflectors:
            def handler(id, code, ss=self.SendScintilla,
                        tosend=reflection): return ss(tosend) and 0
        ...
        self.HookCommand(self.OnCmdViewIndentationGuides,
        ...
        self.HookCommandUpdate(self.OnUpdateViewRightEdge,
        ...
        self.HookCommandUpdate(self.OnUpdateViewFixedFont,
...
#		self.HookMessage(self.OnMouseWheel, win32con.WM_MOUSEWHEEL)
    ...
    def OnInitialUpdate(self):
        ...
        # Enable Unicode
        self.SendScintilla(scintillacon.SCI_SETCODEPAGE,
                           scintillacon.SC_CP_UTF8, 0)
        self.SendScintilla(scintillacon.SCI_SETKEYSUNICODE, 1, 0)

        # Create margins
        self.SendScintilla(scintillacon.SCI_SETMARGINTYPEN,
                           1, scintillacon.SC_MARGIN_SYMBOL)
        self.SendScintilla(scintillacon.SCI_SETMARGINMASKN, 1, 0xF)
        self.SendScintilla(scintillacon.SCI_SETMARGINTYPEN,
                           2, scintillacon.SC_MARGIN_SYMBOL)
        self.SendScintilla(scintillacon.SCI_SETMARGINMASKN,
                           2, scintillacon.SC_MASK_FOLDERS)
        self.SendScintilla(scintillacon.SCI_SETMARGINSENSITIVEN, 2, 1)

        self.GetDocument().HookViewNotifications(
            self)  # is there an MFC way to grab this?
        self.HookHandlers()
    ...
    def DoConfigChange(self):
        ...
        self.bAutoCompleteAttributes = GetEditorOption(
    ...
    # Update the tick on the UI.
    def OnUpdateViewIndentationGuides(self, cmdui):
    ...
    def _AutoComplete(self):
        ...
        if ob is not None:
            try:  # Catch unexpected errors when fetching attribute names from the object
                ...
                if hasattr(ob, "__class__"):
                    items_dict.update(
                        list2dict(_get_class_attributes(ob.__class__)))
                ...
                win32ui.SetStatusText(
                    "Error attempting to get object attributes - %s" % (repr(sys.exc_info()[0]),))
        ...
        if not items:
            ...
            items = [word for word in unique.iterkeys() if word[:2] !=
                     '__' or word[-2:] != '__']
    ...
    def _GetClassInfoFromBrowser(self, pos=-1):
        ...
        import pywin.framework.scriptutils
        curmodule, path = pywin.framework.scriptutils.GetPackageModuleName(
            path)
        try:
...
#		pInfo.SetPrintDialog(self.prtDlg)
    ...
    def FormatRange(self, dc, pageStart, lengthDoc, rc, draw):
        ...
        fr = struct.pack(fmt, hdcRender, hdcFormat,
                         rc[0], rc[1], rc[2], rc[3], rc[0], rc[1], rc[2], rc[3], pageStart, lengthDoc)
...
#		print "dev", w, h, l, metrics['tmAscent'], metrics['tmDescent']
        ...
        pagenum_str = win32ui.LoadString(
            afxres.AFX_IDS_PRINTPAGENUM) % (pageNum+1,)
...
def LoadConfiguration():
    ...
    configName = rc = win32ui.GetProfileVal(
        "Editor", "Keyboard Config", "default")
    configManager = ConfigManager(configName)
    if configManager.last_error:
        bTryDefault = 0
        msg = "Error loading configuration '%s'\n\n%s" % (
            configName, configManager.last_error)

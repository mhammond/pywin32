...
class FormatterBase:
    ...
    def _ReformatStyle(self, style):
        ...
        if style.name == STYLE_SELECTION:
            ...
            self.scintilla.SendScintilla(
                scintillacon.SCI_SETSELBACK, True, clr)
        ...
        scintilla.SCIStyleSetEOLFilled(stylenum, 1)
    ...
    def LoadPreferences(self):
        self.baseFormatFixed = eval(self.LoadPreference(
            "Base Format Fixed", str(self.baseFormatFixed)))
        self.baseFormatProp = eval(self.LoadPreference(
            "Base Format Proportional", str(self.baseFormatProp)))
        ...
        for style in self.styles.values():
            ...
            style.background = int(self.LoadPreference(
    ...
    def SavePreferences(self):
        ...
        self.SavePreference("Base Format Proportional",
                            str(self.baseFormatProp))
...
class Formatter(FormatterBase):
    ...
    def OnStyleNeeded(self, std, extra):
        ...
        endStyledChar = self.scintilla.SendScintilla(
            scintillacon.SCI_GETENDSTYLED)
...
STRING_STYLES = [STYLE_STRING, STYLE_SQSTRING,
                 STYLE_TQSSTRING, STYLE_TQDSTRING, STYLE_STRINGEOL]
...
PYTHON_STYLES = [
    (STYLE_DEFAULT,      (0, 0, 200, 0, 0x808080),
     CLR_INVALID,     scintillacon.SCE_P_DEFAULT),
    (STYLE_COMMENT,      (0, 2, 200, 0, 0x008000),
     CLR_INVALID,     scintillacon.SCE_P_COMMENTLINE),
    (STYLE_COMMENT_BLOCK, (0, 2, 200, 0, 0x808080),
     CLR_INVALID,     scintillacon.SCE_P_COMMENTBLOCK),
    (STYLE_NUMBER,       (0, 0, 200, 0, 0x808000),
     CLR_INVALID,     scintillacon.SCE_P_NUMBER),
    (STYLE_STRING,       (0, 0, 200, 0, 0x008080),
     CLR_INVALID,     scintillacon.SCE_P_STRING),
    (STYLE_SQSTRING,     STYLE_STRING,
     CLR_INVALID,     scintillacon.SCE_P_CHARACTER),
    (STYLE_TQSSTRING,    STYLE_STRING,
     CLR_INVALID,     scintillacon.SCE_P_TRIPLE),
    (STYLE_TQDSTRING,    STYLE_STRING,
     CLR_INVALID,     scintillacon.SCE_P_TRIPLEDOUBLE),
    (STYLE_STRINGEOL,    (0, 0, 200, 0, 0x000000),
     0x008080, scintillacon.SCE_P_STRINGEOL),
    (STYLE_KEYWORD,      (0, 1, 200, 0, 0x800000),
     CLR_INVALID,     scintillacon.SCE_P_WORD),
    (STYLE_CLASS,        (0, 1, 200, 0, 0xFF0000),
     CLR_INVALID,     scintillacon.SCE_P_CLASSNAME),
    (STYLE_METHOD,       (0, 1, 200, 0, 0x808000),
     CLR_INVALID,     scintillacon.SCE_P_DEFNAME),
    (STYLE_OPERATOR,     (0, 0, 200, 0, 0x000000),
     CLR_INVALID,     scintillacon.SCE_P_OPERATOR),
    (STYLE_IDENTIFIER,   (0, 0, 200, 0, 0x000000),
     CLR_INVALID,     scintillacon.SCE_P_IDENTIFIER),
]

# These styles _always_ have this specific style number, regardless of
# internal or external formatter.
SPECIAL_STYLES = [
    (STYLE_BRACE,        (0, 0, 200, 0, 0x000000),
     0xffff80, scintillacon.STYLE_BRACELIGHT),
    (STYLE_BRACEBAD,     (0, 0, 200, 0, 0x000000),
     0x8ea5f2, scintillacon.STYLE_BRACEBAD),
    (STYLE_LINENUMBER,   (0, 0, 200, 0, 0x000000), win32api.GetSysColor(
        win32con.COLOR_3DFACE), scintillacon.STYLE_LINENUMBER),
    (STYLE_INDENTGUIDE,  (0, 0, 200, 0, 0x000000),
     CLR_INVALID, scintillacon.STYLE_INDENTGUIDE),
...
class BuiltinSourceFormatter(FormatterBase):
    ...
    def RegisterStyle(self, style, stylenum=None):
        ...
        if stylenum is None:
            stylenum = self.nextstylenum
            self.nextstylenum = self.nextstylenum + 1
        assert self.styles.get(
            stylenum) is None, "We are reusing a style number!"
        style.stylenum = stylenum
        self.styles[style.name] = style
        self.styles_by_id[stylenum] = style

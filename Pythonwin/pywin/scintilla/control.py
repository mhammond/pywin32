...
if win32ui.debug:  # If running _d version of Pythonwin...
    try:
        dllid = win32api.LoadLibrary(os.path.join(
...
if dllid is None:
    try:
        dllid = win32api.LoadLibrary(os.path.join(
            os.path.split(win32ui.__file__)[0], "Scintilla.DLL"))
    except win32api.error:
        pass
if dllid is None:
...
class ScintillaControlInterface:
    ...
    def SCIAddText(self, text):
        self.SendMessage(scintillacon.SCI_ADDTEXT,
                         text.encode(default_scintilla_encoding))
    ...
    def SCIAddStyledText(self, text, style=None):
        ...
        if style is not None:
            ...
            text = ''.join(text)
        self.SendMessage(scintillacon.SCI_ADDSTYLEDTEXT,
                         text.encode(default_scintilla_encoding))
...
#   NOTE - dependent on of we use builtin lexer, so handled below.
    ...
    def SCIStyleSetFont(self, num, name, characterset=0):
        ...
        self.SendScintilla(
            scintillacon.SCI_STYLESETCHARACTERSET, num, characterset)
    ...
    def SCIMarkerDefine(self, markerNum, markerType):
        self.SendScintilla(scintillacon.SCI_MARKERDEFINE,
                           markerNum, markerType)
    ...
    def SCISetProperty(self, name, value):
        name_buff = array.array(
            'b',  (name + '\0').encode(default_scintilla_encoding))
        ...
        self.SendScintilla(scintillacon.SCI_SETPROPERTY,
                           address_name_buffer, address_val_buffer)
...
class CScintillaEditInterface(ScintillaControlInterface):
    ...
    def FindText(self, flags, range, findText):
        ...
        rc = self.SendScintilla(EM_FINDTEXTEX, flags,
                                ft_array.buffer_info()[0])
    ...
    def SetSel(self, start=0, end=None):
        ...
        assert start <= self.GetTextLength(
        ), "The start postion is invalid (%d/%d)" % (start, self.GetTextLength())
        assert end <= self.GetTextLength(), "The end postion is invalid (%d/%d)" % (end,
                                                                                    self.GetTextLength())
    ...
    def LineFromChar(self, charPos=-1):
        ...
        ), "The charPos postion (%s) is invalid (max=%s)" % (charPos, self.GetTextLength())

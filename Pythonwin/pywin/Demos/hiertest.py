...
class DirHierList(hierlist.HierList):
    def __init__(self, root, listBoxID=win32ui.IDC_LIST1):
        hierlist.HierList.__init__(
            self, root, win32ui.IDB_HIERFOLDERS, listBoxID)
    ...
    def GetSelectedBitmapColumn(self, item):
        # Use different color for selection
...
class TestDocument(docview.Document):
    def __init__(self, template):
        ...
        self.hierlist = hierlist.HierListWithItems(HLIFileDir(
            "\\"), win32ui.IDB_HIERFOLDERS, win32ui.AFX_IDW_PANE_FIRST)
...
def demohli():
    template = docview.DocTemplate(
        win32ui.IDR_PYTHONTYPE, TestDocument, hierlist.HierListFrame, hierlist.HierListView)
    template.OpenDocumentFile(None).SetTitle("Hierlist demo")

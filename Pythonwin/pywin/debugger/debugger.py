...
#		print self.debugger.stack, self.debugger.curframe
        ...
        if debugger.debuggerState == DBGSTATE_BREAK:
            ...
            for frame, lineno in stackUse:
                ...
                if frame is debugger.userbotframe:
                    break
...
class DebuggerWindow(window.Wnd):
    ...
    def OnDestroy(self, msg):
        ...
        pywin.framework.app.SaveWindowSize(
            "Debugger Windows\\" + self.title, newSize)
...
class DebuggerStackWindow(DebuggerWindow):
    ...
    def SaveState(self):
        ...
        self.listOK = 0
        win32ui.WriteProfileVal("Debugger Windows\\" +
                                self.title, "Visible", self.IsWindowVisible())
    ...
    def CreateWindow(self, parent):
        ...
        self._obj_.CreateWindow(style, self.GetDefRect(),
                                parent, win32ui.IDC_LIST1)
...
class DebuggerListViewWindow(DebuggerWindow):
    ...
    def OnItemRightClick(self, notify_data, extra):
        ...
        if hItem == -1 or commctrl.TVHT_ONITEM & flags == 0:
            return None
        self.SetItemState(hItem, commctrl.LVIS_SELECTED,
                          commctrl.LVIS_SELECTED)

        menu = win32ui.CreatePopupMenu()
        menu.AppendMenu(win32con.MF_STRING |
                        win32con.MF_ENABLED, 1000, "Edit item")
        menu.AppendMenu(win32con.MF_STRING |
                        win32con.MF_ENABLED, 1001, "Delete item")
...
class DebuggerBreakpointsWindow(DebuggerListViewWindow):
    ...
    def SaveState(self):
        ...
        win32ui.WriteProfileVal("Debugger Windows\\" +
                                self.title, "BreakpointList", "\t".join(items))
        win32ui.WriteProfileVal("Debugger Windows\\" +
                                self.title, "Visible", self.IsWindowVisible())
        return 1
...
class DebuggerWatchWindow(DebuggerListViewWindow):
    ...
    def CreateWindow(self, parent):
        ...
        items = win32ui.GetProfileVal(
            "Debugger Windows\\" + self.title, "Items", "").split("\t")
        index = -1
        for item in items:
            if item:
...
def PrepareControlBars(frame):
    ...
    for id, klass, float in DebuggerDialogInfos:
        ...
        bar.CreateWindow(frame, CreateDebuggerDialog, klass.title,
                         id, style, childCreatorArgs=(klass,))
...
#		frame.RecalcLayout()
    ...
    def GUIAboutToBreak(self):
        ...
        if self.pumping:
            ...
            return
        self.pumping = 1
        # NOTE - This will NOT return until the user is finished interacting
...
#			fw.EnableWindow(0) Leave enabled for now?
        ...
        if self.inForcedGUI and not frame.IsWindowVisible():
            ...
            frame.UpdateWindow()
        if self.curframe:
            SetInteractiveContext(self.curframe.f_globals,

...
class ListDialog (dialog.Dialog):
    ...
    def _maketemplate(self, title):
        ...
        return [[title, (0, 0, 200, 200), style, None, (8, "MS Sans Serif")],
                ["SysListView32", None, win32ui.IDC_LIST1,
                    (0, 0, 200, 200), ls],
                [128,	"OK", win32con.IDOK,
                    (10, 0, 50, 14), bs | win32con.BS_DEFPUSHBUTTON],
    ...
    def OnListItemChange(self, std, extra):
        (hwndFrom, idFrom, code), (itemNotify, sub, newState,
                                   oldState, change, point, lparam) = std, extra
...
class ListsDialog(ListDialog):
    ...
    def FillList(self):
        ...
        size = self.GetWindowRect()
        width = size[2] - size[0] - \
            (10) - win32api.GetSystemMetrics(win32con.SM_CXVSCROLL)

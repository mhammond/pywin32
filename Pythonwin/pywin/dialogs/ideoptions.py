...
class OptionsPropPage(dialog.PropertyPage):
    ...
    def OnInitDialog(self):
        ...
        format = eval(win32ui.GetProfileVal(interact.sectionProfile,
                                            interact.STYLE_INTERACTIVE_PROMPT, str(interact.formatInput)))
        edit.SetDefaultCharFormat(format)
        edit.SetWindowText("Input Text")

        edit = self.GetDlgItem(win32ui.IDC_EDIT2)
        format = eval(win32ui.GetProfileVal(interact.sectionProfile,
                                            interact.STYLE_INTERACTIVE_OUTPUT, str(interact.formatOutput)))
        edit.SetDefaultCharFormat(format)
        edit.SetWindowText("Output Text")

        edit = self.GetDlgItem(win32ui.IDC_EDIT3)
        format = eval(win32ui.GetProfileVal(interact.sectionProfile,
                                            interact.STYLE_INTERACTIVE_ERROR, str(interact.formatOutputError)))
        edit.SetDefaultCharFormat(format)
        ...
        self['MRUSize'] = win32ui.GetProfileVal(
            "Settings", "Recent File List Size", 10)
    ...
    def OnOK(self):
        ...
        win32ui.WriteProfileVal(
            "Settings", "Recent File List Size", self['MRUSize'])

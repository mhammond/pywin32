...
class DockingBar(window.Wnd):
    ...
    def CreateWindow(self, parent, childCreator, title, id, style=win32con.WS_CHILD | win32con.WS_VISIBLE | afxres.CBRS_LEFT, childCreatorArgs=()):
        ...
        self._obj_.CreateWindow(wndClass, title, style,
                                (0, 0, 0, 0), parent, id)
        ...
        self.sizeHorz = self.sizeVert = self.sizeFloat = rect[2] - \
            rect[0], rect[3]-rect[1]

        self.sizeHorz = self.sizeHorz[0], self.sizeHorz[1] + \
            self.cxEdge + self.cxBorder
        self.sizeVert = self.sizeVert[0] + \
            self.cxEdge + self.cxBorder, self.sizeVert[1]
        self.HookMessages()

    def CalcFixedLayout(self, bStretch, bHorz):
        rectTop = self.dockSite.GetControlBar(
            afxres.AFX_IDW_DOCKBAR_TOP).GetWindowRect()
        rectLeft = self.dockSite.GetControlBar(
            afxres.AFX_IDW_DOCKBAR_LEFT).GetWindowRect()
        if bStretch:
    ...
    def CalcDynamicLayout(self, length, mode):
        ...
        if self.IsFloating():
            ...
            if hittest == win32con.HTTOPLEFT:
                ...
                cy = max(windowRect[3] - self.cCaptionSize -
                         pt[1], self.cMinHeight) - 1
                self.sizeFloat = cx, cy

                top = min(pt[1], windowRect[3] - self.cCaptionSize -
                          self.cMinHeight) - self.cyBorder
            ...
            if hittest == win32con.HTTOPRIGHT:
                ...
                top = min(pt[1], windowRect[3] - self.cCaptionSize -
                          self.cMinHeight) - self.cyBorder
                dc.rectFrameDragHorz = dc.rectFrameDragHorz[
                    0], top, dc.rectFrameDragHorz[2], dc.rectFrameDragHorz[3]
                return self.sizeFloat

            if hittest == win32con.HTBOTTOMLEFT:
                cx = max(windowRect[2] - pt[0], self.cMinWidth) - self.cxBorder
                cy = max(pt[1] - windowRect[1] -
                         self.cCaptionSize, self.cMinHeight)
                self.sizeFloat = cx, cy

                left = min(pt[0], windowRect[2] - self.cMinWidth) - 1
                dc.rectFrameDragHorz = left, dc.rectFrameDragHorz[
                    1], dc.rectFrameDragHorz[2], dc.rectFrameDragHorz[3]
                return self.sizeFloat
    ...
    def OnMouseMove(self, msg):
        ...
        if self.IsHorz():
            if cpt[1] != pt[1]:
                ...
                self.rectTracker = OffsetRect(
                    self.rectTracker, (0, pt[1] - cpt[1]))
                self.OnInvertTracker(self.rectTracker)
        else:
            if cpt[0] != pt[0]:
                self.OnInvertTracker(self.rectTracker)
                self.rectTracker = OffsetRect(
                    self.rectTracker, (pt[0]-cpt[0], 0))
                self.OnInvertTracker(self.rectTracker)
...
#	def OnBarStyleChange(self, old, new):
    ...
    def OnNcCalcSize(self, bCalcValid, size_info):
        ...
        if self.nDockBarID == afxres.AFX_IDW_DOCKBAR_TOP:
            ...
            self.rectBorder = self.rectBorder[0], self.rectBorder[3] - \
                self.cxEdge, self.rectBorder[2], self.rectBorder[3]
            ...
            self.rectBorder = self.rectBorder[2] - \
                self.cxEdge, self.rectBorder[1], self.rectBorder[2], self.rectBorder[3]
            ...
            rc0.bottom = rc0.bottom - self.cxBorder
            rc0.top = rc0.top + self.cxGripper
            self.rectBorder = self.rectBorder[0], self.rectBorder[1], self.rectBorder[0] + \
                self.cxEdge, self.rectBorder[3]
        else:
    ...
    def StartTracking(self):
        ...
        self.RedrawWindow(None, None, win32con.RDW_ALLCHILDREN |
    ...
    def OnInvertTracker(self, rect):
        ...
        try:
            ...
            dc.PatBlt((rect[0], rect[1]), (rect[2]-rect[0],
                                           rect[3]-rect[1]), win32con.PATINVERT)
    ...
    def DrawGripper(self, dc):
        ...
        if self._obj_.dwStyle & afxres.CBRS_ORIENT_HORZ:
            ...
            dc.Draw3dRect(OffsetRect(gripper, (4, 0)),
                          clrBtnHilight, clrBtnShadow)
        else:
            # gripper at top
            ...
            gripper = gl, gt, gr, gb
            dc.Draw3dRect(gripper, clrBtnHilight, clrBtnShadow)
            dc.Draw3dRect(OffsetRect(gripper, (0, 4)),
                          clrBtnHilight, clrBtnShadow)

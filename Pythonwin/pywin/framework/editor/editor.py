...
MODIFYING_VK_KEYS = [win32con.VK_BACK, win32con.VK_TAB,
                     win32con.VK_RETURN, win32con.VK_SPACE, win32con.VK_DELETE]
...
MODIFYING_VK_KEYS_CTRL = [
    win32con.VK_BACK, win32con.VK_RETURN, win32con.VK_SPACE, win32con.VK_DELETE]
...
class EditorView(ParentEditorView):
    def __init__(self, doc):
        ...
        self.defCharFormat = GetEditorFontOption(
            "Default Font", defaultCharacterFormat)
    ...
    def SetAllLineColors(self, color=None):
        if isRichText:
            ...
            try:
                ...
                self.SetSel(0, -1)
                self.SetSelectionCharFormat(
                    (win32con.CFM_COLOR, 0, 0, 0, color))
            finally:
...
#		print "curCol is", curCol, "nextColumn is", nextColumn
    ...
    def OnRClick(self, params):
        ...
        if patImport.match(line) == len(line):
            menu.AppendMenu(flags, ID_LOCATE_FILE, "&Locate %s.py" %
                            patImport.group('name'))
    ...
    def OnKeyTab(self, key):
        ...
        self.BlockDent(win32api.GetKeyState(
            win32con.VK_SHIFT) >= 0, startLine, endLine)
        return 0

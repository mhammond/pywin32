...
class SyntEditView(SyntEditViewParent):
    ...
    def OnInitialUpdate(self):
        ...
        self.HookCommand(self.OnCmdViewFoldTopLevel,
                         win32ui.ID_VIEW_FOLD_TOPLEVEL)

        # Define the markers
#		self.SCIMarkerDeleteAll()
        self.SCIMarkerDefineAll(MARKER_BOOKMARK, scintillacon.SC_MARK_ROUNDRECT, win32api.RGB(
            0x0, 0x0, 0x0), win32api.RGB(0, 0xff, 0xff))
        ...
        if 1:  # traditional markers
            self.SCIMarkerDefineAll(scintillacon.SC_MARKNUM_FOLDEROPEN, scintillacon.SC_MARK_MINUS, win32api.RGB(
                0xff, 0xff, 0xff), win32api.RGB(0, 0, 0))
            self.SCIMarkerDefineAll(scintillacon.SC_MARKNUM_FOLDER, scintillacon.SC_MARK_PLUS, win32api.RGB(
                0xff, 0xff, 0xff), win32api.RGB(0, 0, 0))
            self.SCIMarkerDefineAll(scintillacon.SC_MARKNUM_FOLDERSUB, scintillacon.SC_MARK_EMPTY, win32api.RGB(
                0xff, 0xff, 0xff), win32api.RGB(0, 0, 0))
            self.SCIMarkerDefineAll(scintillacon.SC_MARKNUM_FOLDERTAIL, scintillacon.SC_MARK_EMPTY, win32api.RGB(
                0xff, 0xff, 0xff), win32api.RGB(0, 0, 0))
            self.SCIMarkerDefineAll(scintillacon.SC_MARKNUM_FOLDEREND, scintillacon.SC_MARK_EMPTY, win32api.RGB(
                0xff, 0xff, 0xff), win32api.RGB(0, 0, 0))
            self.SCIMarkerDefineAll(scintillacon.SC_MARKNUM_FOLDEROPENMID, scintillacon.SC_MARK_EMPTY, win32api.RGB(
                0xff, 0xff, 0xff), win32api.RGB(0, 0, 0))
            self.SCIMarkerDefineAll(scintillacon.SC_MARKNUM_FOLDERMIDTAIL, scintillacon.SC_MARK_EMPTY, win32api.RGB(
                0xff, 0xff, 0xff), win32api.RGB(0, 0, 0))
        else:  # curved markers
            self.SCIMarkerDefineAll(scintillacon.SC_MARKNUM_FOLDEROPEN, scintillacon.SC_MARK_CIRCLEMINUS, win32api.RGB(
                0xff, 0xff, 0xff), win32api.RGB(0, 0, 0))
            self.SCIMarkerDefineAll(scintillacon.SC_MARKNUM_FOLDER, scintillacon.SC_MARK_CIRCLEPLUS, win32api.RGB(
                0xff, 0xff, 0xff), win32api.RGB(0, 0, 0))
            self.SCIMarkerDefineAll(scintillacon.SC_MARKNUM_FOLDERSUB, scintillacon.SC_MARK_VLINE, win32api.RGB(
                0xff, 0xff, 0xff), win32api.RGB(0, 0, 0))
            self.SCIMarkerDefineAll(scintillacon.SC_MARKNUM_FOLDERTAIL, scintillacon.SC_MARK_LCORNERCURVE, win32api.RGB(
                0xff, 0xff, 0xff), win32api.RGB(0, 0, 0))
            self.SCIMarkerDefineAll(scintillacon.SC_MARKNUM_FOLDEREND, scintillacon.SC_MARK_CIRCLEPLUSCONNECTED, win32api.RGB(
                0xff, 0xff, 0xff), win32api.RGB(0, 0, 0))
            self.SCIMarkerDefineAll(scintillacon.SC_MARKNUM_FOLDEROPENMID, scintillacon.SC_MARK_CIRCLEMINUSCONNECTED, win32api.RGB(
                0xff, 0xff, 0xff), win32api.RGB(0, 0, 0))
            self.SCIMarkerDefineAll(scintillacon.SC_MARKNUM_FOLDERMIDTAIL, scintillacon.SC_MARK_TCORNERCURVE, win32api.RGB(
                0xff, 0xff, 0xff), win32api.RGB(0, 0, 0))
    ...
    def DoConfigChange(self):
        ...
        self.SCISetIndentationGuides(
            GetEditorOption("View Indentation Guides", 0))
        ...
        self.SCISetEdgeColor(GetEditorOption(
        ...
        fold_flags = 0
        self.SendScintilla(scintillacon.SCI_SETMODEVENTMASK,
                           scintillacon.SC_MOD_CHANGEFOLD)
        if self.bFolding:
        ...
        if bSmartTabs:
            ...
            ext.config(usetabs=usetabs, indentwidth=indentwidth,
                       tabwidth=tabSize)
        else:
            # Dont want smart-tabs - just set the options!
            ext.config(usetabs=bUseTabs, tabwidth=tabSize,
                       indentwidth=indentSize)
    ...
    def OnDebuggerStateChange(self, state):
        if state == dbgcon.DBGSTATE_NOT_DEBUGGING:
            ...
            self.SCIMarkerSetBack(
                MARKER_BREAKPOINT, win32api.RGB(0xef, 0xef, 0xef))
        else:
            # A light-red, so still readable when no marker margin.
            self.SCIMarkerSetBack(
                MARKER_BREAKPOINT, win32api.RGB(0xff, 0x80, 0x80))

    def HookDocumentHandlers(self):
        SyntEditViewParent.HookDocumentHandlers(self)
        self.HookMessage(self.OnCheckExternalDocumentUpdated,
...
#			max_line = self.GetLineCount()
    ...
    def OnRClick(self, params):
        ...
        self.AppendMenu(menu, 'View &Whitespace',
                        'ViewWhitespace', checked=self.SCIGetViewWS())
        self.AppendMenu(menu, "&Fixed Font", "ViewFixedFont",
                        checked=self._GetColorizer().bUseFixed)
        self.AppendMenu(menu, flags=win32con.MF_SEPARATOR)
    ...
    def OnUpdateViewFold(self, cmdui):  # Update the tick on the UI.
        ...
        if id in [win32ui.ID_VIEW_FOLD_EXPAND_ALL, win32ui.ID_VIEW_FOLD_COLLAPSE_ALL]:
            ...
            foldable = self.SCIGetFoldLevel(
                lineno) & scintillacon.SC_FOLDLEVELHEADERFLAG
    ...
    def FoldTopLevelEvent(self, event=None):
        ...
        try:
            ...
            for lineSeek in xrange(lineSeek, maxLine):
                ...
                level_no = level & scintillacon.SC_FOLDLEVELNUMBERMASK - \
...
class SyntEditTemplate(EditorTemplateBase):
    ...
    def CheckIDLEMenus(self, idle):
        ...
        for text, event in newitems:
            ...
            if id is not None:
                keyname = pywin.scintilla.view.configManager.get_key_binding(event, [
                                                                             "editor"])
        ...
        editMenu.AppendMenu(win32con.MF_STRING | win32con.MF_POPUP |
                            win32con.MF_ENABLED, submenu.GetHandle(), "&Source Code")

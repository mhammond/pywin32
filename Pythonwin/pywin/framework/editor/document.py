    ...
    def CheckExternalDocumentUpdated(self):
        ...
        try:
            ...
            if not self.bReportedFileNotFound:
                print(
                    "The file '{}' is open for editing, but\nchecking it for changes caused the error: {}".format(self.GetPathName(), exc.strerror)
                )
                self.bReportedFileNotFound = 1
            return
        if self.bReportedFileNotFound:
            print(
                "The file '{}' has re-appeared - continuing to watch for changes...".format(self.GetPathName())
            )

    ...
    def TakeDefaultAction(self):
        if self.file:
            ...
            win32ui.SetStatusText(
                "Can not locate the source code for this object.")

    def PerformItemSelected(self):
        if self.file is None:
            msg = "%s - source can not be located." % (self.name, )
        else:
            msg = "%s defined at line %d of %s" % (
                self.name, self.lineno, self.file)
...
class BrowserView(pywin.mfc.docview.TreeView):
    ...
    def _MakeRoot(self):
        ...
        if not path:
            ...
            if self.bDirty:
                ...
                what = "Building"
            win32ui.SetStatusText(
                "%s class list - please wait..." % (what,), 1)
    ...
    def CheckMadeList(self):
        ...
        self.list = list = hierlist.HierListWithItems(
            root, win32ui.IDB_BROWSER_HIER)

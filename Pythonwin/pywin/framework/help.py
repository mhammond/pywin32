...
def FinalizeHelp():
    ...
    if htmlhelp_handle is not None:
        ...
        try:
            ...
            frame = 0
            win32help.HtmlHelp(
                frame, None, win32help.HH_UNINITIALIZE, htmlhelp_handle)
...
def OpenHelpFile(fileName, helpCmd=None, helpArg=None):
    ...
    try:
        ...
        if ext == ".hlp":
            win32api.WinHelp(win32ui.GetMainFrame(
            ).GetSafeHwnd(), fileName, helpCmd, helpArg)
        ...
        # which may or may not be related.
            ...
            win32api.ShellExecute(0, "open", fileName,
                                  None, "", win32con.SW_SHOW)
        return fileName
    finally:
        win32ui.DoWaitCursor(-1)
...
def SetHelpMenuOtherHelp(mainMenu):
    ...
    if helpIDMap:
        for id, (desc, fname) in helpIDMap.iteritems():
            otherMenu.AppendMenu(win32con.MF_ENABLED |
                                 win32con.MF_STRING, id, desc)
    else:

...
#		self.oldCallbackCaller = win32ui.InstallCallbackCaller(self.CallbackManager)
    ...
    def ExitInstance(self):
        ...
        # clean Python references to the now destroyed window object.
        self.frame = None
        self.idleHandlers = []
    ...
    def OnIdle(self, count):
        try:
            ret = 0
            # copy list, as may be modified during loop
            handlers = self.idleHandlers[:]
    ...
    def LoadMainFrame(self):
        ...
        self.frame.LoadFrame(win32ui.IDR_MAINFRAME,
    ...
    def OnHelp(self, id, code):
        try:
            if id == win32ui.ID_HELP_GUI_REF:
                ...
                helpFile = regutil.GetRegisteredHelpFile(
    ...
    def SetApplicationPaths(self):
        ...
        apppath = win32ui.GetProfileVal(
            'Python', 'Application Path', '').split(';')
        for path in apppath:
            if len(path) > 0:
                new_path.append(win32ui.FullPath(path))
        for extra_num in range(1, 11):
            apppath = win32ui.GetProfileVal(
                'Python', 'Application Path %d' % extra_num, '').split(';')
            if len(apppath) == 0:
                break
            for path in apppath:
                if len(path) > 0:
...
class AboutBox(dialog.Dialog):
    ...
    def OnInitDialog(self):
        ...
        try:
            build_no = open(os.path.join(
                site_packages, "pywin32.version.txt")).read().strip()

...
def SetToolsMenu(menu, menuPos=None):
    ...
    for menuString, cmd in items:
        ...
        toolsMenu.AppendMenu(win32con.MF_ENABLED |
                             win32con.MF_STRING, idPos, menuString)
    ...
    menu.InsertMenu(menuPos, win32con.MF_BYPOSITION | win32con.MF_ENABLED |
                    win32con.MF_STRING | win32con.MF_POPUP, toolsMenu.GetHandle(), '&Tools')
...
class ToolMenuPropPage(dialog.PropertyPage):
    ...
    def OnInitDialog(self):
        ...
        self.HookNotify(self.OnNotifyListControlEndLabelEdit,
                        commctrl.LVN_ENDLABELEDIT)

        # Hook the button clicks.
        self.HookCommand(self.OnButtonNew, win32ui.IDC_BUTTON3)  # New Item
        self.HookCommand(self.OnButtonDelete,
                         win32ui.IDC_BUTTON4)  # Delete item
    ...
    def OnButtonMove(self, id, cmd):
        if cmd == win32con.BN_CLICKED:
            try:
                itemNo = self.listControl.GetNextItem(-1,

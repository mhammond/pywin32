...
class MainFrame(app.MainFrame):
    ...
    def OnClose(self):
        ...
        # Otherwise MFC's OnClose may _not_ prompt for save.
...
class InteractivePythonApp(app.CApp):
    ...
    def HookCommands(self):
        ...
        self.HookCommandUpdate(self.OnUpdateViewToolbarDbg,
    ...
    def InitDDE(self):
        ...
        if intpydde is not None:
            ...
            self.ddeServer.Create(
                "Pythonwin", intpydde.CBF_FAIL_SELFCONNECTIONS)
            try:
    ...
    def ProcessArgs(self, args, dde=None):
        ...
        # activate it!
        if dde is not None:
            dde.Exec("self.Activate()")
        if len(args) and args[0] in ['/nodde', '/new']:
        ...
        try:
            if args[0] and args[0][0] != '/':
                ...
                argType = win32ui.GetProfileVal(
                    "Python", "Default Arg Type", "/edit").lower()
            else:
                argStart = 1
                argType = args[0]
            ...
            if argType == "/edit":
                ...
                if dde:
                    ...
                    import scriptutils
                    scriptutils.RunScript(
                        args[argStart], ' '.join(args[argStart+1:]), 0)
            elif argType == "/app":
                raise RuntimeError(
    ...
    def LoadUserModules(self, moduleNames=None):
        ...
        if moduleNames is None:
            ...
            moduleNames = win32ui.GetProfileVal(
                'Python', 'Startup Modules', default)
    ...
    def OnViewBrowse(self, id, code):
        ...
        obName = dialog.GetSimpleInput(
            'Object', '__builtins__', 'Browse Python Object')
        if obName is None:
            return
        try:
    ...
    def OnFileLocate(self, id, code):
        ...
        if newName is None:
            win32ui.MessageBox(
                "The file '%s' can not be located" % lastLocateFileName)
        else:

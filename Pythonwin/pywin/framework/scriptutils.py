...
# Just run under the debugger, stopping only at break-points.
RS_DEBUGGER_GO = 2
# Dont run under debugger, but do post-mortem analysis on exception.
RS_DEBUGGER_PM = 3
...
def RunScript(defName=None, defArgs=None, bShowDialog=1, debuggingType=None):
    ...
    if not pathName or bShowDialog:
        ...
        if debuggingType == RS_DEBUGGER_GO and debugger is not None:
            ...
            if rd is not None and len(rd.breaks) == 0:
                ...
                rc = win32ui.MessageBox(msg, win32ui.LoadString(
                    win32ui.IDR_DEBUGGER), win32con.MB_YESNOCANCEL | win32con.MB_ICONINFORMATION)
    ...
    if bWorked:
        win32ui.SetStatusText(
            "Script '%s' returned exit code %s" % (script, exitCode))
    else:
        win32ui.SetStatusText(
            'Exception raised while running script  %s' % base)
    try:
        sys.stdout.flush()
    except AttributeError:
        pass
...
#	win32ui.GetMainFrame().BeginWaitCursor()
    ...
    try:
        ...
        win32ui.SetStatusText('Successfully ' + what + "ed module '" +
                              modName+"': %s" % getattr(mod, '__file__', "<unkown file>"))
    except:
...
def CheckFile():
    ...
    try:
        ...
        if RunTabNanny(pathName):
            win32ui.SetStatusText(
                "Python and the TabNanny successfully checked the file '"+os.path.basename(pathName)+"'")
    except SyntaxError:
...
def RunTabNanny(filename):
    ...
    if tabnanny is None:
        win32ui.MessageBox(
            "The TabNanny is not around, so the children can run amok!")
        return
    ...
    if data:
        try:
            ...
            try:  # Try and display whitespace
                ...
                pass
            win32ui.SetStatusText(
                "The TabNanny found trouble at line %d" % lineno)
...
def FindTabNanny():
    ...
    try:
        path = win32api.RegQueryValue(
            win32con.HKEY_LOCAL_MACHINE, "SOFTWARE\\Python\\PythonCore\\%s\\InstallPath" % (sys.winver))

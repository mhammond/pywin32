...
class TheDocument(docview.RichEditDoc):
    ...
    def setInitParams(self, paramstr):
        if paramstr is None:
            paramstr = win32ui.GetProfileVal(
                "Pychecker", "Params", '\t\t\t1\t0\t0')
        ...
        if not self.dirpattern:
            try:
                editor = win32ui.GetMainFrame().MDIGetActive()[
                    0].GetEditorView()
    ...
    def OnNewDocument(self):
        ...
        d = TheDialog(self.dirpattern, self.filpattern, self.greppattern,
                      self.casesensitive, self.recurse, self.verbose)
    ...
    def doSearch(self):
        ...
        #self.text = []
        self.GetFirstView().Append('#Pychecker Run in ' +
                                   self.dirpattern+'  %s\n' % time.asctime())
        if self.verbose:
        ...
        if not self.dp:
            ...
            import operator
            self.flist = reduce(operator.add, list(
                map(glob.glob, self.fplist)))
        ...
        try:
            ...
            if not os.path.isfile(py):
                if "64 bit" in sys.version:
                    py = os.path.join(sys.prefix, 'PCBuild',
                                      'amd64', 'python.exe')
                else:
            ...
            pychecker = os.path.join(
                get_python_lib(), 'pychecker', 'checker.py')
            if not os.path.isfile(py):
...
class TheView(docview.RichEditView):
    ...
    def OnRClick(self, params):
        ...
        if regexGrepResult:
            ...
            menu.AppendMenu(flags, ID_ADDPYCHECKNO2,
...
class TheDialog(dialog.Dialog):
    def __init__(self, dp, fp, gp, cs, r, v):
        ...
        tmp = [["Pychecker Run", (0, 0, 210, 90), style,
                None, (8, "MS Sans Serif")], ]
        tmp.append([STATIC, "Files:",            -1, (7,   7,  50,  9), CS])
        tmp.append([EDIT,   gp,                    103, (52,  7, 144,  11), CS |
                    win32con.WS_TABSTOP | win32con.ES_AUTOHSCROLL | win32con.WS_BORDER])
        tmp.append([STATIC, "Directories:",         -1, (7,  20,  50,  9), CS])
        tmp.append([EDIT,   dp,                    102, (52, 20, 128,  11), CS |
                    win32con.WS_TABSTOP | win32con.ES_AUTOHSCROLL | win32con.WS_BORDER])
        tmp.append([BUTTON, '...',                 110, (182, 20,  16,
                                                         11), CS | win32con.BS_PUSHBUTTON | win32con.WS_TABSTOP])
        tmp.append([STATIC, "Options:",            -1, (7,  33,  50,  9), CS])
        tmp.append([EDIT,   fp,                    101, (52, 33, 128,  11), CS |
                    win32con.WS_TABSTOP | win32con.ES_AUTOHSCROLL | win32con.WS_BORDER])
        tmp.append([BUTTON, '...',                 111, (182, 33,  16,
                                                         11), CS | win32con.BS_PUSHBUTTON | win32con.WS_TABSTOP])
        ...
        tmp.append([BUTTON, 'OK',         win32con.IDOK, (166, 53,  32, 12),
                    CS | win32con.BS_DEFPUSHBUTTON | win32con.WS_TABSTOP])
        tmp.append([BUTTON, 'Cancel', win32con.IDCANCEL, (166, 67,
                                                          32, 12), CS | win32con.BS_PUSHBUTTON | win32con.WS_TABSTOP])
...
class TheParamsDialog(dialog.Dialog):
    def __init__(self, items):
        ...
        style = win32con.DS_MODALFRAME | win32con.WS_POPUP | win32con.WS_VISIBLE | win32con.WS_CAPTION | win32con.WS_SYSMENU | win32con.DS_SETFONT
        CS = win32con.WS_CHILD | win32con.WS_VISIBLE
        tmp = [["Pychecker Parameters",
                (0, 0, 205, 100), style, None, (8, "MS Sans Serif")], ]
        ...
        tmp.append([BUTTON, 'OK',         win32con.IDOK, (167, 7,  32, 12),
                    CS | win32con.BS_DEFPUSHBUTTON | win32con.WS_TABSTOP])
        tmp.append([BUTTON, 'Cancel', win32con.IDCANCEL, (167, 23,
                                                          32, 12), CS | win32con.BS_PUSHBUTTON | win32con.WS_TABSTOP])
        tmp.append([STATIC, 'New:',                  -
                    1, (2,  83,  15,  12), CS])
        tmp.append([EDIT,  '',                     108, (18, 83,  139,  12), CS |
                    win32con.WS_TABSTOP | win32con.ES_AUTOHSCROLL | win32con.WS_BORDER])
        tmp.append([BUTTON, 'Add',                  109, (167, 83,
                                                          32, 12), CS | win32con.BS_PUSHBUTTON | win32con.WS_TABSTOP])
        dialog.Dialog.__init__(self, tmp)

...
INTERACTIVE_STYLES = [STYLE_INTERACTIVE_EOL, STYLE_INTERACTIVE_OUTPUT, STYLE_INTERACTIVE_PROMPT,
                      STYLE_INTERACTIVE_BANNER, STYLE_INTERACTIVE_ERROR, STYLE_INTERACTIVE_ERROR_FINALLINE]
...
class InteractiveFormatter(FormatterParent):
    ...
    def SetStyles(self):
        ...
        self.RegisterStyle(
            Style(STYLE_INTERACTIVE_EOL, STYLE_INTERACTIVE_PROMPT))
            ...
            if look and look < start-1:  # Did we find a char before the \n\r sets?
                ...
                if strstyle is not None:
                    ...
                    if quote_char is not None:
                        ...
                        if look > 2:
                            look_str = self.scintilla.SCIGetCharAt(
                                look-2) + self.scintilla.SCIGetCharAt(look-1) + self.scintilla.SCIGetCharAt(look)
...
class PythonwinInteractiveInterpreter(code.InteractiveInterpreter):
    ...
    def showsyntaxerror(self, filename=None):
        ...
        sys.stderr.write(tracebackHeader.decode('ascii'))
...
#		self.SetWordWrap(win32ui.CRichEditView_WrapNone)
    ...
    def AppendToPrompt(self, bufLines, oldPrompt=None):
        ...
        if oldPrompt and line == oldPrompt:
            self.SetSel(self.GetTextLength() -
                        len(oldPrompt), self.GetTextLength())
    ...
    def _GetSubConfigNames(self):
        # Allow [Keys:Interactive] sections to be specific
        return ["interactive"]
    ...
    def ProcessEnterEvent(self, event):
        ...
        bNeedIndent = win32api.GetKeyState(
            win32con.VK_SHIFT) < 0 or win32api.GetKeyState(win32con.VK_CONTROL) < 0
        if bNeedIndent:
            self.ReplaceSel("\n")
        else:
            self.SetSel(-2)
            ...
            try:
                # Need more input!
                if self.interp.runsource(source, "<interactive input>"):
                    bNeedIndent = 1
                else:
...
#					win32ui.SetStatusText('Successfully executed statement')
    ...
    def OnEditCopyCode(self, command, code):
        ...
        try:
            win32clipboard.SetClipboardData(
                win32clipboard.CF_UNICODETEXT, unicode(out_code))
        finally:
    ...
    def GetRightMenuItems(self):
        ...
        if win32clipboard.IsClipboardFormatAvailable(win32clipboard.CF_UNICODETEXT):
            ret.append((flags, ID_EDIT_EXEC_CLIPBOARD,
...
class DockedInteractiveView(DockedInteractiveViewParent):
    ...
    def OnDestroy(self, msg):
        ...
        pywin.framework.app.SaveWindowSize(
            "Interactive Window", newSize, "docked")
        try:
...
class CDockedInteractivePython(CInteractivePython):
    ...
    def Create(self):
        ...
        defRect = pywin.framework.app.LoadWindowSize(
            "Interactive Window", "docked")
...
def CreateDockedInteractiveWindow():
    ...
    bar.CreateWindow(win32ui.GetMainFrame(), creator,
                     "Interactive Window", ID_DOCKED_INTERACTIVE_CONTROLBAR)

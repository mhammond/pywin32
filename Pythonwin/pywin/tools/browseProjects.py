...
class HLIRoot(hierlist.HierListItem):
    ...
    def GetSubList(self):
        ...
        try:
            ...
            while 1:
                try:
                    ret.append(HLIProjectRoot(
                        win32api.RegEnumKey(hKey, index)))
                    index = index + 1
                except win32api.error:
                    break
            return ret
...
class dynamic_browser (dialog.Dialog):
    ...
    dt = [
        ["Python Projects", (0, 0, 200, 200), style,
...
def DockablePathBrowser():
    ...
    bar.CreateWindow(win32ui.GetMainFrame(),
                     DockableBrowserCreator, "Path Browser", 0x8e0a)

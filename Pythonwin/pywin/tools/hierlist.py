...
class HierList(object.Object):
    ...
    def HierInit(self, parent, listControl=None):
        ...
        self.imageList = win32ui.CreateImageList(
            self.bitmapID, 16, 0, bitmapMask)
        if listControl is None:
            ...
            assert self.listBoxId is None or self.listBoxId == lbid, "An invalid listbox control ID has been specified (specified as %s, but exists as %s)" % (
                self.listBoxId, lbid)
            self.listBoxId = lbid
...
#		self.list.AttachObject(self)
        ...
        if sys.version_info[0] < 3:
            parent.HookNotify(self.OnTreeItemExpanding,
                              commctrl.TVN_ITEMEXPANDINGA)
            parent.HookNotify(self.OnTreeItemSelChanged,
                              commctrl.TVN_SELCHANGEDA)
        else:
            parent.HookNotify(self.OnTreeItemExpanding,
                              commctrl.TVN_ITEMEXPANDINGW)
            parent.HookNotify(self.OnTreeItemSelChanged,
                              commctrl.TVN_SELCHANGEDW)
    ...
    def AddItem(self, parentHandle, item, hInsertAfter=commctrl.TVI_LAST):
        ...
        hitem = self.listControl.InsertItem(
            parentHandle, hInsertAfter, (None, None, None, text, bitmapCol, bitmapSel, cItems, 0))
    ...
    def _GetChildHandles(self, handle):
        ...
        try:
            ...
            while 1:
                ret.append(handle)
                handle = self.listControl.GetNextItem(
                    handle, commctrl.TVGN_NEXT)

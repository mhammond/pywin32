...
class HLICode(HLIPythonObject):
    ...
    def GetSubList(self):
        ...
        ret.append(MakeHLI(self.myobject.co_argcount,
                           "Number of args (co_argcount)"))
        ret.append(MakeHLI(self.myobject.co_varnames,
                           "Param names (co_varnames)"))
...
class dynamic_browser (dialog.Dialog):
    ...
    dt = [
        ["Python Object Browser", (0, 0, 200, 200),
...
class BrowserView(docview.TreeView):
    def OnInitialUpdate(self):
        ...
        list = hierlist.HierListWithItems(
            self.GetDocument().root, win32ui.IDB_BROWSER_HIER, win32ui.AFX_IDW_PANE_FIRST)
        list.HierInit(self.GetParent())
        list.SetStyle(commctrl.TVS_HASLINES |
                      commctrl.TVS_LINESATROOT | commctrl.TVS_HASBUTTONS)
        return rc

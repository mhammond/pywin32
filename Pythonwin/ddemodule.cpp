...
{
    if (!PyArg_ParseTuple(args, ":CreateServer"))
        return NULL;
    GUI_BGN_SAVE;
...
PyObject *PyDDE_CreateTopic(PyObject *s, PyObject *args)
{
    TCHAR *name;
    PyObject *obname;
    if (!PyArg_ParseTuple(args, "O:CreateTopic", &obname))
        return NULL;
...
{
    if (!PyArg_ParseTuple(args, ":CreateServerSystemTopic"))
        return NULL;
    GUI_BGN_SAVE;
...
{
    PyObject *obServer;
    if (!PyArg_ParseTuple(args, "O:CreateConversation", &obServer))
        return NULL;
...
PyObject *PyDDE_CreateStringItem(PyObject *s, PyObject *args)
{
    TCHAR *name;
    PyObject *obname;
    if (!PyArg_ParseTuple(args, "O:CreateStringItem", &obname))
        return NULL;
    if (!PyWinObject_AsTCHAR(obname, &name, FALSE))
        return NULL;
    GUI_BGN_SAVE;
...
static struct PyMethodDef dde_functions[] = {{"CreateConversation", PyDDE_CreateConversation, 1},
...
#define ADD_CONSTANT(tok)                           \
    if (PyModule_AddIntConstant(module, #tok, tok)) \
...
PYWIN_MODULE_INIT_FUNC(dde)
{
    ...
    PYWIN_MODULE_INIT_PREPARE(dde, dde_functions, "A module for Dynamic Data Exchange support");

    dde_module_error = PyErr_NewException("dde.error", NULL, NULL);
    PyDict_SetItemString(dict, "error", dde_module_error);

    ADD_CONSTANT(APPCLASS_MONITOR);  // Makes it possible for the application to monitor DDE activity in the system.
    ...
    ADD_CONSTANT(APPCMD_CLIENTONLY);  // Prevents the application from becoming a server in a DDE conversation. The
    ...
    ADD_CONSTANT(
        APPCMD_FILTERINITS);  // Prevents the DDEML from sending XTYP_CONNECT and XTYP_WILDCONNECT transactions to the
                              // application until the application has created its string handles and registered its
                              // service names or has turned off filtering by a subsequent call to the DdeNameService or
                              // DdeInitialize function. This flag is always in effect when an application calls
                              // DdeInitialize for the first time, regardless of whether the application specifies the
                              // flag. On subsequent calls to DdeInitialize, not specifying this flag turns off the
                              // application's service-name filters, but specifying it turns on the application's
                              // service name filters.
    ADD_CONSTANT(
        CBF_FAIL_ALLSVRXACTIONS);    // Prevents the callback function from receiving server transactions. The system
    ...
    ADD_CONSTANT(CBF_FAIL_ADVISES);  // Prevents the callback function from receiving XTYP_ADVSTART and XTYP_ADVSTOP
                                     // transactions. The system returns DDE_FNOTPROCESSED to each client that sends an
                                     // XTYP_ADVSTART or XTYP_ADVSTOP transaction to the server.
    ADD_CONSTANT(CBF_FAIL_CONNECTIONS);  // Prevents the callback function from receiving XTYP_CONNECT and
                                         // XTYP_WILDCONNECT transactions.
    ADD_CONSTANT(CBF_FAIL_EXECUTES);     // Prevents the callback function from receiving XTYP_EXECUTE transactions. The
                                         // system returns DDE_FNOTPROCESSED to a client that sends an XTYP_EXECUTE
                                         // transaction to the server.
    ADD_CONSTANT(
        CBF_FAIL_POKES);  // Prevents the callback function from receiving XTYP_POKE transactions. The system returns
                          // DDE_FNOTPROCESSED to a client that sends an XTYP_POKE transaction to the server.
    ADD_CONSTANT(CBF_FAIL_REQUESTS);  // Prevents the callback function from receiving XTYP_REQUEST transactions. The
                                      // system returns DDE_FNOTPROCESSED to a client that sends an XTYP_REQUEST
                                      // transaction to the server.
    ADD_CONSTANT(
        CBF_FAIL_SELFCONNECTIONS);  // Prevents the callback function from receiving XTYP_CONNECT transactions from the
                                    // application's own instance. This flag prevents an application from establishing a
                                    // DDE conversation with its own instance. An application should use this flag if it
                                    // needs to communicate with other instances of itself but not with itself.
    ADD_CONSTANT(CBF_SKIP_ALLNOTIFICATIONS);  // Prevents the callback function from receiving any notifications. This
                                              // flag is equivalent to combining all CBF_SKIP_ flags.
    ADD_CONSTANT(CBF_SKIP_CONNECT_CONFIRMS);  // Prevents the callback function from receiving XTYP_CONNECT_CONFIRM
                                              // notifications.
    ...
    //	ADD_CONSTANT(MF_SKIP_UNREGISTRATIONS); // Prevents the callback function from receiving XTYP_UNREGISTER
    // notifications.
    ADD_CONSTANT(MF_CALLBACKS);  // Notifies the callback function whenever a transaction is sent to any DDE callback
    ...
    ADD_CONSTANT(MF_HSZ_INFO);   // Notifies the callback function whenever a DDE application creates, frees, or
                                // increments the usage count of a string handle or whenever a string handle is freed as
                                // a result of a call to the DdeUninitialize function.

...
class ArgFormatter:
    ...
    def GetInterfaceCppObjectInfo(self):
        ...
        return self.GetIndirectedArgName(self.builtinIndirection,
    ...
    def GetAutoduckString(self):
        return '// @pyparm %s|%s||Description for %s' % (self._GetPythonTypeDesc(),
                                                         self.arg.name,
                                                         self.arg.name)
...
class ArgFormatterTime(ArgFormatterPythonCOM):
    def __init__(self, arg, builtinIndirection, declaredIndirection=0):
        ...
        ArgFormatterPythonCOM.__init__(self,
                                       arg,
                                       builtinIndirection,
                                       declaredIndirection)
...
def make_arg_converter(arg):
    try:
        ...
        raise error_not_supported("The type '%s' (%s) is unknown." % (arg.type,
                                                                      arg.name
                                                                      )
                                  )
...
class Argument:
    ...
    def BuildFromFile(self, file):
        ...
        description.  If not a valid argument line, an error_not_found
        exception is raised.
        """
        line = file.readline()
        ...
        if VERBOSE:
            print "       Arg %s of type %s%s (%s)" % (self.name,
                                                       self.type,

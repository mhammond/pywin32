...
def SetupEnvironment():
    ...
    try:
        ...
        key = win32api.RegOpenKey(HKEY_LOCAL_MACHINE,
                                  keyName,
                                  0,
                                  KEY_QUERY_VALUE,
                                  )
    ...
    try:
        ...
        # If extension modules add support, we can load their .pyd's
        # from a completely different directory (see the comments below)
        try:
            if key is not None:

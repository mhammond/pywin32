...
class PythonSemanticClass:
    ...
    # DISPIDs specified by the object.
...
#       assert ob.GetMemberName(10, 0)=="add", "Policy did not give me the correct function for the dispid"
    ...
    assert ob.GetDispID(
        "_NewEnum", 0) == pythoncom.DISPID_NEWENUM, "_NewEnum() got unexpected DISPID"
    dispids = []
    dispid = -1
    while 1:
        try:
...
def SemanticTest(ob):
    ...
    rc = dispob.Invoke(pythoncom.DISPID_EVALUATE, 0,
                       pythoncom.DISPATCH_METHOD | pythoncom.DISPATCH_PROPERTYGET, 1)
    if rc != 6:

...
def test():
    ...
    try:
        ...
        if not exc or exc[-1] != winerror.E_UNEXPECTED:
            raise error(
                "The scode element of the exception tuple did not yield the correct scode", com_exc)
        if exc[2] != "Not today":
            raise error(
                "The description in the exception tuple did not yield the correct string", com_exc)
    cap = CaptureWriter()
    try:
        cap.capture()
        try:
        ...
        if com_exc.hresult != winerror.E_FAIL:
            raise error(
                "The hresult was not E_FAIL for an internal error", com_exc)
    ...
    try:
        ...
        if com_exc.hresult != winerror.DISP_E_EXCEPTION:
            raise error(
                "Calling the object via IDispatch did not yield the correct scode", com_exc)
        exc = com_exc.excepinfo
        if not exc or exc[-1] != winerror.E_UNEXPECTED:
            raise error(
                "The scode element of the exception tuple did not yield the correct scode", com_exc)

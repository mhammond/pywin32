...
class BasicWrapPolicy:
    ...
    def _wrap_(self, object):
        ...
        This function keeps a reference to the passed object, and may
        interogate it to determine how to respond to COM requests, etc.
        """
    ...
    def _QueryInterface_(self, iid):
        ...
        This checks the _com_interfaces_ attribute and if the interface is
        not specified there, it calls the derived helper _query_interface_.
        """
    ...
    def _query_interface_(self, iid):
        """Called if the object does not provide the requested interface
        in _com_interfaces_, and does not provide a _query_interface_ handler.

        Returns a result to the COM framework indicating the interface is
        not supported.
        """
        return 0
    ...
    def _GetIDsOfNames_(self, names, lcid):
        ...
        """
        if len(names) > 1:
            raise COMException(scode=winerror.DISP_E_INVALID,
    ...
    # policies to explicitely implement all this functionality just to
    # not implement it!
...
class DesignatedWrapPolicy(MappedWrapPolicy):
    ...
    def _wrap_(self, ob):
        ...
        if tlb_guid is not None:
            ...
            universal_data = universal.RegisterInterfaces(tlb_guid,
                                                          tlb_lcid,
                                                          tlb_major,
                                                          tlb_minor,
                                                          interfaces)
        ...
        for dispid, invkind, name in universal_data:
            ...
            if invkind == DISPATCH_METHOD:
                ...
                raise ValueError("unexpected invkind: %d (%s)" % (invkind,
                                                                  name))
    ...
    def _invokeex_(self, dispid, lcid, wFlags, args, kwArgs, serviceProvider):
            ...
            try:
                ...
                try:
                    ...
                    if str(v).find("arguments") >= 0:
                        print "** TypeError %s calling function %r(%r)" % (v,
                                                                           func,
                                                                           args)
                    raise
        ...
        if wFlags & DISPATCH_PROPERTYGET:
            ...
            # a method as a property - call it.
...
class EventHandlerPolicy(DesignatedWrapPolicy):
    ...
    def _transform_args_(self,
        ...
        for arg in args:
            ...
            if arg_type == IDispatchType:
                ...
                try:
                    import win32com.client
                    query = arg.QueryInterface(pythoncom.IID_IDispatch)
                    arg = win32com.client.Dispatch(query)
    ...
    def _invokeex_(self, dispid, lcid, wFlags, args, kwArgs, serviceProvider):
        # transform the args.
        args, kwArgs = self._transform_args_(args,
                                             kwArgs,
                                             dispid,
                                             lcid,
                                             wFlags,
...
class DynamicPolicy(BasicWrapPolicy):
    ...
    def _invoke_(self, dispid, lcid, wFlags, args):
        return S_OK, -1, self._invokeex_(dispid,
                                         lcid,
                                         wFlags,
                                         args,
                                         None,
...
# Quite likely a frozen executable that doesnt need dispatchers
except ImportError:
    pass

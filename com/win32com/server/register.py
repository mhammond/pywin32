...
def _find_localserver_exe(mustfind):
    ...
    exeName = os.path.join(os.path.split(sys.executable)[0],
                           exeBaseName)
    ...
    if clsctx & pythoncom.CLSCTX_INPROC_SERVER:
        ...
        if pythoncom.frozen:
            ...
            if pythoncom.__file__.find("_d") < 0:
                ...
                suffix = "_d"
            loadername = "pythoncomloader%d%d%s.dll" % (sys.version_info[0],
                                                        sys.version_info[1],
                                                        suffix)
...
def GetUnregisterServerKeys(clsid, progID=None,
                            verProgID=None,
                            customKeys=None):
    """Given a server, return a list of of ("key", root),
    which are keys recursively and uncondtionally deleted at unregister
    or uninstall time.
    """
...
def RegisterClasses(*classes, **flags):
    ...
    for cls in classes:
        ...
        addPyComCat = not _get(cls,
                               '_reg_disable_pycomcat_',
                               pythoncom.frozen != 0)
        addnPath = None
        if debugging:
            # If the class has a debugging dispatcher specified, use it,
            # otherwise use our default dispatcher.
...
def UnregisterInfoClasses(*classes, **flags):
    ...
    for cls in classes:
        ...
        verProgID = _get(cls, '_reg_verprogid_')
        customKeys = _get(cls, '_reg_remove_keys_')
        ret = ret + GetUnregisterServerKeys(clsid,
                                            progID,
                                            verProgID,
                                            customKeys
                                            )
    ...
    try:
        ...
        try:
            ...
            print >> batf, "set PYTHONPATH=%s" % os.environ.get('PYTHONPATH',
                                                                '')
            # May be on a different drive
            # Select that before attempting to CD.
            print >> batf, os.path.splitdrive(cwd)[0]
            print >> batf, 'cd "%s"' % os.getcwd()
            print >> batf, '%s %s > "%s" 2>&1' % (win32api.GetShortPathName(exe_to_run),
                                                  new_params,
                                                  outfile)
        finally:
            batf.close()

...
class DispatcherBase:
    ...
    This base class supports the printing of "unexpected" exceptions.
    Note, however, that exactly where the output of print goes may
    not be useful! A derived class may provide additional semantics for this.
    """
    ...
    def _InvokeEx_(self, dispid, lcid, wFlags, args, kwargs, serviceProvider):
        try:
            return self.policy._InvokeEx_(dispid,
                                          lcid,
                                          wFlags,
                                          args,
                                          kwargs,
...
class DispatcherTrace(DispatcherBase):
    ...
    def _InvokeEx_(self, dispid, lcid, wFlags, args, kwargs, serviceProvider):
        self._trace_("in %r._InvokeEx_-%s%r [%x,%s,%r]" % (self.policy._obj_,
                                                           dispid,
                                                           args,
                                                           wFlags,
                                                           lcid,
        ...
        return DispatcherBase._InvokeEx_(self,
                                         dispid,
                                         lcid,
                                         wFlags,
                                         args,

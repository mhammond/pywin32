...
class DispatcherTrace(DispatcherBase):
    ...
    def _QueryInterface_(self, iid):
        ...
        if not rc:
            self._trace_(
                "in {}._QueryInterface_ with unsupported IID {} ({})".format(repr(self.policy._obj_), IIDToInterfaceName(iid), iid)
            )
        return rc
    ...
    def _InvokeEx_(self, dispid, lcid, wFlags, args, kwargs, serviceProvider):
        self._trace_(
            "in {!r}._InvokeEx_-{}{!r} [{:x},{},{!r}]".format(self.policy._obj_, dispid, args, wFlags, lcid, serviceProvider)
        )

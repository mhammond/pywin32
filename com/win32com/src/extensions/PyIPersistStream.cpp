...
/////////////////////////////////////////////////////////////
PyIPersistStream::PyIPersistStream(IUnknown *pDisp) : PyIPersist(pDisp) { ob_type = &type; }

PyIPersistStream::~PyIPersistStream() {}

/*static*/ IPersistStream *PyIPersistStream::GetI(PyObject *self) { return (IPersistStream *)PyIPersist::GetI(self); }
...
PyObject *PyIPersistStream::Load(PyObject *self, PyObject *args)
{
    ...
    Py_INCREF(Py_None);
    return Py_None;
    // @comm This method loads an object from its associated stream. The seek pointer is set as it was in the most
    // recent <om PyIPersistStream.Save> method. This method can seek and read from the stream, but cannot write to it.
    // @comm On exit, the seek pointer must be in the same position it was in on entry, immediately past the end of the
    // data.
}
...
// @pymethod ULARGE_INTEGER|PyIPersistStream|GetSizeMax|Returns the size in bytes of the stream needed to save the
// object.
...
static struct PyMethodDef PyIPersistStream_methods[] = {
    {"IsDirty", PyIPersistStream::IsDirty,
     1},  // @pymeth IsDirty|Checks the object for changes since it was last saved.
    {"Load", PyIPersistStream::Load,
     1},  // @pymeth Load|Initializes an object from the stream where it was previously saved.
    {"Save", PyIPersistStream::Save, 1},  // @pymeth Save|Saves an object to the specified stream.
    {"GetSizeMax", PyIPersistStream::GetSizeMax,
     1},  // @pymeth GetSizeMax|Returns the size in bytes of the stream needed to save the object.
    {NULL, NULL}};

PyComTypeObject PyIPersistStream::type("PyIPersistStream",
                                       &PyIPersist::type,  // @base PyIPersistStream|PyIPersist
                                       sizeof(PyIPersistStream), PyIPersistStream_methods,

...
def WriteSinkEventMap(obj, stream):
    ...
    for name, entry in obj.propMapGet.items() + obj.propMapPut.items() + obj.mapFuncs.items():
        fdesc = entry.desc
        print >> stream, '\t\t%9d : "%s",' % (
            entry.desc[0], MakeEventMethodName(entry.names[0]))
    print >> stream, '\t\t}'
        ...
        if self.aliasDoc:
            ...
            if type(ai) == type(0):
                try:
                    ...
                    print >> stream, self.doc[0] + \
                        " = None # Can't convert alias info " + str(ai)
        print >> stream
        self.bWritten = 1
...
class VTableItem(build.VTableItem, WritableItem):
    ...
    def WriteVTableMap(self, generator):
        ...
        print >> stream, "%s_vtables_dispatch_ = %d" % (
            self.python_name, self.bIsDispatch)
...
class DispatchItem(build.DispatchItem, WritableItem):
    ...
    def WriteCallbackClassBody(self, generator):
        ...
        for name, entry in self.propMapGet.items() + self.propMapPut.items() + self.mapFuncs.items():
            ...
            print >> stream, '#\tdef ' + methName + '(self' + build.BuildCallList(
                fdesc, entry.names, "defaultNamedOptArg", "defaultNamedNotOptArg", "defaultUnnamedArg", "pythoncom.Missing", is_comment=True) + '):'
    ...
    def WriteClassBody(self, generator):
        ...
        specialItems = {"count": None, "item": None,
                        "value": None, "_newenum": None}
        itemCount = None
        for name in names:
        ...
        for key in names:
            ...
            if generator.bBuildHidden or not entry.hidden:
                ...
                mapEntry = MakeMapLineEntry(
                    details[0], pythoncom.DISPATCH_PROPERTYGET, resultDesc, argDesc, key, entry.GetResultCLSIDStr())
        ...
        for key in names:
            ...
            if generator.bBuildHidden or not entry.hidden:
                ...
                print >> stream, '\t\t"%s" : ((%s, LCID, %d, 0),(%s)),' % (
                    build.MakePublicAttributeName(key), details[0], pythoncom.DISPATCH_PROPERTYPUT, defArgDesc)
        ...
        for key in names:
            ...
            if generator.bBuildHidden or not entry.hidden:
                ...
                print >> stream, '\t\t"%s": ((%s, LCID, %d, 0),%s),' % (
                    build.MakePublicAttributeName(key), details[0], details[4], defArgDesc)
        print >> stream, "\t}"
...
class CoClassItem(build.OleItem, WritableItem):
    ...
    def __init__(self, typeinfo, attr, doc=None, sources=[], interfaces=[], bForUser=1):
        ...
        # Pretend it is so it is written to the class map.
        self.bIsDispatch = 1
        ...
        if generator.generate_type == GEN_DEMAND_CHILD:
            ...
            for ref in referenced_items:
                ...
                print >> stream, "%s = sys.modules['%s.%s'].%s" % (
                    ref.python_name, generator.base_mod_name, ref.python_name, ref.python_name)
...
class Generator:
    ...
    def _Build_CoClass(self, type_info_tuple):
        ...
        for j in range(attr[8]):
            ...
            child_infos.append(
                (info, refAttr.typekind, refType, refType.GetDocumentation(-1), refAttr, flags))
            ...
            if refAttr.typekind == pythoncom.TKIND_DISPATCH or \
               (refAttr.typekind == pythoncom.TKIND_INTERFACE and refAttr[11] & pythoncom.TYPEFLAG_FDISPATCHABLE):
                ...
                if clsid not in vtableItems and refAttr[11] & pythoncom.TYPEFLAG_FDUAL:
                    refType = refType.GetRefTypeInfo(
                        refType.GetRefTypeOfImplType(-1))
                    refAttr = refType.GetTypeAttr()
        ...
        if infotype == pythoncom.TKIND_DISPATCH or \
           (infotype == pythoncom.TKIND_INTERFACE and attr[11] & pythoncom.TYPEFLAG_FDISPATCHABLE):
            ...
            if (attr.wTypeFlags & pythoncom.TYPEFLAG_FDUAL):
                ...
                infotype = None
        assert infotype in [
            None, pythoncom.TKIND_INTERFACE], "Must be a real interface at this point"
        ...
        for type_info_tuple in self.CollectOleItemInfosFromType():
            ...
            # hidden, assuming that you only ever use them via the CoClass)
                ...
                self._Build_CoClassChildren(
                    newItem, child_infos, oleItems, vtableItems)
    ...
    def do_gen_file_header(self):
        ...
        if self.sourceFilename:
            print >> self.file, "# From type library '%s'" % (
                os.path.split(self.sourceFilename)[1],)
    ...
    def do_generate(self):
        ...
        self.progress.SetDescription("Generating...", len(
            oleItems)+len(enumItems)+len(vtableItems))
        ...
        for record in recordItems.itervalues():
            if record.clsid == pythoncom.IID_NULL:
                print >> stream, "\t###%s: %s, # Record disabled because it doesn't have a non-null GUID" % (
        ...
        if self.generate_type == GEN_FULL:
            ...
            for item in oleItems.itervalues():
                if item is not None and item.bWritten:
                    print >> stream, "\t'%s' : %s," % (
                        str(item.clsid), item.python_name)
            print >> stream, '}'
            ...
            for item in oleItems.itervalues():
                if item is not None:
                    print >> stream, "\t'%s' : %s," % (
                        str(item.clsid), repr(item.python_name))
            print >> stream, '}'
            print >> stream, "VTablesToClassMap = {}"
            print >> stream, "VTablesToPackageMap = {"
            for item in vtableItems.itervalues():
    ...
    def generate_child(self, child, dir):
        ...
        major = la[3]
        minor = la[4]
        self.base_mod_name = "win32com.gen_py." + \
            str(clsid)[1:-1] + "x%sx%sx%s" % (lcid, major, minor)
        try:
    ...
    def do_gen_child_item(self, oleitem):
        ...
        oleitem.WriteClass(self)
        if oleitem.bWritten:
            print >> self.file, 'win32com.client.CLSIDToClass.RegisterCLSID( "%s", %s )' % (

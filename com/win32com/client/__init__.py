...
def DispatchEx(clsid, machine=None, userName=None, resultCLSID=None, typeinfo=None, UnicodeToString=None, clsctx=None):
    ...
    dispatch = pythoncom.CoCreateInstanceEx(
        clsid, None, clsctx, serverInfo, (pythoncom.IID_IDispatch,))[0]
...
def CastTo(ob, target, typelib=None):
    ...
    if typelib is not None:
        mod = gencache.MakeModuleForTypelib(typelib.clsid, typelib.lcid, int(
            typelib.major, 16), int(typelib.minor, 16))
...
def DispatchWithEvents(clsid, user_event_class):
    ...
    * The user_event_class as passed to this function.
    ...
    if not disp.__class__.__dict__.get("CLSID"):
        try:
            ...
            gencache.EnsureModule(
                tla[0], tla[1], tla[3], tla[4], bValidateFile=0)
    ...
    result_class = new_type("COMEventClass", (disp_class, events_class, user_event_class), {
...
def WithEvents(disp, user_event_class):
    ...
    result_class = new_type(
        "COMEventClass", (events_class, user_event_class), {})
    # This only calls the first base class __init__.
        ...
        if oobj is None:
            ...
            try:
                oobj = oobj._oleobj_.QueryInterface(
                    self.CLSID, pythoncom.IID_IDispatch)  # Must be a valid COM instance

...
tsf_flags = list((k, v)
                 for k, v in shellcon.__dict__.items() if k.startswith('TSF_'))
...
class TransferAdviseSink(DesignatedWrapPolicy):
    ...
    def UpdateTransferState(self, State):
        print('Current state: ', TRANSFER_ADVISE_STATES.get(
            State, '??? Unknown state %s ???' % State))
    ...
    def ConfirmEncryptionLoss(self, Source):
        print('ConfirmEncryptionLoss:', Source.GetDisplayName(
            shellcon.SHGDN_FORPARSING))

    def FileFailure(self, Item, ItemName, Error):
        print('FileFailure:', Item.GetDisplayName(
            shellcon.SHGDN_FORPARSING), ItemName)

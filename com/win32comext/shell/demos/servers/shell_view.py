...
class FileSystemView:
    ...
    def __init__(self, folder, hwnd):
        ...
        # intermediate window for catching command notifications.
        self.hwnd = None
   ...
   # IShellView
    def CreateViewWindow(self, prev, settings, browser, rect):
        ...
        browser_ad = win32gui.SendMessage(
            self.hwnd_parent, win32con.WM_USER+7, 0, 0)
        browser_ob = pythoncom.ObjectFromAddress(
            browser_ad, shell.IID_IShellBrowser)
...
#        win32gui.SetWindowLong(self.hwnd, win32con.GWL_WNDPROC, message_map)
    ...
    def _SetupLexer(self):
        ...
        for f, bg, stylenum in styles:
            ...
            self._SendSci(scintillacon.SCI_STYLESETFONT,
                          stylenum, baseFormat[7])
            if f[1] & 1:
            ...
            self._SendSci(scintillacon.SCI_STYLESETSIZE,
                          stylenum, int(baseFormat[2]/20))
            if bg is not None:

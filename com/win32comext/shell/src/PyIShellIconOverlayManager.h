...
class PyGShellIconOverlayManager : public PyGatewayBase, public IShellIconOverlayManager {
   protected:
    ...
    PYGATEWAY_MAKE_SUPPORT2(PyGShellIconOverlayManager, IShellIconOverlayManager, IID_IShellIconOverlayManager,
                            PyGatewayBase)

    // IShellIconOverlayManager
    STDMETHOD(GetFileOverlayInfo)(LPCWSTR path, DWORD attrib, int __RPC_FAR *index, DWORD flags);

    STDMETHOD(GetReservedOverlayInfo)(LPCWSTR path, DWORD attrib, int __RPC_FAR *index, DWORD flags, int ireservedID);

    STDMETHOD(RefreshOverlayImages)(DWORD flags);

    STDMETHOD(LoadNonloadedOverlayIdentifiers)(void);

    STDMETHOD(OverlayIndexFromImageIndex)(int iImage, int __RPC_FAR *index, BOOL fAdd);
};

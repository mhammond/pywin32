...
class SimpleSite:
    ...
    def OnPosRectChange(self, rect):
        browser_ob = self.hw.browser.QueryInterface(
            axcontrol.IID_IOleInPlaceObject)
...
class IEHost:
    ...
    def create_window(self):
        ...
        site = wrap(SimpleSite(self), axcontrol.IID_IOleClientSite,
                    useDispatcher=debugging)

        browser.SetClientSite(site)
        browser.SetHostNames("IE demo", "Hi there")
        axcontrol.OleSetContainedObject(self.browser, True)
        rect = win32gui.GetWindowRect(self.hwnd)
        browser.DoVerb(axcontrol.OLEIVERB_SHOW, None,
                       site, -1, self.hwnd, rect)

...
def _CheckEngineState(engine, name, state):
    ...
    if got != state:
        ...
        raise RuntimeError(
            "Warning - engine %s has state %s, but expected %s" % (name, got_name, state_name))
        ...
        try:
            ...
            try:
                ...
                if expected_exc is None:
                    self.fail("Unexpected failure from script code: %s" %
                              (site.exception_seen,))
                if expected_exc not in site.exception_seen[2]:
                    self.fail("Could not find %r in %r" %

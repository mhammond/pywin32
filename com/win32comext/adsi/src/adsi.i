...
static PyObject *PyADsEnumerateNext(PyObject *self, PyObject *args)
{
    ...
    int i;
    for (i = celt; i--;) VariantInit(&rgVar[i]);
    ...
    for (i = celtFetched; i--;) VariantClear(&rgVar[i]);
    delete[] rgVar;
...
%{
// @pyswig <o PyDS_SELECTION_LIST>|StringAsDS_SELECTION_LIST|Unpacks a string (generally fetched via <om
// PyIDataObject.GetData>) into a <o PyDS_SELECTION_LIST> list.
...
if (PyCom_RegisterClientType(&PyIADs::type, &IID_IADs) != 0)
    return NULL;
ADD_IID(IID_IADs);
...
if (PyCom_RegisterClientType(&PyIDirectoryObject::type, &IID_IDirectoryObject) != 0)
    return NULL;
ADD_IID(IID_IDirectoryObject);

if (PyCom_RegisterClientType(&PyIDirectorySearch::type, &IID_IDirectorySearch) != 0)
    return NULL;
ADD_IID(IID_IDirectorySearch);

if (PyCom_RegisterClientType(&PyIADsUser::type, &IID_IADsUser) != 0)
    return NULL;
ADD_IID(IID_IADsUser);

if (PyCom_RegisterClientType(&PyIADsContainer::type, &IID_IADsContainer) != 0)
    return NULL;
ADD_IID(IID_IADsContainer);

if (PyCom_RegisterClientType(&PyIDsObjectPicker::type, &IID_IDsObjectPicker) != 0)
    return NULL;
ADD_IID(IID_IDsObjectPicker);

if (PyCom_RegisterClientType(&PyIADsDeleteOps::type, &IID_IADsDeleteOps) != 0)
    return NULL;

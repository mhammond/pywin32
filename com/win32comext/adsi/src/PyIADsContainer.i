...
PyIADsContainer::PyIADsContainer(IUnknown *pDisp) : PyIDispatch(pDisp) { ob_type = &type; }

PyIADsContainer::~PyIADsContainer() {}

IADsContainer *PyIADsContainer::GetI(PyObject *self) { return (IADsContainer *)PyIDispatch::GetI(self); }

%}

// @pyswig <o PyIDispatch>|GetObject|
// @pyparm string|class||Specifies the name of the object class as known in the underlying directory and identical to
// the one retrieved through the get_Class property method. If the class name is None, the provider returns the first
// item found in the container.
// @pyparm string|relativeName||Specifies the name of the object as known in the underlying directory and identical to
// the one retrieved through the get_Name property method.
...
#define MAKE_PUT_VARIANT(name)                                                 \
    ...
    {                                                                          \
        ...
        if (!PyCom_VariantFromPyObject(_obj0, &var))                           \
            return NULL;                                                       \
        Py_BEGIN_ALLOW_THREADS _result = (HRESULT)_swig_self->put_##name(var); \
        Py_END_ALLOW_THREADS VariantClear(&var);                               \
        if (FAILED(_result))                                                   \

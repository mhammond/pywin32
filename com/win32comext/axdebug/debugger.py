...
def BuildModule(module, built_nodes, rootNode, create_node_fn, create_node_args):
    if module:
        ...
        if keep and hasattr(module, '__file__'):
            keep = string.lower(os.path.splitext(module.__file__)[
                                1]) not in [".pyd", ".dll"]
#               keep = keep and module.__name__=='__main__'
    if module and keep:
        ...
        if parent:
            ...
            BuildModule(parentModule, built_nodes, rootNode,
...
def RefreshAllModules(builtItems, rootNode, create_node, create_node_args):
    for module in sys.modules.values():
        BuildModule(module, builtItems, rootNode,
                    create_node, create_node_args)
...
class AXDebugger:
    def __init__(self, interfaceMaker=None, processName=None):
        ...
        expressionProvider = _wrap(
            expressions.ProvideExpressionContexts(), axdebug.IID_IProvideExpressionContexts)
...
#               self.app.CauseBreak()
    ...
    def RefreshAllModules(self, nodes, containerProvider):
        RefreshAllModules(nodes, self.root,
                          self.CreateApplicationNode, (containerProvider,))

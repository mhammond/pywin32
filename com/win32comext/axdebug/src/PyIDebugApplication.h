...
class PyGDebugApplication : public PyGRemoteDebugApplication, public IDebugApplication {
   protected:
    ...
    STDMETHOD(StepOutComplete)(void);

    STDMETHOD(DebugOutput)(LPCOLESTR pstr);

    STDMETHOD(StartDebugSession)(void);

    STDMETHOD(HandleBreakPoint)(BREAKREASON br, BREAKRESUMEACTION __RPC_FAR *pbra);
    ...
    STDMETHOD(RemoveStackFrameSniffer)(DWORD dwCookie);

    STDMETHOD(QueryCurrentThreadIsDebuggerThread)(void);

    STDMETHOD(SynchronousCallInDebuggerThread)
    (IDebugThreadCall __RPC_FAR *pptc,
#ifdef _WIN64
     DWORDLONG dwParam1, DWORDLONG dwParam2, DWORDLONG dwParam3);
#else
    ...
    (IActiveScriptErrorDebug __RPC_FAR *pErrorDebug, IActiveScriptSite __RPC_FAR *pScriptSite,
     BREAKRESUMEACTION __RPC_FAR *pbra, ERRORRESUMEACTION __RPC_FAR *perra, BOOL __RPC_FAR *pfCallOnScriptError);

    virtual BOOL STDMETHODCALLTYPE FCanJitDebug(void);

    virtual BOOL STDMETHODCALLTYPE FIsAutoJitDebugEnabled(void);

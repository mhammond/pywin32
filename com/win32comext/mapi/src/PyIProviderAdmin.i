...
PyIProviderAdmin::PyIProviderAdmin(IUnknown *pDisp) : PyIUnknown(pDisp) { ob_type = &type; }

PyIProviderAdmin::~PyIProviderAdmin() {}

/*static*/ IProviderAdmin *PyIProviderAdmin::GetI(PyObject *self) { return (IProviderAdmin *)PyIUnknown::GetI(self); }
...
HRESULT DeleteProvider(MAPIUID *INPUT  // @pyparm <o PyIID>|uuid||The ID of the provider
);
...
//  HRESULT CreateProvider(LPTSTR lpszProvider,ULONG cValues,LPSPropValue lpProps,ULONG ulUIParam,ULONG ulFlags,MAPIUID
//  FAR * lpUID);
...
PyObject *PyIProviderAdmin::CreateProvider(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "slOll", &lpszProvider, &cValues, &py_props, &ulUIParam, &ulFlags)) {
        return NULL;
    }

    // handle spropvalue structure
    SPropValue *pPropValue;
    ULONG len;
    if (py_props == Py_None) {
        pPropValue = NULL;
        cValues = 0;
    ...
    Py_BEGIN_ALLOW_THREADS hr =
        (HRESULT)_swig_self->CreateProvider(lpszProvider, cValues, pPropValue, ulUIParam, ulFlags, pMAPIUID);
    Py_END_ALLOW_THREADS MAPIFreeBuffer(pPropValue);
    if (FAILED(hr)) {
        return NULL;
    }
    GUID *pTemp = (GUID *)&iid;

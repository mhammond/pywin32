...
PyIMAPIFolder::PyIMAPIFolder(IUnknown *pDisp) : PyIMAPIContainer(pDisp) { ob_type = &type; }

PyIMAPIFolder::~PyIMAPIFolder() {}

/*static*/ IMAPIFolder *PyIMAPIFolder::GetI(PyObject *self) { return (IMAPIFolder *)PyIUnknown::GetI(self); }
...
PyObject *PyIMAPIFolder::CreateFolder(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS hRes =
        _swig_self->CreateFolder(ulFolderType, lpszFolderName, lpszFolderComment, lpInterface, ulFlags, &lpFolder);
    Py_END_ALLOW_THREADS

        if (FAILED(hRes)) result = OleSetOleError(hRes);
    else MAKE_OUTPUT_INTERFACE(&lpFolder, result, IID_IMAPIFolder);
...
HRESULT_KEEP_INFO CopyMessages(
    ...
    IID *INPUT_NULLOK,    // @pyparm <o PyIID>|iid||IID representing the interface to be used to access the destination
                          // folder.  Should usually be None.
    IMAPIFolder *INPUT,   // @pyparm <o PyIMAPIFolder>|folder||The destination folder
    unsigned long ulUIParam,      // @pyparm long|ulUIParam||Handle of the parent window for any dialog boxes or windows
...
// @flag MAPI_DECLINE_OK|Informs the message store provider to immediately return MAPI_E_DECLINE_COPY if it implements
// CopyMessage by calling the support object's IMAPISupport::DoCopyTo or IMAPISupport::DoCopyProps method.
// @flag MESSAGE_DIALOG |Displays a progress indicator as the operation proceeds.
// @flag MESSAGE_MOVE|The message or messages are to be moved rather than copied. If MESSAGE_MOVE is not set, the
// messages are copied.
...
// @pyswig |SetReadFlags|Sets or clears the MSGFLAG_READ flag in the PR_MESSAGE_FLAGS (PidTagMessageFlags) property of
// one or more of the folder's messages, and manages the sending of read reports.
HRESULT_KEEP_INFO SetReadFlags(
    SBinaryArray *INPUT,          // @pyparm <o PySBinaryArray>|msgs||
    ULONG ulUIParam,              // @pyparm int|uiParam||A HWND for the progress
    IMAPIProgress *INPUT_NULLOK,  // @pyparm <o PyIMAPIProgress>|progress||A progress object, or None
    ULONG
        ulFlags  // @pyparm int|flag||Bitmask of flags that controls the setting of a message's read flag - that is, the
                 // message's MSGFLAG_READ flag in its PR_MESSAGE_FLAGS property and the processing of read reports.
);

...
IMessage *INPUT_NULLOK, IMAPITable *INPUT, IMAPITable *INPUT_NULLOK, IMAPISession *INPUT, IMAPISession *INPUT_NULLOK,
    IMAPIFolder *INPUT, IMAPIFolder *INPUT_NULLOK, IMAPIProp *INPUT, IMAPIProp *INPUT_NULLOK, IMAPIProgress *INPUT,
    IMAPIProgress *INPUT_NULLOK, IMsgStore *INPUT, IMsgStore *INPUT_NULLOK, IAttach *INPUT, IAttach *INPUT_NULLOK,
    IProfAdmin *INPUT, IProfAdmin *INPUT_NULLOK, IProfSect *INPUT, IProfSect *INPUT_NULLOK, IProviderAdmin *INPUT,
    IProviderAdmin *INPUT_NULLOK, IMAPIAdviseSink *INPUT, IMAPIAdviseSink *INPUT_NULLOK, IAddrBook *INPUT,
    IAddrBook *INPUT_NULLOK, IMsgServiceAdmin *INPUT, IMsgServiceAdmin *INPUT_NULLOK, IMsgServiceAdmin2 *INPUT,
    IMsgServiceAdmin2 *INPUT_NULLOK, IStream *INPUT, IStream *INPUT_NULLOK
{
    if ($source)
        $source->Release();
}
...
if ($source)
    PyMAPIObject_FreeSRowSet($source);
}

%typemap(python,freearg) ADRLIST *INPUT, ADRLIST *INPUT_NULLOK {
if ($source)
    PyMAPIObject_FreeADRLIST($source);
}
...
if (*$source)
    PyMAPIObject_FreeADDRLIST(*$source);
}
...
Py_DECREF($target);
$target = Py_BuildValue("ll", $source->ulVersion, $source->ulFlags);
}

...
PyIMAPIProp::PyIMAPIProp(IUnknown *pDisp) : PyIUnknown(pDisp) { ob_type = &type; }

PyIMAPIProp::~PyIMAPIProp() {}

/*static*/ IMAPIProp *PyIMAPIProp::GetI(PyObject *self) { return (IMAPIProp *)PyIUnknown::GetI(self); }
...
PyObject *PyIMAPIProp::GetProps(PyObject *self, PyObject *args)
{
    ...
    ULONG numValues;
    SPropValue *pv;
    Py_BEGIN_ALLOW_THREADS _result = (HRESULT)_swig_self->GetProps(_arg0, _arg1, &numValues, &pv);
    Py_END_ALLOW_THREADS PyMAPIObject_FreeSPropTagArray(_arg0);
    if (FAILED(_result)) {
...
PyObject *PyIMAPIProp::DeleteProps(PyObject *self, PyObject *args)
{
    ...
    if (!PyMAPIObject_AsSPropTagArray(obs, &_arg0))
        return NULL;
    Py_BEGIN_ALLOW_THREADS hr = pMAPIProp->DeleteProps(_arg0, wantProblems ? &pProblems : NULL);
    Py_END_ALLOW_THREADS PyMAPIObject_FreeSPropTagArray(_arg0);
    if (FAILED(hr))
...
PyObject *PyIMAPIProp::SetProps(PyObject *self, PyObject *args)
{
    ...
    if (!PyMAPIObject_AsSPropValueArray(obs, &pPV, &seqLen))
        return NULL;
    Py_BEGIN_ALLOW_THREADS hr = pMAPIProp->SetProps(seqLen, pPV, wantProblems ? &pProblems : NULL);
    Py_END_ALLOW_THREADS MAPIFreeBuffer(pPV);
    if (FAILED(hr))
        return OleSetOleError(hr);
    if (wantProblems) {
        PyObject *result = Py_BuildValue("iN", hr, PyMAPIObject_FromSPropProblemArray(pProblems));
...
PyObject *PyIMAPIProp::CopyTo(PyObject *self, PyObject *args)
{
    ...
    // @pyparm bool|wantProblems|False|Return detailed error information
    if (!PyArg_ParseTuple(args, "OOlzOOl|O:CopyTo", &obIIDExclude, &obPropTags, &ulUIParam, &szIgnore, &obIID, &obDest,
                          &flags, &obWantProblems))
        return NULL;
    int wantProblems = PyObject_IsTrue(obWantProblems);
    if (wantProblems == -1)
        return NULL;
    ...
    Py_BEGIN_ALLOW_THREADS hr = pMAPIProp->CopyTo(ciidExclude, pExclude, pta, ulUIParam, NULL, &iid, (void *)pUnk,
                                                  flags, wantProblems ? &pProblems : NULL);
...
PyObject *PyIMAPIProp::CopyProps(PyObject *self, PyObject *args)
{
    ...
    // @pyparm int|flags||flags
    // @pyparm bool|wantProblems|False|Return detailed error information
    if (!PyArg_ParseTuple(args, "OlzOOl|O:CopyProps", &obPropTags, &ulUIParam, &szIgnore, &obIID, &obDest, &flags,
                          &obWantProblems))
        return NULL;
    int wantProblems = PyObject_IsTrue(obWantProblems);
    if (wantProblems == -1)
    ...
    Py_BEGIN_ALLOW_THREADS hr =
        pMAPIProp->CopyProps(pta, ulUIParam, NULL, &iid, (void *)pUnk, flags, wantProblems ? &pProblems : NULL);
    Py_END_ALLOW_THREADS if (FAILED(hr))
    {
        OleSetOleError(hr);
        goto error;
...
PyObject *PyIMAPIProp::OpenProperty(PyObject *self, PyObject *args)
{
    ...
    if (!PyWinObject_AsIID(obIID, &iid))
        return NULL;
    // Make the call.
    Py_BEGIN_ALLOW_THREADS hr = pMAPIProp->OpenProperty(propTag, &iid, interfaceOptions, flags, &pUnk);
    Py_END_ALLOW_THREADS if (FAILED(hr)) { return OleSetOleError(hr); }
...
PyObject *PyIMAPIProp::GetIDsFromNames(PyObject *self, PyObject *args)
{
    ...
    HRESULT hr;
    Py_BEGIN_ALLOW_THREADS hr = pMAPIProp->GetIDsFromNames(numIds, ppIds, flags, &pTagResult);
    Py_END_ALLOW_THREADS PyMAPIObject_FreeMAPINAMEIDArray(ppIds);
    if (FAILED(hr))
        return OleSetOleError(hr);
...
PyObject *PyIMAPIProp::GetNamesFromIDs(PyObject *self, PyObject *args)
{
    ...
    HRESULT hr;
    Py_BEGIN_ALLOW_THREADS hr = pMAPIProp->GetNamesFromIDs(&pta, pguid, flags, &numNames, &ppNames);
    Py_END_ALLOW_THREADS PyObject *result;
    if (SUCCEEDED(hr)) {
...
// @pyparm int|flags||flags
HRESULT GetPropList(unsigned long ulFlags, SPropTagArray **OUTPUT);

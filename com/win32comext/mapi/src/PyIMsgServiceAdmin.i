...
PyIMsgServiceAdmin::PyIMsgServiceAdmin(IUnknown *pDisp) : PyIUnknown(pDisp) { ob_type = &type; }
...
PyObject *PyIMsgServiceAdmin::CreateMsgService(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS hRes = _swig_self->CreateMsgService(lpszService, lpszDisplayName, ulUIParam, ulFlags);
    Py_END_ALLOW_THREADS
...
PyObject *PyIMsgServiceAdmin::ConfigureMsgService(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            ...
            &obs))  // @pyparm [values, ...]|[SPropValue, ...]||Property values describing the properties to display in
                    // the property sheet.  Should not be None if the service is to be configured without a message
                    // service.
        return NULL;
    CLSID iid;
    ...
    HRESULT _result;
    Py_BEGIN_ALLOW_THREADS _result = _swig_self->ConfigureMsgService(pMAPIUID, ulUIParam, ulFlags, cValues, pv);
    Py_END_ALLOW_THREADS MAPIFreeBuffer(pv);
    if (FAILED(_result))
...
HRESULT GetMsgServiceTable(unsigned long ulFlags,  // @pyparm int|flags||
                           IMAPITable **OUTPUT);

// @pyswig <o PyIMAPITable>|GetProviderTable|Retrieves a table of service providers.
HRESULT GetProviderTable(unsigned long ulFlags,  // @pyparm int|flags||
                         IMAPITable **OUTPUT);

// @pyswig |DeleteMsgService|Deletes the specified service
HRESULT DeleteMsgService(MAPIUID *INPUT  // @pyparm <o PyIID>|uuid||The ID of the service
);
...
HRESULT RenameMsgService(MAPIUID *INPUT,       // @pyparm <o PyIID>|uuid||The ID of the service
...
HRESULT AdminProviders(MAPIUID *INPUT,         // @pyparm <o PyIID>|uuid||The ID of the service
                       unsigned long ulFlags,  // @pyparm int|flags||

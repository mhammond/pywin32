...
PyIMsgServiceAdmin2::PyIMsgServiceAdmin2(IUnknown *pDisp) : PyIMsgServiceAdmin(pDisp) { ob_type = &type; }
...
PyObject *PyIMsgServiceAdmin2::CreateMsgServiceEx(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS hRes =
        _swig_self->CreateMsgServiceEx(lpszService, lpszDisplayName, ulUIParam, ulFlags, &uidService);
    Py_END_ALLOW_THREADS

        if (FAILED(hRes)) result = OleSetOleError(hRes);
    else result = PyWinObject_FromIID(reinterpret_cast<GUID &>(uidService));

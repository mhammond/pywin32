...
PyIMsgStore::PyIMsgStore(IUnknown *pDisp) : PyIMAPIProp(pDisp) { ob_type = &type; }

PyIMsgStore::~PyIMsgStore() {}

/*static*/ IMsgStore *PyIMsgStore::GetI(PyObject *self) { return (IMsgStore *)PyIUnknown::GetI(self); }
%}
...
PyObject *PyIMsgStore::StoreLogoff(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS _result = (HRESULT)_swig_self->StoreLogoff(&flags);
    Py_END_ALLOW_THREADS if (FAILED(_result)) { return OleSetOleError(_result); }
...
%{
// @pyswig <o PyIID>, string|GetReceiveFolder|Obtains the folder that was established as the destination for incoming
// messages of a specified message class or the default receive folder for the message store.
PyObject *PyIMsgStore::GetReceiveFolder(PyObject *self, PyObject *args)
{
    ...
    if ((_swig_self = GetI(self)) == NULL)
        return NULL;
    // @pyparm string|messageClass|None|Message class that is associated with a receive folder. If this parameter is set
    // to None or an empty string, GetReceiveFolder returns the default receive folder for the message store.
    // @pyparm int|flags|0|
    ...
    Py_BEGIN_ALLOW_THREADS _result =
        (HRESULT)_swig_self->GetReceiveFolder(szClass, flags, &eid_cb, &eid_out, &sz_explicit_class);
    Py_END_ALLOW_THREADS if (FAILED(_result))
    {
...
// @pyswig <o PyIMAPITable>|GetReceiveFolderTable|provides access to the receive folder table, a table that includes
// information about all of the receive folders for the message store.
HRESULT GetReceiveFolderTable(unsigned long ulFlags,  // @pyparm int|flags||Bitmask of flags that controls table access
                              IMAPITable **OUTPUT);
...
PyObject *PyIMsgStore::AbortSubmit(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS hr = _swig_self->AbortSubmit(cb, peid, flags);
    Py_END_ALLOW_THREADS if (FAILED(hr)) rc = OleSetOleError(hr);
    else
    {
        rc = Py_None;
...
PyObject *PyIMsgStore::Advise(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            args, "OkO:Advise",
            &obEntryId,  // @pyparm string|entryId||entry identifier of the folder or message about which notifications
                         // should be generated, or None
...
HRESULT Unadvise(ULONG handle);  // @pyparm int|connection||Connection number returned from <om PyIMsgStore.Advise>

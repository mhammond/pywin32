...
#define MAPI_ISPOOLERHOOK_METHODS(IPURE)                                                                            \
    ...
    (THIS_ LPMESSAGE lpMessage, LPMAPIFOLDER lpFolder, LPMDB lpMDB, ULONG FAR * lpulFlags, ULONG FAR * lpcbEntryID, \
...
DECLARE_MAPI_INTERFACE_(ISpoolerHook,
                        IUnknown){BEGIN_INTERFACE MAPI_IUNKNOWN_METHODS(PURE) MAPI_ISPOOLERHOOK_METHODS(PURE)};
...
typedef HRESULT(STDMAPIINITCALLTYPE HPPROVIDERINIT)(LPMAPISESSION lpSession, HINSTANCE hInstance,
                                                    LPALLOCATEBUFFER lpAllocateBuffer, LPALLOCATEMORE lpAllocateMore,
                                                    LPFREEBUFFER lpFreeBuffer, LPMAPIUID lpSectionUID, ULONG ulFlags,

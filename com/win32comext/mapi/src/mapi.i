...
// @pyswig |MAPIUninitialize|Decrements the reference count, cleans up, and deletes per-instance global data for the
// MAPI DLL.
...
%init %{
if (PyCom_RegisterClientType(&PyIMAPISession::type, &IID_IMAPISession) != 0)
    return NULL;
ADD_IID(IID_IMAPISession);

if (PyCom_RegisterClientType(&PyIMAPIStatus::type, &IID_IMAPIStatus) != 0)
    return NULL;
ADD_IID(IID_IMAPIStatus);

if (PyCom_RegisterClientType(&PyIMAPITable::type, &IID_IMAPITable) != 0)
    return NULL;
ADD_IID(IID_IMAPITable);

if (PyCom_RegisterClientType(&PyIMAPIProp::type, &IID_IMAPIProp) != 0)
    return NULL;
ADD_IID(IID_IMAPIProp);

if (PyCom_RegisterClientType(&PyIMAPIFolder::type, &IID_IMAPIFolder) != 0)
    return NULL;
ADD_IID(IID_IMAPIFolder);

if (PyCom_RegisterClientType(&PyIMAPIContainer::type, &IID_IMAPIContainer) != 0)
    return NULL;
ADD_IID(IID_IMAPIContainer);

if (PyCom_RegisterClientType(&PyIMessage::type, &IID_IMessage) != 0)
    return NULL;
ADD_IID(IID_IMessage);

if (PyCom_RegisterClientType(&PyIMsgStore::type, &IID_IMsgStore) != 0)
    return NULL;
ADD_IID(IID_IMsgStore);

if (PyCom_RegisterClientType(&PyIAttach::type, &IID_IAttachment) != 0)
    return NULL;
ADD_IID(IID_IAttachment);

if (PyCom_RegisterClientType(&PyIProfAdmin::type, &IID_IProfAdmin) != 0)
    return NULL;
ADD_IID(IID_IProfAdmin);

if (PyCom_RegisterClientType(&PyIAddrBook::type, &IID_IAddrBook) != 0)
    return NULL;
ADD_IID(IID_IAddrBook);

if (PyCom_RegisterClientType(&PyIDistList::type, &IID_IDistList) != 0)
    return NULL;
ADD_IID(IID_IDistList);

if (PyCom_RegisterClientType(&PyIMailUser::type, &IID_IMailUser) != 0)
    return NULL;
ADD_IID(IID_IMailUser);

if (PyCom_RegisterClientType(&PyIABContainer::type, &IID_IABContainer) != 0)
    return NULL;
ADD_IID(IID_IABContainer);

if (PyCom_RegisterClientType(&PyIProfSect::type, &IID_IProfSect) != 0)
    return NULL;
ADD_IID(IID_IProfSect);

if (PyCom_RegisterClientType(&PyIMsgServiceAdmin::type, &IID_IMsgServiceAdmin) != 0)
    return NULL;
ADD_IID(IID_IMsgServiceAdmin);

if (PyCom_RegisterClientType(&PyIMsgServiceAdmin2::type, &IID_IMsgServiceAdmin2) != 0)
    return NULL;
ADD_IID(IID_IMsgServiceAdmin2);

if (PyCom_RegisterClientType(&PyIProviderAdmin::type, &IID_IProviderAdmin) != 0)
    return NULL;
ADD_IID(IID_IProviderAdmin);

if (PyCom_RegisterClientType(&PyIMAPIAdviseSink::type, &IID_IMAPIAdviseSink) != 0)
    return NULL;
ADD_IID(IID_IMAPIAdviseSink);
if (PyCom_RegisterGatewayObject(IID_IMAPIAdviseSink, GET_PYGATEWAY_CTOR(PyGMAPIAdviseSink), "IMAPIAdviseSink") != 0)
    return NULL;
ADD_IID(IID_IMAPIAdviseSink);

if (PyCom_RegisterClientType(&PyIConverterSession::type, &IID_IConverterSession) != 0)
    return NULL;
{
...
#define ATTACH_BY_REFERENCE \
    ATTACH_BY_REFERENCE  // The PR_ATTACH_PATHNAME or PR_ATTACH_LONG_PATHNAME property contains a fully qualified path
                         // identifying the attachment to recipients with access to a common file server.
#define ATTACH_BY_REF_RESOLVE \
    ATTACH_BY_REF_RESOLVE  // The PR_ATTACH_PATHNAME or PR_ATTACH_LONG_PATHNAME property contains a fully qualified path
                           // identifying the attachment.
#define ATTACH_BY_REF_ONLY \
    ATTACH_BY_REF_ONLY  // The PR_ATTACH_PATHNAME or PR_ATTACH_LONG_PATHNAME property contains a fully qualified path
                        // identifying the attachment.
#define ATTACH_EMBEDDED_MSG \
    ATTACH_EMBEDDED_MSG        // The PR_ATTACH_DATA_OBJ property contains an embedded object that supports the IMessage
                               // interface.
#define ATTACH_OLE ATTACH_OLE  // The attachment is an embedded OLE object

#define CONVENIENT_DEPTH \
    CONVENIENT_DEPTH  // Fills the hierarchy table with containers from multiple levels. If CONVENIENT_DEPTH is not set,
                      // the hierarchy table contains only the container's immediate child containers.
...
#define FORCE_SAVE \
    FORCE_SAVE  // Changes should be written to the object, overriding any previous changes made to the object, and the
                // object closed. Read/write access must have been set for the operation to succeed. The FORCE_SAVE flag
                // is used after a previous call to SaveChanges returned MAPI_E_OBJECT_CHANGED.

#define KEEP_OPEN_READONLY \
    KEEP_OPEN_READONLY  // Changes should be committed and the object should be kept open for reading. No further
                        // changes will be made.
#define KEEP_OPEN_READWRITE \
    KEEP_OPEN_READWRITE  // Changes should be committed and the object should be kept open for read/write access. This
                         // flag is usually set when the object was initially opened for read/write access. Subsequent
                         // changes to the object are allowed.
...
#define MAPI_ASSOCIATED \
    MAPI_ASSOCIATED  // The container's associated contents table should be returned rather than the standard contents
                     // table. This flag is used only with folders. The messages that are included in the associated
                     // contents table were created with the MAPI_ASSOCIATED flag set in the call to
                     // IMAPIFolder::CreateMessage. Clients typically use the associated contents table to retrieve
                     // forms and views.
#define MAPI_ALLOW_OTHERS \
    MAPI_ALLOW_OTHERS  // The shared session should be returned, allowing subsequent clients to acquire the session
                       // without providing any user credentials.
#define MAPI_EXPLICIT_PROFILE \
    MAPI_EXPLICIT_PROFILE  // The default profile should not be used, and the user should be required to supply a
                           // profile.
#define MAPI_EXTENDED \
    MAPI_EXTENDED  // Log on with extended capabilities. This flag should always be set. The older MAPILogon function is
                   // no longer available.
#define MAPI_FORCE_DOWNLOAD \
    MAPI_FORCE_DOWNLOAD  // An attempt should be made to download all of the user's messages before returning. If the
                         // MAPI_FORCE_DOWNLOAD flag is not set, messages can be downloaded in the background after the
                         // call to MAPILogonEx returns.
#define MAPI_LOGON_UI \
    MAPI_LOGON_UI  // A dialog box should be displayed to prompt the user for logon information if required. When the
                   // MAPI_LOGON_UI flag is not set, the calling client does not display a logon dialog box and returns
                   // an error value if the user is not logged on. MAPI_LOGON_UI and MAPI_PASSWORD_UI are mutually
                   // exclusive.
#define MAPI_NEW_SESSION \
    MAPI_NEW_SESSION  // An attempt should be made to create a new MAPI session rather than acquire the shared session.
                      // If the MAPI_NEW_SESSION flag is not set, MAPILogonEx uses an existing shared session even if
                      // the lpszprofileName parameter is not NULL.
#define MAPI_NO_MAIL \
    MAPI_NO_MAIL  // MAPI should not inform the MAPI spooler of the session's existence. The result is that no messages
                  // can be sent or received within the session except through a tightly coupled store and transport
                  // pair. A calling client sets this flag if it is acting as an agent, if configuration work must be
                  // done, or if the client is browsing the available message stores.
#define MAPI_MULTITHREAD_NOTIFICATIONS \
    MAPI_MULTITHREAD_NOTIFICATIONS  // MAPI should generate notifications using a thread dedicated to notification
                                    // handling rather than the first thread used to call <om mapi.MAPIInitialize>.
#define MAPI_NT_SERVICE \
    MAPI_NT_SERVICE  // The caller is running as a Windows NT service. Callers that are not running as a Windows NT
                     // service should not set this flag; callers that are running as a service must set this flag.
#define MAPI_PASSWORD_UI \
    MAPI_PASSWORD_UI  // A dialog box should be displayed to prompt the user for the profile password. MAPI_PASSWORD_UI
                      // cannot be set if MAPI_LOGON_UI is set because the calling client can only present one of the
                      // two dialog boxes. This dialog box does not allow the profile name to be changed; the
                      // lpszProfileName parameter must be non-NULL.
#define MAPI_SERVICE_UI_ALWAYS \
    MAPI_SERVICE_UI_ALWAYS  // MAPILogonEx should display a configuration dialog box for each message service in the
                            // profile. The dialog boxes are displayed after the profile has been chosen but before any
                            // message service is logged on. The MAPI common dialog box for logon also contains a check
                            // box that requests the same operation.
#define MAPI_TIMEOUT_SHORT MAPI_TIMEOUT_SHORT  // The logon should fail if blocked for more than a few seconds.
#define MAPI_UNICODE \
    MAPI_UNICODE  // The passed-in strings are in Unicode format. If the MAPI_UNICODE flag is not set, the strings are
                  // in ANSI format.
#define MAPI_USE_DEFAULT \
    MAPI_USE_DEFAULT  // The messaging subsystem should substitute the profile name of the default profile for the
                      // lpszProfileName parameter. The MAPI_EXPLICIT_PROFILE flag is ignored unless lpszProfileName is
                      // NULL or empty.
...
#define MAPI_DEFERRED_ERRORS \
    MAPI_DEFERRED_ERRORS  // Allows a method to return successfully, possibly before the changes have been fully
                          // committed.
#define MAPI_INIT_VERSION MAPI_INIT_VERSION

#define MDB_NO_DIALOG \
    MDB_NO_DIALOG  // Prevents the display of logon dialog boxes. If this flag is set, and OpenMsgStore does not have
                   // enough configuration information to open the message store without the user's help, it returns
                   // MAPI_E_LOGON_FAILED. If this flag is not set, the message store provider can prompt the user to
                   // correct a name or password, to insert a disk, or to perform other actions necessary to establish
                   // connection to the message store.
#define MDB_NO_MAIL \
    MDB_NO_MAIL  // The message store should not be used for sending or receiving mail. When this flag is set, MAPI does
                 // not notify the MAPI spooler that this message store is being opened.
#define MDB_TEMPORARY \
    MDB_TEMPORARY  // Instructs MAPI that the message store is not permanent and should not be added to the message
                   // store table. This flag is used to log on the message store so that information can be retrieved
                   // programmatically from the profile section.
...
#define MODRECIP_MODIFY \
    MODRECIP_MODIFY  // The recipients should replace existing recipients. All of the existing properties are replaced
                     // by those in the corresponding ADRENTRY structure.

#define MODRECIP_REMOVE \
    MODRECIP_REMOVE  // Existing recipients should be removed from the recipient list using as an index the PR_ROWID
                     // property included in the property value array of each recipient entry in the mods parameter.

#define MAPI_TO \
    MAPI_TO  // The recipient is a primary (To) recipient. Clients are required to handle primary recipients; all other
             // types are optional.

#define MAPI_CC \
    MAPI_CC  // The recipient is a carbon copy (CC) recipient, a recipient that receives a message in addition to the
             // primary recipients.

#define MAPI_BCC \
    MAPI_BCC  // The recipient is a blind carbon copy (BCC) recipient. Primary and carbon copy recipients are unaware of
              // the existence of BCC recipients.

#define MAPI_P1 \
    MAPI_P1  // The recipient did not successfully receive the message on the previous attempt. This is a resend of an
             // earlier transmission.

#define MAPI_SUBMITTED \
    MAPI_SUBMITTED  // The recipient has already received the message and does not need to receive it again. This is a
                    // resend of an earlier transmission. This flag is set in conjunction with the MAPI_TO, MAPI_CC, and
                    // MAPI_BCC values.

#define MAPI_DEFAULT_SERVICES \
    MAPI_DEFAULT_SERVICES  // MAPI should populate the new profile with the message services that are included in the
                           // [Default Services] section of the MAPISVC.INF file.
...
// #define MSG_SERVICE_UI_READ_ONLY MSG_SERVICE_UI_READ_ONLY // The message service should display its configuration
// property sheet but not enable the user to change it. Most message services ignore this flag.

#define SERVICE_UI_ALLOWED \
    SERVICE_UI_ALLOWED  // The message service should display its configuration property sheet only if the service is
                        // not completely configured.

#define SERVICE_UI_ALWAYS \
    SERVICE_UI_ALWAYS  // The message service must always display its configuration property sheet. If SERVICE_UI_ALWAYS
                       // is not set, a configuration property sheet can still be displayed if SERVICE_UI_ALLOWED is set
                       // and valid configuration information is not available from the property value array in the
                       // lpProps parameter. Either SERVICE_UI_ALLOWED or SERVICE_UI_ALWAYS must be set for a property
                       // sheet to be displayed.
...
#define RES_AND \
    RES_AND  // SRestriction structure describes an AND restriction, which applies a bitwise AND operation to a
             // restriction.
...
#define RES_COMMENT \
    RES_COMMENT  // SRestriction structure describes a comment restriction, which associates a comment with a
                 // restriction.

#define RES_COMPAREPROPS \
    RES_COMPAREPROPS  // SRestriction structure describes a compare properties restriction, which compares two property
                      // values.

#define RES_CONTENT \
    RES_CONTENT  // SRestriction structure describes a content restriction, which searches a property value for specific
                 // content.
...
#define RES_NOT \
    RES_NOT  // SRestriction structure describes a NOT restriction, which applies a logical NOT operation to a
             // restriction.

#define RES_OR \
    RES_OR  // SRestriction structure describes an OR restriction, which applies a logical OR operation to a
            // restriction.

#define RES_PROPERTY \
    RES_PROPERTY  // SRestriction structure describes a property restriction, which determines if a property value
                  // matches a particular value.

#define RES_SIZE \
    RES_SIZE  // SRestriction structure describes a size restriction, which determines if a property value is a
              // particular size.

#define RES_SUBRESTRICTION \
    RES_SUBRESTRICTION  // SRestriction structure describes a subobject restriction, which applies a restriction to a
                        // message's attachments or recipients.
...
#define BMR_EQZ \
    BMR_EQZ  // Perform a bitwise AND operation of the mask in the ulMask member with the property represented by the
             // ulPropTag member and test for being equal to zero.

#define BMR_NEZ \
    BMR_NEZ  // Perform a bitwise AND operation of the mask in the ulMask member with the property represented by the
             // ulPropTag member and test for being not equal to zero.

#define DIR_BACKWARD DIR_BACKWARD  // Searches backward from the row identified by the bookmark.

#define FL_FULLSTRING \
    FL_FULLSTRING  // To match, the lpProp search string must be completely contained in the property identified by
                   // ulPropTag.

#define FL_PREFIX \
    FL_PREFIX  // To match, the lpProp search string must appear at the beginning of the property identified by
               // ulPropTag. The two strings should be compared only up to the length of the search string indicated by
               // lpProp.

#define FL_SUBSTRING \
    FL_SUBSTRING  // To match, the lpProp search string must be contained anywhere within the property identified by
                  // ulPropTag.
...
#define FL_LOOSE \
    FL_LOOSE  // The comparison should result in a match whenever possible, ignoring case and nonspacing characters
...
#define TABLE_SORT_COMBINE \
    TABLE_SORT_COMBINE  // The sort operation should create a category that combines the property identified as the sort
                        // key column in the ulPropTag member with the sort key column specified in the previous
                        // SSortOrder structure.<nl>TABLE_SORT_COMBINE can only be used when the SSortOrder structure is
                        // being used as an entry in an SSortOrderSet structure to specify multiple sort orders for a
                        // categorized sort. TABLE_SORT_COMBINE cannot be used in the first SSortOrder structure in an
...
#define CLEAR_READ_FLAG \
    CLEAR_READ_FLAG  // The MSGFLAG_READ flag should be cleared in PR_MESSAGE_FLAGS and a read report should not be
                     // sent.
#define CLEAR_NRN_PENDING \
    CLEAR_NRN_PENDING  // The MSGFLAG_NRN_PENDING flag should be cleared in PR_MESSAGE_FLAGS and an unread report should
                       // not be sent.
#define CLEAR_RN_PENDING \
    CLEAR_RN_PENDING  // The MSGFLAG_RN_PENDING flag should be cleared in PR_MESSAGE_FLAGS and a read report should not
                      // be sent.
#define GENERATE_RECEIPT_ONLY \
    GENERATE_RECEIPT_ONLY  // A read report should be sent if one is pending, but there should be no change in the state
                           // of the MSGFLAG_READ flag.
#define SUPPRESS_RECEIPT \
    SUPPRESS_RECEIPT  // A pending read report should be cancelled if a read report had been requested and this call
                      // changes the state of the message from unread to read.
...
// @flagh Value|Meaning
// @flag MAPI_MULTITHREAD_NOTIFICATIONS|MAPI should generate notifications using a thread dedicated to notification
// handling rather than the first thread used to call <om mapi.MAPIInitialize>.
// @flag MAPI_NT_SERVICE|The caller is running as a NT service. Callers that are not running in a Windows NT service
// should not set this flag; callers that are running as a service must set this flag.
// @comm Multithreaded clients should set MAPI_MULTITHREAD_NOTIFICATIONS flag so that they can receive notifications on
// threads other than the first thread to call <om mapi.MAPIInitialize>.

// @pyswig |MAPIInitialize|Increments the MAPI subsystem reference count and initializes global data for the MAPI DLL.
HRESULT MAPIInitialize(MAPIINIT_0 *INPUT  // @pyparm <o MAPIINIT_0>|init||MAPI Initialization flags.
);
...
PyObject *PyMAPILogonEx(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "lO|Ol",
                          &ulUIParam,      // @pyparm int|uiParam||Handle to the window to which the logon dialog box is
                                           // modal. If no dialog box appears during the call, the uiParam parameter is
                                           // ignored. This parameter can be zero.
                          &obProfileName,  // @pyparm string|profileName||A string that contains the name of the profile
                                           // to use when the user logs on. This string is limited to 64 characters.
                          &obPassword,     // @pyparm string|password|None|A string that contains the password of the
                                           // profile. The password parameter must be None.
    ...
    Py_BEGIN_ALLOW_THREADS hRes = ::MAPILogonEx(ulUIParam, lpszProfileName, lpszPassword, ulFlags, &lpSession);
    Py_END_ALLOW_THREADS

        if (FAILED(hRes)) result = OleSetOleError(hRes);
    else MAKE_OUTPUT_INTERFACE(&lpSession, result, IID_IMAPISession);
...
HRESULT MAPIAdminProfiles(unsigned long ulFlags,  // @pyparm int|fFlags||
                          IProfAdmin **OUTPUT);
...
HRESULT HrQueryAllRows(
    ...
    SPropTagArray *INPUT,  // @pyparm <o PySPropTagArray>|properties||A sequence of property tags indicating table
                           // columns. These tags are used to select the specific columns to be retrieved. If this
                           // parameter is None, HrQueryAllRows retrieves the entire column set of the current table
                           // view passed in the table parameter.
    SRestriction *INPUT,   // @pyparm <o PySRestriction>|restrictions||Defines the retrieval restrictions. If this
                           // parameter is None, HrQueryAllRows makes no restrictions.
    SSortOrderSet *INPUT,  // @pyparm <o PySSortOrderSet>|sortOrderSet||Identifies the sort order of the columns to be
                           // retrieved. If this parameter is None, the default sort order for the table is used.
    long crowsMax,         // @pyparm int|rowsMax||Maximum number of rows to be retrieved. If the value of the rowsMax
                           // parameter is zero, no limit on the number of rows retrieved is set.
    SRowSet **OUTPUT);

// @pyswig int|RTFSync|
HRESULT RTFSync(IMessage *INPUT,        // @pyparm <o PyIMessage>|message||The message.
                unsigned long ulFlags,  // @pyparm int|flags||
                int *OUTPUT             // lpfMessageUpdated
);

// @pyswig <o PyIStream>|WrapCompressedRTFStream|
HRESULT WrapCompressedRTFStream(IStream *INPUT,         // @pyparm <o PyIStream>|stream||Message stream
                                unsigned long ulflags,  // @pyparm int|flags||
                                IStream **OUTPUT);

// WrapCompressedRTFStreamEx
#define MAPI_NATIVE_BODY \
    ((ULONG)0x00010000)  // Indicates whether the decompressed stream is also converted to its native body.
...
STDAPI WrapCompressedRTFStreamEx(LPSTREAM lpCompressedRTFStream, CONST RTF_WCSINFO *pWCSInfo,
...
PyObject *PyWrapCompressedRTFStreamEx(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O|(kkk):PyWrapCompressedRTFStreamEx", &obCompressedStream, &wcsinfo.Flags,
                          &wcsinfo.ulInCodePage, &wcsinfo.ulOutCodePage))
        return NULL;

    if (!PyCom_InterfaceFromPyObject(obCompressedStream, IID_IStream, (void **)&lpCompressedStream, FALSE))
        return NULL;

    Py_BEGIN_ALLOW_THREADS hRes =
        WrapCompressedRTFStreamEx(lpCompressedStream, &wcsinfo, &lpUncompressedStream, &retinfo);
...
// @pyswig <o PyIMessage>|OpenIMsgOnIStg|Builds a new IMessage object on top of an existing OLE IStorage object, to be
// used within a message session.
...
PyObject *PyOpenIMsgOnIStg(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            args, "lOO|Oll:OpenIMsgOnIStg",
            &lSession,       // @pyparm object|session||
            &obMapiSupport,  // @pyparm <o PyIMAPISupport>|support||May be None
            &obStorage,  // @pyparm <o PyIStorage>|storage||A <o PyIStorage> object that is open and has read-only or
                         // read/write access. Because IMessage does not support write-only access, OpenIMsgOnIStg does
                         // not accept a storage object opened in write-only mode.
...

    {
        PY_INTERFACE_PRECALL;
        LPMALLOC pMalloc = MAPIGetDefaultMalloc();
        hr = ::OpenIMsgOnIStg((LPMSGSESS)lSession, MAPIAllocateBuffer, MAPIAllocateMore, MAPIFreeBuffer, pMalloc,
                              pSupport, pStorage, NULL, dwCallRelFlags, flags, &pRet);
        if (pMalloc)
            pMalloc->Release();
        PY_INTERFACE_POSTCALL;
    }
    ...
    while (c < max) {
        if (*c == '{')
            ...
            *((unsigned char *)d) = (unsigned char)(hi * 16 + lo);
            ...
            c++;
            while (c < max && strncmp(c, "\\htmlrtf0", 9) != 0) c++;
            if (c < max)
                c += 9;
            if (*c == ' ')
                c++;
...
static PyObject *MyRTFStreamToHTML(PyObject *self, PyObject *args)
{
    ...
    bool done = (htmlbuf == NULL);
    while (!done) {
        ULONG red;
        hr = pStream->Read(htmlbuf + htmlsize, bufsize - htmlsize, &red);
        if (hr != S_OK) {
            htmlbuf[htmlsize] = 0;
            done = true;
        }
...
// @pyswig <o PyIStream>|OpenStreamOnFile|Allocates and initializes an OLE IStream object to access the contents of a
// file.
...
// @pyswig <o PyIStream>|OpenStreamOnFileW|Allocates and initializes an OLE IStream object to access the contents of a
// file.
%native(OpenStreamOnFileW) PyOpenStreamOnFileW;
%{

// Missing declaration copied from MFCMAPI
_Check_return_ STDAPI OpenStreamOnFileW(_In_ LPALLOCATEBUFFER lpAllocateBuffer, _In_ LPFREEBUFFER lpFreeBuffer,
                                        ULONG ulFlags, _In_z_ LPCWSTR lpszFileName, _In_opt_z_ LPCWSTR lpszPrefix,
...
PyObject *PyHrGetOneProp(PyObject *self, PyObject *args)
{
    ...
    if (PyTuple_GET_ITEM(ret, 1) == Py_None && PyLong_AsUnsignedLong(PyTuple_GET_ITEM(ret, 0)) != PT_NULL) {
        char buf[128];
...
// @pyswig <o PyIMAPIAdviseSink>|HrAllocAdviseSink|Creates an advise sink object, given a context specified by the
// calling implementation and a callback function to be triggered by an event notification.
...
// @pyswig <o PyIMAPIAdviseSink>|HrThisThreadAdviseSink|Creates an advise sink that wraps an existing advise sink for
// thread safety.

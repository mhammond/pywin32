...
%init %{
if (PyCom_RegisterClientType(&PyIExchangeManageStore::type, &IID_IExchangeManageStore) != 0)
    return NULL;
ADD_IID(IID_IExchangeManageStore);
if (PyCom_RegisterClientType(&PyIExchangeManageStoreEx::type, &IID_IExchangeManageStoreEx) != 0)
    return NULL;
...
HRESULT HrGetExchangeStatus(char *server,  // @pyparm string/<o PyUnicode>|server||The name of the server to query.
                            unsigned long *OUTPUT, unsigned long *OUTPUT);

// @pyswig string|HrGetMailboxDN|Retrieves the distinguished name (DN) for a mailbox
HRESULT HrGetMailboxDN(IMAPISession *INPUT,  // @pyparm <o IMAPISession>|session||The root folder.
                       char **OUTPUT_MAPI    // mailboxDN
);

// @pyswig string|HrGetServerDN|Retrieves the distinguished name (DN) for a server
HRESULT HrGetServerDN(IMAPISession *INPUT,  // @pyparm <o IMAPISession>|session||The root folder.
                      char **OUTPUT_MAPI    // mailboxDN
);
...
%{
PyObject *MyHrMAPIFindSubfolderEx(IMAPIFolder *lpRootFolder, TCHAR chSep, TCHAR *lpszName)
{
    DWORD idSize;
    ENTRYID *id;
...
// @pyswig <o PyIMsgStore>|HrMAPIFindSubfolderEx|Retrieves a subfolder in an information store using the hierarchical
...
%name(HrMAPIFindSubfolderEx) PyObject *MyHrMAPIFindSubfolderEx(
    ...
    // @pyswig string|HrMAPIFindFolder|Retrieves the entry ID for a folder in an information store using the
    // hierarchical path name of the folder.
...
%}
    ...
    // @pyswig string|HrMAPIFindFolderEx|Retrieves the entry ID of a folder in an information store using the
    // hierarchical path name of the folder.
...
%}
    ...
    // @pyswig <o PyIMsgStore>|HrMAPIFindStore|Retrieves a pointer to the entry identifier of an information store from
...
%}
    ...
    // @pyswig string|HrCreateDirEntryIdEx|Creates a directory identifier for a MAPI object, given the address of the
...
%{
    PyObject *PyHrCreateDirEntryIdEx(PyObject *self, PyObject *args)
    {
        ...
        if (!PyCom_InterfaceFromPyInstanceOrObject(obAddrBook, IID_IAddrBook, (void **)&pAddrBook,
                                                   /*BOOL bNoneOK=*/FALSE))
            goto done;
...
%name(HrMailboxLogoff) HRESULT MyHrMailboxLogoff(
    ...
    // @pyswig <o PyIMAPIFolder>|HrMAPIOpenFolderEx|Opens a folder in the information store from the hierarchical path
    // name of the folder.
%name(HrMAPIOpenFolderEx) HRESULT HrMAPIOpenFolderExW(
    ...
    HRESULT HrMAPISetPropBoolean(IMAPIProp *INPUT,         // @pyparm <o PyIMAPIProp>|obj||The object to set
    ...
    HRESULT HrMAPISetPropLong(IMAPIProp *INPUT,         // @pyparm <o PyIMAPIProp>|obj||The object to set
                              unsigned long ulPropTag,  // @pyparm int|tag||The property tag
                              long INPUT                // int|val||The property value.
    );

    // @pyswig <o PyIMsgStore>|HrOpenExchangePublicStore|Retrieves an interface to the public information store
    // provider.
    ...
    // @pyswig <o PyIMAPIFolder>|HrOpenExchangePublicFolders|Opens the root of the public folder hierarchy in the public
    // information store.
    HRESULT HrOpenExchangePublicFolders(IMsgStore *INPUT,  // @pyparm <o PyIMsgStore>|store||
                                        IMAPIFolder **OUTPUT);

    // @pyswig <o PyIMAPIProp>|HrOpenSessionObject|Retrieves a MAPI <o PyIMAPIProp> object for the current session
    // object.
    HRESULT HrOpenSessionObject(IMAPISession *INPUT,  // @pyparm <o PyIMAPISession>|session||The MAPI session object
                                IMAPIProp **OUTPUT);

    // @pyswig <o PyIMAPIProp>|HrOpenSiteContainer|Retrieves a MAPI <o PyIMAPIProp> object for a site object.
    HRESULT HrOpenSiteContainer(IMAPISession *INPUT,  // @pyparm <o PyIMAPISession>|session||The MAPI session object
                                IMAPIProp **OUTPUT);

    // @pyswig <o PyIMAPIProp>|HrOpenSiteContainerAddressing|Retrieves a MAPI <o PyIMAPIProp> object for a

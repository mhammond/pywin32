...
PyIMessage::PyIMessage(IUnknown *pDisp) : PyIMAPIProp(pDisp) { ob_type = &type; }

PyIMessage::~PyIMessage() {}

/*static*/ IMessage *PyIMessage::GetI(PyObject *self) { return (IMessage *)PyIUnknown::GetI(self); }

%}

// @pyswig |SetReadFlag|Sets the read flags for a message
HRESULT SetReadFlag(ULONG ulFlags  // @pyparm int|flag||Bitmask of flags that controls the setting of a message's read
                                   // flag - that is, the message's MSGFLAG_READ flag in its PR_MESSAGE_FLAGS property
                                   // and the processing of read reports.
);
...
HRESULT OpenAttach(ULONG ulAttachmentNum,  // @pyparm int|attachmentNum||
...
HRESULT CreateAttach(IID *INPUT_NULLOK,  // @pyparm <o PyIID>|interface||The interface to use, or None
...
HRESULT ModifyRecipients(
    unsigned long flags,  // @pyparm int|flags||Bitmask of flags that controls the recipient changes. If zero is passed
                          // for the ulFlags parameter, ModifyRecipients replaces all existing recipients with the
                          // recipient list in the mods parameter.
...
HRESULT SubmitMessage(unsigned long flags  // @pyparm int|flags||Flags which specify how the message is submitted.
);

...
PyIProfAdmin::PyIProfAdmin(IUnknown *pDisp) : PyIUnknown(pDisp) { ob_type = &type; }

PyIProfAdmin::~PyIProfAdmin() {}

/*static*/ IProfAdmin *PyIProfAdmin::GetI(PyObject *self) { return (IProfAdmin *)PyIUnknown::GetI(self); }
...
PyObject *PyIProfAdmin::CreateProfile(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OO|ll",
                          ...
                          &ulUIParam,  // @pyparm int|uiParam|0|A handle of the parent window for any dialog boxes or
                                       // windows that this method displays.
    ...
    Py_BEGIN_ALLOW_THREADS hRes = _swig_self->CreateProfile(lpszProfileName, lpszPassword, ulUIParam, ulFlags);
    Py_END_ALLOW_THREADS

        if (FAILED(hRes)) result = OleSetOleError(hRes);
    else result = Py_BuildValue("");
...
HRESULT GetProfileTable(unsigned long ulFlags, IMAPITable **OUTPUT);
...
HRESULT ChangeProfilePassword(TCHAR *INPUT,         // lpszProfileName
...
PyObject *PyIProfAdmin::CopyProfile(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS hRes =
        _swig_self->CopyProfile(lpszOldProfileName, lpszOldPassword, lpszNewProfileName, ulUIParam, ulFlags);
    Py_END_ALLOW_THREADS
...
PyObject *PyIProfAdmin::RenameProfile(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            ...
            &ulUIParam,  // @pyparm int|uiParam|0|A handle of the parent window for any dialog boxes or windows that
    ...
    Py_BEGIN_ALLOW_THREADS hRes =
        _swig_self->RenameProfile(lpszOldProfileName, lpszOldPassword, lpszNewProfileName, ulUIParam, ulFlags);
    Py_END_ALLOW_THREADS
...
PyObject *PyIProfAdmin::SetDefaultProfile(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            args, "O|l",
            &obProfileName,  // @pyparm string|profileName||The name of the profile that will become the default, or
                             // None. Setting profileName to None indicates that SetDefaultProfile should remove the
                             // existing default profile, leaving the client without a default.
            &ulFlags))       // @pyparm int|flags|0|
        return NULL;
    ...
    Py_BEGIN_ALLOW_THREADS hRes = _swig_self->SetDefaultProfile(lpszProfileName, ulFlags);
    Py_END_ALLOW_THREADS
...
%{
// @pyswig <o PyIProfAdmin>|AdminServices|Provides access to a message service administration object for making changes
// to the message services in a profile.
PyObject *PyIProfAdmin::AdminServices(PyObject *self, PyObject *args)
{
    ...
    Py_BEGIN_ALLOW_THREADS hRes =
        _swig_self->AdminServices(lpszProfileName, lpszPassword, ulUIParam, ulFlags, &lpServiceAdmin);
    Py_END_ALLOW_THREADS

        if (FAILED(hRes)) result = OleSetOleError(hRes);
    else MAKE_OUTPUT_INTERFACE(&lpServiceAdmin, result, IID_IMsgServiceAdmin);

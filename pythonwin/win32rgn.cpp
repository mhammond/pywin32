...
CRgn *PyCRgn::GetRgn(PyObject *self) { return (CRgn *)GetGoodCppObject(self, &type); }

// @pymethod <o PyCRgn>|win32ui|CreateRgn|Creates a new rgn object.
// Return Values: a PyCRgn object
PyObject *PyCRgn::create(PyObject *self, PyObject *args)
{
    CHECK_NO_ARGS(args);
    CRgn *pRgn = new CRgn;
...
PyObject *PyCRgn::create_rect_rgn(PyObject *self, PyObject *args)
{
    ...
    int x1, y1, x2, y2;
    if (!PyArg_ParseTuple(args, "(iiii):CreateRectRgn", &x1, &y1, &x2, &y2))
        return NULL;
...
// Return Values: success or failure flag (BOOL)
PyObject *PyCRgn::create_elliptic_rgn(PyObject *self, PyObject *args)
{
    CRgn *pRgn = PyCRgn::GetRgn(self);
    if (!pRgn)
        return NULL;
    ...
    int x1, y1, x2, y2;
    if (!PyArg_ParseTuple(args, "(iiii):CreateEllipticRgn", &x1, &y1, &x2, &y2))
        return NULL;
...
// as specified by nCombineMode Return Values: success or failure flag (BOOL)
PyObject *PyCRgn::combine_rgn(PyObject *self, PyObject *args)
{
    CRgn *pRgn = PyCRgn::GetRgn(self);
    if (!pRgn)
        return NULL;
    ...
    if (!PyArg_ParseTuple(args, "OOi:CombineRgn", &objRgn1, &objRgn2, &nCombineMode))
        return NULL;
...
// Return Values: success or failure flag (BOOL)
PyObject *PyCRgn::copy_rgn(PyObject *self, PyObject *args)
{
    CRgn *pRgn = PyCRgn::GetRgn(self);
    if (!pRgn)
        return NULL;
...
PyObject *PyCRgn::get_rgn_box(PyObject *self, PyObject *args)
{
    CRgn *pRgn = PyCRgn::GetRgn(self);
    if (!pRgn)
        return NULL;
...
// @pymethod int|PyCRgn|DeleteObject|Deletes the attached Windows GDI Rgn object from memory by freeing all system
// storage associated with the Windows GDI object Return Values: None
PyObject *PyCRgn::delete_object(PyObject *self, PyObject *args)
{
    CRgn *pRgn = PyCRgn::GetRgn(self);
    if (!pRgn)
        return NULL;
...
// @pymethod int|PyCRgn|GetSafeHandle|A HANDLE to the attached Windows GDI object; otherwise NULL if no object is
// attached Return Values: the handle of the CRgn object
PyObject *PyCRgn::get_safe_handle(PyObject *self, PyObject *args)
{
    CRgn *pRgn = PyCRgn::GetRgn(self);
    if (!pRgn)
        return NULL;
...
static struct PyMethodDef PyCRgn_methods[] = {{"CreateRectRgn", PyCRgn::create_rect_rgn, 1},
...
ui_type_CObject PyCRgn::type("PyCRgn", &PyCGdiObject::type, RUNTIME_CLASS(CRgn), sizeof(PyCRgn), PYOBJ_OFFSET(PyCRgn),
                             PyCRgn_methods, GET_PY_CTOR(PyCRgn));

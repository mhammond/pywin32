...
char *FormatSysError(const DWORD nErrNo)
{
    ...
    if (!result)
        return NULL;
    result[0] = '\0';
    int nLen = FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, nErrNo, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
                              result, 1024, NULL);
    if (nLen > 2) {
        if (result[nLen - 1] == '\n')
            result[nLen - 1] = 0;
...
static void CheckRegisterEventSourceFile()
{
    ...
    GetModuleFileNameW(g_hInstance, mod_name, sizeof mod_name / sizeof(WCHAR));
    if (!mod_name[0]) {
    ...
    BOOL rc = FALSE;
    if (RegCreateKeyExW(HKEY_LOCAL_MACHINE, keyName, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hkey, NULL) ==
        ERROR_SUCCESS) {
        RegSetValueExW(hkey, L"EventMessageFile", 0, REG_SZ, (const BYTE *)mod_name, wcslen(mod_name) * sizeof(WCHAR));
        DWORD types = EVENTLOG_ERROR_TYPE | EVENTLOG_WARNING_TYPE | EVENTLOG_INFORMATION_TYPE;
        RegSetValueExW(hkey, L"TypesSupported", 0, REG_DWORD, (const BYTE *)&types, sizeof(types));
        RegCloseKey(hkey);
    }
